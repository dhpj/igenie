      ...
      <div class="wl_main_goods" id="goods_list_step1" style="display:<? if($screen_data->psd_step1_yn == "N"){ ?>none<? } ?>;">
        <div class="good_btn_box">
          <span class="btn_excel_down">
            <a href="/uploads/sample/goods_sample.xlsx">엑셀 샘플 다운로드</a>
          </span>
		  ...
		  <dl id="step1_<?=$ii?>" class="dl_step1">
		    ...
		    <dd>
		  	  <ul>
		  		...
		  		<li style="display:none;"><span>옵션명</span><input type="text" id="goods_option_step1_<?=$ii?>" value="<?=$r->psg_option?>" onKeyup="chgGoodsData('step1_<?=$ii?>')" placeholder="5입" class="int140"></li>
              ...
              <dl id="step2-0_<?=$ii?>" class="dl_step2-0">
                ...
                <dd>
                  <ul>
              	    ...
              	    <li style="display:none;"><span>옵션명</span><input type="text" id="goods_option_step2-0_<?=$ii?>" value="" onKeyup="chgGoodsData('step2-0_<?=$ii?>')" placeholder="" class="int140"></li>
			  ...
			  <dl id="step2-<?=$seq?>_<?=$ii?>" class="dl_step2-<?=$seq?>">
                ...
                <dd>
              	<ul>
              	  ...
              	  <li style="display:none;"><span>옵션명</span><input type="text" id="goods_option_step2-<?=$seq?>_<?=$ii?>" value="<?=$r->psg_option?>" onKeyup="chgGoodsData('step2-<?=$seq?>_<?=$ii?>')" placeholder="" class="int140"></li>
            ...
            <ul class="txt_goods_list">
              ...
              <section id="area_goods_step3-0">
                ...
                <li id="step3-0_<?=$ii?>" class="dl_step3-0">
                  <span class="txt_name"><input type="text" id="goods_name_step3-0_<?=$ii?>" value="" onKeyup="chgGoodsData('step3-0_<?=$ii?>')" placeholder="신라면 5입" class="int100_l"></span>
                  ...
                  <input type="hidden" id="goods_option_step3-0_<?=$ii?>" value="">
                </li>
                <? } //for($ii = 0; $ii < $step3_std; $ii++){ ?>
              </section>
            ...
            <ul class="txt_goods_list">
              ...
              <section id="area_goods_step3-<?=$seq?>">
                ...
                <li id="step3-<?=$seq?>_<?=$ii?>" class="dl_step3-<?=$seq?>">
                  ...
                  <input type="hidden" id="goods_option_step3-<?=$seq?>_<?=$ii?>" value="<?=$r->psg_option?>">
                </li>
                <?
						$ii++; //코너별 등록수
						if($ii >= $r->rownum){
			    ...
			    <dd>
                  <ul>
                    ...
                    <li class="name">
					  ...
					  <span id="pre_goods_option_step1_<?=$ii?>" style="display:none;"><?=$r->psg_option?></span><?//STEP1 옵션명?>
            ...
            <section id="pre_goods_list_step2-0" style="display:none;">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step2-0">
				...
				<div class="pop_list01" id="pre_step2-0_<?=$ii?>">
                  ...
                  <div class="pop_name">
                    <span id="pre_goods_name_step2-0_<?=$ii?>">신라면 5입</span><?//STEP2 상품명?>
                    <span id="pre_goods_option_step2-0_<?=$ii?>" style="display:none;"></span><?//STEP2 옵션명?>
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step2-<?=$seq?>">
				...
				<div class="pop_list01" id="pre_step2-<?=$seq?>_<?=$ii?>">
				  ...
				  <div class="pop_name">
                    ...
                    <span id="pre_goods_option_step2-<?=$seq?>_<?=$ii?>" style="display:none;"><?//=$r->psg_option?></span><?//STEP2 옵션명?>
            ...
            <section id="pre_goods_list_step3-0" style="display:none;">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step3-0">
                ...
                <div class="pop_list02" id="pre_step3-0_<?=$ii?>">
                   <span class="name" id="pre_goods_name_step3-0_<?=$ii?>">신라면 5입</span><?//STEP3 상품명?>
                   <span id="pre_goods_option_step3-0_<?=$ii?>" style="display:none;"></span><?//STEP3 옵션명?>
            ...
            <section id="pre_goods_list_step3-<?=$seq?>">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step3-<?=$seq?>">
                ...
                <div class="pop_list02" id="pre_step3-<?=$seq?>_<?=$ii?>">
                   <span class="name" id="pre_goods_name_step3-<?=$seq?>_<?=$ii?>"><?=$r->psg_name?></span><?//STEP3 상품명?>
                   <span id="pre_goods_option_step3-<?=$seq?>_<?=$ii?>" style="display:none;"><?//=$r->psg_option?></span><?//STEP3 옵션명?>
...
<script>
		...
		if(step1_yn_Y){ //STEP1 사용함
			...
			for(var i=0; i < step1_cnt; i++){
				...
				if($("#goods_name_step1_"+ i).length > 0){ //해당 라인이 존재 할 경우
					...
					formData.append("data_id", data_id); //전단번호
					formData.append("goods_id", goods_id); //상품번호
					formData.append("tit_id", 0); //타이틀번호
					formData.append("goods_step", goods_step); //스텝
					formData.append("goods_step_no", 0); //스텝순번
					formData.append("goods_imgpath", goods_imgpath); //이미지경로
					formData.append("goods_name", goods_name); //상품명
					formData.append("goods_option", goods_option); //옵션명
					formData.append("goods_price", goods_price); //정상가
					formData.append("goods_dcprice", goods_dcprice); //할인가
					formData.append("goods_seq", goods_seq); //할인가
					formData.append("goods_badge", goods_badge); //할인뱃지
					formData.append("<?=$this->security->get_csrf_token_name()?>", "<?=$this->security->get_csrf_hash()?>");
					//request.onreadystatechange = callbackF;
					request.open("POST", "/pop/screen/screen_goods_save");
					request.send(formData);
			...
			for(var j=1; j <= step2_cnt; j++){
				...
				for(var i=0; i < line; i++){
					...
					if($("#goods_name_"+ stepId +"_"+ i).length > 0){ //해당 라인이 존재 할 경우
						...
						formData.append("goods_step", goods_step); //스텝
						formData.append("goods_step_no", j); //스텝순번
						formData.append("goods_imgpath", goods_imgpath); //이미지경로
						formData.append("goods_name", goods_name); //상품명
						formData.append("goods_option", goods_option); //옵션명
						formData.append("goods_price", goods_price); //정상가
	...
	function goods_append(step, id){
		...
		if(step == "3"){ //STEP3
			...
			html += '  <span class="txt_name"><input type="text" id="goods_name_'+ id +'_'+ no +'" value="" onKeyup="chgGoodsData(\''+ id +'_'+ no +'\')" placeholder="신라면 5입" class="int100_l"></span>';
			...
			html += '          <li><span>상품명</span> <input type="text" id="goods_name_'+ id +'_'+ no +'" onKeyup="chgGoodsData(\''+ id +'_'+ no +'\')" placeholder="신라면 5입" class="int140"></li>';
			html += '          <li style="display:none;"><span>옵션명</span> <input type="text" id="goods_option_'+ id +'_'+ no +'" onKeyup="chgGoodsData(\''+ id +'_'+ no +'\')" placeholder="" class="int140"></li>';
		...
		if(step == "2"){ //STEP2
			...
			html += '    <span id="pre_goods_option_'+ id +'_'+ no +'" style="display:none;"></span>'; //STEP2 옵션명
			...
			html += '   <span class="name" id="pre_goods_name_'+ id +'_'+ no +'">농심 신라면 120g 5입 1+1</span>'; //STEP3 상품명
			html += '   <span id="pre_goods_option_'+ id +'_'+ no +'" style="display:none;"></span>'; //STEP3 옵션명
			...
			html += '          <span id="pre_goods_option_'+ id +'_'+ no +'" style="display:none;"></span>'; //STEP1 옵션명
			...
			$.ajax({
				...
				success: function (json) {
					...
					$.each(json, function(key, value){
						...
						//alert("["+ no +"] "+ goods_name +", "+ goods_price +", "+ goods_dcprice +", "+ rtn_data);
