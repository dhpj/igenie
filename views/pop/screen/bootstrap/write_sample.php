<?
	...
	//echo "psd_id : ". $screen_data->psd_id .", add : ". $add ."<br>";
	$test_yn = "N"; //테스트여부
	if($screen_data->psd_id == "" and $add == "_test"){ //테스트의 경우
		$test_yn = "Y";
	}
    ...
    <div class="wl_lbox">
	  ...
	  <div id="goods_copy_list">
		...
		<div class="box_wrap" id="goods_list_step9-0" style="display:none;"><?//행사이미지 복사대상 ?>
          ...
          <div class="add_corner">
            <a class="" id="btn_del_step9-0" onClick="goods_area_del('goods_box-0', 'step9', 'step9-0');">
        ...
        <div class="box_wrap" id="goods_list_step2-0" style="display:none;"><?//이미지형 복사대상 ?>
          ...
          <div class="add_corner">
            ...
            <a class="" id="btn_del_step2-0" onClick="goods_area_del('goods_box-0', 'step2', 'step2-0');">
        ...
        <div class="box_wrap" id="goods_list_step3-0" style="display:none;"><?//텍스트형 복사대상 ?>
          ...
          <div class="add_corner">
            <a class="" id="btn_del_step3-0" onClick="goods_area_del('goods_box-0', 'step3', 'step3-0');">
	  ...
	  <div id="box_area"><?//박스 영역?>
        ...
        <div id="goods_box-<?=$goods_box_cnt?>"><?//이미지형?>
          <input type="text" name="goods_box_id" id="goods_box-<?=$goods_box_cnt?>_name" value="goods_box-<?=$goods_box_cnt?>" style="width:120px;">
          ...
          <div class="box_wrap" id="goods_list_step2-<?=$seq?>">
            ...
            <div class="add_corner">
              <a class="" id="btn_del_step2-<?=$seq?>" onClick="goods_area_del('goods_box-<?=$goods_box_cnt?>', 'step2', 'step2-<?=$seq?>');">
                <i class="xi-minus-circle-o"></i> 이미지형 상품코너 삭제
              </a>
            </div><!--//코너 추가/삭제-->
        ...
        <div id="goods_box-<?=$goods_box_cnt?>"><?//텍스트형?>
          ...
          <div id="goods_list_step3-<?=$seq?>">
            ...
            <div class="add_corner">
              <a class="" id="btn_del_step3-<?=$seq?>" onClick="goods_area_del('goods_box-<?=$goods_box_cnt?>', 'step3', 'step3-<?=$seq?>');">
               <i class="xi-minus-circle-o"></i> 텍스트형 상품코너 삭제
              </a>
            </div><!--//코너 추가/삭제-->
      ...
      <div class="smart_btn_group">
        ...
        <label for="event_file"><span><i class="xi-image-o"></i> 행사이미지 직접추가</span></label>
...
<script>
	...
	function modal_sort_open(page){
		...
		$("input[name=goods_box_id]").each(function(index, item){ //상품코너별 조회
			...
			html += "  <input type='text' name='modal_sort_box_id' id='modal_sort_box_id_"+ no +"' value='"+ goods_box_id +"' style='width:110px;'>";
	...
	function saved(flag){
		...
		$.ajax({
			...
			type: "POST",
			data: {"flag" : flag, "data_id" : data_id, "tem_id" : tem_id, "psd_title" : psd_title, "psd_date" : psd_date, "step1_yn" : step1_yn, "step2_yn" : step2_yn, "step3_yn" : step3_yn, "psd_viewyn" : "<?=$psd_viewyn?>", "psd_ver_no" : "<?=$psd_ver_no?>", "<?=$this->security->get_csrf_token_name()?>" : "<?=$this->security->get_csrf_hash()?>"},
			success: function (json) {
	...
	function goods_save(data_id){
		...
		//--------------------------------------------------------------------------------------------------------------
		var goods_box_cnt = Number($("#goods_box_cnt").val()); //상품 박스 갯수
		//alert("goods_box_cnt : "+ goods_box_cnt);
		for(var j=1; j <= goods_box_cnt; j++){ //박스 갯수만큼
			var box_id = "goods_box-"+ j;
			var box_no = $("#"+ box_id +"_box_no").val();
			var step = $("#"+ box_id +"_step").val();
			var stepId = $("#"+ box_id +"_step_id").val();
			var stepNo = $("#"+ box_id +"_step_no").val();
	...
	function goods_area_copy(area, step){
		...
		var html = '';
		html += '<div id="'+ newId +'">\n';
		html += '  <input type="text" name="goods_box_id" id="'+ newId +'_name" value="'+ newId +'" style="width:120px;">';
		html += '  <input type="text" id="'+ newId +'_step" value="'+ step +'" style="width:80px;">';
		html += '  <input type="text" id="'+ newId +'_step_id" value="'+ stepId +'" style="width:80px;">';
		html += '  <input type="text" id="'+ newId +'_step_no" value="'+ no +'" style="width:60px;">';
		html += '  <input type="text" id="'+ newId +'_box_no" value="'+ next_box_no +'" style="width:60px;">';
