				...
				<div class="pop_list01" id="pre_step2-0_<?=$ii?>">
                  ...
                  <div class="pop_price">
                    ...
                    <p class="price2" id="pre_goods_dcprice_step2-0_<?=$ii?>">3,000원</p><?//STEP2 할인가?>
									...
									<div class="pop_name">
                    <span id="pre_goods_name_step2-0_<?=$ii?>">상품명</span><?//STEP2 상품명?>
                    <span id="pre_goods_option_step2-0_<?=$ii?>">옵션명</span><?//STEP2 옵션명?>
            ...
            <section id="pre_goods_list_step3-0" style="display:none;">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step3-0">
                ...
                <div class="pop_list02" id="pre_step3-0_<?=$ii?>">
                  <div class="name">
                    <span id="pre_goods_name_step3-0_<?=$ii?>">상품명</span><?//STEP3 상품명?>
                    <span id="pre_goods_option_step3-0_<?=$ii?>"> 옵션명</span><?//STEP3 옵션명?>
                  </div>
			...
			$.ajax({
				...
				success: function (json) {
					...
					$.each(json, function(key, value){
						...
						/*if(goods_price != "" && goods_price != undefined){ //정상가 (숫자만 콤마 처리)
							//goods_price = goods_price.trim();
							//goods_price = goods_price.replace(/(^\s*)|(\s*$)/, ''); //앞과 뒤의 공백을 제거
							var rtn_price = goods_price;
							var temp_price = goods_price.replace(/[^0-9]/g,''); //정상가 숫자만 추출
							if(temp_price !=""){
								rtn_price = rtn_price.replace(/,/g,'');
								var coma_price = temp_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","); //금액 콤마 찍기
								rtn_price = rtn_price.replace(temp_price, coma_price); //최종 금액
								//$("#goods_price_"+ id).val(rtn_price);
							}
							goods_price = rtn_price;
						}
						if(goods_dcprice != "" && goods_dcprice != undefined){ //할인가 (숫자만 콤마 처리)
							//goods_dcprice = goods_dcprice.trim();
							//goods_dcprice = goods_dcprice.replace(/(^\s*)|(\s*$)/, ''); //앞과 뒤의 공백을 제거
							var rtn_price = goods_dcprice;
							var temp_price = goods_dcprice.replace(/[^0-9]/g,''); //정상가 숫자만 추출
							if(temp_price !=""){
								rtn_price = rtn_price.replace(/,/g,'');
								var coma_price = temp_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","); //금액 콤마 찍기
								rtn_price = rtn_price.replace(temp_price, coma_price); //최종 금액
								//$("#goods_dcprice_"+ id).val(rtn_price);
							}
							goods_dcprice = rtn_price;
						}*/
