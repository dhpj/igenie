...
<?php echo show_alert_message($this->session->flashdata('message'), '<div class="alert alert-auto-close alert-dismissible alert-info"><button type="button" class="close alertclose" >&times;</button>', '</div>'); ?>
	<h3><i class="icon-home"></i><?php echo html_escape(element('board_name', element('board', element('list', $view)))); ?></h3>
	<div class="wrap">
		<h4><?php if (element('category', element('post', $view))) { ?>[<?php echo html_escape(element('bca_value', element('category', element('post', $view)))); ?>] <?php } ?>
<?php echo html_escape(element('post_title', element('post', $view))); ?></h4>
		<ul class="info">
...
<?php
		...
		<style type="text/css">
		.list-group-tr { position:relative;display:block;padding:0;background-color:#fff;border:1px solid #ddd; }
		.col-group { width:49.5%; height:40px; display:inline-block !important;clear:both;margin:0;padding:0; }
		.col-group div { height:40px; padding-top:10px; }
		.control-label {padding-top:10px;height:40px;background:#f2f2f2;}
		.contents-view { border:1px solid #ddd;min-height:300px;margin-bottom:20px; }
		</style>
<?php if (element('extra_content', $view)) { ?>
		<div class="list-group">
			<div class="list-group-tr">
				<div class="col-group">
					<label for="post_nickname" class="col-sm-4 control-label">회사명</label>
					<div class="col-sm-8 pd7"><?php echo set_value('post_nickname', element('post_nickname', element('post', $view))); ?></div>
				</div>
			</div>
			<div class="list-group-tr">
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_manager">담당자명</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][0]['output']?></div>
				</div>
				<div class="col-group">
					<label for="post_email" class="col-sm-4 control-label">이메일</label>
					<div class="col-sm-8 pd7"><?php echo set_value('post_email', element('post_email', element('post', $view))); ?></div>
				</div>
			</div>
			<div class="list-group-tr">
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_hp">휴대폰번호</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][1]['output']?></div>
				</div>
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_tel">전화번호</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][2]['output']?></div>
				</div>
			</div>
			<div class="list-group-tr">
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_kakao_name">카카오플러스 친구이름</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][3]['output']?></div>
				</div>
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_kakao_id">카카오 검색용 아이디</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][4]['output']?></div>
				</div>
			</div>
			<div class="list-group-tr">
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_kakao_home">카카오 홈 URL</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][5]['output']?></div>
				</div>

				<div class="col-group">
					<label for="post_homepage" class="col-sm-4 control-label">회사홈 도메인</label>
					<div class="col-sm-8 pd7"><?php echo set_value('post_homepage', element('post_homepage', element('post', $view))); ?></div>
				</div>
			</div>
			<div class="list-group-tr">
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_send_qty">문자발송건수(월)</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][6]['output']?></div>
				</div>
				<div class="col-group">
					<label class="col-sm-4 control-label" for="ct_hosting">호스팅종류</label>
					<div class="col-sm-8 pd7"><?=$view['extra_content'][7]['output']?></div>
				</div>
			</div>
...
<?php if (element('file_image', $view)) {
...
<?php		}
			if (element('use_post_dislike', element('board', $view))) { ?>
...
<?php		if (element('attended', $poll) OR element('ended_poll', $poll)) {
				if ($poll_item) {
					$i = 1;
...
<?php			$attributes = array('name' => 'fpostpoll', 'id' => 'fpostpoll');
...
<?php }
		if ( ! element('post_del', element('post', $view)) && element('use_blame', element('board', $view)) && ( ! element('blame_blind_count', element('board', $view)) OR element('post_blame', element('post', $view)) < element('blame_blind_count', element('board', $view)))) { ?>
...
<?php }
		if ( ! element('post_del', element('post', $view)) && element('is_admin', $view)) { ?>
...
<?php		}
			if (element('post_notice', element('post', $view))) { ?>
...
<?php		}
		if (element('post_blame', element('post', $view)) >= element('blame_blind_count', element('board', $view))) { ?>
...
<?php	if (element('use_sns_button', $view)) {
...
<?php	if( ! element('post_hide_comment', element('post', $view))) {?>
