        ...
        <ul>
          ...
          <li>
            ...
            <div class="ll_title">
              ...
              <? if($this->member->item('mem_level') >= 100){ //최고관리자만 ?>
			  <button class="del" style="cursor:pointer;" onClick="samplen('<?=$r->psd_id?>');">샘플삭제</button>
			  <? } ?>
            </div>
            <div class="ll_btn1">
...
<script>
              ...
              html += "              <dt>";
						...
						if(value.psg_badge != "0"){ //행사뱃지(0.표기안함, 1.할인율, x.뱃지번호)
							if(value.psg_badge == "1"){ //행사뱃지(0.표기안함, 1.할인율, x.뱃지번호)
								...
								html += "                <div class=\"design_badge\" style=\"display:;\"><img src=\""+ value.badge_imgpath +"\"></div>"; //행사뱃지
							}
						}
              ...
              if(value.psg_stock == "N"){
                ...
                html += "              <button class=\"icon_add_cart\">장바구니</button>"; //장바구니
						...
						if(value.psg_imgpath != "" && value.psg_imgpath != null){
							html += "            <p class=\"pre_tit\"><img src=\""+ value.psg_imgpath +"\" alt=\"\"></p>"; //STEP2 타이틀 이미지
						}
						html += "          </div><!--//pre_box4-->";
					}
				}); //코너별 리스트 종료
	...
	//스마트전단 삭제
	function samplen(data_id){
		if(confirm("해당 스마트전단 샘플 지정을 취소 하시겠습니까?")){
			$.ajax({
				url: "/mall/compare/samplen",
				type: "POST",
				data: {"data_id" : data_id, "<?=$this->security->get_csrf_token_name()?>" : "<?=$this->security->get_csrf_hash()?>"},
				success: function (json) {
					showSnackbar("정상적으로 지정 취소 되었습니다.", 1500);
					setTimeout(function() {
						//location.reload();
						location.href = "/mall/compare/sample";
					}, 1000); //1초 지연
				}
			});
		}
	...
	function open_page(page) {
		...
		form.setAttribute("action", "/mall/compare/sample");
