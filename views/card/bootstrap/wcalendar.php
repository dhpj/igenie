        ...
        <table><?//리스트 영역?>
            ...
            <thead>
                <tr>
                    ...
                    <th class="align-center">정산받은금액</th>
                    <th class="align-center">수익금</th>
...
<script>
                    ...
                    if (json.list.length > 0){
                        $.each(json.list, function(idx, val){
                            ...
                            schedules.push({
                                ...
                                , title: val['vs_title']
                                , isAllDay: true
                                , start: moment(Number(val['vs_start2'])).format("YYYY-MM-DDTHH:mm:ss+09:00")
                                , end: moment(Number(val['vs_end2'])).format("YYYY-MM-DDTHH:mm:ss+09:00")
                                , goingDuration: 30
                                , comingDuration: 30
                                , color: 'red'
                                , isVisible: true
    ...
    $(document).on('click', '.tui-full-calendar-weekday-schedule-title', function(){
        ...
        s_dt = $(this).parent().data("scheduleId");
        $.ajax({
            url: "/card/settledetail2",
            type: "POST",
            data: {
                "<?=$this->security->get_csrf_token_name()?>" : "<?=$this->security->get_csrf_hash()?>"
    ...
    function download_() {
        ...
        searchTypeField.setAttribute("name", "dt");
        searchTypeField.setAttribute("value", s_dt);
