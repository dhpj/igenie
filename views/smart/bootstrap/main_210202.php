    ...
    <div class="wl_rbox">
      <div class="wl_r_preview">
        <div class="pre_box_wrap">
          ...
          <!--//템플릿 이미지가 들어갈 공간-->
          <div class="pre_box1">
            <p id="pre_wl_tit" class="pre_tit"><?=($screen_data->psd_title == "") ? "행사제목" : $screen_data->psd_title?>/<?=$screen_data->tem_useyn?>/</p>
			...
			<div class="pre_box3"><?//이미지형?>
              ...
              <div class="pop_list_wrap" >
				...
				<div class="<?=$pop_list_class?>">
				  <div <? if($r->psg_badge == "0"){ ?>style="display:none;"<? }else if($r->psg_badge == "1"){ ?>class="sale_badge"<? }else{ ?>class="design_badge"<? } ?>"><? if($r->psg_badge == "1"){ ?><?=$badge_rate?><span>%</span><? }else if($r->psg_badge > 1){ ?><img src="<?=$r->badge_imgpath?>"><? } ?></div>
				  <div class="templet_img_in3" style="background-image : url('<?=($r->psg_imgpath == "") ? $sample_imgpath : $r->psg_imgpath?>');">
				  ...
				  <div class="pop_name">
					...
					<div style="display:none;"><?=($r->psg_imgpath == "") ? $sample_imgpath : $r->psg_imgpath?></div><?//이미지경로?>
                  </div>
                </div>
                <?
						$ii++; //코너별 등록수
