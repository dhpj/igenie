...
<? }else{ //if($ie_yn == "Y"){ ?>
<div id="smart_map" style="width:100%;height:1720px;">
	...
	geocoder.addressSearch('<?=$this->member->item("mem_shop_addr")?>', function(result, status) {
		// 정상적으로 검색이 완료됐으면
		 if (status === kakao.maps.services.Status.OK) {
            ...
            $("#map_km").html((radius >= 1000 ? radius.toString().replace("000", "") + "Km" : radius + "m"));
		}

	});
}
//setMapCircle(1000, 4); //m단위 거리 원 그리기
setMapCircle(2000, 5); //m단위 거리 원 그리기
