...
<?
	...
	$stmall_yn = $this->funn->get_stmall_yn($mem_stmall_yn, $psd_order_yn, $psd_order_sdt, $psd_order_edt, $psd_order_st, $psd_order_et); //주문하기 사용여부
...
<div class="wrap_smart">
  ...
  <button type="button" class="btnScreenShot" id="btnScreenShot" style="bottom:90px;"><span class="material-icons">save_alt</span> 이미지 다운로드</button>
  ...
  <script>
    ...
    $(document).ready(function() {
           <? if($screen_data->psd_mem_id=='2'||$screen_data->psd_mem_id=='1789'){ ?>
            spreadGnb();
           <? } ?>
  ...
  <div class="smart_menu_s" style="display:none;">
	...
	<div class="smart_search_result" style="display:none;">
        <button type="button" class="btn_close_result" id="btn_close_result" onclick="search_box_onoff()">
        ...
        <div id="search_item">
    		<dl>
    			<dt>
    				<img src="/uploads/goods/78a834fe621dbcb2278f583e5ad31e51.jpg" />
    			</dt>
    			<dd>
    				<ul>
...
<!-- 상단메뉴 코너 펼치기 -->
<button type="button" class="btn_spread_gnb" id="btn_spread_gnb" onclick="spreadGnb()" <?=($tit_cnt<5)? "style='display:none;'" : ""?>>
    <i class="material-symbols-outlined">expand_more</i>
</button>
<button type="button" class="btn_close_gnb" id="btn_close_gnb" onclick="spreadGnb()" style="display:none">
    <i class="material-symbols-outlined">close</i>
</button>
<script>
function spreadGnb() {
    <? if($tit_cnt>4){ ?>
    let classList = document.getElementById('event_gnb').classList.value;
    if(classList === "event_gnb") {
        document.getElementById("event_gnb").classList = "event_gnb spread_gnb";
        document.getElementById("btn_spread_gnb").style.display = "none";
        document.getElementById("btn_close_gnb").style.display = "";
        $('.img_next_gnb').hide();
    } else {
        document.getElementById("event_gnb").classList = "event_gnb";
        document.getElementById("btn_spread_gnb").style.display = "";
        document.getElementById("btn_close_gnb").style.display = "none";
        $('.img_next_gnb').show();
    }
    <? } ?>
}
...
<?
     if(!empty($tit_text_info)){ //타이틀 텍스트 정보 체크
         ...
         <div class="event_gnb" id="event_gnb">

         <?
            foreach($tit_text_info as $r) {
                if($r->pst_tit_text_info !=''){?>
                    <div  onclick="tit_text_info_click(<?=$r->pst_box_no?>);autoConnerOpen(<?=$r->pst_box_no?>);spreadGnb2();" class=""><?=$r->pst_tit_text_info?></div>
            <?	}
            }
            ?>
            </div>
            <span class="material-symbols-outlined img_next_gnb" <?=($tit_cnt<5)? "style='display:none;'" : "" ?>>double_arrow</span>
            <?
    } ?>
  ...
  <div class="smart_con" id="smart_capture" <?=($tit_cnt<5)? "style='border-top:0px;'" : "" ?>>
    <div class="wl_rbox">
      <div class="wl_r_preview<?=($screen_data->psd_mem_id == "3"||$screen_data->psd_mem_id == "1121")? " twinkle" : "" ?>">
        <div class="pre_box_wrap">
		  ...
		  <div class="pre_box2<?=($screen_data->tem_useyn == "S") ? " mg_t0" : ""?>" id="pre_goods_list_step1">
            ...
            <section id="pre_area_goods_step1">
		      ...
		      ?>
		      <dl>
                <? if($cart_yn == "Y"&&$r->psg_stock != "N"&&$r->psg_no_order != "Y"){ //스마트전단 주문하기 사용의 경우 ?><button class="icon_add_cart" onclick="addcart('<?=$r->psg_id?>')">장바구니</button><? } ?>
                <dt>
			    ...
			    <dd>
                  <ul>
                    <div class="gid" style="display:none;"><?=$r->psg_id?></div>
                    <div style="display:none;" class="gpath"><?=($r->psg_imgpath == "") ? config_item('no_img_path') : config_item('igenie_path').$imgnamepext?></div>
                    <div style="display:none;" class="gnoorder"><?=$r->psg_no_order?></div>
        ...
        <?
			...
			if(!empty($screen_box)){
				foreach($screen_box as $r) {
                    ...
                    // log_message("ERROR", $_SERVER['REQUEST_URI'] ." > r->psg_step : ". $r->psg_step." / r[psg_step] : ". $r['psg_step']);
					$badge_rate = "";
					if($r->psg_price != "" && $r->psg_dcprice != ""){
						$goods_price = preg_replace("/[^0-9]*/s", "", $r->psg_price); //정상가 숫자만 출력
						$goods_dcprice = preg_replace("/[^0-9]*/s", "", $r->psg_dcprice); //할인가 숫자만 출력
					...
					if($psg_step == "1" or $psg_step == "2" or $psg_step == "4" or $psg_step == "5"){ //이미지형
                        if($psg_step == "1"){ //2.2단 이미지형
							$pop_list_class = "pop_list00 pop_item ";
                            if(strlen($r->psg_dcprice)>5&&$screen_data->psd_font_LN!="N"){
                                $minfont = 42-((strlen($r->psg_dcprice)-5)*1);
                                if($minfont<30){
                                    $minfont = 30;
                                }
                                $price_font_size="style='font-size:".$minfont."px;'";
                            }
						}else if($psg_step == "2"){ //2.2단 이미지형
							$pop_list_class = "pop_list01 pop_item ";
                            if(strlen($r->psg_dcprice)>5&&$screen_data->psd_font_LN!="N"){
							...
							$pop_list_class = "pop_list03 pop_item ";
                            $r->psg_badge='0';
                            if(strlen($r->psg_dcprice)>5&&$screen_data->psd_font_LN!="N"){
							...
							$pop_list_class = "pop_list04 pop_item ";
                            $r->psg_badge='0';
                            if(strlen($r->psg_dcprice)>5&&$screen_data->psd_font_LN!="N"){
        ...
        ?>
				...
				<div id="pre_box_n_<?=$r->psg_box_no?>" class="pre_box3" <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?>><?//이미지형?>
                    <? if($r->tit_imgpath != ""){ ?>
    					<div class="pre_tit demo tit_custom" data-effectJs="zoom-in-up">
                            <? if($r->tit_type == "C"){
                                ...
                                ?>
                                <div><span style='font-size:<?=$txsize?>px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>'><?=$r->pst_tit_text_info?></span></div>
                            <? } ?>
    						<img src="<?=config_item('igenie_path').$r->tit_imgpath?>"><?//STEP2 타이틀 이미지 ?>
    					</div>
    			  <? } ?>
             <? if($r->sub_title != ""){ ?>
              <div style="padding:5px;margin-top:10px;position : relative;text-align:center;font-size: <?=(!empty($r->sub_title_size))? $r->sub_title_size : "35px"?>; color: <?=(!empty($r->sub_title_color))? $r->sub_title_color : "#333333"?>;font-weight:bold;"><?=$r->sub_title?></div>
              <? } ?>
              <div class="pop_list_wrap" >
				...
				<div class="<?=$pop_list_class?> <?=($ii>19&&$psd_mode=='L')? "over20 over20_".$r->psg_box_no : "" ?>"<?=($ii>19&&$psd_mode=='L')? "style='display:none;'" : "" ?>>
				  <? if($cart_yn == "Y"&&$r->psg_stock!='N'&&$r->psg_no_order!='Y'){ //스마트전단 주문하기 사용의 경우 ?><button class="icon_add_cart" onclick="addcart('<?=$r->psg_id?>')">장바구니</button><? } ?>
                  <div style="display:none;" class="gid"><?=$r->psg_id?></div>
                  <div style="display:none;" class="gpath"><?=($r->psg_imgpath == "") ? config_item('no_img_path') : config_item('igenie_path').$imgnamepext?></div>
                  <div style="display:none;" class="gnoorder"><?=$r->psg_no_order?></div>
				  <div <? if($r->psg_badge == "0"){ ?>style="display:none;"<? }else if($r->psg_badge == "1"){ ?>class="sale_badge demo" data-effectJs="flip-left"<? }else{ ?>class="design_badge demo" data-effectJs="flip-left"<? } ?>"><? if($r->psg_badge == "1"){ ?><?=$badge_rate?><span>%</span><? }else if($r->psg_badge > 1){ ?><img src="<?=config_item('igenie_path').$r->badge_imgpath?>"><? } ?></div>
				  <div class="templet_img_in3<?=($r->psg_stock == "N") ? " soldout" : ""?>" style="background-image : url('<?=(($psd_mode=='L'&&$ii<20)||$psd_mode!='L')?(($r->psg_imgpath == "") ? config_item('no_img_path') : config_item('igenie_path').$imgnamepext): "" ?>');">
              ...
              <div class="btn_more_wrap">
                    <button type="button" id="pre_goods_more-<?=$r->psg_box_no?>" class="goods_more_btn" onclick="conner_more('<?=$r->psg_box_no?>');">
        ...
        <?
						...
						if((($psg_step*10) + $r->psg_step_no) != $chk_step_no){
							...
							$seq++;
		?>
						<div id="pre_box_n_<?=$r->psg_box_no?>" class="pre_box4"  <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?>><?//텍스트형?>
                            <? if($r->tit_imgpath != ""){ ?>
        								<div class="pre_tit demo tit_custom" data-effectJs="zoom-in-up" id="pre_tit_id_step3-<?=$seq?>">
                                            <? if($r->tit_type == "C"){
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step3-<?=$seq?>"  <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?>>
                ...
                ?>
                <div class="pop_list02  pop_item <? if($stmall_yn == "Y"){ ?> cartplus_t<? }else{?> cartplus <?} ?><?=($r->psg_stock == "N") ? " soldout" : ""?>" id="pre_step3-<?=$seq?>_<?=$ii?>">
                  <div style="display:none;" class="gid"><?=$r->psg_id?></div>
                  <div style="display:none;" class="gnoorder"><?=$r->psg_no_order?></div>
                  <div class="name">
  ...
  <div class="page_but" <?=($screen_data->psd_mem_id == "3")? "style='display:none;" : "style='display:none;'" ?>> <span class="material-icons page_but_prev" onclick="pre_page('<?=$page?>');">chevron_left</span> <div class="page"><?=$page?> / <?=$total?></div> <span class="material-icons page_but_next" onclick="nex_page('<?=$page?>');">chevron_right</span> </div>

  <? include_once($_SERVER['DOCUMENT_ROOT'].'/views/smart/bootstrap/_inc_smart_footer.php'); ?>

  <div class="fix_bar fix_bar_up">
      <!-- <div> <span class="material-icons" onclick="pre_page('<?=$page?>');">history</span> <?=$page?> / <?=$total?><span class="material-icons" onclick="nex_page('<?=$page?>');">history</span> </div> -->
...
<script>
    ...
    function autoConnerOpen(num){
        if($("#pre_goods_more-"+num).length > 0){
            if($("#pre_goods_more-"+num).children("i").text()=="add"){
                conner_more(num);
            }
        }else{
            $(".over20").hide();
            $(".goods_more_btn").children("i").text("add");
            $(".goods_more_btn").children("span").text("상품 더보기");
        }
    }
    ...
    function search_goods(){
		...
		// 	}
		// });
        $('#pre_area_goods_step1').children('dl').each(function(index,item){

            var itemname = $(this).find('.name').text();
            // var itemoption = $(this).find('.pop_option').text();
            if(itemname.includes(word)==true){
                var itemprice1 = $(this).find('.price1').text();
                var itemprice2 = $(this).find('.price2').text();

                var gid = $(this).find('.gid').text();
                var soldout = "Y";
                var gnoorder = $(this).find('.gnoorder').text();
                var imgpath = "/dhn/images/leaflets/sample_img.jpg";
                if($(this).find('div').hasClass("soldout") == true){
                    soldout = "N";
                }

                if($(this).find('div').hasClass("templet_img_in3") == true){
                    imgpath=$(this).find('.gpath').text();
                }
                html += "<dl>";
                html += "<dt><img src='"
                html += imgpath;
                ...
                html += "<dd><ul>";
                html += "<li>"+itemname+"</li>";
                html += "<li>"+itemprice1+"</li>";
                html += "<li>"+itemprice2+"</li>";
                html += "</ul></dd>";
                <? if($cart_yn == "Y"){ //스마트전단 주문하기 사용의 경우 ?>
                if(soldout=="Y"&&gnoorder!="Y"){
                    html += "<button class='icon_add_cart' onclick='addcart(\""+gid+"\")'>장바구니</button>";
                }
                <? } ?>
                html += "</dl>";
            }
         });
        $('.pop_item').each(function(index,item){

            var itemname = $(this).find('.pop_name').text();
            if(itemname==""||itemname==undefined){
                itemname = $(this).find('.name').text();
            }
            var itemoption = $(this).find('.pop_option').text();
            if(itemname.includes(word)==true||itemoption.includes(word)==true){
                var itemprice1 = $(this).find('.price1').text();
                var itemprice2 = $(this).find('.price2').text();
                ...
                var gnoorder = $(this).find('.gnoorder').text();
                var imgpath = "/dhn/images/leaflets/sample_img.jpg";
                if($(this).hasClass("soldout") == true||$(this).children('div').hasClass("soldout") == true){
                    soldout = "N";
                }

                if($(this).children('div').hasClass("templet_img_in3") == true){
                    imgpath=$(this).children('.gpath').text();
                    // imgpath=$(this).children('.templet_img_in3').css("background-image");
                    // imgpath = imgpath.replace(/^url\(['"](.+)['"]\)/, '$1');
                }
                html += "<dl>";
                html += "<dt><img src='"
                html += imgpath;
                html += "' /></dt>";
                html += "<dd><ul>";
                html += "<li>"+itemname+ " " + itemoption + "</li>";
                html += "<li>"+itemprice1+"</li>";
                html += "<li>"+itemprice2+"</li>";
                html += "</ul></dd>";
                <? if($cart_yn == "Y"){ //스마트전단 주문하기 사용의 경우 ?>
    ...
    function close_pop() {
     ...
     function conner_more(no){
         if(no==""){
             ...
             if($("#pre_goods_more-"+no).children("i").text()=="remove"){
                 $(".over20_"+no).hide();
                 $("#pre_goods_more-"+no).children("i").text("add");
                 $("#pre_goods_more-"+no).children("span").text("상품 더보기");
                 tit_text_info_click(no);
                 // $("#pre_goods_more-"+no).attr("tabindex", -1).focus(); //포커스
             }else{
                 $(".over20").hide();
                 $(".goods_more_btn").children("i").text("add");
                 $(".goods_more_btn").children("span").text("상품 더보기");
                 ...
                 $(".over20_"+no).each(function(index,item){
                     var gpath = $(this).children('.gpath').text();
                     $(this).find('.templet_img_in3').css("background-image", "url('"+gpath+"')");
                  });
                // templet_img_in3
                 // tit_text_info_click(no);
                 // $("#pre_goods_more-"+no).attr("tabindex", -1).focus(); //포커스
