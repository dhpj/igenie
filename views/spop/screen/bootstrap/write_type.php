...
<link rel="stylesheet" type="text/css" href="/views/spop/screen/bootstrap/css/custom_style.css?v=<?=date("ymdHis")?>"/>
  ...
  <div class="write_leaflets">
    <div class="wl_lbox">
      ...
      <div class="wl_main_goods" id="goods_list_step1" style="display:<? if($psd_step1_yn == "N"){ ?>none<? } ?>;">
        ...
        <? } ?>
					<button class="btn_badge_all" type="button" onclick="modal_all_badge_open('1', 'step1_')"><span class="material-icons">class</span> 뱃지 일괄적용</button>
                    <button class="btn_badge_all order_all_x" type="button" onclick="all_noorder('goods_box-<?=$goods_box_cnt?>')" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>><span class="material-icons">remove_shopping_cart</span>주문X 일괄적용</button>
          <? if($mem_bigpos_yn == "Y"){ ?>
		...
		<section id="area_goods_step1">
		  		...
		  		if(!empty($screen_step1)){
					foreach($screen_step1 as $r) {
						...
						if($r->psg_price != "" && $r->psg_dcprice != ""){
							...
							$badge_rate = $goods_price > 0 ? 100 - round( ( ($goods_dcprice / $goods_price) * 100) ) : 0; //할인율
						}
		  ?>
		  ...
		  <dl id="step1_<?=$ii?>" class="dl_step1">
		    ...
		    <dd>
              ...
              <div class="order_sel" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>>
					<input type="hidden" id="goods_noorder_step1_<?=$ii?>" name="hdn_goods_noorder_step1" value="<?=$r->psg_no_order?>">
					<label class="checkbox_container">
						<input type="checkbox" name="noorder_chk_step1" id="noorder_chk_step1_<?=$ii?>" onClick="chgGoodsData('step1_<?=$ii?>', '', this)" value="Y" <?=($r->psg_no_order == "Y") ? " checked" : ""?> >
						<span class="checkmark"></span>
					</label>
				</div>
				<span class="order_sel_text" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>>주문X</span>
	  ...
	  <div id="goods_copy_list">
        ...
        <? //행사이미지 복사대상 [E] ------------------------------------------------------------------------------------------------------------------------------------ ?>
        <? //1단 이미지형 복사대상 [S] -------------------------------------------------------------------------------------------------------------------------------------- ?>
        <div class="box_wrap gb_box" id="goods_list_step1-0" style="display:none;"><?//1단 이미지형 복사대상 ?>
  	      <div class="tit_box1">
          ...
          <div class="wl_img_goods" id="step1-0">
              <!-- 커스텀 타이틀 선택 -->
              <div class="tit_box tit_sel_corner_tit">
                  <!-- <div class="tit">코너 타이틀</div> -->
                  <div class="use_choice">
                      <label class="uc_container"> 이미지 타이틀
                          <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="I" id="goods_box-<?=$goods_box_cnt?>_title_type_I" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step1-0', 'I');" checked >
                          <span class="checkmark"></span>
                      </label>
                      <label class="uc_container"> 커스텀 타이틀
                          <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="C" id="goods_box-<?=$goods_box_cnt?>_title_type_C" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step1-0', 'C')" >
                          <span class="checkmark"></span>
                      </label>
                  </div>
                   <input type="hidden" id="tit_pre_select_step1-0">
              </div>
            <!--타이틀 이미지가 들어갈 공간-->
            <div class="list_title_img">
              <ul>
                ...
                <?
					$dti = 0;
					foreach($design_title_custom_list_i as $r) { //타이틀 이미지
						if($r->tit_type_self != "S"){ //타이틀이미지 직접입력 체크
                            ...
                            $align_text = "center";
                            $padding_plus_text = "";
                            if(!empty($r->tit_align)){
                                if($r->tit_align=="L"){
                                    $align_text = "left";
                                    $padding_plus_text = "padding:0 21% 0 1%;";
                                }else if($r->tit_align=="C"){
                                    $align_text = "center";
                                }else{
                                    $align_text = "right";
                                    $padding_plus_text = "padding:0 1% 0 21%;";
                                }
                            }

                            $color_text = "#333333";
                            if(!empty($r->tit_txcolor)){
                                $color_text = $r->tit_txcolor;
                            }

                            if(!empty($r->tit_imgpath)&&$r->tit_imgpath != ""){
                                $file_ext = substr(strrchr($r->tit_imgpath, "."), 1);
                                $fileNameWithoutExt = substr($r->tit_imgpath, 0, strrpos($r->tit_imgpath, "."));
                                $findthumb = strripos($fileNameWithoutExt, '_thumb');
                                //echo $fileNameWithoutExt."---".$file_ext;
                                $plustext = "_thumb.";
                                if($findthumb>0){
                                ...
                                $imgtitthumb = $fileNameWithoutExt.$plustext.$file_ext;
                            }else{
                                $imgtitthumb = "";
                            }
					?>
	    			<li class="<?=($r->tit_type=="C")? "tit_custom_y" : "tit_custom_n" ?>" <?=($r->tit_type!="C")? "" : "style='display:none;'" ?>><label class="uc_container">
                        <? if($r->tit_type=="C"){ ?>
							  <input type="radio" name="tit_id_step1-0" id="tit_id_step1-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step1-0', '<div><span style=\'font-size:61px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>\'><?=$r->tit_text_info?></span></div><img src=\'<?=$r->tit_imgpath?>\'>' ,'<?=$r->tit_text_info?>','<?=$r->tit_bgcolor?>','<?=$r->tit_txcolor?>','<?=$r->tit_align?>');">
                            <? }else{ ?>
                              <input type="radio" name="tit_id_step1-0" id="tit_id_step1-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step1-0', '<img src=\'<?=$r->tit_imgpath?>\'>','<?=$r->tit_text_info?>');"<? if($dti == 1){ ?> checked<? } ?>>
                            <? } ?>


                        <span class="checkmark"></span><img src="<?=$imgtitthumb?>"></label></li>
	    			<?
						}
					}
				?>
            ...
            <div class="tit_box2">
              ...
              <ul class="tit_img_menu">
    				...
    				<li><span><i class="xi-bars"></i> 스마트전단 메뉴명</span> <input type="text" id="t_tit_text_info_step1-0" placeholder="예시)야채코너" value="<?=$tit_text_info_first_i?>" onkeyup="chgCustomData('step1-0', 'goods_box-<?=$goods_box_cnt?>', this)"></li>
    				<li><span><i class="xi-camera-o"></i> 타이틀이미지 직접입력</span>
                        <label for="titimg_self_file_step1-0" class="templet_img_box" ><div id="div_preview_tit_step1-0"><img id="tit_img_preview_step1-0" style="display:block;width:100%"></div></label>
                         ...
                         <input type="hidden" id="tit_id_custom_yn_step1-0" value="I">
    				</li>
    			</ul>
                <ul class="tit_custom_plus_menu">

                    <li id="plus_menu_step1-0" style='display:none;'>
						<span><i class="xi-text-color"></i> 글꼴 </span>
                        <input type="text" id="t_tit_txcolor_step1-0" placeholder="예시)#333333" value="<?=$r->tit_txcolor?>"  onkeyup="chgtxColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '')">
                        <ul class="sel_color_CT">
                            <li style="background-color:#333333" onclick="chgtxColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
							<li style="background-color:#ffffff" onclick="chgtxColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
							<li style="background-color:#da281c" onclick="chgtxColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
							<li style="background-color:#2c53d2" onclick="chgtxColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
							<li style="background-color:#ff5a00" onclick="chgtxColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li>
						</ul>
                        <input type="hidden" id="tit_id_custom_align_step1-0" value="<?=$r->tit_align?>">
                        <input type="hidden" id="tit_id_custom_txcolor_step1-0" value="<?=$r->tit_txcolor?>">
                        <input type="hidden" id="tit_id_custom_bgcolor_step1-0" value="<?=$r->tit_bgcolor?>">
					</li>

                    <li>
						<span><i class="xi-comment-o"></i> 부제목 </span>
                        <input type="text" id="t_sub_txt_step1-0" placeholder="예시)12.01~12.21" value="<?=$r->sub_title?>"  onkeyup="chgsubTxtData('step1-0')">
                        <span class="counter_minus button" onclick="subt_size_change('minus','step1-0');">-</span>
                        <span class="counter_plus button" onclick="subt_size_change('plus','step1-0');">+</span>
                        <input type="text" id="t_sub_txcolor_step1-0" placeholder="예시)#333333" value="<?=$r->sub_title_color?>"  onkeyup="chgsubColorData('step1-0', '')">
						<ul class="sel_color_CT">
							<li style="background-color:#ffffff" onclick="chgsubColorData('step1-0', '#ffffff')"></li>
                            <li style="background-color:#fcff00" onclick="chgsubColorData('step1-0', '#fcff00')"></li>
							<li style="background-color:#fd1979" onclick="chgsubColorData('step1-0', '#fd1979')"></li>
                            <li style="background-color:#6bff60" onclick="chgsubColorData('step1-0', '#6bff60')"></li>
                            <li style="background-color:#e12028" onclick="chgsubColorData('step1-0', '#e12028')"></li>
                            <li style="background-color:#0034df" onclick="chgsubColorData('step1-0', '#0034df')"></li>
                            <li style="background-color:#000000" onclick="chgsubColorData('step1-0', '#000000')"></li>
                            <li style="background-color:#333333" onclick="chgsubColorData('step1-0', '#333333')"></li>
						</ul>
					</li>
                    <li>
						<span><i class="xi-color-fill"></i> 배경 </span>
                        <input type="text" id="t_tit_bgcolor_step1-0" placeholder="예시)#ffffff" value=""  onkeyup="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '')">
						<ul class="sel_color_CT CT_bg">
							<li style="background-color:#333333" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                            <li style="background-color:#d9d9d9" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#d9d9d9')"></li>
							<li style="background-color:#ffffff" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                            <li style="background-color:#01508d" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#01508d')"></li>
                            <li style="background-color:#6299fe" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#6299fe')"></li>
                            <li style="background-color:#0138ed" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#0138ed')"></li>
                            <li style="background-color:#132062" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#132062')"></li>
                            <li style="background-color:#c138c1" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#c138c1')"></li>
                            <li style="background-color:#9032d2" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#9032d2')"></li>
                            <li style="background-color:#410098" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#410098')"></li>
                            <li style="background-color:#00aacf" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#00aacf')"></li>
                            <li style="background-color:#0050c7" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#0050c7')"></li>
                            <li style="background-color:#00acaa" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#00acaa')"></li>
                            <li style="background-color:#8794a3" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#8794a3')"></li>
                            <li style="background-color:#87a395" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#87a395')"></li>
                            <li style="background-color:#409834" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#409834')"></li>
                            <li style="background-color:#ceff0c" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ceff0c')"></li>
                            <li style="background-color:#a9d44f" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#a9d44f')"></li>
                            <li style="background-color:#005e3c" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#005e3c')"></li>
                            <li style="background-color:#008b6e" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#008b6e')"></li>
                            <li style="background-color:#299e00" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#299e00')"></li>
                            <li style="background-color:#59301a" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#59301a')"></li>
                            <li style="background-color:#f4eee2" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#f4eee2')"></li>
                            <li style="background-color:#71675d" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#71675d')"></li>
                            <li style="background-color:#a39b87" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#a39b87')"></li>
                            <li style="background-color:#d08e01" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#d08e01')"></li>
                            <li style="background-color:#fedc00" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#fedc00')"></li>
                            <li style="background-color:#ffa400" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ffa400')"></li>
                            <li style="background-color:#e9ab00" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#e9ab00')"></li>
                            <li style="background-color:#e7ca02" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#e7ca02')"></li>
                            <li style="background-color:#ff9400" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ff9400')"></li>
                            <li style="background-color:#f85333" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#f85333')"></li>
                            <li style="background-color:#ff1b41" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ff1b41')"></li>
                            <li style="background-color:#ff7800" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ff7800')"></li>
                            <li style="background-color:#f63756" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#f63756')"></li>
                            <li style="background-color:#ae0009" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#ae0009')"></li>
                            <li style="background-color:#9d1000" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#9d1000')"></li>
                            <li style="background-color:#d8282f" onclick="chgbgColorData('step1-0', 'goods_box-<?=$goods_box_cnt?>', '#d8282f')"></li>
						</ul>
					</li>
                </ul>
            ...
            <? } ?>
              <button class="btn_badge_all" type="button" onclick="modal_all_badge_open('1', 'step1-0_')"><span class="material-icons">class</span> 뱃지 일괄적용</button>
              <button class="btn_badge_all order_all_x" type="button" onclick="all_noorder('goods_box-<?=$goods_box_cnt?>')" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>><span class="material-icons">remove_shopping_cart</span>주문X 일괄적용</button>
              <? if($mem_bigpos_yn == "Y"){ ?>
		      <span class="btn_pos_view">
              ...
              <dl id="step1-0_<?=$ii?>" class="dl_step1-0">
                ...
                <dd>
                  <ul>
              	    ...
              	    <input type="hidden" id="goods_step_step1-0_<?=$ii?>" value="1" style="width:30px;">
              	    <input type="hidden" id="goods_div_step1-0_<?=$ii?>" value="step1-0_<?=$ii?>" style="width:80px;">
              	    ...
              	    <li><span>정상가</span><input type="text" id="goods_price_step1-0_<?=$ii?>" value="" onClick="chkGoodsData('step1-0_<?=$ii?>', '', this)" onKeyup="chgGoodsData('step1-0_<?=$ii?>', '', this)" placeholder="4,000원" class="int140"></li>
              	    <li><span>할인가</span><input type="text" id="goods_dcprice_step1-0_<?=$ii?>" value="" onClick="chkGoodsData('step1-0_<?=$ii?>', '', this)" onKeyup="chgGoodsData('step1-0_<?=$ii?>', '', this)" placeholder="3,000원" class="int140"></li>
        ...
        <div class="box_wrap gb_box" id="goods_list_step2-0" style="display:none;"><?//2단 이미지형 복사대상 ?>
          ...
          <div class="wl_img_goods" id="step2-0">
              ...
              <div class="tit_box tit_sel_corner_tit">
                  ...
                  <div class="use_choice">
                      <label class="uc_container"> 이미지 타이틀
                          <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="I" id="goods_box-<?=$goods_box_cnt?>_title_type_I" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step2-0', 'I');" checked >
                          <span class="checkmark"></span>
                      </label>
                      <label class="uc_container"> 커스텀 타이틀
                          <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="C" id="goods_box-<?=$goods_box_cnt?>_title_type_C" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step2-0', 'C')" >
                          <span class="checkmark"></span>
                      </label>
                  </div>
                   <input type="hidden" id="tit_pre_select_step2-0">
              </div>
            <!--타이틀 이미지가 들어갈 공간-->
            <div class="list_title_img">
              <ul>
                ...
                <?
                    ...
                    <li class="<?=($r->tit_type=="C")? "tit_custom_y" : "tit_custom_n" ?>" <?=($r->tit_type!="C")? "" : "style='display:none;'" ?>><label class="uc_container">
                                        <? if($r->tit_type=="C"){ ?>
                							  <input type="radio" name="tit_id_step2-0" id="tit_id_step2-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step2-0', '<div><span style=\'font-size:61px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>\'><?=$r->tit_text_info?></span></div><img src=\'<?=$r->tit_imgpath?>\'>' ,'<?=$r->tit_text_info?>','<?=$r->tit_bgcolor?>','<?=$r->tit_txcolor?>','<?=$r->tit_align?>');">
                                            <? }else{ ?>
                                              <input type="radio" name="tit_id_step2-0" id="tit_id_step2-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step2-0', '<img src=\'<?=$r->tit_imgpath?>\'>','<?=$r->tit_text_info?>');"<? if($dti == 1){ ?> checked<? } ?>>
                                            <? } ?>
            ...
            <div class="tit_box2">
              ...
              <ul class="tit_img_menu">
					...
					<li><span><i class="xi-bars"></i> 스마트전단 메뉴명</span> <input type="text" id="t_tit_text_info_step2-0" placeholder="예시)야채코너" value="<?=$tit_text_info_first_i?>" onkeyup="chgCustomData('step2-0', 'goods_box-<?=$goods_box_cnt?>', this)"></li>
					<li><span><i class="xi-camera-o"></i> 타이틀이미지 직접입력</span>
                         ...
                         <input type="hidden" id="tit_id_custom_yn_step2-0" value="I">
					</li>
				</ul>
                <ul class="tit_custom_plus_menu">

                    <li id="plus_menu_step2-0" style='display:none;'>
						<span><i class="xi-text-color"></i> 글꼴 </span>
                        <input type="text" id="t_tit_txcolor_step2-0" placeholder="예시)#333333" value="<?=$r->tit_txcolor?>"  onkeyup="chgtxColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '')">
                        <ul class="sel_color_CT">
                            <li style="background-color:#333333" onclick="chgtxColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
							<li style="background-color:#ffffff" onclick="chgtxColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
							<li style="background-color:#da281c" onclick="chgtxColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
							<li style="background-color:#2c53d2" onclick="chgtxColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
							<li style="background-color:#ff5a00" onclick="chgtxColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li>
						</ul>
                        <input type="hidden" id="tit_id_custom_align_step2-0" value="<?=$r->tit_align?>">
                        <input type="hidden" id="tit_id_custom_txcolor_step2-0" value="<?=$r->tit_txcolor?>">
                        <input type="hidden" id="tit_id_custom_bgcolor_step2-0" value="<?=$r->tit_bgcolor?>">
					</li>
                    <li>
						<span><i class="xi-comment-o"></i> 부제목 </span>
                        <input type="text" id="t_sub_txt_step2-0" placeholder="예시)12.01~12.21" value="<?=$r->sub_title?>"  onkeyup="chgsubTxtData('step2-0')">
                        <span class="counter_minus button" onclick="subt_size_change('minus','step2-0');">-</span>
                        <span class="counter_plus button" onclick="subt_size_change('plus','step2-0');">+</span>
                        <input type="text" id="t_sub_txcolor_step2-0" placeholder="예시)#333333" value="<?=$r->sub_title_color?>"  onkeyup="chgsubColorData('step2-0', '')">
						<ul class="sel_color_CT">
							<li style="background-color:#ffffff" onclick="chgsubColorData('step2-0', '#ffffff')"></li>
                            <li style="background-color:#fcff00" onclick="chgsubColorData('step2-0', '#fcff00')"></li>
							<li style="background-color:#fd1979" onclick="chgsubColorData('step2-0', '#fd1979')"></li>
                            <li style="background-color:#6bff60" onclick="chgsubColorData('step2-0', '#6bff60')"></li>
                            <li style="background-color:#e12028" onclick="chgsubColorData('step2-0', '#e12028')"></li>
                            <li style="background-color:#0034df" onclick="chgsubColorData('step2-0', '#0034df')"></li>
                            <li style="background-color:#000000" onclick="chgsubColorData('step2-0', '#000000')"></li>
                            <li style="background-color:#333333" onclick="chgsubColorData('step2-0', '#333333')"></li>
						</ul>
					</li>
                    <li>
						<span><i class="xi-color-fill"></i> 배경 </span>
                        <input type="text" id="t_tit_bgcolor_step2-0" placeholder="예시)#ffffff" value=""  onkeyup="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '')">
						<ul class="sel_color_CT CT_bg">
							<li style="background-color:#333333" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                            <li style="background-color:#d9d9d9" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#d9d9d9')"></li>
							<li style="background-color:#ffffff" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                            <li style="background-color:#01508d" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#01508d')"></li>
                            <li style="background-color:#6299fe" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#6299fe')"></li>
                            <li style="background-color:#0138ed" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#0138ed')"></li>
                            <li style="background-color:#132062" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#132062')"></li>
                            <li style="background-color:#c138c1" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#c138c1')"></li>
                            <li style="background-color:#9032d2" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#9032d2')"></li>
                            <li style="background-color:#410098" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#410098')"></li>
                            <li style="background-color:#00aacf" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#00aacf')"></li>
                            <li style="background-color:#0050c7" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#0050c7')"></li>
                            <li style="background-color:#00acaa" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#00acaa')"></li>
                            <li style="background-color:#8794a3" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#8794a3')"></li>
                            <li style="background-color:#87a395" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#87a395')"></li>
                            <li style="background-color:#409834" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#409834')"></li>
                            <li style="background-color:#ceff0c" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ceff0c')"></li>
                            <li style="background-color:#a9d44f" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#a9d44f')"></li>
                            <li style="background-color:#005e3c" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#005e3c')"></li>
                            <li style="background-color:#008b6e" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#008b6e')"></li>
                            <li style="background-color:#299e00" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#299e00')"></li>
                            <li style="background-color:#59301a" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#59301a')"></li>
                            <li style="background-color:#f4eee2" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#f4eee2')"></li>
                            <li style="background-color:#71675d" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#71675d')"></li>
                            <li style="background-color:#a39b87" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#a39b87')"></li>
                            <li style="background-color:#d08e01" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#d08e01')"></li>
                            <li style="background-color:#fedc00" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#fedc00')"></li>
                            <li style="background-color:#ffa400" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ffa400')"></li>
                            <li style="background-color:#e9ab00" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#e9ab00')"></li>
                            <li style="background-color:#e7ca02" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#e7ca02')"></li>
                            <li style="background-color:#ff9400" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ff9400')"></li>
                            <li style="background-color:#f85333" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#f85333')"></li>
                            <li style="background-color:#ff1b41" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ff1b41')"></li>
                            <li style="background-color:#ff7800" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ff7800')"></li>
                            <li style="background-color:#f63756" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#f63756')"></li>
                            <li style="background-color:#ae0009" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#ae0009')"></li>
                            <li style="background-color:#9d1000" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#9d1000')"></li>
                            <li style="background-color:#d8282f" onclick="chgbgColorData('step2-0', 'goods_box-<?=$goods_box_cnt?>', '#d8282f')"></li>
						</ul>
					</li>
                </ul>
			</div>
            ...
            <? } ?>
            <button class="btn_badge_all" type="button" onclick="modal_all_badge_open('2', 'step2-0_')"><span class="material-icons">class</span> 뱃지 일괄적용</button>
            <button class="btn_badge_all order_all_x" type="button" onclick="all_noorder('goods_box-<?=$goods_box_cnt?>')" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>><span class="material-icons">remove_shopping_cart</span>주문X 일괄적용</button>
              <? if($mem_bigpos_yn == "Y"){ ?>
		      <span class="btn_pos_view">
        ...
        <div class="box_wrap gb_box" id="goods_list_step4-0" style="display:none;"><?//3단 이미지형 복사대상 ?>
          ...
          <div class="wl_img_goods" id="step4-0">
              <div class="tit_box tit_sel_corner_tit">
                ...
                <div class="use_choice">
                    <label class="uc_container"> 이미지 타이틀
                        <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="I" id="goods_box-<?=$goods_box_cnt?>_title_type_I" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step4-0', 'I');" checked >
                        <span class="checkmark"></span>
                    </label>
                    <label class="uc_container"> 커스텀 타이틀
                        <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="C" id="goods_box-<?=$goods_box_cnt?>_title_type_C" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step4-0', 'C')" >
                        <span class="checkmark"></span>
                    </label>
                </div>
                 <input type="hidden" id="tit_pre_select_step4-0">
            </div>
            <!--타이틀 이미지가 들어갈 공간-->
            <div class="list_title_img">
              <ul>
					...
					foreach($design_title_custom_list_i as $r) { //타이틀 이미지
						if($r->tit_type_self != "S"){ //타이틀이미지 직접입력 체크
                           ...
                           if(!empty($r->tit_imgpath)&&$r->tit_imgpath != ""){
                            ...
                            <li class="<?=($r->tit_type=="C")? "tit_custom_y" : "tit_custom_n" ?>" <?=($r->tit_type!="C")? "" : "style='display:none;'" ?>><label class="uc_container">
                        <? if($r->tit_type=="C"){ ?>
							  <input type="radio" name="tit_id_step4-0" id="tit_id_step4-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step4-0', '<div><span style=\'font-size:61px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>\'><?=$r->tit_text_info?></span></div><img src=\'<?=$r->tit_imgpath?>\'>' ,'<?=$r->tit_text_info?>','<?=$r->tit_bgcolor?>','<?=$r->tit_txcolor?>','<?=$r->tit_align?>');">
                            <? }else{ ?>
                              <input type="radio" name="tit_id_step4-0" id="tit_id_step4-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step4-0', '<img src=\'<?=$r->tit_imgpath?>\'>','<?=$r->tit_text_info?>');"<? if($dti == 1){ ?> checked<? } ?>>
                            <? } ?>
            ...
            <div class="tit_box2">
    		  ...
    		  <ul class="tit_img_menu">
    				...
    				<li><span><i class="xi-bars"></i> 스마트전단 메뉴명</span> <input type="text" id="t_tit_text_info_step4-0" placeholder="예시)야채코너" value="<?=$tit_text_info_first_i?>" onkeyup="chgCustomData('step4-0', 'goods_box-<?=$goods_box_cnt?>', this)"></li>
    				<li><span><i class="xi-camera-o"></i> 타이틀이미지 직접입력</span>
                         ...
                         <input type="hidden" id="tit_id_custom_yn_step4-0" value="I">
    				</li>
    		   </ul>
               <ul class="tit_custom_plus_menu">

                   <li id="plus_menu_step4-0" style='display:none;'>
                       <span><i class="xi-text-color"></i> 글꼴 </span>
                       <input type="text" id="t_tit_txcolor_step4-0" placeholder="예시)#333333" value="<?=$r->tit_txcolor?>"  onkeyup="chgtxColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '')">
                       <ul class="sel_color_CT">
                           <li style="background-color:#333333" onclick="chgtxColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                           <li style="background-color:#ffffff" onclick="chgtxColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                           <li style="background-color:#da281c" onclick="chgtxColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
                           <li style="background-color:#2c53d2" onclick="chgtxColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
                           <li style="background-color:#ff5a00" onclick="chgtxColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li>
                       </ul>
                       <input type="hidden" id="tit_id_custom_align_step4-0" value="<?=$r->tit_align?>">
                       <input type="hidden" id="tit_id_custom_txcolor_step4-0" value="<?=$r->tit_txcolor?>">
                       <input type="hidden" id="tit_id_custom_bgcolor_step4-0" value="<?=$r->tit_bgcolor?>">
                   </li>
                   <li>
                       <span><i class="xi-comment-o"></i> 부제목 </span>
                       <input type="text" id="t_sub_txt_step4-0" placeholder="예시)12.01~12.21" value="<?=$r->sub_title?>"  onkeyup="chgsubTxtData('step4-0')">
                       <span class="counter_minus button" onclick="subt_size_change('minus','step4-0');">-</span>
                       <span class="counter_plus button" onclick="subt_size_change('plus','step4-0');">+</span>
                       <input type="text" id="t_sub_txcolor_step4-0" placeholder="예시)#333333" value="<?=$r->sub_title_color?>"  onkeyup="chgsubColorData('step4-0', '')">
                       <ul class="sel_color_CT">
                           <li style="background-color:#ffffff" onclick="chgsubColorData('step4-0', '#ffffff')"></li>
                           <li style="background-color:#fcff00" onclick="chgsubColorData('step4-0', '#fcff00')"></li>
                           <li style="background-color:#fd1979" onclick="chgsubColorData('step4-0', '#fd1979')"></li>
                           <li style="background-color:#6bff60" onclick="chgsubColorData('step4-0', '#6bff60')"></li>
                           <li style="background-color:#e12028" onclick="chgsubColorData('step4-0', '#e12028')"></li>
                           <li style="background-color:#0034df" onclick="chgsubColorData('step4-0', '#0034df')"></li>
                           <li style="background-color:#000000" onclick="chgsubColorData('step4-0', '#000000')"></li>
                           <li style="background-color:#333333" onclick="chgsubColorData('step4-0', '#333333')"></li>
                       </ul>
                   </li>
                   <li>
                       <span><i class="xi-color-fill"></i> 배경 </span>
                       <input type="text" id="t_tit_bgcolor_step4-0" placeholder="예시)#ffffff" value=""  onkeyup="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '')">
                       <ul class="sel_color_CT CT_bg">
                           <li style="background-color:#333333" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                           <li style="background-color:#d9d9d9" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#d9d9d9')"></li>
                           <li style="background-color:#ffffff" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                           <li style="background-color:#01508d" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#01508d')"></li>
                           <li style="background-color:#6299fe" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#6299fe')"></li>
                           <li style="background-color:#0138ed" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#0138ed')"></li>
                           <li style="background-color:#132062" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#132062')"></li>
                           <li style="background-color:#c138c1" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#c138c1')"></li>
                           <li style="background-color:#9032d2" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#9032d2')"></li>
                           <li style="background-color:#410098" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#410098')"></li>
                           <li style="background-color:#00aacf" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#00aacf')"></li>
                           <li style="background-color:#0050c7" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#0050c7')"></li>
                           <li style="background-color:#00acaa" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#00acaa')"></li>
                           <li style="background-color:#8794a3" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#8794a3')"></li>
                           <li style="background-color:#87a395" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#87a395')"></li>
                           <li style="background-color:#409834" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#409834')"></li>
                           <li style="background-color:#ceff0c" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ceff0c')"></li>
                           <li style="background-color:#a9d44f" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#a9d44f')"></li>
                           <li style="background-color:#005e3c" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#005e3c')"></li>
                           <li style="background-color:#008b6e" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#008b6e')"></li>
                           <li style="background-color:#299e00" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#299e00')"></li>
                           <li style="background-color:#59301a" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#59301a')"></li>
                           <li style="background-color:#f4eee2" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#f4eee2')"></li>
                           <li style="background-color:#71675d" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#71675d')"></li>
                           <li style="background-color:#a39b87" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#a39b87')"></li>
                           <li style="background-color:#d08e01" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#d08e01')"></li>
                           <li style="background-color:#fedc00" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#fedc00')"></li>
                           <li style="background-color:#ffa400" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ffa400')"></li>
                           <li style="background-color:#e9ab00" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#e9ab00')"></li>
                           <li style="background-color:#e7ca02" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#e7ca02')"></li>
                           <li style="background-color:#ff9400" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ff9400')"></li>
                           <li style="background-color:#f85333" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#f85333')"></li>
                           <li style="background-color:#ff1b41" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ff1b41')"></li>
                           <li style="background-color:#ff7800" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ff7800')"></li>
                           <li style="background-color:#f63756" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#f63756')"></li>
                           <li style="background-color:#ae0009" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#ae0009')"></li>
                           <li style="background-color:#9d1000" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#9d1000')"></li>
                           <li style="background-color:#d8282f" onclick="chgbgColorData('step4-0', 'goods_box-<?=$goods_box_cnt?>', '#d8282f')"></li>
                       </ul>
                   </li>
               </ul>
            </div>
        ...
        <div class="box_wrap gb_box" id="goods_list_step5-0" style="display:none;"><?//4단 이미지형 복사대상 ?>
          ...
          <div class="wl_img_goods" id="step5-0">
              ...
              <div class="tit_box tit_sel_corner_tit">
                  ...
                  <div class="use_choice">
                      <label class="uc_container"> 이미지 타이틀
                          <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="I" id="goods_box-<?=$goods_box_cnt?>_title_type_I" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step5-0', 'I');" checked >
                          <span class="checkmark"></span>
                      </label>
                      <label class="uc_container"> 커스텀 타이틀
                          <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="C" id="goods_box-<?=$goods_box_cnt?>_title_type_C" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step5-0', 'C')" >
                          <span class="checkmark"></span>
                      </label>
                  </div>
                   <input type="hidden" id="tit_pre_select_step5-0">
              </div>
            <!--타이틀 이미지가 들어갈 공간-->
                ...
                <?
					...
					foreach($design_title_custom_list_i as $r) { //타이틀 이미지
						if($r->tit_type_self != "S"){ //타이틀이미지 직접입력 체크
                            ...
                            <li class="<?=($r->tit_type=="C")? "tit_custom_y" : "tit_custom_n" ?>" <?=($r->tit_type!="C")? "" : "style='display:none;'" ?>><label class="uc_container">
                                <? if($r->tit_type=="C"){ ?>
        							  <input type="radio" name="tit_id_step5-0" id="tit_id_step5-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step5-0', '<div><span style=\'font-size:61px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>\'><?=$r->tit_text_info?></span></div><img src=\'<?=$r->tit_imgpath?>\'>' ,'<?=$r->tit_text_info?>','<?=$r->tit_bgcolor?>','<?=$r->tit_txcolor?>','<?=$r->tit_align?>');">
                                    <? }else{ ?>
                                      <input type="radio" name="tit_id_step5-0" id="tit_id_step5-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step5-0', '<img src=\'<?=$r->tit_imgpath?>\'>','<?=$r->tit_text_info?>');"<? if($dti == 1){ ?> checked<? } ?>>
                                    <? } ?>
            ...
            <div class="tit_box2">
              <div class="tit">* 진열할 상품에 맞는 타이틀을 선택하세요~! 선택안함 또는 이미지 직접입력 선택시 스마트전단 메뉴로 등록될 타이틀명을 입력하셔야 합니다.</div>
							<ul class="tit_img_menu">
								...
								<li><span><i class="xi-bars"></i> 스마트전단 메뉴명</span> <input type="text" id="t_tit_text_info_step5-0" placeholder="예시)야채코너" value="<?=$tit_text_info_first_i?>" onkeyup="chgCustomData('step5-0', 'goods_box-<?=$goods_box_cnt?>', this)"></li>
								<li><span><i class="xi-camera-o"></i> 타이틀이미지 직접입력</span>
                                     ...
                                     <input type="hidden" id="tit_id_custom_yn_step5-0" value="I">
								</li>
							</ul>
                    <ul class="tit_custom_plus_menu">

                        <li id="plus_menu_step5-0" style='display:none;'>
							<span><i class="xi-text-color"></i> 글꼴 </span>
                            <input type="text" id="t_tit_txcolor_step5-0" placeholder="예시)#333333" value="<?=$r->tit_txcolor?>"  onkeyup="chgtxColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '')">
                            <ul class="sel_color_CT">
                                <li style="background-color:#333333" onclick="chgtxColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
								<li style="background-color:#ffffff" onclick="chgtxColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
								<li style="background-color:#da281c" onclick="chgtxColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
								<li style="background-color:#2c53d2" onclick="chgtxColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
								<li style="background-color:#ff5a00" onclick="chgtxColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li>
							</ul>
                            <input type="hidden" id="tit_id_custom_align_step5-0" value="<?=$r->tit_align?>">
                            <input type="hidden" id="tit_id_custom_txcolor_step5-0" value="<?=$r->tit_txcolor?>">
                            <input type="hidden" id="tit_id_custom_bgcolor_step5-0" value="<?=$r->tit_bgcolor?>">
						</li>
                        <li>
							<span><i class="xi-comment-o"></i> 부제목 </span>
                            <input type="text" id="t_sub_txt_step5-0" placeholder="예시)12.01~12.21" value="<?=$r->sub_title?>"  onkeyup="chgsubTxtData('step5-0')">
                            <span class="counter_minus button" onclick="subt_size_change('minus','step5-0');">-</span>
                            <span class="counter_plus button" onclick="subt_size_change('plus','step5-0');">+</span>
                            <input type="text" id="t_sub_txcolor_step5-0" placeholder="예시)#333333" value="<?=$r->sub_title_color?>"  onkeyup="chgsubColorData('step5-0', '')">
							<ul class="sel_color_CT">
								<li style="background-color:#ffffff" onclick="chgsubColorData('step5-0', '#ffffff')"></li>
                                <li style="background-color:#fcff00" onclick="chgsubColorData('step5-0', '#fcff00')"></li>
								<li style="background-color:#fd1979" onclick="chgsubColorData('step5-0', '#fd1979')"></li>
                                <li style="background-color:#6bff60" onclick="chgsubColorData('step5-0', '#6bff60')"></li>
                                <li style="background-color:#e12028" onclick="chgsubColorData('step5-0', '#e12028')"></li>
                                <li style="background-color:#0034df" onclick="chgsubColorData('step5-0', '#0034df')"></li>
                                <li style="background-color:#000000" onclick="chgsubColorData('step5-0', '#000000')"></li>
                                <li style="background-color:#333333" onclick="chgsubColorData('step5-0', '#333333')"></li>
							</ul>
						</li>
                        <li>
							<span><i class="xi-color-fill"></i> 배경 </span>
                            <input type="text" id="t_tit_bgcolor_step5-0" placeholder="예시)#ffffff" value=""  onkeyup="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '')">
							<ul class="sel_color_CT CT_bg">
								<li style="background-color:#333333" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                                <li style="background-color:#d9d9d9" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#d9d9d9')"></li>
								<li style="background-color:#ffffff" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                                <li style="background-color:#01508d" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#01508d')"></li>
                                <li style="background-color:#6299fe" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#6299fe')"></li>
                                <li style="background-color:#0138ed" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#0138ed')"></li>
                                <li style="background-color:#132062" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#132062')"></li>
                                <li style="background-color:#c138c1" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#c138c1')"></li>
                                <li style="background-color:#9032d2" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#9032d2')"></li>
                                <li style="background-color:#410098" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#410098')"></li>
                                <li style="background-color:#00aacf" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#00aacf')"></li>
                                <li style="background-color:#0050c7" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#0050c7')"></li>
                                <li style="background-color:#00acaa" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#00acaa')"></li>
                                <li style="background-color:#8794a3" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#8794a3')"></li>
                                <li style="background-color:#87a395" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#87a395')"></li>
                                <li style="background-color:#409834" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#409834')"></li>
                                <li style="background-color:#ceff0c" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ceff0c')"></li>
                                <li style="background-color:#a9d44f" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#a9d44f')"></li>
                                <li style="background-color:#005e3c" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#005e3c')"></li>
                                <li style="background-color:#008b6e" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#008b6e')"></li>
                                <li style="background-color:#299e00" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#299e00')"></li>
                                <li style="background-color:#59301a" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#59301a')"></li>
                                <li style="background-color:#f4eee2" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#f4eee2')"></li>
                                <li style="background-color:#71675d" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#71675d')"></li>
                                <li style="background-color:#a39b87" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#a39b87')"></li>
                                <li style="background-color:#d08e01" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#d08e01')"></li>
                                <li style="background-color:#fedc00" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#fedc00')"></li>
                                <li style="background-color:#ffa400" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ffa400')"></li>
                                <li style="background-color:#e9ab00" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#e9ab00')"></li>
                                <li style="background-color:#e7ca02" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#e7ca02')"></li>
                                <li style="background-color:#ff9400" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ff9400')"></li>
                                <li style="background-color:#f85333" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#f85333')"></li>
                                <li style="background-color:#ff1b41" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ff1b41')"></li>
                                <li style="background-color:#ff7800" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ff7800')"></li>
                                <li style="background-color:#f63756" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#f63756')"></li>
                                <li style="background-color:#ae0009" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#ae0009')"></li>
                                <li style="background-color:#9d1000" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#9d1000')"></li>
                                <li style="background-color:#d8282f" onclick="chgbgColorData('step5-0', 'goods_box-<?=$goods_box_cnt?>', '#d8282f')"></li>
							</ul>
						</li>
                    </ul>
            </div>
        ...
        <div class="box_wrap gb_box" id="goods_list_step3-0" style="display:none;"><?//텍스트형 복사대상 ?>
           ...
           <div class="tit_box tit_sel_corner_tit">
               ...
               <div class="use_choice">
                   <label class="uc_container"> 이미지 타이틀
                       <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="I" id="goods_box-<?=$goods_box_cnt?>_title_type_I" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step3-0', 'I');" checked >
                       <span class="checkmark"></span>
                   </label>
                   <label class="uc_container"> 커스텀 타이틀
                       <input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="C" id="goods_box-<?=$goods_box_cnt?>_title_type_C" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step3-0', 'C')" >
                       <span class="checkmark"></span>
                   </label>
               </div>
                <input type="hidden" id="tit_pre_select_step3-0">
           </div>
            <!--타이틀 이미지가 들어갈 공간-->
            <div class="list_title_img">
              <ul>
                ...
                <?
  					$dti = 0;
  					foreach($design_title_custom_list_t as $r) { //타이틀 이미지
							if($r->tit_type_self != "S"){ //타이틀이미지 직접입력 체크
	  						$dti++;
                    ...
                    <li class="<?=($r->tit_type=="C")? "tit_custom_y" : "tit_custom_n" ?>" <?=($r->tit_type!="C")? "" : "style='display:none;'" ?>><label class="uc_container">
                        <? if($r->tit_type=="C"){ ?>
							  <input type="radio" name="tit_id_step3-0" id="tit_id_step3-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step3-0', '<div><span style=\'font-size:61px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>\'><?=$r->tit_text_info?></span></div><img src=\'<?=$r->tit_imgpath?>\'>' ,'<?=$r->tit_text_info?>','<?=$r->tit_bgcolor?>','<?=$r->tit_txcolor?>','<?=$r->tit_align?>');">
                            <? }else{ ?>
                              <input type="radio" name="tit_id_step3-0" id="tit_id_step3-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step3-0', '<img src=\'<?=$r->tit_imgpath?>\'>','<?=$r->tit_text_info?>');"<? if($dti == 1){ ?> checked<? } ?>>
                            <? } ?>
            ...
            <div class="tit_box2">
              <div class="tit">* 진열할 상품에 맞는 타이틀을 선택하세요~! 선택안함 또는 이미지 직접입력 선택시 스마트전단 메뉴로 등록될 타이틀명을 입력하셔야 합니다.</div>
							<ul class="tit_img_menu">
								...
								<li><span><i class="xi-bars"></i> 스마트전단 메뉴명</span> <input type="text" id="t_tit_text_info_step3-0" placeholder="예시)야채코너" value="<?=$tit_text_info_first_t?>" onkeyup="chgCustomData('step3-0', 'goods_box-<?=$goods_box_cnt?>', this)"></li>
								<li><span><i class="xi-camera-o"></i> 타이틀이미지 직접입력</span>
                                     ...
                                     <input type="hidden" id="tit_id_custom_yn_step3-0" value="T">
								</li>
							</ul>

                    <ul class="tit_custom_plus_menu">

                        <li id="plus_menu_step3-0" style='display:none;'>
							<span><i class="xi-text-color"></i> 글꼴 </span>
                            <input type="text" id="t_tit_txcolor_step3-0" placeholder="예시)#333333" value="<?=$r->tit_txcolor?>"  onkeyup="chgtxColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '')">
                            <ul class="sel_color_CT">
                                <li style="background-color:#333333" onclick="chgtxColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
								<li style="background-color:#ffffff" onclick="chgtxColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
								<li style="background-color:#da281c" onclick="chgtxColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
								<li style="background-color:#2c53d2" onclick="chgtxColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
								<li style="background-color:#ff5a00" onclick="chgtxColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li>
							</ul>
                            <input type="hidden" id="tit_id_custom_align_step3-0" value="<?=$r->tit_align?>">
                            <input type="hidden" id="tit_id_custom_txcolor_step3-0" value="<?=$r->tit_txcolor?>">
                            <input type="hidden" id="tit_id_custom_bgcolor_step3-0" value="<?=$r->tit_bgcolor?>">
						</li>
                        <li>
							<span><i class="xi-comment-o"></i> 부제목 </span>
                            <input type="text" id="t_sub_txt_step3-0" placeholder="예시)12.01~12.21" value="<?=$r->sub_title?>"  onkeyup="chgsubTxtData('step3-0')">
                            <span class="counter_minus button" onclick="subt_size_change('minus','step3-0');">-</span>
                            <span class="counter_plus button" onclick="subt_size_change('plus','step3-0');">+</span>
                            <input type="text" id="t_sub_txcolor_step3-0" placeholder="예시)#333333" value="<?=$r->sub_title_color?>"  onkeyup="chgsubColorData('step3-0', '')">
							<ul class="sel_color_CT">
								<li style="background-color:#ffffff" onclick="chgsubColorData('step3-0', '#ffffff')"></li>
                                <li style="background-color:#fcff00" onclick="chgsubColorData('step3-0', '#fcff00')"></li>
								<li style="background-color:#fd1979" onclick="chgsubColorData('step3-0', '#fd1979')"></li>
                                <li style="background-color:#6bff60" onclick="chgsubColorData('step3-0', '#6bff60')"></li>
                                <li style="background-color:#e12028" onclick="chgsubColorData('step3-0', '#e12028')"></li>
                                <li style="background-color:#0034df" onclick="chgsubColorData('step3-0', '#0034df')"></li>
                                <li style="background-color:#000000" onclick="chgsubColorData('step3-0', '#000000')"></li>
                                <li style="background-color:#333333" onclick="chgsubColorData('step3-0', '#333333')"></li>
							</ul>
						</li>
                        <li>
							<span><i class="xi-color-fill"></i> 배경 </span>
                            <input type="text" id="t_tit_bgcolor_step3-0" placeholder="예시)#ffffff" value=""  onkeyup="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '')">
							<ul class="sel_color_CT CT_bg">
								<li style="background-color:#333333" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                                <li style="background-color:#d9d9d9" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#d9d9d9')"></li>
								<li style="background-color:#ffffff" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                                <li style="background-color:#01508d" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#01508d')"></li>
                                <li style="background-color:#6299fe" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#6299fe')"></li>
                                <li style="background-color:#0138ed" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#0138ed')"></li>
                                <li style="background-color:#132062" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#132062')"></li>
                                <li style="background-color:#c138c1" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#c138c1')"></li>
                                <li style="background-color:#9032d2" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#9032d2')"></li>
                                <li style="background-color:#410098" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#410098')"></li>
                                <li style="background-color:#00aacf" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#00aacf')"></li>
                                <li style="background-color:#0050c7" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#0050c7')"></li>
                                <li style="background-color:#00acaa" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#00acaa')"></li>
                                <li style="background-color:#8794a3" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#8794a3')"></li>
                                <li style="background-color:#87a395" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#87a395')"></li>
                                <li style="background-color:#409834" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#409834')"></li>
                                <li style="background-color:#ceff0c" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ceff0c')"></li>
                                <li style="background-color:#a9d44f" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#a9d44f')"></li>
                                <li style="background-color:#005e3c" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#005e3c')"></li>
                                <li style="background-color:#008b6e" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#008b6e')"></li>
                                <li style="background-color:#299e00" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#299e00')"></li>
                                <li style="background-color:#59301a" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#59301a')"></li>
                                <li style="background-color:#f4eee2" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#f4eee2')"></li>
                                <li style="background-color:#71675d" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#71675d')"></li>
                                <li style="background-color:#a39b87" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#a39b87')"></li>
                                <li style="background-color:#d08e01" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#d08e01')"></li>
                                <li style="background-color:#fedc00" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#fedc00')"></li>
                                <li style="background-color:#ffa400" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ffa400')"></li>
                                <li style="background-color:#e9ab00" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#e9ab00')"></li>
                                <li style="background-color:#e7ca02" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#e7ca02')"></li>
                                <li style="background-color:#ff9400" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ff9400')"></li>
                                <li style="background-color:#f85333" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#f85333')"></li>
                                <li style="background-color:#ff1b41" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ff1b41')"></li>
                                <li style="background-color:#ff7800" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ff7800')"></li>
                                <li style="background-color:#f63756" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#f63756')"></li>
                                <li style="background-color:#ae0009" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#ae0009')"></li>
                                <li style="background-color:#9d1000" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#9d1000')"></li>
                                <li style="background-color:#d8282f" onclick="chgbgColorData('step3-0', 'goods_box-<?=$goods_box_cnt?>', '#d8282f')"></li>
							</ul>
						</li>
                    </ul>
            </div>
	  ...
	  <div id="box_area"><?//박스 영역?>
        ...
        ?>
        <div id="goods_box-<?=$goods_box_cnt?>" class="gb_box"><?//이미지형?>
           ...
           <div class="wl_img_goods" id="step<?=$psg_step?>-<?=$seq?>">
            <!-- 커스텀 타이틀 선택 -->
  			<div class="tit_box tit_sel_corner_tit">
  				...
  				<div class="use_choice">
  					<label class="uc_container"> 이미지 타이틀
  						<input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="I" id="goods_box-<?=$goods_box_cnt?>_title_type_I" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step<?=$psg_step?>-<?=$seq?>', 'I');" <?=($r->tit_type!="C"||empty($r->tit_type))? "checked" : ""?>>
  						<span class="checkmark"></span>
  					</label>
  					<label class="uc_container"> 커스텀 타이틀
  						<input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="C" id="goods_box-<?=$goods_box_cnt?>_title_type_C" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step<?=$psg_step?>-<?=$seq?>', 'C')"  <?=($r->tit_type=="C")? "checked" : ""?>>
  						<span class="checkmark"></span>
  					</label>
  				</div>
                 <input type="hidden" id="tit_pre_select_step<?=$psg_step?>-<?=$seq?>">
  			</div>
              <!--타이틀 이미지가 들어갈 공간-->
              <div class="list_title_img">
                <ul>
                  ...
                  <?
                        ...
                        $tit_typeELSE = "";
                        if($r->tit_type=="C"){
                            $tit_typeELSE = " style='display:none;'";
                        }else{
                            $tit_typeC = " style='display:none;'";
                        }

						foreach($design_title_custom_list_i as $dt) { //타이틀 이미지
							if($dt->tit_type_self != "S"){ //타이틀이미지 직접입력 체크
                                    ...
                                    $align_text = "center";
                                    $padding_plus_text = "";
                                    if(!empty($dt->tit_align)){
                                        if($dt->tit_align=="L"){
                                            $align_text = "left";
                                            $padding_plus_text = "padding:0 21% 0 1%;";
                                        }else if($dt->tit_align=="C"){
                                            $align_text = "center";
                                        }else{
                                            $align_text = "right";
                                            $padding_plus_text = "padding:0 1% 0 21%;";
                                    ...
                                    $color_text = "#333333";
                                    if(!empty($dt->tit_txcolor)){
                                        $color_text = $dt->tit_txcolor;
                                    }

                                    if(!empty($dt->tit_imgpath)&&$dt->tit_imgpath != ""){
                                        $file_ext = substr(strrchr($dt->tit_imgpath, "."), 1);
                                        $fileNameWithoutExt = substr($dt->tit_imgpath, 0, strrpos($dt->tit_imgpath, "."));
                                        $findthumb = strripos($fileNameWithoutExt, '_thumb');
                                        //echo $fileNameWithoutExt."---".$file_ext;
                                        $plustext = "_thumb.";
                                        if($findthumb>0){
						  ...
						  ?>
		                  <li class="<?=($dt->tit_type=="C")? "tit_custom_y" : "tit_custom_n" ?>" <?=($dt->tit_type=="C")? $tit_typeC : $tit_typeELSE ?>>
		                    <label class="uc_container">
                              <? if($dt->tit_type=="C"){ ?>
							  <input type="radio" name="tit_id_step<?=$psg_step?>-<?=$seq?>" id="tit_id_step<?=$psg_step?>-<?=$seq?>_<?=$dti?>" value="<?=$dt->tit_id?>" onClick="areaSet('pre_tit_id_step<?=$psg_step?>-<?=$seq?>', '<div><span style=\'font-size:61px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>\'><?=$dt->tit_text_info?></span></div><img src=\'<?=$dt->tit_imgpath?>\'>' ,'<?=$dt->tit_text_info?>','<?=$dt->tit_bgcolor?>','<?=$dt->tit_txcolor?>','<?=$dt->tit_align?>');"<? if($r->psg_tit_id == $dt->tit_id){ $ex_tit_text_info = $dt->tit_text_info ?> checked<? } ?>>
                            <? }else{ ?>
                            ...
                            <? } ?>
		                      <span class="checkmark"></span><img id="tit_img_step<?=$psg_step?>-<?=$seq?>_<?=$dti?>" src="<?=$imgtitthumb?>">
		                    </label>
		                  </li>
		    			  <?
							}
              ...
              <!--//타이틀 이미지가 들어갈 공간-->
              <div class="tit_box2">
                  <? if($r->tit_type=="C"&&!empty($r->tit_type)){ ?>
                        <div class="tit">
                            * 진열할 상품에 맞는 타이틀용 배경이미지를 먼저 선택하세요~! 그리고 원하시는 타이틀을 직접 입력하고 색상, 위치를 선택하시면 됩니다.
                        </div>
                    <? }else{ ?>
                        <div class="tit">* 진열할 상품에 맞는 타이틀을 선택하세요~! 선택안함 또는 이미지 직접입력 선택시 스마트전단 메뉴로 등록될 타이틀명을 입력하셔야 합니다.</div>
                    <? } ?>
					...
					<?if(empty($r->pst_tit_text_info) and !empty($r->psg_tit_id) and empty($r->tit_type_self_s) ){
						...
						<li><span><i class="xi-bars"></i> 스마트전단 메뉴명</span> <input type="text" id="t_tit_text_info_step<?=$psg_step?>-<?=$seq?>" placeholder="예시)야채코너" value="<?=$pst_tit_text?>" onkeyup="chgCustomData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', this)"></li>
						<li><span><i class="xi-camera-o"></i> 타이틀이미지 직접입력</span>
                             ...
                             <input type="hidden" id="tit_id_custom_yn_step<?=$psg_step?>-<?=$seq?>" value="<?=(!empty($r->tit_type))? $r->tit_type : "T" ?>">
						</li>
					</ul>
                    ...
                    <ul class="tit_custom_plus_menu">
                        ...
                        <li id="plus_menu_step<?=$psg_step?>-<?=$seq?>" <?=($r->tit_type=="C")? "" : "style='display:none;'" ?>>
							<span><i class="xi-text-color"></i> 글꼴 </span>
                            <input type="text" id="t_tit_txcolor_step<?=$psg_step?>-<?=$seq?>" placeholder="예시)#333333" value="<?=$r->tit_txcolor?>"  onkeyup="chgtxColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '')">
                            <ul class="sel_color_CT">
                                <li style="background-color:#333333" onclick="chgtxColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
								<li style="background-color:#ffffff" onclick="chgtxColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
								<li style="background-color:#da281c" onclick="chgtxColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
								<li style="background-color:#2c53d2" onclick="chgtxColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
								<li style="background-color:#ff5a00" onclick="chgtxColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li>
							</ul>
                            <input type="hidden" id="tit_id_custom_align_step<?=$psg_step?>-<?=$seq?>" value="<?=$r->tit_align?>">
                            <input type="hidden" id="tit_id_custom_txcolor_step<?=$psg_step?>-<?=$seq?>" value="<?=$r->tit_txcolor?>">
                            <input type="hidden" id="tit_id_custom_bgcolor_step<?=$psg_step?>-<?=$seq?>" value="<?=$r->tit_bgcolor?>">
						</li>

                        <li>
							<span><i class="xi-comment-o"></i> 부제목 </span>
                            <input type="text" id="t_sub_txt_step<?=$psg_step?>-<?=$seq?>" placeholder="예시)12.01~12.21" value="<?=$r->sub_title?>"  onkeyup="chgsubTxtData('step<?=$psg_step?>-<?=$seq?>')">
                            <span class="counter_minus button" onclick="subt_size_change('minus','step<?=$psg_step?>-<?=$seq?>');">-</span>
                            <span class="counter_plus button" onclick="subt_size_change('plus','step<?=$psg_step?>-<?=$seq?>');">+</span>
                            <input type="text" id="t_sub_txcolor_step<?=$psg_step?>-<?=$seq?>" placeholder="예시)#333333" value="<?=$r->sub_title_color?>"  onkeyup="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '')">
							<ul class="sel_color_CT">
								<li style="background-color:#ffffff" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#ffffff')"></li>
                                <li style="background-color:#fcff00" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#fcff00')"></li>
								<li style="background-color:#fd1979" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#fd1979')"></li>
                                <li style="background-color:#6bff60" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#6bff60')"></li>
                                <li style="background-color:#e12028" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#e12028')"></li>
                                <li style="background-color:#0034df" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#0034df')"></li>
                                <li style="background-color:#000000" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#000000')"></li>
                                <li style="background-color:#333333" onclick="chgsubColorData('step<?=$psg_step?>-<?=$seq?>', '#333333')"></li>
							</ul>
						</li>

                        <li>
							<span><i class="xi-color-fill"></i> 배경 </span>
                            <input type="text" id="t_tit_bgcolor_step<?=$psg_step?>-<?=$seq?>" placeholder="예시)#ffffff" value="<?=$r->tit_bgcolor?>"  onkeyup="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '')">
							<ul class="sel_color_CT CT_bg">
								<li style="background-color:#333333" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                                <li style="background-color:#d9d9d9" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#d9d9d9')"></li>
								<li style="background-color:#ffffff" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                                <li style="background-color:#01508d" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#01508d')"></li>
                                <li style="background-color:#6299fe" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#6299fe')"></li>
                                <li style="background-color:#0138ed" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#0138ed')"></li>
                                <li style="background-color:#132062" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#132062')"></li>
                                <li style="background-color:#c138c1" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#c138c1')"></li>
                                <li style="background-color:#9032d2" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#9032d2')"></li>
                                <li style="background-color:#410098" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#410098')"></li>
                                <li style="background-color:#00aacf" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#00aacf')"></li>
                                <li style="background-color:#0050c7" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#0050c7')"></li>
                                <li style="background-color:#00acaa" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#00acaa')"></li>
                                <li style="background-color:#8794a3" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#8794a3')"></li>
                                <li style="background-color:#87a395" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#87a395')"></li>
                                <li style="background-color:#409834" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#409834')"></li>
                                <li style="background-color:#ceff0c" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ceff0c')"></li>
                                <li style="background-color:#a9d44f" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#a9d44f')"></li>
                                <li style="background-color:#005e3c" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#005e3c')"></li>
                                <li style="background-color:#008b6e" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#008b6e')"></li>
                                <li style="background-color:#299e00" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#299e00')"></li>
                                <li style="background-color:#59301a" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#59301a')"></li>
                                <li style="background-color:#f4eee2" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#f4eee2')"></li>
                                <li style="background-color:#71675d" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#71675d')"></li>
                                <li style="background-color:#a39b87" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#a39b87')"></li>
                                <li style="background-color:#d08e01" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#d08e01')"></li>
                                <li style="background-color:#fedc00" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#fedc00')"></li>
                                <li style="background-color:#ffa400" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ffa400')"></li>
                                <li style="background-color:#e9ab00" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#e9ab00')"></li>
                                <li style="background-color:#e7ca02" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#e7ca02')"></li>
                                <li style="background-color:#ff9400" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff9400')"></li>
                                <li style="background-color:#f85333" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#f85333')"></li>
                                <li style="background-color:#ff1b41" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff1b41')"></li>
                                <li style="background-color:#ff7800" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff7800')"></li>
                                <li style="background-color:#f63756" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#f63756')"></li>
                                <li style="background-color:#ae0009" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ae0009')"></li>
                                <li style="background-color:#9d1000" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#9d1000')"></li>
                                <li style="background-color:#d8282f" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#d8282f')"></li>





								<!-- <li style="background-color:#da281c" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
								<li style="background-color:#2c53d2" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
								<li style="background-color:#ff5a00" onclick="chgbgColorData('step<?=$psg_step?>-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li> -->
							</ul>
						</li>
                    </ul>
              </div>
              ...
              <? } ?>
                <? if($psg_step < 3){ ?>
                    <button class="btn_badge_all badge_all<?=$goods_box_cnt?>" type="button" onclick="modal_all_badge_open('<?=$psg_step?>', 'step<?=$psg_step?>-<?=$seq?>_')"><span class="material-icons">class</span> 뱃지 일괄적용</button>
               <? }else{ ?>
                   <button style="display:none;" class="btn_badge_all badge_all<?=$goods_box_cnt?>" type="button" onclick="modal_all_badge_open('<?=$psg_step?>', 'step<?=$psg_step?>-<?=$seq?>_')"><span class="material-icons">class</span> 뱃지 일괄적용</button>
               <? } ?>
               <button class="btn_badge_all order_all_x" type="button" onclick="all_noorder('goods_box-<?=$goods_box_cnt?>')" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>><span class="material-icons">remove_shopping_cart</span>주문X 일괄적용</button>
                <? if($mem_bigpos_yn == "Y"){ ?>
                <span class="btn_pos_view">
              ...
              <section id="area_goods_step<?=$psg_step?>-<?=$seq?>">
                ...
                <dl id="step<?=$psg_step?>-<?=$seq?>_<?=$ii?>" class="dl_step<?=$psg_step?>-<?=$seq?>">
                  ...
                  <dd>
                    ...
                    <div class="order_sel" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>>
                        <input type="hidden" id="goods_noorder_step<?=$psg_step?>-<?=$seq?>_<?=$ii?>" name="hdn_goods_noorder_step<?=$psg_step?>-<?=$seq?>" value="<?=$r->psg_no_order?>">
                        <label class="checkbox_container">
                            <input type="checkbox" name="noorder_chk_step<?=$psg_step?>-<?=$seq?>" id="noorder_chk_step<?=$psg_step?>-<?=$seq?>_<?=$ii?>" onClick="chgGoodsData('step<?=$psg_step?>-<?=$seq?>_<?=$ii?>', '', this)" value="Y" <?=($r->psg_no_order == "Y") ? " checked" : ""?> >
                            <span class="checkmark"></span>
                        </label>
                    </div>
                    <span class="order_sel_text" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>>주문X</span>
        ...
        <?
						...
						if($r->psg_box_no != $chk_step_no){
							...
							$tit_tiers = "1단";
        ?>
        <div id="goods_box-<?=$goods_box_cnt?>" class="gb_box"><?//텍스트형?>
          <input type="hidden" name="goods_box_id" id="goods_box-<?=$goods_box_cnt?>_name" value="goods_box-<?=$goods_box_cnt?>" style="width:120px;">
          <input type="hidden" id="goods_box-<?=$goods_box_cnt?>_step" value="step<?=$psg_step?>" style="width:80px;">
          <input type="hidden" id="goods_box-<?=$goods_box_cnt?>_step_id" value="step<?=$psg_step?>-<?=$seq?>" style="width:80px;">
          <input type="hidden" id="goods_box-<?=$goods_box_cnt?>_step_no" value="<?=$seq?>" style="width:60px;">
          ...
          <div id="goods_list_step3-<?=$seq?>">
            ...
            <div class="wl_txt_goods" id="step3-<?=$seq?>">
     			...
     			<div class="tit_box tit_sel_corner_tit">
     				...
     				<div class="use_choice">
     					<label class="uc_container"> 이미지 타이틀
     						<input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="T" id="goods_box-<?=$goods_box_cnt?>_title_type_T" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step<?=$psg_step?>-<?=$seq?>', 'T');" <?=($r->tit_type!="C"||empty($r->tit_type))? "checked" : ""?>>
     						<span class="checkmark"></span>
     					</label>
     					<label class="uc_container"> 커스텀 타이틀
     						<input type="radio" name="goods_box-<?=$goods_box_cnt?>_title_type" value="C" id="goods_box-<?=$goods_box_cnt?>_title_type_C" onclick="goods_conner_type('goods_box-<?=$goods_box_cnt?>', 'step<?=$psg_step?>-<?=$seq?>', 'C')"  <?=($r->tit_type=="C")? "checked" : ""?>>
                    ...
                    <input type="hidden" id="tit_pre_select_step3-<?=$seq?>">
     			</div>
   			...
   			<!-- 이미지 타이틀 선택시 -->
   				...
   				<div class="list_title_img">
   					<ul>
   						...
   						foreach($design_title_custom_list_t as $dt) { //타이틀 이미지
   							if($dt->tit_type_self != "S"){ //타이틀이미지 직접입력 체크
   									$dti++;
   									if($r->psg_tit_id == $dt->tit_id and $dti > 14){
                                       ...
                                       // $plustext = "";
                                       // if($dt->tit_type=="C"){
                                       //     $plustext = '<div><span style=\'font-size:61px; text-align:'.$align_text.'; color:'.$color_text.';\'>타이틀명</span></div>';
                                       // }
                                       if(!empty($dt->tit_imgpath)&&$dt->tit_imgpath != ""){
                                           $file_ext = substr(strrchr($dt->tit_imgpath, "."), 1);
                                           $fileNameWithoutExt = substr($dt->tit_imgpath, 0, strrpos($dt->tit_imgpath, "."));
   						  ...
   						  <li class="<?=($dt->tit_type=="C")? "tit_custom_y" : "tit_custom_n" ?>" <?=($dt->tit_type=="C")? $tit_typeC : $tit_typeELSE ?>>
   							<label class="uc_container">
                                   <? if($dt->tit_type=="C"){ ?>
   							  <input type="radio" name="tit_id_step3-<?=$seq?>" id="tit_id_step3-<?=$seq?>_<?=$dti?>" value="<?=$dt->tit_id?>" onClick="areaSet('pre_tit_id_step3-<?=$seq?>', '<div><span style=\'font-size:61px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>\'><?=$dt->tit_text_info?></span></div><img src=\'<?=$dt->tit_imgpath?>\'>' ,'<?=$dt->tit_text_info?>','<?=$dt->tit_bgcolor?>','<?=$dt->tit_txcolor?>','<?=$dt->tit_align?>');"<? if($r->psg_tit_id == $dt->tit_id){ $ex_tit_text_info = $dt->tit_text_info ?> checked<? } ?>>
                               <? }else{ ?>
   				...
   				<div class="tit_box2">
   					...
   					<ul class="tit_img_menu">
   						...
   						<li><span><i class="xi-bars"></i> 스마트전단 메뉴명</span> <input type="text" id="t_tit_text_info_step3-<?=$seq?>" placeholder="예시)야채코너" value="<?=$pst_tit_text?>" onkeyup="chgCustomData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', this)"></li>
   						<li><span><i class="xi-camera-o"></i> 타이틀이미지 직접입력</span>
   							 <label for="titimg_self_file_step3-<?=$seq?>" class="templet_img_box" >
   								 <div id="div_preview_tit_step3-<?=$seq?>"><img id="tit_img_preview_step3-<?=$seq?>" style="display:block;width:100%"></div>
   							 </label>
   							<input type="file" title="타이틀 이미지 파일" id="titimg_self_file_step3-<?=$seq?>" onchange="titimg_self_img(this, 'step3-<?=$seq?>');" class="upload-hidden" accept=".jpg, .jepg, .png, .gif" style="display:none;">
                                ...
                                <input type="hidden" id="tit_id_custom_yn_step3-<?=$seq?>" value="<?=(!empty($r->tit_type))? $r->tit_type : "I" ?>">
   						</li>

   					</ul>
                       <ul class="tit_custom_plus_menu">

                           <li id="plus_menu_step3-<?=$seq?>" <?=($r->tit_type=="C")? "" : "style='display:none;'" ?>>
   							<span><i class="xi-text-color"></i> 글꼴 </span>
                               <input type="text" id="t_tit_txcolor_step3-<?=$seq?>" placeholder="예시)#333333" value="<?=$r->tit_txcolor?>"  onkeyup="chgtxColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '')">
                               <ul class="sel_color_CT">
                                   <li style="background-color:#333333" onclick="chgtxColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
   								<li style="background-color:#ffffff" onclick="chgtxColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
   								<li style="background-color:#da281c" onclick="chgtxColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
   								<li style="background-color:#2c53d2" onclick="chgtxColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
   								<li style="background-color:#ff5a00" onclick="chgtxColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li>
   							</ul>
                               <input type="hidden" id="tit_id_custom_align_step3-<?=$seq?>" value="<?=$r->tit_align?>">
                               <input type="hidden" id="tit_id_custom_txcolor_step3-<?=$seq?>" value="<?=$r->tit_txcolor?>">
                               <input type="hidden" id="tit_id_custom_bgcolor_step3-<?=$seq?>" value="<?=$r->tit_bgcolor?>">
   						</li>

                           <li>
   							<span><i class="xi-comment-o"></i> 부제목 </span>
                               <input type="text" id="t_sub_txt_step3-<?=$seq?>" placeholder="예시)12.01~12.21" value="<?=$r->sub_title?>"  onkeyup="chgsubTxtData('step3-<?=$seq?>')">
                               <span class="counter_minus button" onclick="subt_size_change('minus','step3-<?=$seq?>');">-</span>
                               <span class="counter_plus button" onclick="subt_size_change('plus','step3-<?=$seq?>');">+</span>
                               <input type="text" id="t_sub_txcolor_step3-<?=$seq?>" placeholder="예시)#333333" value="<?=$r->sub_title_color?>"  onkeyup="chgsubColorData('step3-<?=$seq?>', '')">
   							<ul class="sel_color_CT">
   								<li style="background-color:#ffffff" onclick="chgsubColorData('step3-<?=$seq?>', '#ffffff')"></li>
                                   <li style="background-color:#fcff00" onclick="chgsubColorData('step3-<?=$seq?>', '#fcff00')"></li>
   								<li style="background-color:#fd1979" onclick="chgsubColorData('step3-<?=$seq?>', '#fd1979')"></li>
                                   <li style="background-color:#6bff60" onclick="chgsubColorData('step3-<?=$seq?>', '#6bff60')"></li>
                                   <li style="background-color:#e12028" onclick="chgsubColorData('step3-<?=$seq?>', '#e12028')"></li>
                                   <li style="background-color:#0034df" onclick="chgsubColorData('step3-<?=$seq?>', '#0034df')"></li>
                                   <li style="background-color:#000000" onclick="chgsubColorData('step3-<?=$seq?>', '#000000')"></li>
                                   <li style="background-color:#333333" onclick="chgsubColorData('step3-<?=$seq?>', '#333333')"></li>
   							</ul>
   						</li>

                        <li>
                         <span><i class="xi-color-fill"></i> 배경 </span>
                            <input type="text" id="t_tit_bgcolor_step3-<?=$seq?>" placeholder="예시)#ffffff" value="<?=$r->tit_bgcolor?>"  onkeyup="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '')">
                         <ul class="sel_color_CT CT_bg">
                             <li style="background-color:#333333" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#333333')"></li>
                                <li style="background-color:#d9d9d9" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#d9d9d9')"></li>
                             <li style="background-color:#ffffff" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ffffff')"></li>
                                <li style="background-color:#01508d" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#01508d')"></li>
                                <li style="background-color:#6299fe" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#6299fe')"></li>
                                <li style="background-color:#0138ed" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#0138ed')"></li>
                                <li style="background-color:#132062" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#132062')"></li>
                                <li style="background-color:#c138c1" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#c138c1')"></li>
                                <li style="background-color:#9032d2" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#9032d2')"></li>
                                <li style="background-color:#410098" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#410098')"></li>
                                <li style="background-color:#00aacf" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#00aacf')"></li>
                                <li style="background-color:#0050c7" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#0050c7')"></li>
                                <li style="background-color:#00acaa" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#00acaa')"></li>
                                <li style="background-color:#8794a3" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#8794a3')"></li>
                                <li style="background-color:#87a395" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#87a395')"></li>
                                <li style="background-color:#409834" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#409834')"></li>
                                <li style="background-color:#ceff0c" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ceff0c')"></li>
                                <li style="background-color:#a9d44f" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#a9d44f')"></li>
                                <li style="background-color:#005e3c" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#005e3c')"></li>
                                <li style="background-color:#008b6e" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#008b6e')"></li>
                                <li style="background-color:#299e00" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#299e00')"></li>
                                <li style="background-color:#59301a" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#59301a')"></li>
                                <li style="background-color:#f4eee2" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#f4eee2')"></li>
                                <li style="background-color:#71675d" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#71675d')"></li>
                                <li style="background-color:#a39b87" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#a39b87')"></li>
                                <li style="background-color:#d08e01" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#d08e01')"></li>
                                <li style="background-color:#fedc00" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#fedc00')"></li>
                                <li style="background-color:#ffa400" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ffa400')"></li>
                                <li style="background-color:#e9ab00" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#e9ab00')"></li>
                                <li style="background-color:#e7ca02" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#e7ca02')"></li>
                                <li style="background-color:#ff9400" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff9400')"></li>
                                <li style="background-color:#f85333" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#f85333')"></li>
                                <li style="background-color:#ff1b41" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff1b41')"></li>
                                <li style="background-color:#ff7800" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff7800')"></li>
                                <li style="background-color:#f63756" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#f63756')"></li>
                                <li style="background-color:#ae0009" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ae0009')"></li>
                                <li style="background-color:#9d1000" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#9d1000')"></li>
                                <li style="background-color:#d8282f" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#d8282f')"></li>





                             <!-- <li style="background-color:#da281c" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#da281c')"></li>
                             <li style="background-color:#2c53d2" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#2c53d2')"></li>
                             <li style="background-color:#ff5a00" onclick="chgbgColorData('step3-<?=$seq?>', 'goods_box-<?=$goods_box_cnt?>', '#ff5a00')"></li> -->
                         </ul>
                     </li>
                    </ul>
              ...
              <ul class="txt_goods_list">
                <li>
                  ...
                  <span class="txt_price2">할인가</span>
                  <span class="txt_btn">삭제</span>
                  <span class="txt_soldout order_col_x" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>>주문X</span>
                ...
                <section id="area_goods_step3-<?=$seq?>">
                  ...
                  <li id="step3-<?=$seq?>_<?=$ii?>" class="dl_step3-<?=$seq?>">
                    <span class="txt_name"><input type="text" id="goods_name_step3-<?=$seq?>_<?=$ii?>" value="<?=$r->psg_name?>" onClick="chkGoodsData('step3-<?=$seq?>_<?=$ii?>', '', this);" onKeyup="chgGoodsData('step3-<?=$seq?>_<?=$ii?>', '', this)" placeholder="" class="int100_l search_goods_input"></span>
                    ...
                    <div class="txt_order_sel" <?=($mem_stmall_yn!="Y"||$psd_order_yn == "N")? " style='display:none;'" : "" ?>>
						<label class="checkbox_container">
							<input type="hidden" id="goods_noorder_step3-<?=$seq?>_<?=$ii?>" name="hdn_goods_noorder_step3-<?=$seq?>" value="<?=$r->psg_no_order?>">
							<input type="checkbox" name="noorder_chk_step3-<?=$seq?>" id="noorder_chk_step3-<?=$seq?>_<?=$ii?>" onClick="chgGoodsData('step3-<?=$seq?>_<?=$ii?>', '', this)" value="Y" <?=($r->psg_no_order == "Y") ? " checked" : ""?>>
							<span class="checkmark"></span>
						</label>
					</div>
        ...
        <div id="goods_box-<?=$goods_box_cnt?>"><?//행사이미지?>
          ...
          <div class="box_wrap" id="goods_list_step9-<?=$seq?>">
            ...
            <div class="smart_add_images">
              <img id="goods_img_step9-0" src="<?=config_item('igenie_path').$r->psg_imgpath?>">
            </div>
          </div>
        </div><!--//goods_box-<?=$goods_box_cnt?>-->
        <?
      ...
      <div class="smart_btn_group sbg2">
        <span><a href="javascript:modal_search_open(1);"><i class="material-icons">search</i> 전체 상품 검색</a></span>
      ...
      <div class="wl_r_preview_n">
		      ...
		      <dl id="pre_step1_<?=$ii?>" onclick="$('#step1_<?=$ii?>').attr('tabindex', -1).focus();">
								<? //장바구니 버튼 품절처리 - 2021.06.17
									if($r->psg_stock == "N"){
										...
										if($mem_stmall_yn == "Y"){
                                            if($psd_order_yn == "Y"&&$r->psg_no_order!="Y"){
													$button_add_cart = "<button class='icon_add_cart' name='yesstock'>장바구니</button>";
											}else{
                                                $button_add_cart = "<button class='icon_add_cart' name='yesstock' style='display:none;'>장바구니</button>";
                                            }
										}
									}
								?>
		...
		<div class="pre_box_wrap" id="pre_box_sample">
           ...
           <div class="pre_box3" id="pre_goods_list_step1-0" style="display:none;">
              <div class="pre_tit tit_custom" id="pre_tit_id_step1-0">
  			    <img src="<?=$title_img_imgpath?>"><?//이미지형 타이틀 이미지 ?>
              </div>
              <div id="sub_title_text_step1-0" style="display:none;padding:5px;margin-top:10px;position : relative;text-align:center;font-size: 35px; color: #333333;font-weight:bold;"></div>
             <div class="pop_list_wrap" id="pre_area_goods_step1-0">
               ...
               <? for($ii = 0; $ii < $step1_std; $ii++){ ?>
                <div class="pop_list00" id="pre_step1-0_<?=$ii?>" onclick="$('#step1-0_<?=$ii?>').attr('tabindex', -1).focus();">
                 ...
                 <div class="pop_price">
                   <p class="price1" id="pre_goods_price_step1-0_<?=$ii?>">4,000원</p><?//이미지형 정상가?>
                   <p class="price2" id="pre_goods_dcprice_step1-0_<?=$ii?>">3,000원</p><?//이미지형 할인가?>
                 </div>
           ...
           <? //화면 미리보기 1단 이미지형 [E] ------------------------------------------------------------------------------------------------ ?>
			...
			<div class="pre_box3" id="pre_goods_list_step2-0" style="display:none;">
                <div class="pre_tit tit_custom" id="pre_tit_id_step2-0">
  			    <img src="<?=$title_img_imgpath?>"><?//이미지형 타이틀 이미지 ?>
              </div>
              <div id="sub_title_text_step2-0" style="display:none;padding:5px;margin-top:10px;position : relative;text-align:center;font-size: 35px; color: #333333;font-weight:bold;"></div>
			...
			<div class="pre_box3" id="pre_goods_list_step4-0" style="display:none;">
              <div class="pre_tit tit_custom" id="pre_tit_id_step4-0">
  			    <img src="<?=$title_img_imgpath?>"><?//이미지형 타이틀 이미지 ?>
              </div>
              <div id="sub_title_text_step4-0" style="display:none;padding:5px;margin-top:10px;position : relative;text-align:center;font-size: 35px; color: #333333;font-weight:bold;"></div>
			...
			<div class="pre_box3" id="pre_goods_list_step5-0" style="display:none;">
              <div class="pre_tit tit_custom" id="pre_tit_id_step5-0">
  			    <img src="<?=$title_img_imgpath?>"><?//이미지형 타이틀 이미지 ?>
              </div>
              <div id="sub_title_text_step5-0" style="display:none;padding:5px;margin-top:10px;position : relative;text-align:center;font-size: 35px; color: #333333;font-weight:bold;"></div>
			...
			<div class="pre_box4" id="pre_goods_list_step3-0" style="display:none;">
              <div class="pre_tit tit_custom" id="pre_tit_id_step3-0">
                  <img src="<?=$title_text_imgpath?>"><?//텍스트형 타이틀 이미지 ?>
              </div>
              <div id="sub_title_text_step3-0" style="display:none;padding:5px;margin-top:10px;position : relative;text-align:center;font-size: 35px; color: #333333;font-weight:bold;"></div>
        ...
        <div class="pre_box_wrap" id="pre_box_area">
					...
					foreach($screen_box as $r) {
                        ...
                        if($psg_step != "9"){
                            ...
                            $padding_plus_text = "";
                                ...
                                $len_text =  mb_strlen($r->pst_tit_text_info);
                                $txsize = 63;
                                if($len_text>5){
                                    $txsize = $txsize - (($txsize - 5) * 3);
                                    if($txsize<45){
                                        $txsize=45;
                                    }
                                }
                        ...
                        if($psg_step == "1"){ //1단 이미지형
							...
							if($r->psg_box_no != $chk_step_no){
								...
								$seq = $step1_seq; //코너번호
			?>
			<div class="pre_box3" id="pre_goods_box-<?=$box_seq?>" <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?> >
                <div class="pre_tit tit_custom" id="pre_tit_id_step1-<?=$seq?>" onclick="gb_box_view('<?=$box_seq?>');chkBoxData('step1-<?=$seq?>','<?=$box_seq?>', 'step1');"><? if($r->tit_type == "C"&&$r->tit_imgpath != ""){ ?><div><span style='font-size:<?=$txsize?>px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>'><?=$r->pst_tit_text_info?></span></div><? } ?><? if($r->tit_imgpath != ""){ ?><img src="<?=$r->tit_imgpath?>"><? } ?><?//이미지형 타이틀 이미지 ?>

                </div>
                <div id="sub_title_text_step1-<?=$seq?>" style="<?=(empty($r->sub_title))? "display:none;" : ""?>padding:5px;margin-top:10px;position : relative;text-align:center;font-size: <?=(!empty($r->sub_title_size))? $r->sub_title_size : "35px"?>; color: <?=(!empty($r->sub_title_color))? $r->sub_title_color : "#333333"?>;font-weight:bold;"><?=(!empty($r->sub_title))? $r->sub_title : ""?></div>
              ...
              <div class="btn_more_wrap" <?=($psd_mode=='L')? "'" : "style='display:none;" ?>>
			...
			?>
			<div class="pre_box3" id="pre_goods_box-<?=$box_seq?>" <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?> >
                <div class="pre_tit tit_custom" id="pre_tit_id_step2-<?=$seq?>" onclick="gb_box_view('<?=$box_seq?>');chkBoxData('step2-<?=$seq?>','<?=$box_seq?>', 'step2');"><? if($r->tit_type == "C"&&$r->tit_imgpath != ""){ ?><div><span style='font-size:<?=$txsize?>px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>'><?=$r->pst_tit_text_info?></span></div><? } ?><? if($r->tit_imgpath != ""){ ?><img src="<?=$r->tit_imgpath?>"><? } ?><?//이미지형 타이틀 이미지 ?></div>
                <div id="sub_title_text_step2-<?=$seq?>" style="<?=(empty($r->sub_title))? "display:none;" : ""?>padding:5px;margin-top:10px;position : relative;text-align:center;font-size: <?=(!empty($r->sub_title_size))? $r->sub_title_size : "35px"?>; color: <?=(!empty($r->sub_title_color))? $r->sub_title_color : "#333333"?>;font-weight:bold;"><?=(!empty($r->sub_title))? $r->sub_title : ""?></div>
			...
			?>
			<div class="pre_box3" id="pre_goods_box-<?=$box_seq?>" <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?> >
                <div class="pre_tit tit_custom" id="pre_tit_id_step4-<?=$seq?>" onclick="gb_box_view('<?=$box_seq?>');chkBoxData('step4-<?=$seq?>','<?=$box_seq?>', 'step4');"><? if($r->tit_type == "C"&&$r->tit_imgpath != ""){ ?><div><span style='font-size:<?=$txsize?>px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>'><?=$r->pst_tit_text_info?></span></div><? } ?><? if($r->tit_imgpath != ""){ ?><img src="<?=$r->tit_imgpath?>"><? } ?><?//이미지형 타이틀 이미지 ?></div>
               <div id="sub_title_text_step4-<?=$seq?>" style="<?=(empty($r->sub_title))? "display:none;" : ""?>padding:5px;margin-top:10px;position : relative;text-align:center;font-size: <?=(!empty($r->sub_title_size))? $r->sub_title_size : "35px"?>; color: <?=(!empty($r->sub_title_color))? $r->sub_title_color : "#333333"?>;font-weight:bold;"><?=(!empty($r->sub_title))? $r->sub_title : ""?></div>
			...
			?>
			<div class="pre_box3" id="pre_goods_box-<?=$box_seq?>" <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?> >
                <div class="pre_tit tit_custom" id="pre_tit_id_step5-<?=$seq?>" onclick="gb_box_view('<?=$box_seq?>');chkBoxData('step5-<?=$seq?>','<?=$box_seq?>', 'step5');"><? if($r->tit_type == "C"&&$r->tit_imgpath != ""){ ?><div><span style='font-size:<?=$txsize?>px; text-align:<?=$align_text?>; color:<?=$color_text?>;word-break: break-word;line-height:1;<?=$padding_plus_text?>'><?=$r->pst_tit_text_info?></span></div><? } ?><? if($r->tit_imgpath != ""){ ?><img src="<?=$r->tit_imgpath?>"><? } ?><?//이미지형 타이틀 이미지 ?></div>
                <div id="sub_title_text_step5-<?=$seq?>" style="<?=(empty($r->sub_title))? "display:none;" : ""?>padding:5px;margin-top:10px;position : relative;text-align:center;font-size: <?=(!empty($r->sub_title_size))? $r->sub_title_size : "35px"?>; color: <?=(!empty($r->sub_title_color))? $r->sub_title_color : "#333333"?>;font-weight:bold;"><?=(!empty($r->sub_title))? $r->sub_title : ""?></div>
			...
			<?
							...
							if($r->psg_box_no != $chk_step_no){
								...
								$seq = $step3_seq;
			?>
			<div class="pre_box4" id="pre_goods_box-<?=$box_seq?>" <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?> >
                <?
                  $align_text = "center";
                  if(!empty($r->tit_align)){
                      if($r->tit_align=="L"){
                          $align_text = "left";
                          $padding_plus_text = "padding-right:20%;";
                      }else if($r->tit_align=="C"){
                          $align_text = "center";
                      }else{
                          $align_text = "right";
                          $padding_plus_text = "padding-left:20%;";
                      }
                  }
                ...
                <div class="pre_tit tit_custom" id="pre_tit_id_step3-<?=$seq?>" onclick="gb_box_view('<?=$box_seq?>');chkBoxData('step3-<?=$seq?>','<?=$box_seq?>', 'step3');"><? if($r->tit_type=="C"&&$r->tit_imgpath != ""){?> <div><span style="font-size:<?=$txsize?>px; text-align:<?=$align_text?>; color:<?=$color_text?>;line-height:1;word-break: break-word;<?=$padding_plus_text?>"><?=$r->pst_tit_text_info?></span></div> <? } ?><? if($r->tit_imgpath != ""){ ?><img src="<?=$r->tit_imgpath?>"><? } ?><?//텍스트형 타이틀 이미지 ?></div>
                <div id="sub_title_text_step3-<?=$seq?>" style="<?=(empty($r->sub_title))? "display:none;" : ""?>padding:5px;margin-top:10px;position : relative;text-align:center;font-size: <?=(!empty($r->sub_title_size))? $r->sub_title_size : "35px"?>; color: <?=(!empty($r->sub_title_color))? $r->sub_title_color : "#333333"?>;font-weight:bold;"><?=(!empty($r->sub_title))? $r->sub_title : ""?></div>


                <div class="pop_list_wrap pre_list_view" id="pre_area_goods_step3-<?=$seq?>"  <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?>>
              <!-- <div class="pop_list_wrap" id="pre_area_goods_step3-<?=$seq?>" <? if(!empty($r->tit_bgcolor)){ ?> style='background-color:<?=$r->tit_bgcolor?>;' <? } ?>> -->
                <!--상품추가시 looping-->
                <?
							} //if($r->psg_step_no != $chk_step_no){
                ?>
			...
			<div class="pre_box4" id="pre_goods_box-<?=$box_seq?>">
              <p class="pre_tit" id="pre_tit_id_step9-<?=$seq?>" onclick="gb_box_view('<?=$box_seq?>');chkBoxData('step9-<?=$seq?>','<?=$box_seq?>', 'step9');"><? if($r->psg_imgpath != ""){ ?><img src="<?=config_item('igenie_path').$r->psg_imgpath?>"><? } ?><?//행사이미지 타이틀 이미지 ?></p>
		    </div>
			<?
						} //}else if($psg_step == "9"){ //행사이미지
					} //foreach($screen_box as $r) {
...
?>
    <div class="write_leaflets_btn">
      ...
      <button type="button" class="pop_btn_send" onClick="modal_search_open(1);"><span class="material-icons">search</span> 상품검색</button>
...
<!-- 이미지 템플릿선택 Modal -->
<div id="dh_myModal1" class="dh_modal templet_wrap">
...
<!-- 행사코너 순서변경 Modal -->
<div id="modal_sort" class="dh_modal modal_sort">
	<div class="modal-content">
...
<script>
    ...
    function goods_conner_type(box_id, step_id, type){
        var select_type_id = $(":input:radio[name="+box_id+"_title_type]:checked").val();
        var pre_select_id = $(":input:radio[name=tit_id_"+step_id+"]:checked").val();
        var pre_selection = $("#tit_pre_select_"+step_id).val();

        var pre_type = $("#tit_id_custom_yn_"+step_id).val();
        if(pre_type!=type){
            // $("#"+step_id).children('.list_title_img').html('');
            if(select_type_id=="C"){
                $("#"+step_id).find(".tit_custom_n").hide();
                $("#"+step_id).find(".tit_custom_y").show();
                $("#plus_menu_"+step_id).show();
            }else{
                $("#"+step_id).find(".tit_custom_y").hide();
                $("#"+step_id).find(".tit_custom_n").show();
                $("#plus_menu_"+step_id).hide();
            }
            $("#tit_id_custom_yn_"+step_id).val(type);
            if(pre_selection!=""){
                // $("input:radio[name=tit_id_"+step_id+"]:radio[value='"+pre_selection+"']").prop('checked', true);
                $("input:radio[name=tit_id_"+step_id+"]:radio[value='"+pre_selection+"']").click();
            }
            $("#tit_pre_select_"+step_id).val(pre_select_id);
        }

    }

    function subt_size_change(type, id){

		var currentSize = $("#sub_title_text_"+id).css("fontSize"); //변경대상 폰트사이즈를 알아낸다.
		...
		//alert("id : "+ id +"\n"+ "num : "+ num);
		var chkSize = 60; //1~20px 까지 조절 가능
		var increase = 5; // 증감폭
		if(type == "plus"){
			//console.log("sam_num : " + sam_num + " chkSize : " + chkSize + " num : " + num);
			if(60 > num){
				num+=increase;
			}
		} else if(type == "minus"){
        ...
        $("#sub_title_text_"+id).css("fontSize", num + unit);
	}
	//POP인쇄 클릭시
	// function popChk(obj){
	// 	//alert("pop_chk click");
  ...
  //       window.open("", pop_title);
	...
	function templet_list(tag_id, page){
		...
		$.ajax({
			...
			type: "POST",
			data: {"tag_txt":tag_id, "page":page, "<?=$this->security->get_csrf_token_name()?>":"<?=$this->security->get_csrf_hash()?>"},
			success: function (json) {
	...
	function modal_sort_open(page){
		...
		$("input[name=goods_box_id]").each(function(index, item){ //상품코너별 조회
			...
			if(goods_box_img == "" && goods_box_step != "step9"){
				html += "  <div class=\"move_img no_title\">타이틀이미지 사용안함</div>";
			}else{
				html += "  <div class=\"move_img\">"+ goods_box_img +"</div>";
			}
        ...
        $(".move_img").find('span').each(function(index, item){
            var title_text = $(item).text();
            if(title_text!=""){
                var leng_text = title_text.length;
                if(leng_text>5){

                    var conner_size = 40;
                    conner_size = conner_size - ((leng_text - 5) * 2);

                    if(conner_size>30){
                        $(item).css("font-size",conner_size+"px");
                    }else{
                        $(item).css("font-size","30px");
                    }

                }else{
                    $(item).css("font-size","40px");
                }
            }else{
                $(item).css("font-size","40px");
            }
    ...
    function conner_sort_move(type, id){
		...
		if(cnt > 1){
			...
			if(type == "first" || type == "last"){ //first.처음으로, last.마지막으로
				if(type == "first"){ //first.처음으로
                    $("#modal_sort_li_"+ id).prependTo("#modal_sort_list");
				...
				if(type == "last"){ //last.마지막으로
					// html += "<li id='modal_sort_li_"+ id +"'>"+ $("#modal_sort_li_"+ id).html() +"</li>";
                    $("#modal_sort_li_"+ id).appendTo("#modal_sort_list");
    ...
    function set_img_library(imgpath){
		...
		if(imgpath != ""){
            ...
            if(url.indexOf('_thumb')>0){
                fileurl = igenie_path + url;
            }else{
                fileurl = igenie_path + fileUrlName+'_thumb.'+fileExt;
            }
    ...
    function modal_search_run(){
        ...
        $(".search_goods_input").each(function(key, value){
            ...
            var txtflag = $(this).parent('span').hasClass('txt_name');
            // console.log(txtflag);
            // $(this).parent('span').hasClass('txt_name');
            ...
            if (thistxt.includes(search_text)) {
                ...
                if(txtflag==false){
                    ...
                    console.log("search_imgpath : "+search_imgpath);
                    ...
                    var fileUrlName = url.substring(0, lastDot);
                    console.log("fileUrlName : "+fileUrlName);
                    var fileExt = url.substring(lastDot+1, fileLength);
                    ...
                    var fileurl = "/dhn/images/leaflets/sample_img.jpg";
                }
                ...
                if(search_title_name == "대표할인상품"){
                    ...
                    var bid = $("#"+step).parent(".gb_box").attr("id");
                    if(bid==undefined){
                        bid = $("#goods_list_"+step).parent(".gb_box").attr("id");
                    }
                    // var bid = $("#goods_list_"+step).parent("div").attr("id");
                    console.log("bid : "+bid);
    ...
    //상품 데이타 변경시
	function chgCustomData(id, boxno, text) {
		// var stepId = id.substring(0, 5); //스텝 ID
		// //alert("stepId : "+ stepId +"\n"+ "id : "+ id);
		// var preId = "pre_"+ id;
        var i_type = $("#tit_id_custom_yn_"+id).val();

        if(i_type=="C"){
            var chgtext = $("#t_tit_text_info_"+id).val();

            $("#pre_tit_id_"+id).find('span').html(chgtext);
            $("#modal_sort_li_"+boxno).children('.move_img').find('span').html(chgtext);

            var leng_text = chgtext.length;
            if(leng_text>5){
                var pre_size = 61;
                var conner_size = 30;
                pre_size = pre_size - ((leng_text - 5) * 3);
                conner_size = conner_size - ((leng_text - 5) * 2);
                // console.log("leng_text : "+ leng_text + " pre_size : " + pre_size + " conner_size : "+ conner_size);
                if(pre_size>45){
                    $("#pre_tit_id_"+id).find('span').css("font-size", pre_size+"px");
                }else{
                    $("#pre_tit_id_"+id).find('span').css("font-size", "45px");
                }

                if(conner_size>22){
                    $("#modal_sort_li_"+boxno).children('.move_img').find('span').css("font-size",conner_size+"px");
                }else{
                    $("#modal_sort_li_"+boxno).children('.move_img').find('span').css("font-size","22px");
                }


            }else{
                $("#pre_tit_id_"+id).find('span').css("font-size", "61px");
                $("#modal_sort_li_"+boxno).children('.move_img').find('span').css("font-size","30px");
            }
        }
		//alert("preId : "+ preId);
	}
    ...
    //상품 데이타 변경시
	function chgbgColorData(id, boxno, color) {
		var stepId = id.substring(0, 5); //스텝 ID
		// //alert("stepId : "+ stepId +"\n"+ "id : "+ id);
		// var preId = "pre_"+ id;
        // var i_type = $("#tit_id_custom_yn_"+id).val();
        ...
        // if(i_type=="C"){
            var chgcolor = $("#t_tit_bgcolor_"+id).val();
            if(color!=""){
                chgcolor = color;
                $("#t_tit_bgcolor_"+id).val(color);
            }

            $("#pre_"+boxno).css("background-color", chgcolor);
            if(stepId=="step3"){
                $("#pre_"+boxno).find('.pop_list_wrap').css("background-color", chgcolor);
            }
            // $("#modal_sort_li_"+boxno).children('.move_img').find('span').css("background-color", chgcolor);
        // }
		//alert("preId : "+ preId);
	}

    //상품 데이타 변경시
	function chgtxColorData(id, boxno, color) {
		// var stepId = id.substring(0, 5); //스텝 ID
		// //alert("stepId : "+ stepId +"\n"+ "id : "+ id);
		// var preId = "pre_"+ id;
        var i_type = $("#tit_id_custom_yn_"+id).val();
        ...
        if(i_type=="C"){
            var chgcolor = $("#t_tit_txcolor_"+id).val();
            if(color!=""){
                chgcolor = color;
                $("#t_tit_txcolor_"+id).val(color);
            }

            // $("#pre_"+boxno).css("background-color", chgcolor);
            $("#pre_tit_id_"+id).find('span').css("color", chgcolor);
            $("#modal_sort_li_"+boxno).children('.move_img').find('span').css("color", chgcolor);
        }
		//alert("preId : "+ preId);
	}

    function chgsubTxtData(id){
        var con = $("#t_sub_txt_"+id).val();
        if(con){
            $("#sub_title_text_"+id).show();
            $("#sub_title_text_"+id).html(con);
        }else{
            $("#sub_title_text_"+id).hide();
        }

    }

    function chgsubColorData(id, color){
        var chgcolor = $("#t_sub_txcolor_"+id).val();
        if(color!=""){
            chgcolor = color;
            $("#t_sub_txcolor_"+id).val(color);
        }
        $("#sub_title_text_"+id).css("color", chgcolor);
    }

    function all_noorder(box_id){

        var s_id = $("#"+box_id+"_step_id").val();

        if(box_id=="goods_box-0"){
            s_id = "step1";
            box_id = "area_goods_step1";
        }

        var allchk = $("input:checkbox[name=noorder_chk_"+s_id+"]").length;
        var allchk_y = $("input:checkbox[name=noorder_chk_"+s_id+"]:checked").length;
        if(allchk>0){
            if(allchk==allchk_y){
                $("input:checkbox[name=noorder_chk_"+s_id+"]").prop("checked", false);
                $("input[name=hdn_goods_noorder_"+s_id+"]").val('N');
                $("#pre_"+box_id).find("button[name=yesstock]").show();
            }else{
                $("input:checkbox[name=noorder_chk_"+s_id+"]").prop("checked", true);
                $("input[name=hdn_goods_noorder_"+s_id+"]").val('Y');
                $("#pre_"+box_id).find("button[name=yesstock]").hide();
            }
        }
	...
	function chgGoodsData(id, fl, obj) {
        ...
        if($("input:checkbox[id='noorder_chk_"+id+"']").is(":checked") == true){
            ...
            if($('#pre_'+id).find('.icon_add_cart').attr('name')=="yesstock"){
                $('#pre_'+id).find('.icon_add_cart').hide();
            }
		}else{
		...
		if(goods_dcprice != ""){ //할인가 (숫자만 콤마 처리)
            ...
            if($("input:checkbox[id='noorder_chk_"+id+"']").is(":checked") == false){
                if(temp_price !=""){
                    goods_dcprice = str_price(goods_dcprice, stepId);
                }else{
				...
				if(isNaN(data)){ //문자가 포함되어 있는 경우
					...
					if(array_data.length > 1){
						if(array_data.length == 2){
                            ...
                            if(isNaN(array_data[array_data.length - 1])){

                            }else{
                                data = "<span class='sm_small'>"+str.trim()+"</span>" + comma(array_data[array_data.length - 1]);
                                if(unit!=""){
                                    if(step=="step1"){
                                        data =  data + "<span class='sm_small'>원</span>";
                                    }else{
                                        data =  data + unit;
                                    }
	...
	function imgCallback(){
		...
		if(request.readyState  == 4) {
			...
			if(obj.code == "0") { //성공
                ...
                $("#"+ set_div_preview_id).css({"background":"url(" + igenie_path + obj.imgpath + ")"});
				$("#pre_"+ set_div_preview_id).css({"background":"url(" + igenie_path + obj.imgpath + ")"});
			...
			success: function (json) {
				...
				if(json.code == "0") { //성공
					...
					if(json.flag == "lms"){ //문자메시지 발송
						...
						location.href = "/dhnbiz/sender/send/talk_adv_v4?psd_code="+ json.psd_code; //알림톡 발송 페이지
	...
	function goods_save(data_id, flag){
		...
		if(step1_yn_Y){ //STEP1 사용함
			...
			$('#area_goods_step1').children('dl').each(function(index, item){
                var gid = $(item).prop('id');
				var goods_id = $("#goods_id_"+ gid).val(); //상품번호
				var goods_step = $("#goods_step_"+ gid).val(); //스텝
				var goods_imgpath = $("#goods_imgpath_"+ gid).val(); //이미지경로
				...
				if($("#goods_name_"+ gid).length > 0){ //해당 라인이 존재 할 경우
					var pushData = {
                        ...
                        , "goods_noorder" : goods_noorder //주문불가
					};
					ui.push(pushData);
					cnt++;
		...
		if( cnt > 0 ) {
			...
			formData.append("<?=$this->security->get_csrf_token_name()?>", "<?=$this->security->get_csrf_hash()?>");
			$.ajax({
				url: "/spop/screen/screen_goods_arr_save_batch",
				type: "POST",
				data: formData,
				async: false,
				processData: false,
	...
	function goods_append(step, id, flag, first="N"){
		...
		// console.log("#"+ id +"_cnt : "+ no);
$("#"+ id +"_cnt").val(Number(no)+1); //상품 등록 상품수

		// console.log("no+1 :"+ id +"_cnt : "+$("#"+ id +"_cnt").val());
		//버그수정 시작 - 230908 21:25 윤재박
		if(flag == "new"){
			var eachtag = "dl";
			...
			var max_id = 0;
			$("#area_goods_"+id).children(eachtag).each(function(index, item){ //상품코너별 조회
					var sort_dl_id = $(item).prop('id'); //상품코너별 ID
					var sp = sort_dl_id.split("_");
					// console.log("sort_dl_id : "+ sort_dl_id);
					// console.log("pre_max_id : "+ max_id);
					// console.log("sp1 : "+ sp[1]);


					if(Number(max_id) < Number(sp[1])){
							max_id = sp[1];
					}

					// console.log("real_max_id : "+ max_id);

			});
			if(max_id!=0&&max_id>=Number(no)){
					$("#"+ id +"_cnt").val(Number(max_id)+2);
					no = Number(max_id)+1;
					// console.log("after cnt :"+ id +"_cnt : "+$("#"+ id +"_cnt").val());
					// console.log("after no :"+ no );
			}
		}
		// 버그수정 끝 - 230908 21:25 윤재박
		...
		if(step == "3"){ //3.1단 텍스트형
			...
			html += '  <span class="txt_name"><input type="text" id="goods_name_'+ id +'_'+ no +'" value="" onClick="chkGoodsData(\''+ id +'_'+ no +'\', \'\', this)" onKeyup="chgGoodsData(\''+ id +'_'+ no +'\', \'\', this)" placeholder="상품명" class="int100_l search_goods_input"></span>';
            ...
            html += '      <div class="order_sel">';
            html += '          <input type="hidden" id="goods_noorder_'+ id +'_'+ no +'" name="hdn_goods_noorder_'+ id +'" value="N">';
            html += '          <label class="checkbox_container">';
            html += '           <input type="checkbox" name="noorder_chk_'+ id +'" id="noorder_chk_'+ id +'_'+ no +'" onclick="chgGoodsData(\''+ id +'_'+ no +'\', \'\', this)" value="Y">';
            html += '           <span class="checkmark"></span>';
            html += '           </label>';
            html += '       </div>';
            html += '       <span class="order_sel_text">주문X</span>';

            html += '      <div class="soldout_box">';
            html += '          <label class="checkbox_container">';
            html += '          <input type="hidden" id="goods_soldout_'+ id +'_'+ no +'" value="Y">';
            html += '           <input type="checkbox" name="soldout_chk_'+ id +'" id="soldout_chk_'+ id +'_'+ no +'" onclick="chgGoodsData(\''+ id +'_'+ no +'\', \'\', this)" value="N">';
            html += '           <span class="checkmark"></span>';
            html += '           </label>';
            html += '       </div>';
            html += '       <span class="soldout_text">품절</span>';
    ...
    function goods_move(type, step, id){
        ...
        $("#area_goods_"+step).children('dl').each(function(index, item){ //상품코너별 조회
            var sort_dl_id = $(item).prop('id'); //상품코너별 ID
            $("#goods_seq_"+sort_dl_id).val(index);
        });
	...
	//영역에 값 넣기
	function areaSet(id, html, text, bgcolor='', txcolor='', txalign='C'){
        var s_id = id.substring(11);
		// alert("id : "+ id +" html : " + html + " text : " +text+ " s_id : "+ s_id);
        var i_type = $("#tit_id_custom_yn_"+s_id).val();
		...
		if(html ==''){
            ...
            if(bnid){
                ...
                $('#modal_sort_li_'+bnid).children(".move_img").find('span').css("font-size", "32px");
                if(bgcolor!=""){
                    $("#pre_"+bnid).css("background-color", bgcolor);
                    $("#t_tit_bgcolor_"+s_id).val(bgcolor);
                }else{
                    $("#pre_"+bnid).css("background-color", "");
                    $("#t_tit_bgcolor_"+s_id).val('');
                }

                if(i_type=="C"){

                    if(txcolor!=""){
                        $("#pre_tit_id_"+s_id).find('span').css("color", txcolor);
                        $("#modal_sort_li_"+bnid).children('.move_img').find('span').css("color", txcolor);
                        $("#t_tit_txcolor_"+s_id).val(txcolor);
                    }else{
                        $("#pre_tit_id_"+s_id).find('span').css("color", "#333333");
                        $("#modal_sort_li_"+bnid).children('.move_img').find('span').css("color","#333333");
                        $("#t_tit_txcolor_"+s_id).val('');
                    }
                    if(text!=""){
                        ...
                        if(leng_text>5){
                            ...
                            // console.log("leng_text : "+ leng_text + " pre_size : " + pre_size + " conner_size : "+ conner_size);
                            if(pre_size>45){
                                $("#pre_tit_id_"+s_id).find('span').css("font-size", pre_size+"px");
                            }else{
                                $("#pre_tit_id_"+s_id).find('span').css("font-size", "45px");
                            }

                            if(conner_size>22){
                                $("#modal_sort_li_"+bnid).children('.move_img').find('span').css("font-size",conner_size+"px");
                            }else{
                                $("#modal_sort_li_"+bnid).children('.move_img').find('span').css("font-size","22px");
                            }


                        }else{
                            $("#pre_tit_id_"+s_id).find('span').css("font-size", "61px");
                            $("#modal_sort_li_"+bnid).children('.move_img').find('span').css("font-size","30px");
                        }
                    }else{
                        $("#pre_tit_id_"+s_id).find('span').css("font-size", "61px");
                        $("#modal_sort_li_"+bnid).children('.move_img').find('span').css("font-size","30px");
                    }
                    $("#tit_id_custom_align_"+s_id).val(txalign);
                }
            }
	...
	function excelExport(input, step, id, first="N"){
		...
		if (ext == "xls" || ext == "xlsx") {
            ...
            var inconner = "";
            if(first=="N"){
                inconner = $("#t_tit_text_info_"+id).val();
            }
			var formData = new FormData();
			formData.append("file", file_data);
			formData.append("rtn_data", rtn_data);
            formData.append("inconner", inconner);
			formData.append("<?=$this->security->get_csrf_token_name()?>", "<?=$this->security->get_csrf_hash()?>");
			$.ajax({
				url: "/spop/screen/excel_upload_ok_neo",
				type: "POST",
				data: formData,
				processData: false,
				contentType: false,
				success: function (json) {
					...
					$.each(json, function(key, value){
                        ...
                        var goods_stock = value.stock;
                        var goods_noorder = value.noorder;
						...
						if(goods_name != undefined && goods_name != "" && goods_name != null){ //상품명이 있는 경우
                            ...
                            $("#goods_soldout_"+ stepId).val(goods_stock);
                            $("#goods_noorder_"+ stepId).val(goods_noorder);
                            if(goods_noorder=="Y"){
                                $("#noorder_chk_"+ stepId).attr("checked", true);
                            }
                            ...
                            if(goods_stock=="N"){
                                $("#soldout_chk_"+ stepId).attr("checked", true);
                            }
							...
							if(rtn_sno != "3" && goods_name != "" && goods_name != "상품명"){ //이미지형 상품등록
								...
								if(img_path != "" && img_path != null){
									img_img_path = igenie_path + img_path;
								}
								$("#goods_imgpath_"+ stepId).val(img_path); //이미지 경로
								$("#div_preview_"+ stepId).css({"background":"url(" + img_img_path + ")"});
								if(img_path == "" || img_path == null){
									img_path = "<?=$sample_imgpath?>";
									img_img_path = img_path;
								}
								//alert("img_path : "+ img_path);
								$("#pre_goods_imgpath_"+ stepId).html(img_path); //미리보기 이미지 경로
								$("#pre_div_preview_"+ stepId).css({"background":"url(" + img_img_path + ")"});
    ...
    function conner_download_new(data_id, conner_id, bno, flag){
		...
		if(flag == 'Y'){ //STEP1 사용함
			...
			for(var i=0; i < step1_cnt; i++){
                ...
                var goods_noorder = $("#goods_noorder_step1_"+ i).val();
		...
		if( cnt > 0 ) {
        	...
        	document.body.appendChild(form);
        	form.setAttribute("method", "post");
        	form.setAttribute("action", "/spop/screen/conner_download_new");
