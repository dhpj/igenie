  ...
  <div class="write_leaflets">
    <div class="wl_lbox">
	  ...
	  <span class="btn_imgtem">
        <button id="dh_myBtn" class="btn_myModal" onClick="templet_open(1);"><span class="material-icons">photo_filter</span> 이미지 템플릿선택</button>
      ...
      <div id="psd_order_option" class="order_option" style="display:<?=($mem_stmall_yn == "Y" and $psd_order_yn == "Y") ? "" : "none"?>">
        ...
        <span class="text"><span><font id="span_order_sdt"><?=($psd_order_sdt != "") ? $psd_order_sdt : date("Y").".00.00"?></font> ~ <font id="span_order_edt"><?=($psd_order_edt != "") ? $psd_order_edt : date("Y").".00.00"?></font></span> (설정된 기간동안 주문하기 기능이 활성화됩니다.)</span>
      </div>
	  <? //할인&대표상품 등록 [S] --------------------------------------------------------------------------------------------------------------------------------------- ?>
	  <div class="tit_box">
              ...
              <ul>
                ...
                <?
					...
					foreach($design_title_img_list as $r) { //타이틀 이미지
						...
						if($dti == 1) $title_img_imgpath = $r->tit_imgpath;
				?>
    			<li><label class="uc_container"><input type="radio" name="tit_id_step4-0" id="tit_id_step4-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step4-0', '<img src=\'<?=$r->tit_imgpath?>\'>');"<? if($dti == 1){ ?> checked<? } ?>><span class="checkmark"></span><img src="<?=$r->tit_imgpath?>"></label></li>
    			<?
					}
				?>
              </ul>
              ...
              <ul>
                ...
                <?
					...
					foreach($design_title_img_list as $r) { //타이틀 이미지
						...
						if($dti == 1) $title_img_imgpath = $r->tit_imgpath;
				?>
    			<li><label class="uc_container"><input type="radio" name="tit_id_step5-0" id="tit_id_step5-0_<?=$dti?>" value="<?=$r->tit_id?>" onClick="areaSet('pre_tit_id_step5-0', '<img src=\'<?=$r->tit_imgpath?>\'>');"<? if($dti == 1){ ?> checked<? } ?>><span class="checkmark"></span><img src="<?=$r->tit_imgpath?>"></label></li>
    			<?
					}
				?>
              </ul>
	  ...
	  <div id="box_area"><?//박스 영역?>
        ...
        <div id="goods_box-<?=$goods_box_cnt?>"><?//이미지형?>
          ...
          <div class="box_wrap" id="goods_list_step<?=$psg_step?>-<?=$seq?>">
            <div class="tit_box1">
              <div class="tit"><span id="goods_tit_tiers_step<?=$psg_step?>-<?=$seq?>"><?=$tit_tiers?></span> 이미지형 상품등록 <span id="goods_tit_step<?=$psg_step?>-<?=$seq?>"><?=$seq?></span></div>
            ...
            <div class="wl_img_goods" id="step<?=$psg_step?>-<?=$seq?>">
              ...
              <div class="list_title_img">
                <ul>
                  ...
                  <li>
                    <label class="uc_container">
                      <input type="radio" name="tit_id_step<?=$psg_step?>-<?=$seq?>" id="tit_id_step<?=$psg_step?>-<?=$seq?>_<?=$dti?>" value="<?=$dt->tit_id?>" onClick="areaSet('pre_tit_id_step<?=$psg_step?>-<?=$seq?>', '<img src=\'<?=$dt->tit_imgpath?>\'>');"<? if($r->psg_tit_id == $dt->tit_id){ ?> checked<? } ?>>
                ...
                <dl id="step<?=$psg_step?>-<?=$seq?>_<?=$ii?>" class="dl_step<?=$psg_step?>-<?=$seq?>">
                  <dt>
                    <div class="templet_img_in" onclick="showImg('step<?=$psg_step?>-<?=$seq?>_<?=$ii?>')">
                      <div id="div_preview_step<?=$psg_step?>-<?=$seq?>_<?=$ii?>" class="templet_img_in2" style="background-image : url('<?=$r->psg_imgpath?>');">
					...
					<button class="btn_good_badge" type="button" id="badgeBtn" onclick="modal_badge_open('<?=$psg_step?>', 'step<?=$psg_step?>-<?=$seq?>_<?=$ii?>')" style="display:<? if($psg_step != "2"){ ?>none<? } ?>;">행사뱃지 선택</button>
                  </dt>
                  <dd>
                    <ul>
                      ...
                      <? if($psg_step == "2"){ //2단 이미지형 ?>
					  <li><span>옵&nbsp;&nbsp;&nbsp;션</span><input type="text" id="goods_option2_step<?=$psg_step?>-<?=$seq?>_<?=$ii?>" value="<?=$r->psg_option2?>" onClick="chkGoodsData('step<?=$psg_step?>-<?=$seq?>_<?=$ii?>', '' , this);" onKeyup="chgGoodsData('step<?=$psg_step?>-<?=$seq?>_<?=$ii?>', '' , this)" placeholder="옵션" class="int140"></li>
					  <? }else{ ?>
            ...
            <section id="pre_area_goods_step1">
		      ...
		      <dl id="pre_step1_<?=$ii?>">
                ...
                <dt>
				  ...
				  <div id="pre_div_preview_step1_<?=$ii?>" class="templet_img_in3" style="background-image : url('<?=($r->psg_imgpath == "") ? $sample_imgpath : $r->psg_imgpath?>');">
        ...
        <div class="pre_box_wrap" id="pre_box_area">
			...
			<div class="pre_box3" id="pre_goods_box-<?=$box_seq?>">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step2-<?=$seq?>">
				...
				<div class="pop_list01" id="pre_step2-<?=$seq?>_<?=$ii?>">
				  ...
				  <div id="pre_div_preview_step2-<?=$seq?>_<?=$ii?>" class="templet_img_in3" style="background-image : url('<?=($r->psg_imgpath == "") ? $sample_imgpath : $r->psg_imgpath?>');">
			...
			<div class="pre_box3" id="pre_goods_box-<?=$box_seq?>">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step4-<?=$seq?>">
				...
				<div class="pop_list03" id="pre_step4-<?=$seq?>_<?=$ii?>">
				  ...
				  <div id="pre_div_preview_step4-<?=$seq?>_<?=$ii?>" class="templet_img_in3" style="background-image : url('<?=($r->psg_imgpath == "") ? $sample_imgpath : $r->psg_imgpath?>');">
				  ...
				  <div class="pop_price">
                    ...
                    <p class="price2" id="pre_goods_dcprice_step4-<?=$seq?>_<?=$ii?>"><?=$r->psg_dcprice?></p><?//이미지형 할인가?>
				  </div>
				  <div><span class="pop_option" id="pre_goods_option2_step4-<?=$seq?>_<?=$ii?>" style="display:<?=($r->psg_option2 != "") ? "" : "none"?>;"><?=$r->psg_option2?></span></div><?//이미지형 옵션?>
				  <div class="pop_name">
			...
			<div class="pre_box3" id="pre_goods_box-<?=$box_seq?>">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step5-<?=$seq?>">
				...
				<div class="pop_list04" id="pre_step5-<?=$seq?>_<?=$ii?>">
				  ...
				  <div id="pre_div_preview_step5-<?=$seq?>_<?=$ii?>" class="templet_img_in3" style="background-image : url('<?=($r->psg_imgpath == "") ? $sample_imgpath : $r->psg_imgpath?>');">
				  ...
				  <div class="pop_price">
                    ...
                    <p class="price2" id="pre_goods_dcprice_step5-<?=$seq?>_<?=$ii?>"><?=$r->psg_dcprice?></p><?//이미지형 할인가?>
				  </div>
				  <div><span class="pop_option" id="pre_goods_option2_step5-<?=$seq?>_<?=$ii?>" style="display:<?=($r->psg_option2 != "") ? "" : "none"?>;"><?=$r->psg_option2?></span></div><?//이미지형 옵션?>
				  <div class="pop_name">
			...
			<div class="pre_box4" id="pre_goods_box-<?=$box_seq?>">
              ...
              <div class="pop_list_wrap" id="pre_area_goods_step3-<?=$seq?>">
                ...
                ?>
                <div class="pop_list02 cartplus" id="pre_step3-<?=$seq?>_<?=$ii?>">
                  <div class="name">
...
<div id="dh_myModal1" class="dh_modal">
  <div class="modal-content">
	...
	<input type="file" title="행사이미지 파일" id="template_self_file" onchange="template_self_img(this);" class="upload-hidden" accept=".jpg, .jepg, .png, .gif" style="display:none;">
...
<script>
	...
	function popPrint(id){
		...
		if(pop_chk_cnt == 0){ //체그된 갯수가 없을 경우
			...
			pop_chk_cnt_sw = 1;
		}else if(pop_chk_cnt >= 1 && pop_chk_cnt <= 4){
