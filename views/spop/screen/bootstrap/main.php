<div class="wrap_leaflets">
  ...
  <div class="list_leaflets">
      <?
      $onoffy = "Y";
      if($this->member->item('mem_id') == '1554') {
          $onoffy = "N";
          $login_stack = $this->session->userdata('login_stack');
          if($this->session->userdata('login_stack')) {
              $login_stack = $this->session->userdata('login_stack');
              if($login_stack[0] == '3'||$login_stack[0] == '2'||$login_stack[0] == '962') {
                $onoffy = "Y";
             }
          }
      }?>
	...
	<ul>
	  ...
	  <li>
        ...
        <div class="ll_title"  >
          ...
          <button <?=($onoffy=="N")? " style='display:none;'" : "" ?> class="<?=($this->member->item('mem_level') >= 100) ? "cus_open2" : (($chk_test=="Y")? "cus_open3" : "cus_open")?> <?=($r->psd_open_yn=="N")? "cus_close_b" : "cus_open_b" ?>" onClick="psd_onoff('<?=$r->psd_id?>', '<?=($r->psd_open_yn=="N")? "Y" : "N" ?>')"><?=($r->psd_open_yn=="N")? "공개_OFF" : "공개_ON" ?></button>
