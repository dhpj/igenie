...
<style>
@media only print{
	.genie_container .contents_wrap{background: url(/dhn/images/leaflets/pop/pop_bg12.jpg) no-repeat bottom center; background-size: 100%;}
	.pop_ntype3_03{background:none;}
}
</style>
<div id="print_div">
  ...
  for($ii = 1; $ii <= $goods_cnt; $ii++){
    $number++;?>
<div class="pop_ntype3_03 <?=$style?>" id="screen<?=$number?>" onClick="modal_open(<?=$ii?>);">
  <p class="goods_sale">
    <span id="view_goods_tit<?=$ii?>" <?=empty($ppd_font_size->goods_tit)?"":'style="font-size:'.$ppd_font_size->goods_tit.'"'?>><?=($title_str[$ii] != "") ? $title_str[$ii] : $base_goods_tit?></span>
  </p>
