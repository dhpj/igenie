...
<?php
if(!empty($title->type3_09)) {
    $base_goods_tit = $title->type3_09;
}
?>
<div id="print_div">
  ...
  for($ii = 1; $ii <= $goods_cnt; $ii++){?>
    <? if($ii % 4 == 1){
      $number++;?>
<div class="pop_ntype3_13 <?=$style?>" id="screen<?=$number?>">
  <? } ?>
  ...
  <p class="goods_tit">
    <span id="view_goods_tit<?=$ii?>" <?=empty($goods_data[$ii][fontsize]->goods_tit)?'':'style="font-size:'.$goods_data[$ii][fontsize]->goods_tit.'"'?>><?=($title_str[$ii] != "") ? $title_str[$ii] : $base_goods_tit?></span><?//타이틀?>
  </p>
