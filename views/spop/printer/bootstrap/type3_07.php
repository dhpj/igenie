...
<div id="print_div">
  ...
  for($ii = 1; $ii <= $goods_cnt; $ii++){
    $number++;?>
<div class="pop_ntype3_07 <?=$style?>" id="screen<?=$number?>" onClick="modal_open(<?=$ii?>);">
    <p class="goods_tit">
        <?=$ppd_font_size->goods_tit?>
      <span id="view_goods_tit<?=$ii?>" <?=empty($goods_data[$ii][fontsize]->goods_tit)?'':'style="font-size:'.$goods_data[$ii][fontsize]->goods_tit.'"'?>><?=($ppd_title != "") ? $ppd_title : $base_goods_tit?></span><?//타이틀?>
    </p>
