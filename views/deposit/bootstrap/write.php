			...
			<table class="table_list">
				...
				<tr>
					...
					<td class="tl">
						...
						<button type="button" class="btn_myModal3" onClick="partner_open();">업체검색</button>
				...
				<tr>
					...
					<td class="tl">
                        ...
                        <?="현재 선충전 금액 : " . number_format($total_amt) . "원"?>
...
function form_check(){
    ...
    $.ajax({
         url: '/deposit/check_amount',
         type: "POST",
         data: {
               "<?=$this->security->get_csrf_token_name()?>": "<?=$this->security->get_csrf_hash()?>"
             , kind : $("#dep_kind").val()
             , dep_deposit : frm.dep_deposit.value
             , userid : frm.userid.value
         },
         success: function (json) {
             console.log(json.code);
             if (json.code == "NG"){
                 alert(json.msg + "\n" + "(선충전된 금액 : " + json.sum.replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") + "원)");
                 return;
             }
             if(frm.userid.value == ""){
                 alert("업체명을 선택하세요.");
...
<div id="partner_modal" class="dh_modal">
  <!-- Modal content -->
  <div class="modal-content3"> <span id="partner_close" class="dh_close">&times;</span>
