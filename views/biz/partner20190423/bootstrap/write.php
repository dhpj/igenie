...
<?if($this->member->item('mem_level')>=100) {?>                                
                                <tr>
                                    ...
                                    <td >
   										<input class="uniform" id="id_mem_payment_bank" name="mem_payment_bank" type="checkbox" <?=($rs->mem_payment_bank)?"checked":""?> />무통장입금
   										<input class="uniform" id="id_mem_payment_card" name="mem_payment_card" type="checkbox" <?=($rs->mem_payment_card)?"checked":""?> />신용카드
   										<input class="uniform" id="id_mem_payment_realtime" name="mem_payment_realtime" type="checkbox" <?=($rs->mem_payment_realtime)?"checked":""?> />계좌이체
   										<input class="uniform" id="id_mem_payment_vbank" name="mem_payment_vbank" type="checkbox" <?=($rs->mem_payment_vbank)?"checked":""?> />가상계좌
   										<input class="uniform" id="id_mem_payment_phone" name="mem_payment_phone" type="checkbox" <?=($rs->mem_payment_phone)?"checked":""?> />휴대폰결제
                                    </td>                             
								 </tr>
...
<?}?>
                            ...
                            <table class="tpl_ver_form" width="100%">
                                ...
                                <tr>
                                    ...
                                    <th<?=($rs->mem_level >= 100) ? " style='color:pink;'" : ""?>>015저가문자</th>
                                    <th<?=($rs->mem_level >= 100) ? " style='color:pink;'" : ""?>>폰문자</th>
                                ...
                                <tr>
                                    ...
                                    <td style="line-height:25pt;"><input class="form-control align-center inline price" id="id_price_nas" name="price_nas" value="<?=($param['key']) ? number_format($rs->mad_price_nas, 2) : number_format($this->Biz_model->base_price_nas, 2)?>" type="number" step="any" <?=($rs->mem_level >= 100) ? " style='color:pink;'" : ""?> min="<?=$parent_amt['mad_price_nas']?>"/></td>
                                    ...
                                    <td><input class="form-control align-center inline price" id="id_price_mms" name="price_mms"  value="<?=($param['key']) ? number_format($rs->mad_price_mms, 2) : number_format($this->Biz_model->base_price_mms, 2)?>" type="number" step="any" <?=($rs->mem_level >= 100) ? " style='color:pink;'" : ""?> min="<?=$parent_amt['mad_price_mms']?>"/></td>
                                </tr>
										  </tbody>
									 </table>
                            ...
                            <div class="mt30 align-center">
                                <a  href="/biz/partner/lists"><span class="btn">취소</span></a>&nbsp;
    ...
    <script type="text/javascript">
		...
		function methodChange() {
			...
			$.ajax({
			url: "/biz/partner/getMemberPriceJSON",
			type: "POST",
