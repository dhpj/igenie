...
// 	}
	...
	function all_send() {
		...
		if($("#templi_code").val() == null) {
			...
			$.ajax({
				url: "/biz/sender/talk/equal_send_mem_id",
				type: "POST",
				data: {<?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				...
				success: function (json) {
					...
					if (senderCheck == "F") {
	    				check = false;
	    				$(".content").html("친구모으기 알림톡 작성한 계정과 발송계정이 틀립니다.<br>발송계정 다시 로그인 후 다시 작성하세요.");
	    				$('#myModal').modal({backdrop: 'static'});
	    				$(document).unbind("keyup").keyup(function (e) {
	    					var code = e.which;
	    					if (code == 13) {
										...
										success: function (json) {
											...
											if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
												...
												//if (mem_pay_type === "A") {
													...
													content_msg += "발송 건수 : " + row_index.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").replace(/NaN/,'0') + "건<br><br>";
													if (ms!="" && ($('#lms').val().replace(/ /gi, "") != "") && $("#lms_select").is(":checked") == true) {	// 2차문자 발송
														content_msg += "예상 발송 금액 : "+ ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").replace(/NaN/,'0') + "원";
													} else {	// 2차문자 미발송
														content_msg += "예상 발송 금액 : "+ ((Number(at_price) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").replace(/NaN/,'0') + "원";
													}
										...
										success: function (json) {
											...
											if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
												...
												//if (mem_pay_type === "A") {
													...
													content_msg += "발송 건수 : " + row_index.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").replace(/NaN/,'0') + "건<br><br>";
													if (ms!="" && ($('#lms').val().replace(/ /gi, "") != "") && $("#lms_select").is(":checked") == true) {	// 2차문자 발송
														content_msg += "예상 발송 금액1 : "+ ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").replace(/NaN/,'0') + "원";
													} else {	// 2차문자 미발송
														content_msg += "예상 발송 금액2 : "+ ((Number(at_price) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").replace(/NaN/,'0') + "원";
													}
