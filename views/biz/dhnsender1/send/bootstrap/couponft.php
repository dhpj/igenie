...
// 	});
	...
	$(document).ready(function() {
		<?if((date('H') < '08' || date('H') > '19') && $this->member->item('mem_id') != '3') { ?>
		...
		$('#send_friend_content').html('').load(
			(document.location.href.indexOf("/sender/send/couponft") > -1) ? "/biz/sender/send/select_profile" : "",
			{ <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
			  "iscoupon" : "<?=$iscoupon?>"
					 },
			function() {
				if(document.location.href.indexOf("/sender/send/couponft") > -1) {
					...
					$("input[name=btn_url21]").val("http://#{url}");
					onPreviewText();
					onPreviewLms();
	...
	*/
	...
	function all_move() {
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) {		// upload 발송
			...
			file_data.append("couponid", "<?=$iscoupon?>");
			...
			$.ajax({
				...
				data: {
					...
					"mst_type1" : mst_type1, "mst_type2" : mst_type2, "couponid":"<?=$iscoupon?>"	// 2019.01.31 이수환 추가 ; mst_type1(1차 문자 = 카카오 관련 문자), mst_type2(2차 문자 = 카카오톡 외의 문자)
		...
		if ($("#customer_all_send").val() != undefined) { // 전체 고객 발송
			...
			$.ajax({
				...
				data: {
					...
					"btn1": btn1, "btn2": btn2, "btn3": btn3, "btn4": btn4, "btn5": btn5,
					"senderBox": senderBox, "smsOnly": smsOnly,
					"customer_all_count": customer_all_count,"customer_filter": customer_filter, "reserveDt": reserveDt, "couponid":"<?=$iscoupon?>"
				},
				beforeSend: function () {
					$('#overlay').fadeIn();
				},
			...
			$.ajax({
				...
				data: {
					...
					"pf_key": profile, "img_url": img_url, "img_link": img_link,
					"btn1": btn1, "btn2": btn2, "btn3": btn3, "btn4": btn4, "btn5": btn5,
					"smsOnly": smsOnly, "tel_number[]": tel_number, "reserveDt": reserveDt, "couponid":"<?=$iscoupon?>"
				},
				beforeSend: function () {
					$('#overlay').fadeIn();
				},
			...
			$("#friendtalk_table tr").each(function () {
				if($(this).find("#no").text()) {
					if ($(this).find("#no").text() != 1) {
						...
						var pf_yid = $("#pf_yid").val().replace(/[ ]*$/g, '');
						var pf_ynm = $("#pf_ynm").val();
						$("#btn_name2_"+current_btn_num).val("쿠폰받기");//pf_ynm);
						$("input[name=btn_url21]").val("http://#{url}");
						link_name(document.getElementById("btn_type"+current_btn_num),current_btn_num);
					}
				}
