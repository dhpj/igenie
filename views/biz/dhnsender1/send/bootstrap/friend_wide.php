...
// 	});
	...
	$(document).ready(function() {
		...
		$('#send_friend_content').html('').load(
			(document.location.href.indexOf("/sender/send/friend_wide") > -1) ? "/biz/sender/send/select_profile_wide" : "",
			{ <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>" },
			function() {
				if(document.location.href.indexOf("/sender/send/friend_wide") > -1) {
					...
					var wide_price = <?=$this->Biz_model->price_ft_w_img ?>;
					var base_wide_price = <?=$this->Biz_model->base_price_ft_w_img ?>;
					...
					if (wide_price <= 0 && wide_price < base_wide_price) {
						$(".content").html("친구톡 와이드 이미지를 발송할 수 없습니다.<br>관리자에게 문의하세요.");
						$('#myModal').modal({backdrop: 'static'});	
					}
	...
	function select_profile() {
		...
		if(activeTr.length == 0 ) {
			...
			$('#send_friend_content').html('').load(
				"/biz/sender/send/select_profile_wide",
				{
					<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
	...
	var ft_w_img_price = '<?=$this->Biz_model->price_ft_w_img ?>';
...
?>
	...
	function check_resend_method()
	{
		...
		<?/* 친구톡, 친구톡 이미지 가격 산정 */?>
		if ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != "" && $('#lms').val().replace(/ /gi, "") == "" && ft_w_img_price > charge_type) <?/* 친구톡 이미지 */?>
			{ charge_type = ft_w_img_price; }
...
// 					var kakao_img = Math.floor(Number(coin) / Number(ft_w_img_price));
...
// 	}
	...
	function all_send() {
		...
		if (ft_w_img_price <= 0 && ft_w_img_price < base_wide_price) {
			$(".content").html("친구톡 와이드 이미지를 발송할 수 없습니다.<br>관리자에게 문의하세요.");
			$('#myModal').modal({backdrop: 'static'});	
		} else if($("#pf_key").val()==null || $("#pf_key").val()==''){
			$(".content").html("프로필을 먼저 선택해주세요.");
			$('#myModal').modal({backdrop: 'static'});
			...
			$(".content").html("친구톡(와이드)는 반드시 와이드 이미지(800 x 600) 선택하셔야 합니다.");
			$('#myModal').modal({backdrop: 'static'});
			$('#myModal').on('hidden.bs.modal', function () {
				//$("#templi_cont").focus();
			});
		} else if ($("#img_url").val() != undefined && $("#img_url").val() != "" && templi_cont.length > 76) {
			$(".content").html("친구톡(와이드)의 경우 내용을 76자 이내로 입력하여야 합니다.");
			$('#myModal').modal({backdrop: 'static'});
			$('#myModal').on('hidden.bs.modal', function () {
				$("#templi_cont").focus();
			});
			...
			$.ajax({
				url: "/biz/sender/friend/equal_send_mem_id",
				type: "POST",
				data: {<?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				...
				success: function (json) {
					...
					if (senderCheck == "F") {
	    				check = false;
	    				$(".content").html("친구톡 와이드형 작성한 계정과 발송계정이 틀립니다.<br>발송계정 다시 로그인 후 다시 작성하세요.");
	    				$('#myModal').modal({backdrop: 'static'});
	    				$(document).unbind("keyup").keyup(function (e) {
	    					var code = e.which;
	    					if (code == 13) {
											...
											if (!$("#switch_upload").is(":checked")) {
												...
												if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
													...
													//if (mem_pay_type === "A") {
														...
														content_msg = "발송 종류 : 친구톡 와이드 이미지" + s2nd_msg + "<br><br>";
														...
														if (ms!="" && ($('#lms').val().replace(/ /gi, "") != "") && $("#lms_select").is(":checked") == true) {	// 2차문자 발송
															...
															content_msg += "예상 발송 금액 : "+ ((Number(ft_w_img_price) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원";
														}
														$(".content").html(limit_msg + content_msg + "<br/><br/>전체 발송하시겠습니까?");
													//} else {
    												//	var content_msg = ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != "") ? "친구톡 와이드 이미지 발송 가능 건수 : " + k_i + "건" : "친구톡 발송 가능 건수 : " + k + "건";
    												//	if(ms!="" && ($('#lms').val().replace(/ /gi, "") != "" || ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != ""))) {
    												//		content_msg += "<br/>" + "2차문자 발송 가능 건수 : " + resend_cnt + "건";
    												//	}
    												//	content_msg += ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != "") ? "<br/><br/>친구톡 와이드 이미지 발송 예상 금액 : " + (Number(ft_w_img_price) * Number(row_index)).toFixed(2) + " 원(" + row_index + " X " + ft_w_img_price + ")" : "<br/><br/>친구톡 발송 예상 금액 : " + (Number(ft_price) * Number(row_index)).toFixed(2) + " 원(" + row_index + " X " + ft_price + ")";
    												//	if(ms!="" && ($('#lms').val().replace(/ /gi, "") != "" || ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != ""))) {
												...
												if (Number(coin) - (Number(charge_type) * Number(upload_send)) < 0) {
													...
													//if (mem_pay_type === "A") {
														...
														if (ms!="" && ($('#lms').val().replace(/ /gi, "") != "") && $("#lms_select").is(":checked") == true) {	// 2차문자 발송
															...
															content_msg += "예상 발송 금액 : "+ ((Number(ft_w_img_price) * Number(upload_send)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원";
														}
    			    								...
    			    								//		content_msg += "<br/>" + "2차문자 발송 가능 건수 : " + resend_cnt + "건";
    			    								//	} 
    			    								//	content_msg += ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != "") ? "<br/><br/>친구톡 와이드 이미지 발송 예상 금액 : " + (Number(ft_w_img_price) * Number(upload_send)).toFixed(2) + " 원(" + upload_send + " X " + ft_w_img_price + ")" : "<br/><br/>친구톡 발송 예상 금액 : " + (Number(ft_price) * Number(upload_send)).toFixed(2) + " 원(" + upload_send + " X " + ft_price + ")";
    			    								//	if(ms!="" && ($('#lms').val().replace(/ /gi, "") != "" || ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != ""))) {
	...
	*/
	...
	function all_move() {
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) {		// upload 발송
			...
			$.ajaxSettings.traditional = true;
			$.ajax({
				url: "/biz/sender/friend/wide_all",
				type: "POST",
				data:file_data,
				processData: false,
				contentType: false,
...
//----------------------------------------------------------------------------------------------------------------
	...
	function imageSelect() {
		...
		imgSelectBox = window.open("/biz/sender/images_wide", 'tst', 'width=' + sw + ',height=' + sh + ',top=' + mt + ',left=' + ml + ',location=no, resizable=no');
	}
...
//		var image = "<img id='image' name='image' src='" + selctImgValue + "' style='width:100%; margin-bottom:5px;'/>";
		...
		document.getElementById('templi_length').innerHTML = span + "/76자";
...
if($mem->mem_2nd_send=='mms') { /* 2차발신이 mms인 경우 이미지가 없는 발신은 lms 단가로 계산 : resend_price 적용 */ ?>
						var total_price = (Number(ft_count) * Number(ft_price)) + (Number(ft_img_count) * Number(ft_w_img_price)) + (Number(mms_count) * Number(resend_price)) + ((Number(lms_count) - Number(mms_count)) * Number(lms_price));
<?} else { /* 2차발신이 mms가 아닌 경우 이미지 여부와 관계없이 금액 계산 : resend_price 적용 */ ?>
						var total_price = (Number(ft_count) * Number(ft_price)) + (Number(ft_img_count) * Number(ft_w_img_price)) + ((Number(lms_count) - Number(sms_count)) * Number(resend_price)) + (Number(sms_count) * Number(sms_price));
<?} ?>
