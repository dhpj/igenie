  ...
  <script src="/js/fancytree/jquery.fancytree.dnd5.js"></script>
						...
						<div class="inner_content preview_info">
    						...
    						<div class="input_content_wrap">
    									...
    									<div class="input_content">
    											<div class="msg_box">
    												...
    												<span class="txt_byte"><span class="info_desc">90bytes가 넘어가면 장문 문자로 변경됩니다.</span><span class="msg_type_txt sms" id="lms_limit_str"></span><span id="lms_num">0</span>/<span id="lms_character_limit">90</span> bytes</span>
    						...
    						<div class="input_content_wrap">
    							...
    							<div class="input_content">
									...
									<?if($this->member->item("mem_id") == '3') {?>	<input type="radio" name="customer" value="tree" id="switch_tree"/><label for="switch_tree" style="margin-left: -6px;">트리형 선택</label><?php }?>
    								    ...
    								    <div id="tree">
    										<div class="msg_box form_check">
    											...
    											<div class="bottom tr">
    												총 발송 <span class="num" id="groupSendTree">0</span>건
    											</div>
    										</div>    								    
    										...
    										$( document ).ready(function() {
        										$("#tree_view").fancytree({
        											checkbox: true,
        											selectMode: 3,
        										      source: { url: "/dhnbiz/customer/treeload",
        													type: "POST",
        											...
        											init: function(event, data) {
        										      ...
        										      lazyLoad: function(event, data) {
        										          data.result = { url: "/dhnbiz/customer/treeload",
        										  				type: "POST",
        										  				data: {<?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>", 
        										  					   "mem_id":<?=$this->member->item('mem_id')?>,
        										  					   "parent_id":data.node.data.id
        										  					   }
        										  		 };
        											...
        											loadChildren: function(event, ctx) {
        												...
        												$("#tree_view").fancytree("getTree").visit(function(n) {
            												//console.log("Visit :" , n);
            												if(n.data.parent_id != 0)
            													n.setTitle(n.data.group_name +  " ( " + n.data.tel_cnt + " )" );
        											...
        											select: function(event, data) {
        												...
        												var selKeys = $.map(data.tree.getSelectedNodes(), function(node){
            												if(node.data.parent_id != 0) {
            													selected = selected	+ Number(node.data.tel_cnt);
        														return node.data.id;
            												}
        												});

        												$("#tree_select").val(selKeys);
        												$("#groupSendTree").text(selected);
    										...
    										<input type="hidden" id="tree_select" value="">
    								    </div>    								    
    								    <div id="private">
    										    <div class="msg_box">
    												<div class="send_list">
			...
			<div class="clear modal-content">
				...
				<div class="modal-body select-body">
					...
					<div class="btn_wrap" align="center">
						<button type="button" class="btn btn-default" data-dismiss="modal" onclick="cancelClick();">취소</button>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div>
                        <p align="right">
                            <br/><br/>
                            <button type="button" class="btn btn-default dismiss" data-dismiss="modal" onclick="cancelClick();">취소</button>
...
// 	}
	...
	function all_send() {
		...
		//} 
    	...
    	} else if ($("#switch_tree").is(":checked") && parseInt($("#groupSendTree").text()) == 0) {	// 고객 DB에서 트리선택
    		if (parseInt($("#groupSendTree").text()) == 0) {
    			$(".content").html("고객 그룹을 선택하세요.");
    			$('#myModal').modal({backdrop: 'static'});
    		} 
			...
			$.ajax({
				url: "/dhnbiz/sender/lms/equal_send_mem_id",
				type: "POST",
				data: {<?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				...
				success: function (json) {
					...
					if (senderCheck == "F") {
	    				check = false;
	    				$(".content").html("문자 작성한 계정과 발송계정이 틀립니다.<br>발송계정 다시 로그인 후 다시 작성하세요.");
	    				$('#myModal').modal({backdrop: 'static'});
	    				$(document).unbind("keyup").keyup(function (e) {
	    					var code = e.which;
	    					if (code == 13) {
...
//													check_resend_method();
												...
												} else if ($("#switch_tree").is(":checked")) {	// 연락처 직접 입력
													row_index = parseInt($("#groupSendTree").text());
												...
												if (Number(coin) - (Number(send_price) * Number(row_index)) < 0) {
													...
													//if (mem_pay_type === "A") {
    													...
    													if ($("#smslms_kind").val() == "SMS") {
    														content_msg += "발송 종류 : 단문(SMS) 문자<br><br>";
    													} else if ($("#smslms_kind").val() == "LMS") {
    														content_msg += "발송 종류 : 장문(LMS) 문자<br><br>";
    													} else if ($("#smslms_kind").val() == "MMS") {
    														content_msg += "발송 종류 : MMS 문자<br><br> : ";
    													} else {
    														content_msg += "발송 종류 : ";
    													}														
														//content_msg = "발송 종류 : 알림톡" + s2nd_msg + "<br><br>";
														content_msg += "발송 시간 : " + send_time + "<br><br>";
														content_msg += "발송 건수 : " + row_index.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "건<br><br>";

    													if ($("#smslms_kind").val() == "SMS") {
    														content_msg += "예상 발송 금액 : "  + ((Number(send_price) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													} else if ($("#smslms_kind").val() == "LMS") {
    														content_msg += "예상 발송 금액 : "  + ((Number(send_price) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													} else if ($("#smslms_kind").val() == "MMS") {
    														content_msg += "예상 발송 금액 : "  + ((Number(send_price) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													} else {
    														content_msg += "예상 발송 금액 : "  + ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													}
												...
												if (Number(coin) - (Number(send_price) * Number(upload_send)) < 0) {
													...
													//if (mem_pay_type === "A") {
    													...
    													if ($("#smslms_kind").val() == "SMS") {
    														content_msg += "예상 발송 금액 : "  + ((Number(send_price) * Number(upload_send)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													} else if ($("#smslms_kind").val() == "LMS") {
    														content_msg += "예상 발송 금액 : "  + ((Number(send_price) * Number(upload_send)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													} else if ($("#smslms_kind").val() == "MMS") {
    														content_msg += "예상 발송 금액 : "  + ((Number(send_price) * Number(upload_send)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													} else {
    														content_msg += "예상 발송 금액 : "  + ((Number(charge_type) * Number(upload_send)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")  + " 원<br><br>";
    													}
...
//alert($("#switch_tree").is(":checked"));
//		alert(parseInt($("#groupSendTree").text()));
		var reserveDt = "00000000000000";
		if($("#reserve_check").is(":checked") == true) {
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) {		// upload 발송
		...
		} else if ($("#switch_tree").is(":checked") &&  parseInt($("#groupSendTree").text()) > 0) {	// 고객 DB에서 그룹 선택
			var customer_all_count = parseInt($("#groupSendTree").text());
			var customer_filter = $("#tree_select").val();
			var customer_filter_count = 0;
			var smslms_kind = $("#smslms_kind").val();
			var mem_2nd_send = $("#mem_2nd_send").val();
			...
			$.ajax({
				...
				data: {
					...
					"smsOnly": smsOnly,
					"customer_all_count": customer_all_count,
					"tree_customer_filter": customer_filter,
					"reserveDt": reserveDt, 
					"smslms_kind":smslms_kind, 
					"mem_2nd_send":mem_2nd_send,
					"mms_id":mms_id,
