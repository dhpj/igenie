...
<div class="input_content_wrap">
	...
	<div class="input_content">
		<input class="" id="id_cc_title" name="cc_title" style="width:100%;" value="<?=$rs->cc_title?>" <? echo ($rs->cc_status=='P')?"readonly":""; ?>>
	</div>
		...
		<div class="preview_msg_wrap">
			...
			<div class="preview_msg_window" style="height: auto;!important;">
				...
				<div class="draw" style="display:none;">
					<div class="preview_img_coupon"></div>
					<div class="preview_text_box">
						<h3 class="preview_title"><?=$rs->cc_title?></h3>
						<p class="preview_date"></p><?//사용기간?>
						<div id="btn_preview" class="btn_staff">친구추가</div>
						<div id="btn_preview"class="btn_staff">직원확인</div>
						<div class="preview_coupon"></div>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="msg_box">
			...
			<div class="input_msg" style="height: auto;">
				...
				while($m_cnt >= 1) {
					...
					$var_str = '<input name="var_name[]" id="var_name_'. $idx .'" class="var single" onkeyup="return view_preview();" onkeypress="if(event.keyCode==13) { event.preventDefault(); }" placeholder="'.str_replace("#", "", $pla).'" value="'.$var_value.'">';
...
<div class="input_content_wrap">
	...
	<div class="input_content">
		 <input type="text" class="calendar" readonly placeholder="시작날짜 선택"  id="id_cc_start_date" name="cc_start_date" value="<?=$rs->cc_start_date?>" <? echo ($rs->cc_status=='P')?"readonly":""; ?>>
		 <span style="line-height: 38px;">~</span>
		 <input type="text" class="calendar" readonly placeholder="종료날짜 선택" required id="id_cc_end_date" name="cc_end_date" onChange="show_pre_draw();" value="<?=$rs->cc_end_date?>" <? echo ($rs->cc_status=='P')?"readonly":""; ?>>
	</div>
</div>
<div class="input_content_wrap">
	<!--<label class="input_tit">쿠폰 번호 생성</label>-->
	<label class="input_tit">쿠폰 발급수</label>
	<div class="input_content">
		<div class="checks">
		<? if($rs->cc_status!='P') { ?>
...
<div class="input_content_wrap">
	...
	<div class="input_content">
		...
		<button class="btn md fr" type="button" onclick="image_list_open(1)" id="img_select"><i class="icon-folder-open"></i>이미지 선택</button>
		<? } ?>
		<input name="img_link" id="img_link" value="<?=$rs->cc_img_link?>" size="50" style="width:<? if($rs->cc_status!='P') { ?>300px;<? }else{ ?>100%;<? } ?>" readonly>
...
<div class="input_content_wrap" id="btn_0" style="display:none">
  ...
  <script type="text/javascript">
	...
	$("#id_cc_title").on('change keyup paste', function() { //쿠폰명 수정시
		show_pre_draw();
	});

	$("#id_cc_memo").on('change keyup paste', function() { //당첨페이지 메모 수정시
		show_pre_draw();
	});
	...
	function show_pre_draw() {
		var title = $("#id_cc_title").val(); //제목
		$(".preview_title").html(title);
		var end_date = $("#id_cc_end_date").val(); //종료일자
		$(".preview_date").html("사용기간 : "+ end_date +" 까지");
		var imglink = $("#img_link").val(); //이미지 주소
 	 	$(".preview_img_coupon").html("<img src='" + imglink + "'>");
 	 	var ccmemo = $("#id_cc_memo").val(); //당처메이지 메모
 	 	$(".preview_coupon").html(ccmemo.replace(/(\n|\r\n)/g, '<br>'));
 	}
</script>
