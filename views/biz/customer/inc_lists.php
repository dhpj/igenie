...
?>
<input type='hidden' id='inc_glv' name='inc_glv' value='<?=$glv?>'>
<input type='hidden' id='inc_gid' name='inc_gid' value='<?=$gid?>'>
<input type='hidden' id='inc_gnm1' name='inc_gnm1' value='<?=$gnm1?>'>
<input type='hidden' id='inc_gnm2' name='inc_gnm2' value='<?=$gnm2?>'>
<div class="cus_list_l">
  ...
  <ul id="myUL">
    <li>
      <span><i class="xi-folder-check"></i> <span<? if($customer_cnt > 0){ //등록수가 있는 경우 ?> onClick="group_search('', '', '', '');" style="cursor:pointer;"<? } //if($r->cg_cnt > 0){ //등록수가 있는 경우 ?> class="<?=($gid != "") ? "" : "group_on"?>">전체고객목록 <span class="cus_cnt"><?=$customer_cnt?></span></span></span>
      <ul class="nested active">
		<?
			$scnt = 0;
			$tot_gcnt1 = 0; //1그룹 전체등록수
			$org_parent_id = -1;
			foreach($customer_group as $r){
				$cg_gname = $r->cg_gname; //그룹명
				$class_gname = "";
				if($gid == $r->cg_id){ //선택된 그룹인 경우
					$class_gname = "group_on";
				}
				if($r->cg_level == 2){ //2레벨 그룹
					$gnm1 = $r->pname;
					$gnm2 = $cg_gname;
				}else{
					$gnm1 = $cg_gname;
				...
				if($org_parent_id != $r->cg_parent_id){
					$tot_gcnt1 = $tot_gcnt1 + $r->cg_cnt;
		?>
        <li>
        <? if($r->cg_level == 1 && $r->rowspan > 1){ //1차 그룹 등록수가 있는 경우 ?>
		  <span class="caret caret-down"><i class="xi-folder-check"></i> <span class="<?=$class_gname?>" onClick="group_search('<?=$r->cg_level?>', '<?=$r->cg_id?>', '<?=$gnm1?>', '<?=$gnm2?>');"><?=$cg_gname?> <span class="cus_cnt"><?=$r->cg_cnt?></span></span></span><?//1차 그룹?>
		  <? //if($r->rowspan > 1){ ?>
		  <ul class="nested active">
		  <? //} //if($r->rowspan > 1){ ?>
		<? }else if($r->cg_level == 1 && $r->rowspan == 1){ //if($r->cg_level == 1 && $r->rowspan > 1){ //1차 그룹 등록수가 있는 경우 ?>
		  <i class="xi-file-text-o"></i> <span class="<?=$class_gname?>" onClick="group_search('<?=$r->cg_level?>', '<?=$r->cg_id?>', '<?=$gnm1?>', '<?=$gnm2?>');" style="cursor:pointer;"><?=$cg_gname?> <span class="cus_cnt"><?=$r->cg_cnt?></span></span><?//1차 그룹?>
		<? } //if($r->cg_level == 1 && $r->rowspan > 1){ //1차 그룹 등록수가 있는 경우 ?>
        <?
				} //if($org_parent_id != $r->cg_parent_id){
				if($r->cg_level == 2){ //2차 그룹
		?>
		  <li><i class="xi-file-text-o"></i> <span class="<?=$class_gname?>" onClick="group_search('<?=$r->cg_level?>', '<?=$r->cg_id?>', '<?=$gnm1?>', '<?=$gnm2?>');" style="cursor:pointer;"><?=$cg_gname?> <span class="cus_cnt"><?=$r->cg_cnt?></span></span></li><?//2차 그룹?>
		<?
				} //if($r->cg_level == 2){ //2차 그룹
				$scnt++;
				if($scnt >= $r->rowspan){
		...
		?>
		<? if(!empty($customer_group) and $unclassified_cnt > 0){ //그룹이 있고 미분류가 있는 경우 ?>
		<i class="xi-file-text-o"></i> <span class="<?=($gid == "0") ? "group_on" : ""?>" onClick="group_search('1', '0', '미분류', '');" style="cursor:pointer;">미분류 <span class="cus_cnt"><?=$unclassified_cnt?></span></span><?//미분류?>
		<? } ?>
      </ul>
    </li>
  </ul>
...
<? if($this->member->item("mem_id") != 121){  ?>
	<? if($gid != ""){  ?>
		<button class="btn_st1" onclick="groupDelRow('<?=$gid?>');"><span class="material-icons">error_outline</span> 소속그룹 고객 삭제</button>
	<? }else{ ?>
...
<div id="cus_modal" class="cus_modal">
  ...
  <div class="cus_modal-content"> <span id="cus_close" class="cus_close">×</span>
		...
		<li><span><i class="xi-folder-check"></i> 전체고객목록</span>
			...
			<ul class="nested active" id="ul_group_list">
				<li>
					...
					<button class="btn_cus" onclick=""><i class="material-icons">done</i>확인</button>
	...
	<p class="cus_info">* <span>[그룹삭제]</span>는 그룹만 삭제됩니다.(그룹에 속해있던 고객은 전체목록에 그대로 남겨둡니다.) <br  />* 그룹내 속해있는 고객까지 삭제하시려면 <span>[그룹/고객삭제]</span> 버튼을 이용해주세요.</p>
  </div>
</div>
<script>
	// Get the modal
			...
			success: function (json) {
				...
				$.each(json.list, function(key, value){
					...
					if(org_parent_id != cg_parent_id){
						...
						html += '	<button class="btn_cus" onClick="group_save(\'1\', \'group1_nm_'+ no +'\', \''+ cg_parent_id +'\', \''+ cg_id +'\');"><i class="material-icons">done</i>확인</button>';
						//html += '	<button class="btn_cus" onClick="group_del(\'1\', \'li_cg_id_'+ cg_id +'\', \''+ cg_cnt +'\', \''+ cg_id +'\');"><i class="material-icons">clear</i>삭제</button>';
						html += '	<button class="btn_cus" onClick="group_del(\'1\', \'li_cg_id_'+ cg_id +'\', \''+ cg_cnt +'\', \''+ cg_id +'\', \'G\');"><i class="material-icons">layers_clear</i>그룹삭제</button>';
						html += '	<button class="btn_cus" onClick="group_del(\'1\', \'li_cg_id_'+ cg_id +'\', \''+ cg_cnt +'\', \''+ cg_id +'\', \'ALL\');"><i class="material-icons">person_off</i>그룹/고객삭제</button>';
						//html += '	<label class="checkbox_container">';
						//html += '		<input type="checkbox" id="chkCgid" name="chkCgid" value="'+ cg_id +'" class="chk"> 소속고객';
					...
					if(cg_level == 2){ //2차 그룹
						...
						html += '	    <button class="btn_cus" onClick="group_save(\'2\', \'group2_nm_'+ no +'\', \''+ cg_parent_id +'\', \''+ cg_id +'\');"><i class="material-icons">done</i>확인</button>';
	...
	//그룹 추가/수정
	function group_del(glv, id, cnt, gid, gb){
		//alert("gid : "+ gid); return;
		//alert("glv : "+ glv +", id : "+ id +", cnt : "+ cnt +", gid : "+ gid +", gb : "+ gb); return;
		var msg = "해당 그룹을 삭제하시겠습니까?"
		if(cnt > 0){
			if(gb == "ALL"){
				msg = "해당 그룹에 소속된 고객 "+ cnt +"명도 함께 삭제됩니다.\n"+ msg;
			}else{
		...
		$.ajax({
			...
			type: "POST",
			data: {"glv":glv, "gid":gid, "gb":gb, "<?=$this->security->get_csrf_token_name()?>":"<?=$this->security->get_csrf_hash()?>"},
			success: function (json) {
				if(glv == 1){
					id_remove(id); //영역 삭제
				}else{
		...
		<tbody class="tbody">
			<?
				foreach($list as $r) {
					...
					$no = 0;
					$gname = "";
					$get_gname = $r->gname; //그룹명1##그룹번호1@@그룹명2##그룹번호2
					if($get_gname != ""){
						$gname .= "<ul>";
						$exp1 = explode("@@", $get_gname);
						for($ii = 0; $ii < count($exp1); $ii++){
							...
							$gnm = $exp2[0];
							$gname .= "<li><span>그룹". $no ."</span>". $gnm ." <span class='cus_group_close' onClick='del_group(\"".$r->ab_id."\",\"".$gid."\")'><i class='xi-close'></i></span></li>";
						}
			...
			<tr>
                ...
                <td name="kind" class="cus_group_list"><?=$gname?></td><?//그룹?>
...
<script type="text/javascript">
	...
	*/
	...
	<? if($gnm1 != "" and $gnm2 != ""){ ?>
	//parent().$("#div_title").html(" > <?=$gnm1?> > <?=$gnm2?>");
	<? }else if($gnm1 != ""){ ?>
	//parent().$("#div_title").html(" > <?=$gnm1?>");
	<? } ?>
</script>
