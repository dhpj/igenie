    ...
    <li>
      ...
      <ul class="nested active">
		...
		<? }else if($r->cg_level == 1 && $r->rowspan == 1){ //if($r->cg_level == 1 && $r->rowspan > 1){ //1차 그룹 등록수가 있는 경우 ?>
		  <i class="xi-file-text-o"></i> <span class="<?=$class_gname?>" onClick="group_search('<?=$r->cg_level?>', '<?=$r->cg_id?>', '<?=$gnm1?>', '<?=$gnm2?>');");" style="cursor:pointer;"><?=$cg_gname?> <span class="cus_cnt"><?=$r->cg_cnt?></span></span></span><?//1차 그룹?>
		<? } //if($r->cg_level == 1 && $r->rowspan > 1){ //1차 그룹 등록수가 있는 경우 ?>
        <?
				} //if($org_parent_id != $r->cg_parent_id){
