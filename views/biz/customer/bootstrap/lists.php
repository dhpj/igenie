...
<script type="text/javascript">
	...
	//그룹 고객 삭제
	function groupDelRow(delGid) {
		if(confirm("등록된 소속그룹 고객이 삭제됩니다.\n모두 삭제하시겠습니까?")){
			if(confirm("정말 삭제하시겠습니까?\n등록된 소속그룹 고객이 삭제됩니다.")){
				open_page(1, 'group', delGid);
				alert("정상적으로 삭제 되었습니다.");
			}
		}
	}
	...
	//조회
	function open_page(page, delIds, delGid){
		var add = $('#add').val();
			...
			$.ajax({
				...
				success: function (json) {
					...
					if (status == 'error') {
						...
						var ok = json['ok']; //처리수
						var ok_ins = json['ok_ins']; //등록수
						var ok_upd = json['ok_upd']; //수정수
						var ok_del = json['ok_del']; //삭제수
                        var ok_zero = json['ok_zero']; // 0이 안붙은 친구들
						var msg = "등록 : "+ comma(ok_ins) +"건\n수정 : "+ comma(ok_upd) +"건";
						if(ok_del > 0) msg += "\n삭제 : "+ comma(ok_del) +"건";
                        if(ok_zero > 0) msg += "\n0빠짐 : "+ comma(ok_zero) +"건";
						alert(msg +"\n처리 되었습니다.");
						$("#excel_file").val("");
					  ...
					  /*setTimeout(function() {
						...
						hideLoading();//로딩중 닫기  21-06-18 수정
						return;
					}
				}
            });
    ...
    //그룹해제
    function del_group(ab_id, gid){

        $.ajax({
            url: "/biz/customer/del_group",
            type: "POST",
            data: {
				<?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				"ab_id": ab_id,
