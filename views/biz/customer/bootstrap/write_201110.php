		...
		<div class="white_box" style="display:inline-block;">
			<div class="checks">
				...
				<div id="upload">
					<div class="msg_box">
						 <div class="send_list">
							 <button class="btn md excel fr" style="margin-left: 20px;" onclick="download();">엑셀양식 다운로드</button>
							 <input type="file" name="filecount" id="filecount" multiple onchange="readURL();" style="cursor: pointer; padding:5px;">
						 </div>
						...
						<div class="bottom">
							총 <span class="num" id="uploadCount">0</span> 명의 고객이 업로드 되었습니다.
						</div>
					</div>
				</div>
				<div id="private">
...
<script type="text/javascript">
    ...
    function all_save() {
        ...
        //if(file) {
        if ($("#switch_left").is(":checked")) { //파일업로드로 전송
            if (parseInt($("#uploadCount").text()) == 0) {
                $(".content").html("업로드 파일이 없습니다.");
                $('#myModal').modal({backdrop: 'static'});
            } else {
                //$(".content").html("등록 고객수에 따라 시간이 소요될 수 있습니다.");
                //$('#myModalCustomer').modal({backdrop: 'static'});
                //$('#myModalCustomer').unbind("keyup").keyup(function (e) {
                //    var code = e.which;
                //    if (code == 13) {
                //        $(".customer_save").click();
                //    }
            ...
            $.ajax({
                ...
                success: function (json) {
					...
					if (json.hasOwnProperty('success')) {
                        //alert("22222222");
						var dupList = '';
                        if (json.duplicated > 0) {
                            //alert("3333333");
							var phntrs = '';
                            json.dup_list.forEach(function (phn, idx) {
                                /*phntrs += String.format(
                                    '<tr align="center" name="select">' +
                                    '<td class="text" width="130">{0}</td>' +
                                    '</tr>', phn);*/
								phntrs +=
                                    '<tr align="center" name="select">' +
                                    '<td class="text" width="130">'+ phn +'</td>' +
                                    '</tr>';
                            });
                            dupList = '' +
                        ...
                        clearPhoneList();
						//alert("5555555");
                        //$(".content").html("전화번호가 저장되었습니다.");
                        //$(".content").append('<br/><br/>신규저장: ' + json.uploaded + ' 건, ');
                        //$(".content").append('수정: ' + json.duplicated + ' 건, ');
                        //$(".content").append('삭제: ' + json.deleted + ' 건');
                        ////$(".content").append('<br/>' + dupList);
						//alert("전화번호가 저장되었습니다.\n\n신규저장 : "+ json.uploaded +"건\n수정 : "+ json.duplicated +"건");
						alert("전화번호가 저장되었습니다.");
						location.href = "/biz/customer/lists";
						return;
                    } else {
                        //$(".content").html("전화번호를 저장하지 못했습니다.");
                ...
                success: function (json) {
                    if (json['success'] != 'success') {
                        ...
                        if (json.duplicated > 0) {
                            ...
                            dupList = '' +
                                '<div style="max-height: 150px; overflow: auto; margin-top: 20px;"> ' +
                                '<table class="table table-bordered table-highlight-head scrolltbody" style="overflow-y:hidden;width:100% !important;">' +
                                '<thead>' +
                                '<tr style="cursor:default;">' +
                                '<th width="100">중복 전화번호</th>' +
    ...
    function phnHtmlRow(idx, phone, name, addr) {
		...
		html += '  </td>';
		html += '  <td>';
		html += '    <select id="tel_group" name="tel_group" tabindex="1">';
		...
		html += '  </td>';
		html += '  <td>';
		html += '    <input type="text" class="form-control input-width-medium inline tel_addr" style="width:100%;" value="'+ addr +'" id="tel_addr" name="tel_addr" placeholder="주소 입력" tabindex="1">';
		html += '  </td>';
