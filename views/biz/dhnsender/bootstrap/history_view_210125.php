...
<div id="mArticle">
	<div class="form_section">
		...
		<div class="inner_content preview_info clearfix">
			<div class="info_detail fl">
				...
				<div class="input_content_wrap">
					...
					<? }else if($rs->mst_agreeid != ""){ //2021-01-20 개인정보동의 ?>
						개인정보동의
					<? }else if($rs->mst_type2 == "at"){ //2021-01-05 2차 알림톡 추가 ?>
						<span>1차</span> 버튼
					<? }else{ ?>
						버튼
					<? } ?>
				...
				<? if($rs->mst_type2 == "at"){ //2021-01-05 2차 알림톡 추가 ?>
				<div class="input_content_wrap">
					<label class="input_tit">
						<span>2차</span> 버튼
					</label>
				...
				<? } ?>
				<? if($ums->link_type == "editor") { //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력) ?>
				<div class="input_content_wrap">
					<label class="input_tit">에디터 내용수정</label>
					<div class="input_content">
						<p class="info_text">*발송후라도 내용수정후 저장시 수정된 내용이 적용됩니다.</p>
						...
						<script>
							$(document).ready(function() {
								...
								if(!empty($ums)) {
								?>
									$('#summernote').summernote('code', '<?=$ums->html?>');
								<?
								}
								?>
							});
			...
			<div class="info_detail_msg fl">
				<? if( $rs->mst_kind != "MS") { ?>
				<?//=str_replace("\n", "<br/>", $rs->mst_content)?>
					<!-- 친구톡/알림톡 미리보기 -->
					<div class="mobile_preview detail">
						<div class="preview_circle"></div>
						<div class="preview_round"><span>1차</span><? if($rs->mst_type1 == "at"){ ?>알림톡<? }else{ ?>친구톡<? } ?> 미리보기</div>
				...
				<? }
				if((!is_null($rs->mst_content) && strlen($rs->mst_lms_content)>10) or $rs->mst_type2 == "at"){ ?>
					<!-- SMS 미리보기 -->
	...
	function urlConfirm(url, no) {
		...
		if(url != "") {
			...
			if(url != "") {
				...
				if(link_type == "editor" && no == 1) { //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력)
					editor_preview(url); //에디터 전단보기
				}else if(link_type == "smart" && no == 1) { //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력)
					smart_preview(url); //스마트전단 전단보기
				}else if(link_type == "coupon" && no == 1) { //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력)
