<link rel="stylesheet" type="text/css" href="/views/biz/dhnsender/send/bootstrap/css/select_template.css?v=<?=date("ymdHis")?>"/>
<?
	//버튼정보
	$tpl_button = "";
	$str_button = "";
...
<?if($rs->tpl_profile_key) {?>
	...
	<div class="input_content_wrap" style="display:<?=($tpl_button_cnt == 0) ? "none" : ""?>;">
		...
		<div class="input_content">
			...
			<? }else{ ?>
				<?php include_once($_SERVER['DOCUMENT_ROOT'].'/views/biz/dhnsender/send/bootstrap/_inc_leaflet_new_v4.php'); ?>
			<? } ?>
	...
	<!-- 알림톡버튼 변경하기 Modal -->
	<div id="modal_templet" class="dh_modal sel_tem_img_wrap">
	  <!-- Modal content -->
	  <div class="modal-content md_width970">
        <span id="close_templet" class="dh_close">&times;</span>
        ...
        <?//if ($this->member->item("mem_id") == "2"){?>
            ...
            <div class="temp_type_wrap">
                <!-- <span>
                    <input type="radio" name ="temp_type" value="1" id="r_private">
                    <label for="r_private">개별템플릿</label>
                </span> -->
                <span>
                    <input type="radio" name ="temp_type" value="2" id="r_public" checked>
                    <label for="r_public">공용템플릿</label>
                </span>
                <span>
                    <input type="radio" name ="temp_type" value="3" id="r_favorite">
                    <label for="r_favorite">즐겨찾기</label>
                </span>
            </div>
            <script>
            $('input[type=radio][name=temp_type]').change(function(){
                $("#modal_templet_list").html("");
                if (this.value == "1"){
                <?if ($this->member->item("mem_id") == "2"){?>
                ...
                <?}?>
                    ...
                    <?}?>
                    $(".al_talk_cate").css("display", "none")
                } else {
                    <?if ($this->member->item("mem_id") == "2"){?>
                        $("#modal_templet_list").sortable("option", "disabled", false)
                    <?}?>
                    $(".al_talk_cate").css("display", "block")
                }
                $("[name=temp_type]").attr("checked",false);
                $(this).attr("checked", true);
                adv_template_data(this.value)
            });
            </script>
        <?//}?>
	...
	<script>
		...
		function adv_template_data(temp_type) {
			var modal_tpl_no = 0;
			var first_tpl_id = "";
			...
			//알림톡버튼 데이타 조회
			$.ajax({
				url: "/dhnbiz/sender/send/talk_img_adv_v4/ajax_adv_template_data",
				type: "POST",
				data: {
                    "category_ids":category_ids,
                    "tpl_btn_cnts":btn_cnt_ids,
				...
				success: function (json) {
					...
					$.each(json, function(key, value){
						...
						if(tpl_id != "" && tpl_id != undefined){
                            ...
                            <?//if ($this->member->item("mem_id") == "2"){?>
                            if (temp_type == "2"){
                                html += "<span class=\"xi-star-o\" onclick=\"set_favorite(this, 1," + tpl_id + ", '" + value.tpl_profile_key + "', '" + value.tpl_code + "')\" style=\"display:" + (value.favorite_flag == null ? "" : "none") + "\"></span>";
                                html += "<span class=\"xi-star\" onclick=\"set_favorite(this, 0," + tpl_id + ", '" + value.tpl_profile_key + "', '" + value.tpl_code + "')\" style=\"display:" + (value.favorite_flag == null ? "none" : "") + "\"></span>";
                            } else if (temp_type == "3"){
                                html += "<span class=\"xi-star\" onclick=\"set_favorite(this, 0," + tpl_id + ", '" + value.tpl_profile_key + "', '" + value.tpl_code + "', '" + temp_type + "')\" style=\"display:" + (value.favorite_flag == null ? "none" : "") + "\"></span>";
                            }
                            <?//}?>
							if(tpl_imageurl != "") {
		...
		$("#modal_templet_list").sortable({
			stop: function(event, ui) {
				...
				$.ajax({
					url: "/dhnbiz/sender/send/talk_img_adv_v4/ajax_adv_template_sort",
					type: "POST",
...
<script>
    ...
    function set_favorite(t, flag, tmp_id, profile_key, code, temp_type){
        $.ajax({
            url: "/dhnbiz/sender/send/talk_img_adv_v4/ajax_adv_template_favorite",
            type: "POST",
            data:{
                "<?=$this->security->get_csrf_token_name()?>":"<?=$this->security->get_csrf_hash()?>",
                flag : flag,
            ...
            success:function(result) {
                if (result){
                    ...
                    if (flag){
                        ...
                        if (temp_type == 3){
                            $(t).parent("li.li_templet_list").remove();
                        } else {
