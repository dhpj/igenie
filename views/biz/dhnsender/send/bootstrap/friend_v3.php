						...
						<div id="db">
							<div class="msg_box form_check">
								...
								<div class="send_list">
									<ul>
                                        ...
                                        <li <?=($unclassified_cnt>0&&count($customer_group)>0)? "" : "style='display:none;'" ?> ><input type="checkbox" name="nogroupChk" id="nogroup" value="0" checked><label for="nogroup">미분류 <span class="send_amount">(<?=$unclassified_cnt?>명)</span><?=($unclassified_cnt>0)? "<font style='font-size:11px;color:red;'> - 전체고객 선택시에만 포함</font>" : "" ?></label></li>
							...
							$("[name=groupAll]").click(function(){
								if($(this).prop("checked")){
                                    ...
                                    $('#nogroup').prop("disabled", true);
                                    $('#nogroup').prop("checked", false);
                                    $('input:checkbox[name="groupChk"]').each(function(){
...
<script type="text/javascript">
	...
	function chg_second_type(){
		...
		if(second_type == "at"){ //구분(ms.문자, at.알림톡)
			...
			$("#hidden_fields_talk").html("").load(
				"/dhnbiz/sender/send/select_template_second_v3",
	...
	$(document).ready(function() {
		...
		$('#send_friend_content').html('').load(
			(document.location.href.indexOf("/sender/send/friend_v3") > -1) ? "/dhnbiz/sender/send/select_profile_v3" : "",
			{ <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>", "add" : "<?=$add?>" },
			function() {
				if(document.location.href.indexOf("/sender/send/friend_v3") > -1) {
                    // $("radio[name='second_type']").val("at").prop("selected", true);
	...
	function select_profile() {
		...
		if(activeTr.length == 0 ) {
			...
			$('#send_friend_content').html('').load(
				"/dhnbiz/sender/send/select_profile_v3",
				{
					<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
...
//				cont = $("#lms_footer").html().replace("<br>", "\n");
				...
				// chkword_lms();
				// onPreviewLms();
                if ($(check).attr('id')=="lms_kakaolink_select") {
    				if (check.checked) {
    					$("#kakotalk_link_text").html(short_url);
    				} else {
    					$("#kakotalk_link_text").html("");
    				...
    				chkword_lms();
    				onPreviewLms();
				} else if ($(check).attr('id')=="lms_kakaolink_select2"){
					if (check.checked) {
						$("#kakotalk_link_text2").html(short_url);
					} else {
						$("#kakotalk_link_text2").html("");
					}
					chkword_lms2();
					onPreviewLms2();
				}
...
?>
    ...
    var charge_type2 = 0;
    var resend_type2 = ms;			<?/* 3차발신 종류 */?>
	var resend_type2_name = "SMS";	<?/* 3차발신 이름 */?>
	...
	function check_resend_method()
	{
		...
		if ($("input:checkbox[id='lms_select']").is(":checked") == false) {
			...
			if(second_type == "at"){
                ...
                if (($('#lms3').val().replace(/ /gi, "") != "") && $("#lms_select2").is(":checked") == true) {
                    <? if($mem->voucher_deposit>0&&$mem->mem_voucher_yn=='Y'){ ?>
                        sms_price = '<?=$this->Biz_dhn_model->price_v_smt_sms ?>';
            			lms_price = '<?=$this->Biz_dhn_model->price_v_smt ?>';
                    <? }else{ ?>
                    ...
                    <? } ?>
        			if ($('#lms3').val().replace(/ /gi, "") != "" && getByteLength($('#lms3')) > 90 && lms_price > charge_type) {
                		charge_type2 = lms_price;
                		resend_type2_name = "LMS";
                		resend_type2 = "L";
        			} else if ($('#lms3').val().replace(/ /gi, "") != "" && getByteLength($('#lms3')) <= 90 && sms_price > charge_type) {
                		charge_type2 = sms_price;
                		resend_type2_name = "SMS";
                		resend_type2 = "S";
        			}
        		}
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
	...
	function all_send() {
        if ($("input:checkbox[id='lms_select']").is(":checked") == true) {
            var second_type2 = value_check("second_type");
            if(second_type2 == "at"){
                if ($("input:checkbox[id='lms_select2']").is(":checked") == true) {
                    if (!check_pword("ft", "at", "lms")){
                        alert("금칙어가 포함되어 있습니다.(" + pword + ")");
                        return;
                    }
                } else {
                    if (!check_pword("ft", "at")){
                        alert("금칙어가 포함되어 있습니다.(" + pword + ")");
                        return;
                    }
                }
            } else {
                if (!check_pword("ft", "msg")){
                    alert("금칙어가 포함되어 있습니다.(" + pword + ")");
                    return;
                }
            }
        ...
        if(link_type_cnt>1){
            ...
            $("div[name='field-data']").each(function (){
                btn_no++;
                var url = "";
                console.log("btn_no : "+btn_no+" / link_type0 : "+ document.getElementById("link_type"+btn_no).value);
		...
		// var second_type = "ms"; //2차발송 구분(ms.문자, at.알림톡) 선택값
        var second_type = value_check("second_type");
		//alert("second_type : "+ second_type); return false;
		if($("input:checkbox[id='lms_select']").is(":checked") == true && second_type=="ms"){ //2차발송의 경우
        ...
        if($("input:checkbox[id='lms_select2']").is(":checked") == true){ //2차발송의 경우

				if($('#lms3').val().replace(/ /gi,"")==""){ //2차 문자 발송의 경우
					alert("3차 발송 문자 내용을 입력해주세요.");
					$("#lms3").focus();
					return false;
				}
				if(getByteLength($("#lms3")) > 2000){
					alert("3차 발송 문자 내용은 <?=($mem->mem_2nd_send=='sms') ? '90자(한글 45자)' : '2,000자(한글 1,000자)' ?> 이내로 입력하여야 합니다.");
					$("#lms3").focus();
					return false;
				}
					...
					if (check == true) {
						...
						$.ajax({
							...
							success: function (json) {
								...
								// }
								var kakao = Math.floor((Number(coin) + Number(bcoin)) / Number(ft_price));
								var k = kakao.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',').replace(/NaN/, '0');
								var kakao_img = Math.floor((Number(coin) + Number(bcoin)) / Number(ft_img_price));
								...
								if($("input:checkbox[id='lms_select']").is(":checked") == true){ //2차발송의 경우
									if(second_type == "at"){ //2차 알림톡 발송의 경우
                                        ...
                                        if (($('#lms3').val().replace(/ /gi, "") != "") && $("#lms_select2").is(":checked") == true) {
											...
											if ($('#lms3').val().replace(/ /gi, "") != "" && getByteLength($('#lms3')) <= 90) {
												msg_flag = msg_flag + " + 3차 문자(단문)";
											} else {
												msg_flag = msg_flag + " + 3차 문자(장문)";
											}
											// s2nd_msg = " + 2차문자";
										}
									}else{
								...
								if (!$("#switch_upload").is(":checked")) {
									...
									<? } ?>
									if ((Number(coin) + Number(bcoin)) - (Number(msg_price) * Number(row_index)) < 0) {
										$(".content").html("잔액이 부족합니다.<br>잔액 : "+coin+"<br>예상금액 : "+(Number(msg_price) * Number(row_index)));
										modal_open('myModal'); return false;
									} else {
										...
										if($("input:checkbox[id='lms_select']").is(":checked") == true){ //2차발송의 경우
											if(second_type == "at"){ //2차 알림톡 발송의 경우
                                                ...
                                                if($("input:checkbox[id='lms_select2']").is(":checked") == true){ //3차발송의 경우
                                                    content_msg += "<div class='modal_info_row'><label>3차문자 예상 금액</label>"+ ((Number(charge_type2) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + charge_type2 + ")</div>";
                                                }
									...
									if ((Number(coin) + Number(bcoin)) - (Number(msg_price) * Number(upload_send)) < 0) {
										$(".content").html("잔액이 부족합니다.<br>잔액 : "+coin+"<br>예상금액 : "+(Number(msg_price) * Number(upload_send)));
										modal_open('myModal'); return false;
									} else {
										content_msg += "<div class='modal_info_row'><label>발송 종류</label>" + msg_flag + "</div>";
										content_msg += "<div class='modal_info_row'><label>발송 시간</label>" + send_time + "</div>";
	...
	*/
        ...
        if(sendFlag==false){
    		...
    		if($("input:checkbox[id='lms_select']").is(":checked") == true) { //2차 발송이 있는 경우
                ...
                if(second_type=="at"){
    				...
    				$("div[name='field_second-data']").each(function () {
    					...
    					if (btn_type != undefined) {
    						...
    						if (btn_type == "WL") {
    							...
    							obj[btn_num].url_pc = $(this).find(document.getElementsByName("btn_url1")).val();
    							// }
    						} else if (btn_type == "AL") {
    				...
    				link_type = document.getElementById("link_type0").value; //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력)
                    ...
                    if($("input:checkbox[id='lms_select2']").is(":checked") == true && $("#lms3").val().trim() != "") { //3차 발송이 있는 경우

            				//웹(A)LMS : wa, 웹(A)SMS : was, 웹(A) MMS : wam, 웹(B)LMS : wb, 웹(B)SMS : wbs, 웹(B) : wbm, 폰문자: phn

            				var lms3_len = getByteLength($('#lms3'));
            ...
            if(fr_flag==true){
                $("select[name=link_type0]").each(function(index, item){
                    link_type0 = $(item).val();
                    var link0id = $(item).attr("id");
                    if(link0id!="link_type00"){
                        // console.log(link99id);
                        link0id = link0id.slice(-1);

                        if(link_type0=="editor"){
                            dhnl_url = $("#dhnl_url").val(); //에디터 링크 URL
                            biz_url = $("#biz_url").val() //에디터코드
                            editor_html = $('#summernote').summernote('code') //에디터내용
                        }
                        ...
                        if(link_type0 == "smart"||link_type0 == "editor"||link_type0 == "coupon"){
                            link_type = "home";
                        }
                    }
            ...
            if ($("input:checkbox[id='lms_select2']").is(":checked") == true && $("#lms3").val().trim() != ""  ) {
    			<? // 문자 헤드부분 회사명 변경가능하도록 수정 미리보기 처리 2019-07-25 ?>
    					//lms_header_temp = $("#lms_header").html().replace(/<BR>/g, "\n").replace(/<br>/g, "\n");
    					lms_header_temp = $("#span_adv2").text() + $("#companyName2").val();
    					if($("#kakotalk_link_text2").text() == "") {
    						lms_footer_temp = $("#span_unsubscribe2").text() + $("#unsubscribe_num2").val();
    					} else {
    						lms_footer_temp = $("#kakotalk_link_text2").text() + "\n" +$("#span_unsubscribe2").text() + $("#unsubscribe_num2").val();
    		...
    		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) {		// upload 발송
    			...
    			$.ajax({
    				url: "/dhnbiz/sender/friend_v3/all",
    				type: "POST",
    				data:file_data,
    				processData: false,
    				contentType: false,
    		...
    		//} else if ($("#switch_select").is(":checked")) {	// 고객 DB에서 그룹 선택
    			...
    			var customer_filter = "";
    			if ($("#switch_select").is(":checked")) {	// 고객 DB에서 그룹 선택
    				customer_all_count = parseInt($("#groupSendTotal").text().replaceAll(",","")); //발송건수
    				customer_filter = "";
    				...
    				customer_all_count = parseInt($("#posSendTotal").text().replaceAll(",","")); //발송건수
                ...
                if ($("#fail_flag").length > 0){
                    ...
                    $.ajaxSettings.traditional = true;
                    $.ajax({
                        url: "/dhnbiz/sender/friend_v3/all_fail",
                        type: "POST",
                        data: {"<?=$this->security->get_csrf_token_name()?>":"<?=$this->security->get_csrf_hash()?>",
    			...
    			var customer_all_count = parseInt($("#friend_yn_tel_count").text().replaceAll(",",""));
    			var customer_filter = "";
	...
	function test_send() {
		...
		if($("input:checkbox[id='lms_select']").is(":checked") == true){ //2차발송의 경우
			if(second_type == "at"){ //2차 알림톡 발송의 경우
                ...
                if ($("input:checkbox[id='lms_select2']").is(":checked")) {
                    if($('#lms3').val().replace(/ /gi,"")==""){ //2차 문자 발송의 경우
    					alert("3차 발송 문자 내용을 입력해주세요.");
    					$("#lms3").focus();
    					return false;
                    ...
                    if (getByteLength($("#lms3")) > 2000) {
            			alert("3차 발송 내용은 <?=($mem->mem_2nd_send=='sms') ? '90자(한글 45자)' : '2,000자(한글 1,000자)' ?> 이내로 입력하여야 합니다.");
            			$("#lms3").focus();
            			return false;
            		}
        		}
						...
						$.ajax({
							...
							success: function (json) {
								...
								if($("input:checkbox[id='lms_select']").is(":checked") == true){ //2차발송의 경우
									if(second_type == "at"){ //2차 알림톡 발송의 경우
										msg_flag += " + 2차 알림톡";
                                        if($("input:checkbox[id='lms_select2']").is(":checked") == true){ //3차 문자 발송의 경우
                        					msg_flag += " + 3차 문자";
                        				}
									}else{
										msg_flag += " + 2차 문자";
								...
								if ((Number(coin) + Number(bcoin)) - (Number(charge_type) * Number(row_index)) < 0) {
									...
									if($("input:checkbox[id='lms_select']").is(":checked") == true){ //2차발송의 경우
										if(second_type == "at"){ //2차 알림톡 발송의 경우
											content_msg += "<div class='modal_info_row'><label>2차 알림톡 예상 금액</label>"+ ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + charge_type + ")</div>";
                                            if($("input:checkbox[id='lms_select2']").is(":checked") == true){
                                                content_msg += "<div class='modal_info_row'><label>3차 문자 예상 금액</label>"+ ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + charge_type + ")</div>";
                                            }
										}else if(ms!="" && ($('#lms').val().replace(/ /gi, "") != "" || ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != ""))) {
											content_msg += "<div class='modal_info_row'><label>2차 문자 예상 금액</label>"+ ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + charge_type + ")</div>";
										}
		...
		if($("input:checkbox[id='lms_select']").is(":checked") == true) { //2차 발송이 있는 경우
			if(second_type == "at"){ //2차 알림톡 발송의 경우
                ...
                if($("input:checkbox[id='lms_select2']").is(":checked") == true && $("#lms3").val().trim() != "") { //3차 발송이 있는 경우
                        ...
                        // console.log("lms3_len : " + lms3_len);
                        // console.log("mst_type3 : " + mst_type3);
                        // return false;
                        var lms_header_temp = $("#span_adv2").text() + $("#companyName2").val();
            			var lms_footer_temp = "";
            			if($("#kakotalk_link_text2").text() == "") {
	...
	function chkword_lms2() {
		chkbyte2($("#lms_header2"), $("#lms3"), $("#lms_footer2"), $("#lms_num2"), <?=(($mem->mem_2nd_send=='sms') ? '90' : '2000') ?>);
	}
...
// 			}
   ...
   function chkbyte2($obj_header, $obj, $obj_footer, $num, maxByte) {
		var oriMaxByte = maxByte;
		var phn = '<?=$this->Biz_model->reject_phn ?>';
		var phn_len = phn.length + 16;

		<? // 문자 헤드부분 회사명 변경가능하도록 수정 문자길이체크 처리 2019-07-25 ?>
		var lms_header_temp = $obj_header.find("#span_adv2").text() + $obj_header.find("#companyName2").val();
		//var lms_header_temp = $obj_header.html().replace("<BR>", "\n").replace("<br>", "\n");
		var lms_footer_temp = "";
		if($("#kakotalk_link_text2").text() == "") {
			lms_footer_temp = $("#span_unsubscribe2").text() + $("#unsubscribe_num2").val();
...
// 		}
        ...
        var isdhn = $("#hdn_isdhn").val();
		if (totalByte <= 90) {
			$("#lms_limit_str2").html("SMS");
			$("#lms_character_limit2").html("90");
            if(isdhn=="N"){
                $(".msg_type2").attr("class", "msg_type2 sms");
            }
			$(".msg_type_txt2").attr("class", "msg_type_txt2 sms");
            lms2flag="sms";
		} else {
            if(lms2flag=="sms"){
                showSnackbar("90Byte가 초과되어 LMS로 전환됩니다.", 1500);
            }
			$("#lms_limit_str2").html("LMS");
			$("#lms_character_limit2").html("2000");
            if(isdhn=="N"){
                $(".msg_type2").attr("class", "msg_type2 lms");
            }
			$(".msg_type_txt2").attr("class", "msg_type_txt2 lms");
            lms2flag="lms";
		}
...
<?} ?>
	...
	function load_customer_all(filter) {
		...
		if ($("#pf_key").val() == null || $("#pf_key").val()=='') {
			...
			$.ajax({
				url: "/dhnbiz/sender/friend_v3/load_customer",
	...
	function templi_preview(obj) {
		...
		var tpl_imageurl = "";
		if ($("#tpl_emphasizetype_h").val() == "IMAGE") {
			tpl_imageurl = "<img src='"+$("#tpl_imageurl").val()+"' style='width: 100%'/>";
		}
		...
		$("#text_second").html(tpl_imageurl + returnTempli_cont);
		<? //} ?>
	}
	...
	function chg_link_type(id){
        ...
        if(link_type_cnt>1){
            ...
            for(var i=1;i<=link_type_cnt;i++){
                ...
                // if(se_type=="at"){
                //     if($("#link_type"+i).val()=="smart"){
                //         $("input:checkbox[id='chk_a']").prop("checked", true);
                //         $("#div_smart").show();
                //     }
                //     if($("#link_type"+i).val()=="editor"){
                //         $("input:checkbox[id='chk_b']").prop("checked", true);
                //         $("#div_editor").show();
                //     }
                //     if($("#link_type"+i).val()=="coupon"){
                //         $("input:checkbox[id='chk_c']").prop("checked", true);
                //         $("#div_coupon").show();
                //     }
                //
                // }
            ...
            if(link_type){
                ...
                // if(se_type=="at"){
                //     if($("#link_type1").val()=="smart"){
                //         $("input:checkbox[id='chk_a']").prop("checked", true);
                //         $("#div_smart").show();
                //     }
                //     if($("#link_type1").val()=="editor"){
                //         $("input:checkbox[id='chk_b']").prop("checked", true);
                //         $("#div_editor").show();
                //     }
                //     if($("#link_type1").val()=="coupon"){
                //         $("input:checkbox[id='chk_c']").prop("checked", true);
                //         $("#div_coupon").show();
                //     }
                // }
    ...
    //친구톡 버튼 알림톡 자동적용 함수
    function chg_al_btn(){
        var link_type_cnt = buttonArr.length;
        var se_type = $('input[name=second_type]:checked').val();
        if(se_type=="at"){
            if(link_type_cnt>0){
                    $("label[for='chk_a']").hide();
                    $("label[for='chk_b']").hide();
                    $("label[for='chk_c']").hide();
                    $("#div_smart").hide();
                    $("#div_editor").hide();
                    $("#div_coupon").hide();
                    $("input:checkbox[id='chk_a']").prop("checked", false);
                    $("input:checkbox[id='chk_b']").prop("checked", false);
                    $("input:checkbox[id='chk_c']").prop("checked", false);
                for(var i=0;i<=link_type_cnt;i++){
                        if(buttonArr[i]=="smart"){
                            $("label[for='chk_a']").show();
                            $("input:checkbox[id='chk_a']").prop("checked", true);
                            $("#div_smart").show();
                            if($("#psd_title").val()!=""){
                                $(".smart_msg").text($("#psd_title").val());
                                $("#hdn_smart_url").val($("#psd_url").val());
                            }
                        }
                        if(buttonArr[i]=="editor"){
                            $("label[for='chk_b']").show();
                            $("input:checkbox[id='chk_b']").prop("checked", true);
                            $("#div_editor").show();
                            $("#hdn_editor_url").val("http://igenie.co.kr/at/"+$("#biz_url").val());
                        }
                        if(buttonArr[i]=="coupon"){
                            $("label[for='chk_c']").show();
                            $("input:checkbox[id='chk_c']").prop("checked", true);
                            $("#div_coupon").show();
                            if($("#pcd_title").val()!=""){
                                $(".cou_msg").text($("#pcd_title").val());
                                $("#hdn_coupon_url").val($("#pcd_url").val());
                            }
                        }
                }
            }
	...
	//스마트전단 선택
	function smart_choice(code, name=''){
        ...
        if(code != ""){
            var btn_url = "http://smart.dhn.kr/smart/view/"+code;
            ...
            var btn_cnt0 = buttonArr.indexOf("smart");
            ...
            // if (btn_type != undefined) { //첫번째 버튼만 링크 추가
                $("#field-data-"+btn_cnt0).find("[name=btn_url21]").val(btn_url);
            ...
            if(name!=""){
                var short_name = "";
                if(name.length > 15){
                    short_name = name.substring(0, 13)+"...";
                }else{
                    short_name = name;
                }
                $(".smart_msg").text(short_name);
                $("#psd_title").val(short_name);
                $("#hdn_smart_url").val($("#psd_url").val());
                // console.log("여기여기 : "+name + $(".smart_msg").text());
            }
            onPreviewLms();
            chkword_lms();
        }
	...
	//스마트쿠폰 선택
	function coupon_choice(code, type, name){
		//alert("code : "+ code); return;
		//스마트쿠폰 링크 URL 만들기
        ...
        //alert("btn_url : "+ btn_url); return;
        if(code != ""){
            var btn_url = "http://smart.dhn.kr/smart/coupon/"+code;
            ...
            var btn_cnt0 = buttonArr.indexOf("coupon");
            // var btn_type = $("#field-data-"+btn_cnt).find("[name=btn_type]").val();
            // $("#btn_url"+btn_cnt).val(btn_url);
            ...
            if(name!=""){
                ...
                $(".cou_msg").text(short_name);
                $("#pcd_title").val(short_name);
                $("#hdn_coupon_url").val($("#pcd_url").val());
                // console.log("여기여기 : "+name + $(".smart_msg").text());
            }
    ...
    function editor_choice(obj, code){
        ...
        if(obj.checked){
            ...
            $("#div_editor").show();
            $("#hdn_editor_url").val("http://smart.dhn.kr/at/"+$("#biz_url").val());
            // $("#div_editor_box").show();
        }else{
            // $("#div_editor_url").text("");
            // $("#hdn_editor_url").val("");
    ...
    const at_pwords = <?=config_item("at_pword")?>;
    const ft_pwords = <?=config_item("ft_pword")?>;
    const msg_pwords = <?=config_item("msg_pword")?>;

    var pword = "";
    function check_pword(...args){
        var pwords = "";
        var txt = "";
        var ret = true;

        var fttxtid = "#templi_cont";
        var msgtxtid = "#lms";

        function _loop(pwords, ptxt){
            var res = true;
            for (const b of pwords){
                if(ptxt.indexOf(b) !== -1){
                    pword = b;
                    res = false;
                    break;
        ...
        for (const a of args){
            ...
            if (a == "at"){
                $.each($("textarea[name=var_name]"), function (idx, val){
                    txt += $(val).val().toLowerCase() + ",";
                });
                ret = _loop(at_pwords, txt);
            } else if (a == "ft"){
                txt = $(fttxtid).val().toLowerCase();
                ret = _loop(ft_pwords, txt);
            } else if (a == "msg"){
                txt = $(msgtxtid).val().toLowerCase();
                ret = _loop(msg_pwords, txt);
            }
            if (!ret) break;
        }
        return ret
