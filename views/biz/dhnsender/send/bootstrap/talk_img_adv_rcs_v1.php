...
<!--
    ...
    <? if($this->member->item('mem_level') < 100){ ?>
        // alert("점검중입니다");
        // location.href = "http://igenie.co.kr/home";
    <? }  ?>
...
<!--popup-->
<style>
.popStyle0{width:500px; height:539px; background-color: #a5dcef; border:none; padding:0; color:#404040; position:absolute; top:110px; left:50px; z-index:10000;}
...
<img src="/images/pop_20220907.jpg" alt=""/>
</div>
...
<script type="text/javascript">
    ...
    function onPreviewRcs() {
		<? // 문자 헤드부분 회사명 변경가능하도록 수정 미리보기 처리 2019-07-25 ?>
		//var lms_header_temp = $("#lms_header").html().replace(/<BR>/g, "\n").replace(/<br>/g, "\n");
		var rcs_header_temp = $("#span_rcs_adv").text() + $("#rcs_companyName").val();
		var rcs_footer_temp = "";
		// if($("#kakotalk_link_text").text() == "") {
			rcs_footer_temp = $("#span_rcs_unsubscribe").text() + $("#rcs_unsubscribe_num").val();
		// } else {
		// 	lms_footer_temp = $("#kakotalk_link_text").text() + "\n" +$("#span_unsubscribe").text() + $("#unsubscribe_num").val();
		// }
		//var lms_footer_temp = $("#lms_footer").html().replace(/<BR>/g, "\n").replace(/<br>/g, "\n");
		...
		if ($("#rcs").val().replace(/\n/g, "").length > 0) {
			msg_text = msg_text + rcs_header_temp + "\n" + $("#rcs").val() + "\n" + rcs_footer_temp;
		} else {
			msg_text = msg_text + rcs_header_temp + "\n\n메세지를 입력해주세요.\n\n" + rcs_footer_temp;
		}
        ...
        if(url != "") {
    		if(pattern.test(url)) {
				...
				if((link_type == "editor" && bno == "1") || (link_type == "smartEditor" && bno == "2")){ //(에디터 && 1번째 버튼) or (스마트전단+에디터 && 2번째 버튼)
					...
					//alert("html : "+ html); return;
					$.ajax({
						url: "/dhnbiz/sender/talk_img_v3/ums_save",
						type: "POST",
						data: {
							"<?=$this->security->get_csrf_token_name()?>" : "<?=$this->security->get_csrf_hash()?>"
	...
	*----------------------------------*/?>
	...
	function all_send() {
        ...
        if(link_type_cnt>1){
            ...
            //     }
            // }
            if(buttonArr.indexOf("home")>-1){
                link_type = "home";
            }else if(buttonArr.indexOf("addchanel")>-1){
                link_type = "addchanel";
            }else{
                link_type = "self";
            }
		...
		if(1 == 1){
			...
			$.ajax({
				url: "/dhnbiz/sender/talk_img_v3/equal_send_mem_id",
				type: "POST",
        ...
        if(sendFlag==false){
    		...
    		//} else if ($("#switch_select").is(":checked")) {	// 고객 DB에서 그룹 선택
                ...
                if ($("#fail_flag").length > 0){
                    ...
                    $.ajaxSettings.traditional = true;
                    $.ajax({
                        url: "/dhnbiz/sender/talk_img_v3/adv_send_fail",
                        type: "POST",
                        data: {"<?=$this->security->get_csrf_token_name()?>":"<?=$this->security->get_csrf_hash()?>",
                        "tel_number[]":tel_number,"templi_cont":templi_cont,"msg":msg,"kind":kind,
                        "templi_code":code,"pf_key":profile,"senderBox":senderBox,
...
//				onPreviewLms();
    ...
    function chkword_rcs() {
		chklength($("#rcs_header"), $("#rcs"), $("#rcs_footer"), $("#rcs_num"), '100');
	}
...
//		}
	...
	function chklength($obj_header, $obj, $obj_footer, $num, maxlength)
	{


		<? // 문자 헤드부분 회사명 변경가능하도록 수정 문자길이체크 처리 2019-07-25 ?>
		var rcs_header_temp = $obj_header.find("#span_rcs_adv").text() + $obj_header.find("#rcs_companyName").val();
		//var lms_header_temp = $obj_header.html().replace("<BR>", "\n").replace("<br>", "\n");
		var rcs_footer_temp = $("#span_rcs_unsubscribe").text() + $("#rcs_unsubscribe_num").val();

		//var lms_footer_temp = $obj_footer.html().replace("<BR>", "\n").replace("<br>", "\n");

		var strValue = rcs_header_temp + "\n" + $obj.val()+ "\n" + rcs_footer_temp;
		var strLen = strValue.length || 0;
        ...
        if (strLen > limit_length) {
			$("#caution_msg").html('<span style=\'color:red;\'>※ 메시지 내용이 '+ comma(limit_length) +'자가 넘어 발송할 수 없습니다.</span>');
		}else{
            $("#caution_msg").html('');
        }
        // var isdhn = $("#hdn_isdhn").val();
...
/*
	...
	<? if(!empty($spf)) { ?>
		...
		$('#send_template_content').html('').load(
			"/dhnbiz/sender/send/select_template_img_adv_rcs_v1",
			{ <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				add: "<?=$add?>",
                ...
                $("#rcs_select").change(function() {
                    if($(this).is(":checked")) {
                        $("#hidden_fields_rcs").show();
                    } else {
                        $("#hidden_fields_rcs").hide();
                    }
                });
				...
				if($("#lms_header").length >= 1) {
                    ...
                    $("#rcs_companyName").val($("#pf_ynm").val());
    				$("#rcs_unsubscribe_num").val(phn);
                    chkword_rcs();
    				onPreviewRcs();
				}
		   }
	...
	function selected_template(bottonId, tmp_flag) {
		...
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/talk_img_adv_rcs_v1");
    ...
    function chg_link_type2(id){
        ...
        if(link_type_cnt>1){
            ...
            var idx = Number(id);
            console.log("idx : "+idx);
            console.log("buttonArr2[idx] : "+buttonArr2[idx]);
            console.log("buttonArr2 : "+buttonArr2);
            console.log("link_type : "+ link_type);
            console.log("buttonArr2[idx-1] : "+buttonArr2[idx-1]);
            if(buttonArr2[idx-1]!=link_type){
                link_change_ori = buttonArr2[idx-1];
                if(buttonArr2.indexOf(link_type)>-1){
                    var preindex = buttonArr2.indexOf(link_type)+1;
                    ...
                    // alert(buttonArr2.indexOf('editor'));
                    if(link_type != "self"&&link_type != "dialer"){
                        if(link_change_ori == "self"||link_change_ori == "dialer"){ //직접입력
                            ...
                            if(buttonArr2.indexOf('editor')==-1||(buttonArr2.indexOf('editor')>-1&&buttonArr2.indexOf('editor')==id&&link_type!='editor')){
            ...
            for(var i=1;i<=link_type_cnt;i++){
                ...
                buttonArr2.push($("#link_type"+i).val());
            }
            // console.log("여기"+buttonArr2);
        }else{
            if(link_type){
                buttonArr2 = [];
                if(link_type=="smart" || link_type=="editor" || link_type=="coupon"){
                ...
                buttonArr2.push($("#link_type1").val());
            }
        }
	...
	function smart_choice(code){
        ...
        $("#hdn_smart_url").val("http://smart.dhn.kr/smart/view/"+code);
        $("#psd_url").val("http://smart.dhn.kr/smart/view/"+code);
        $("#psd_code").val(code);
	...
	function msg_save(use_yn){
		...
		$.ajax({
    		url: "/dhnbiz/sender/send/talk_img_adv_rcs_v1/save_adv_msg",
    		type: "POST",
    		data: {
    			 <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
	...
	function open_page_user_lms_msg(page, delids) {
		...
		$('#myModalUserMSGList .content').html('').load(
			"/dhnbiz/sender/send/talk_img_adv_rcs_v1/msg_save_list",
			{
				<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
