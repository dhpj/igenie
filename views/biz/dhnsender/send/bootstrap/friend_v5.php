...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#send_friend_content').html('').load(
			(document.location.href.indexOf("/sender/send/friend_v5") > -1) ? "/dhnbiz/sender/send/select_profile_v5" : "",
			{ <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>", "add" : "<?=$add?>" },
			function() {
				if(document.location.href.indexOf("/sender/send/friend_v5") > -1) {
	...
	function select_profile() {
		...
		if(activeTr.length == 0 ) {
			...
			$('#send_friend_content').html('').load(
				"/dhnbiz/sender/send/select_profile_v5",
				{
					<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
