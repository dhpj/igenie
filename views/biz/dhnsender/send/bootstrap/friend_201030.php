...
<div id="mArticle">
					...
					<div class="msg_box">
						<div class="send_list">
							...
							<div class="test_send_num_add">
								<button class="btn_testsend fr" id="btn-test-send" style="margin-left: 5px;" onclick="test_send();">테스트 발송</button>
	...
	<div class="btn_send_cen">
		...
		<button class="btn_send" onclick="all_send();">친구톡발송</button>
	</div>
</div><!-- mArticle END -->
<!-- 컨텐츠 전체 영역 END -->
...
<div class="modal select fade" id="sendbox_select" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="overflow:hidden;">
...
<div class="modal fade" id="profile_select" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog" id="modal">
		<div class="modal-content">
			<i class="material-icons modal_close" data-dismiss="modal">close</i>
			<div class="modal_title">프로필 선택</div>
			<div class="modal-body">
			...
			<button type="button" class="btn md btn-primary modal" id="code" name="code" onclick="select_profile();">확인</button>
		</div>
	</div>
</div>
	...
	<div class="modal-dialog" id="modalCheck">
		<div class="modal-content select_send">
			<div class="modal-body">
				...
				<div class="modal_bottom">
					<button type="button" class="btn md btn-default" data-dismiss="modal">취소</button>
	...
	<div class="modal-dialog" id="modalCheck">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="content"></div>
				<div class="modal_bottom">
					<button type="button" class="btn md btn-default modal_cancel" data-dismiss="modal">취소</button>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content alim_templet">
				...
				<div class="sticky_bottom" style="display:none;">
					<button type="button" class="btn md modal_cancel dismiss" data-dismiss="modal">취소</button>
					<button type="button" class="btn md modal_del include_phns" name="code" onclick="delete_msg()">선택 삭제</button>
					<button type="button" class="btn md include_phns" name="code" onclick="include_msg()">불러오기</button>
				</div>
		</div>
	</div>
</div>
...
?>
	...
	function check_resend_method()
	{
		...
		if ($("input:checkbox[id='lms_select']").is(":checked") == false) {
			//alert("'lms_select : "+ $("input:checkbox[id='lms_select']").is(":checked"));
	...
	*/
	...
	function test_send() {
		...
		if($("#pf_key").val()==null || $("#pf_key").val()==''){
			...
			$(".content").html("테스트 발송 연락처를 입력 후<br>[전화번호 추가] 버튼을 클릭 하세요.");
			$('#myModal').modal({backdrop: 'static'});
		} else {
			var check = true;
						...
						$.ajax({
							...
							success: function (json) {
								...
								if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
									...
									var content_msg = ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != "") ? "친구톡 이미지 발송 가능 건수 : " + k_i + "건" : "친구톡 발송 가능 건수 : " + k + "건";
									if ($("input:checkbox[id='lms_select']").is(":checked") == true) { //2차 발송의 경우
										if(ms!="" && ($('#lms').val().replace(/ /gi, "") != "" || ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != ""))) {
											content_msg += "<br/>" + "2차문자 발송 가능 건수 : " + resend_cnt + "건";
										}
									}
...
// 	}
	...
	function set_lms_by_user_msg(msg_id, msg_type){
		...
		//alert("set_lms_by_user_msg('"+ msg_id +"', '"+ msg_type +"')"); return;
		$.ajax({
