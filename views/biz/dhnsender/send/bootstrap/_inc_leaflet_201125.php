...
<div style="width:100%; display:inline-block;">
	<select style="width: 130px;"  name="link_type" id="link_type" onChange="chg_link_type();">
	  <option value="smart"<?=(($rs->tpl_id == $bt3_tpl_id and $leaflet_link_type != "lms") or $rs->tpl_btn1_type == "L") ? "" : " selected"?>>스마트전단</option>
	  <option value="editor"<?=($rs->tpl_btn1_type == "E") ? " selected" : ""?>>에디터사용</option>
	  <? if($leaflet_link_type == "lms"){ ?>
	  <option value="none"<?=($leaflet_link_type != "lms") ? "" : " selected"?>>사용안함</option>
	  <? }elseif($leaflet_link_type == "at"){ ?>
	  <option value="self"<?=(($rs->tpl_id == $bt3_tpl_id) or $rs->tpl_btn1_type == "S") ? " selected" : ""?>>직접입력</option>
	  <? } ?>
	</select>
	<div id="div_blank_box" style="display:<?=($rs->tpl_btn1_type == "S") ? "" : "none"?>;margin: 0px 0px 10px 0px;"><? //사용안함 영역 ?>
	</div>
	<div id="div_smart_box" style="display:<?=($leaflet_link_type == "lms" or $rs->tpl_btn1_type != "L") ? "none" : "inline"?>;">
	...
	<div class="editor_box" id="div_editor_box" style="margin:15px 0; display:<?=($rs->tpl_btn1_type == "E") ? "" : "none"?>;">
