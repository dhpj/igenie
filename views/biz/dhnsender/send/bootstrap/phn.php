...
$('#test_num_list').on(
	...
	$(document).ready(function() {
        <? if($this->member->item("mem_day_limit")){ ?>
            // $(".content").html("폰문자 발신등록되지 않아 발송 불가합니다. 고객센터로 문의하세요.");
            // modal_open('myModal');
            alert("폰문자 발신등록되지 않아 발송 불가합니다. 고객센터로 문의하세요.");
        <? } ?>
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
    ...
    var pms_price = '<?=$this->Biz_dhn_model->price_imc?>';
...
<?/*----------------------------------*
	...
	| 이부분에 단가를 넣어줘야 합니다. |
	*----------------------------------*/?>
    if ( $('#lms').val().replace(/ /gi, "") != "") <?/* mms가 아니고 글자수가 90byte초과일때 */?>
        { charge_type = pms_price; resend_type_name = "폰문자"; }
	coin=0;
						...
						$.ajax({
							...
							success: function (json) {
								...
								if (!$("#switch_upload").is(":checked")) {
									...
									var content_msg = "<div class='modal_info_row'><label>발송 가능 건수</label>" + slimit + "</div>";
									...
									if (Number(coin) - (Number(send_price) * Number(row_index)) < 0) {
										...
										modal_open('myModal'); return false;
											...
											content_msg += "<div class='modal_info_row'><label>발송 예상 금액</label>"  + (Number(pms_price) * Number(row_index)).toFixed(2)  + " 원(" + row_index + " X " + pms_price + ")</div>";

										$(".content").html(limit_msg + content_msg + "<br>전체 발송하시겠습니까?<br/> (발송할 건수(폰문자) : " + row_index + "건)");
									...
									if (Number(coin) - (Number(send_price) * Number(upload_send)) < 0) {
                                        ...
                                        var content_msg = "";
                                        if(Number(limit - sent)< Number(resend_cnt)){
                                            var slimit = "";
                                            if(limit>0){
                                                slimit = (limit - sent)+"건";
                                            }else{
                                            ...
                                            content_msg = "<div class='modal_info_row'><label>발송 가능 건수</label>" + slimit + "</div>";
                                        }else{
                                            content_msg = "<div class='modal_info_row'><label>발송 가능 건수</label>" + resend_cnt + "건</div>";
                                        }

                                        <? if($this->member->item("mem_pay_type")!="T"){ ?>
    										content_msg += "<div class='modal_info_row'><label>발송 예상 금액</label>"  + (Number(pms_price) * Number(upload_send)).toFixed(2)  + " 원(" + upload_send + " X " + charge_type + ")</div>";
    									<? } ?>
