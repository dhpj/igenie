...
/*
	...
	<? if(!empty($spf)) { ?>
		...
		$('#send_template_content').html('').load(
			"/dhnbiz/sender/send/select_template_img_adv_v4",
			{ <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				add: "<?=$add?>",
	...
	function selected_template(bottonId, tmp_flag) {
		...
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/talk_img_adv_v4");
	...
	function msg_save(use_yn){
		...
		$.ajax({
    		url: "/dhnbiz/sender/send/talk_img_adv_v4/save_adv_msg",
    		type: "POST",
    		data: {
    			 <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
	...
	function open_page_user_lms_msg(page, delids) {
		...
		$('#myModalUserMSGList .content').html('').load(
			"/dhnbiz/sender/send/talk_img_adv_v4/msg_save_list",
			{
				<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
