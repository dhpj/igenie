	...
	*----------------------------------*/?>
	...
	function all_move() {
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) { //파일업로드 발송
			...
			//alert(couponid);
			$.ajax({
				url: "/dhnbiz/sender/talk_img/adv_send",
				type: "POST",
				data: file_data,
				processData: false,
				contentType: false,
...
/*
	...
	function selected_template(bottonId, tmp_flag) {
		...
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/talk_img_adv_v2");
	...
	function msg_save(use_yn){
		...
		$.ajax({
    		url: "/dhnbiz/sender/send/talk_img_adv_v2/save_adv_msg",
    		type: "POST",
    		data: {
    			 <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
	...
	function open_page_user_lms_msg(page, delids) {
		...
		$('#myModalUserMSGList .content').html('').load(
			"/dhnbiz/sender/send/talk_img_adv_v2/msg_save_list",
			{
				<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
