...
<script type="text/javascript">
	...
	console.log("get_buttons : "+ get_buttons);
...
<script type="text/javascript">
	...
	function getArimtalkCont() {
        ...
        returnTempli_cont = returnTempli_cont.replace("#{회원정보 등록일}", "행사 정보");
        returnTempli_cont = returnTempli_cont.replace("#{변수내용}", "행사 정보");
...
/*
		...
		for (var i = 0; i < temp.length; i++) {
			if (i == 0) {
				...
				if (varName == "#{변수내용}" || varName == "#{회원정보 등록일}") {
					...
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" id=\"var_name\" class=\"var multi\" onkeyup=\"return view_preview();\" placeholder=\"행사 정보\">"+ strMsg +"</textarea>";
				} else if (varName == "#{업체명}") {
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" placeholder=\"업체명\"><?= $this->member->item('mem_username')?></textarea>";
				} else if (varName == "#{업체전화번호}" || varName == "#{전화번호}" ) {
					var tel = "<?=$this->funn->format_phone($this->member->item('mem_phone'),"-")?>";
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" placeholder=\"전화번호\">"+ tel +"</textarea>";
				} else if (varName == "#{날짜}") {
					// 멀티라인이 필요없는 변수 한줄짜리.
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" placeholder=\"행사 날짜\">"+ strMsg +"</textarea>";
				} else if (varName == "#{00}") {
					// 멀티라인이 필요없는 변수 한줄짜리.
                    if(strMsg.match(/^[0-9]+$/) != null){
                        returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" placeholder=\"00\" style='width:35px;'>"+ strMsg +"</textarea>";
                    }else{
                        returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" placeholder=\"00\" style='width:35px;'></textarea>";
                    }

				} else {
					// 멀티라인이 필요없는 변수 한줄짜리.
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" placeholder=\"" + varName + "\">"+ strMsg +"</textarea>";
				}
				returnTempli_cont = returnTempli_cont + varsplit[1];
			}
		}
	...
	<? if(!empty($spf)) { ?>
		...
		$('#send_template_content').html('').load(
			"/dhnbiz/sender/send/select_template_adv_v2",
			{ <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				add: "<?=$add?>",
				...
				<? //if($this->member->item("mem_2nd_send")!='NONE'){?>
                    console.log("$sendtelauth : "+"<?=$sendtelauth?>");
	...
	function selected_template(bottonId, tmp_flag) {
		...
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/talk_adv_v2");
	...
	function chg_link_type(id){
        ...
        if(link_type_cnt>1){
            var idx = Number(id);
            if(buttonArr[idx]!=link_type){
                link_change_ori = buttonArr[idx];
                if(buttonArr.indexOf(link_type)>-1){
                    var preindex = buttonArr.indexOf(link_type);

                    console.log(preindex);
                    console.log(id);
                    var prepre = $(".btn_loop_"+preindex).val();
                    var nownow = $(".btn_loop_"+id).val();
                    console.log(".btn_loop_"+preindex + "/" +prepre);
                    console.log(".btn_loop_"+id + "/" +nownow);
                    console.log(buttonArr);
                    if(link_type != "self"){
                        $("#link_type"+preindex).val(link_change_ori).prop("selected", true);
                        if(link_change_ori){
                            // $(".btn_loop_"+preindex).attr("readonly", false);
                            $(".btn_loop_"+preindex).val(nownow);
                            if(link_change_ori=="smart" || link_change_ori=="editor" || link_change_ori=="coupon"){
                                $(".btn_loop_"+preindex).attr("readonly", true);
                                $(".btn_loop_"+preindex).css("background-color", "#f2f2f2"); //링크 배경색 변경
                            }else{
                                $(".btn_loop_"+preindex).attr("readonly", false);
                                $(".btn_loop_"+preindex).val("");
                                $(".btn_loop_"+preindex).css("background-color", "#ffffff"); //링크 배경색 변경
                    ...
                    if(link_type){
                        ...
                        if(link_type=="smart" || link_type=="editor" || link_type=="coupon"){
                            ...
                            if(link_type=="smart"){
                                $(".btn_loop_"+ id).val(document.getElementById("psd_url").value);
                            }else if(link_type=="editor"){
                                $(".btn_loop_"+ id).val(document.getElementById("dhnl_url").value);
                            }else if(link_type=="coupon"){
                                $(".btn_loop_"+ id).val(document.getElementById("pcd_url").value);
                            }
                            // console.log("3");
                        }else{
                            $(".btn_loop_"+id).attr("readonly", false);
                    ...
                    if(link_type != "self"){
                        if(link_change_ori == "self"){ //직접입력
                			...
                			document.getElementById("div_smart_box"+preindex).style.display = "none"; //스마트전단 영역 닫기
                			document.getElementById("div_coupon_box"+preindex).style.display = "inline"; //스마트쿠폰 영역 열기
                            if(buttonArr.indexOf('editor')==-1||(buttonArr.indexOf('editor')>-1&&buttonArr.indexOf('editor')==id&&link_type!='editor')){
                                document.getElementById("div_editor_box").style.display = "none"; //에디터 영역 닫기
                                // alert("002");
                            }
                			...
                			// btn_url = document.getElementById("psd_url").value; //스마트전단 링크 URL
                			document.getElementById("div_smart_box"+preindex).style.display = "inline"; //스마트전단 영역 열기
                			document.getElementById("div_coupon_box"+preindex).style.display = "none"; //스마트쿠폰 영역 닫기
                            if(buttonArr.indexOf('editor')==-1||(buttonArr.indexOf('editor')>-1&&buttonArr.indexOf('editor')==id&&link_type!='editor')){
                                document.getElementById("div_editor_box").style.display = "none"; //에디터 영역 닫기
                                // alert("003");
		...
		if(link_type == "self"){ //직접입력
			...
			btn_url = document.getElementById("pcd_url").value; //스마트쿠폰 링크 URL
			document.getElementById("div_smart_box"+id).style.display = "none"; //스마트전단 영역 닫기
			document.getElementById("div_coupon_box"+id).style.display = "inline"; //스마트쿠폰 영역 열기
            if(link_change_value.indexOf('editor')==-1){
                document.getElementById("div_editor_box").style.display = "none"; //에디터 영역 닫기
                // alert("005");
            }
			...
			// }
			btn_url = document.getElementById("psd_url").value; //스마트전단 링크 URL
			document.getElementById("div_smart_box"+id).style.display = "inline"; //스마트전단 영역 열기
			document.getElementById("div_coupon_box"+id).style.display = "none"; //스마트쿠폰 영역 닫기
            if(link_change_value.indexOf('editor')==-1){
                document.getElementById("div_editor_box").style.display = "none"; //에디터 영역 닫기
                // alert("006");
		...
		$("div[name='field-data']").each(function (){
			...
			if(btn_type != undefined){
				// if(btn_cnt == 1) { //1번째 버튼
					...
					// if(link_type == "smart" || link_type == "coupon" || link_type == "editor"){ //스마트전단 or 스마트전단+에디터 or 스마트쿠폰 or 에디터
					...
					// 		$(this).find(document.getElementsByName("btn_url1")).attr("placeholder", "스마트전단 불러오기 버튼을 클릭하세요."); //링크 placeholder 변경
					// 	}else if(link_type == "coupon"){ //스마트쿠폰
					// 		$(this).find(document.getElementsByName("btn_url1")).attr("placeholder", "스마트쿠폰 불러오기 버튼을 클릭하세요."); //링크 placeholder 변경
					// 	}else{
					// 		$(this).find(document.getElementsByName("btn_url1")).attr("placeholder", "링크주소를 입력해주세요."); //링크 placeholder 변경
					// 	}
					// }else{
					...
					// 	$(this).find(document.getElementsByName("btn_url1")).attr("placeholder", "링크주소를 입력해주세요."); //링크 placeholder 변경
					// }
				// }else if(btn_cnt == 2) { //2번째 버튼
				// 	//$(this).find(document.getElementsByName("btn_url1")).attr("placeholder", "링크주소를 입력해주세요."); //링크 placeholder 변경
				// 	if(link_type == "smartEditor"){ //스마트전단
				// 		var btn_editor_url = document.getElementById("dhnl_url").value; //에디터 링크 URL
				// 		$(this).find(document.getElementsByName("btn_url1")).val(btn_editor_url);
				// 		$(this).find(document.getElementsByName("btn_url2")).val(btn_editor_url);
				// 		$(this).find(document.getElementsByName("btn_url1")).attr("readonly",true); //링크 readonly true
				// 		$(this).find(document.getElementsByName("btn_url1")).css("background-color", "#f2f2f2"); //링크 배경색 변경
				// 	}else{
				// 		//$(this).find(document.getElementsByName("btn_url1")).val("");
				// 		//$(this).find(document.getElementsByName("btn_url2")).val("");
				// 		$(this).find(document.getElementsByName("btn_url1")).attr("readonly",false); //링크 readonly false
				// 		$(this).find(document.getElementsByName("btn_url1")).css("background-color", "#ffffff"); //링크 배경색 변경
				// 	}
				// } else if(btn_cnt == 3) { //3번째 버튼
				// 	//$(this).find(document.getElementsByName("btn_url1")).attr("placeholder", "링크주소를 입력해주세요."); //링크 placeholder 변경
				// 	if(link_type == "smartEditor"){ //스마트전단
				// 		var btn_editor_url = document.getElementById("dhnl_url").value; //에디터 링크 URL
				// 		//$(this).find(document.getElementsByName("btn_url1")).val(btn_editor_url);
		...
		$.ajax({
			...
			success: function(json){
				...
				if(btn_url != ""){
					...
					//alert("psd_url : "+ document.getElementById("psd_url").value); return;
					//알림톡 버튼 URL 변경
                    var btn_cnt = buttonArr.indexOf("coupon");
                    var btn_type = $("#field-data-"+btn_cnt).find("[name=btn_type]").val();
                    //alert("btn_type : "+ btn_type);
                    if (btn_type != undefined) { //첫번째 버튼만 링크 추가
                        $("#field-data-"+btn_cnt).find("[name=btn_url1]").val(btn_url);
	...
	function msg_save(use_yn){
		...
		$.ajax({
    		url: "/dhnbiz/sender/send/talk_adv_v2/save_adv_msg",
    		type: "POST",
    		data: {
    			 <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
	...
	function open_page_user_lms_msg(page, delids) {
		...
		$('#myModalUserMSGList .content').html('').load(
			"/dhnbiz/sender/send/talk_adv_v2/msg_save_list",
			{
				<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
