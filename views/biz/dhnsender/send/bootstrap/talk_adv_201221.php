	...
	function getArimtalkCont() {
		...
		if(dumyTempli_cont != ""){
			...
			if(get_btn_cnt > 0){ //버튼이 있는 경우
				var link_type = document.getElementById("link_type").value; //전단종류(smart.스마트전단, editor.에디터 사용, self.직접입력)
				var btn_url = "";
				...
				if(link_type == "editor" && bno == "1"){ //에디터 && 첫번째 버튼
					...
					$.ajax({
						...
						data: {
							...
							,"bizurl" : bizurl //에디터코드
							,"html" : html //에디터내용
							,"link_type": link_type //전단종류(smart.스마트전단, editor.에디터 사용, self.직접입력)
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
	...
	function all_move() {
		...
		var link_type = $("#link_type").val(); //전단종류(smart.스마트전단, editor.에디터 사용, self.직접입력)
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) { //파일업로드 발송
			...
			file_data.append("link_type", link_type); //전단종류(smart.스마트전단, editor.에디터 사용, self.직접입력) 2020-09-16 추가
		...
		//} else if ($("#switch_select").is(":checked") && $("input[name=checkOne]:checked").length > 0) {	// 고객 DB에서 그룹 선택
			...
			$.ajax({
				...
				data: {
					...
					"bizurl": $("#biz_url").val(),
					"link_type": link_type, //전단종류(smart.스마트전단, editor.에디터 사용, self.직접입력) 2020-09-16 추가
		...
		for (var i = 0; i < temp.length; i++) {
			if (i == 0) {
				...
				if (varName == "#{변수내용}") {
					...
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" id=\"var_name\" class=\"var multi\" onkeyup=\"return view_preview();\" placeholder=\"" + varName + "\">"+ strMsg +"</textarea>";
				} else if (varName == "#{업체명}") {
