...
<div class="smart_btn_box">
	<div id="inhere"></div>
        ...
        if($leaflet_link_type == "S" || $leaflet_link_type == "L" || $leaflet_link_type == "M"){ ?>
	...
	function linkview(id){
		...
		if(link_type == "editor"){ //에디터
			...
			// $("div[name='field<?=(strpos("/".$_SERVER['REQUEST_URI'], "/dhnbiz/sender/send/select_template_second") == true) ? "_second" : ""?>-data']").each(function () {
                ...
                var btn_type = $("#field-data-"+realid).find("#btn_type"+id).val();
                console.log(btn_type);
				if (btn_type != undefined) {
					var focus;
					btn_url =  $("#field-data-"+realid).find("[name=btn_url21]").val() || "";
                    console.log(btn_url);
					if (btn_url == "" || !checkURL(btn_url)) {
						check = false;

						focus = $("#field-data-"+id).find("[name=btn_url21]").val();
			...
			// $("div[name='field<?=(strpos("/".$_SERVER['REQUEST_URI'], "/dhnbiz/sender/send/select_template_second") == true) ? "_second" : ""?>-data']").each(function () {
				...
				if (btn_type != undefined) {
					...
					btn_url =  $("#field-data-"+realid).find("[name=btn_url21]").val() || "";
                    console.log(btn_url);
					if (btn_url == "" || btn_url.length < 10 || btn_url.length > 11) {
						check = false;

						focus = $("#field-data-"+realid).find("[name=btn_url21]").val();
						alert("전화번호가 입력되지 않았거나 전화번호 길이가 올바르지 않습니다.");
						focus.focus();
						return;
					}else{
                        showSnackbar("전화번호 : "+btn_url, 1500);
						// alert("전화번호 : "+btn_url);
					}
				}
			// });
		...
		$.ajax({
			...
			success: function (json) {
				...
				if(pn == "at"){
                    ...
                    // if(rcs_select!="L"){
                        // showSnackbar("RCS LMS만 행사정보 가져오기를 지원합니다.", 1500);
                    // }else{
                        $("#templi_cont").val(html); //문자내용 => 스마트전단 상품정보 가져오기
    					onPreviewText();
    					chkword_templi();
                    // }
