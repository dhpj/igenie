...
<link rel="stylesheet" type="text/css" href="/views/biz/dhnsender/send/bootstrap/css/style.css?v=<?=date("ymdHis")?>"/>
 <!-- head>
...
<input type='hidden' name='pcr_id' id='pcr_id' value='<?=$pcr_id?>' />
		...
		<div class="inner_content preview_info">
			...
			<div class="input_content_wrap">
				<label class="input_tit">
					<p class="txt_st_eng">STEP <?=!empty($para->button) ? "5" : "4"?>.</p>
					<p>수신고객 선택</p>
				</label>
...
<div id="template_select" class="dh_modal md_personal_v1">
	<div class="modal-dialog modal-lg select-dialog" id="modal" data-keyboard="false" data-backdrop="static">
		<div class="modal-content">
			<span id="close_template_select" class="dh_close" onclick="close_md_personal_v1()">&times;</span>
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
	...
	function test_send() {
		...
		if (emptyVar !== true) {
			alert("소재 "+ (emptyVar+1) +"번째 변수를 입력해주세요.");
			$("textarea[name=var_name]")[emptyVar].focus();
			return;
		}
        ...
        var ltt = '';
        var link_val;
        $.each($('.pre_set'), function(){
            link_val = '';
            lt = $(this).find('[name="link_type"]').val();
            ltt = $(this).find('[name="link_type"] option:checked').text();
            link_val = $(this).find('[name="btn_url21"]').val();
            if (link_val == ''){
                button_flag = true;
                if (lt == 'self'){
                    alert("링크주소를 입력하세요.");
            ...
            if (lt == 'editor' && (summ == '' || summ == '<p><br></p>' || summ == '<p><span style="font-size: 18px;">﻿</span><br></p>')){
                button_flag = true;
                alert("에디터 내용을 입력하세요.");
    			window.scroll(0, getOffsetTop(document.getElementById("id_STEP3"))); //STEP 3 영역으로 이동
    			return false;
		...
		if(1 == 1){
			var check = true;
			$.ajax({
				url: "/dhnbiz/sender/personal_v1/equal_send_mem_id",
				type: "POST",
				data: {
                    <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>"
				  , "senderMem_id" : <?=$this->member->item("mem_id") ?>
				...
				success: function (json) {
					...
					if (senderCheck == "F") {
	    				check = false;
	    				$(".content").html("개인화 메시지 작성한 계정과 발송계정이 틀립니다.<br>발송계정 다시 로그인 후 다시 작성하세요.");
						modal_open('myModal');
						return false;
					} else {
						check = true;
...
/*
	...
	$(document).ready(function() {
		...
		$('#send_template_content').html('').load(
			"/dhnbiz/sender/send/select_template_personal_v1",
			{ <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				pcr_id: "<?=$pcr_id?>",
				...
				if($("#lms_header").length >= 1) {
    				...
    				<?}?>
    				<?if($this->member->item("mem_id") == 228 || $this->member->item("mem_id") == 93 || $this->member->item("mem_id") == 519|| $this->member->item("mem_id") == 445 || $this->member->item("mem_id") == 446 || $this->member->item("mem_id") == 447|| $this->member->item("mem_id") == 448 || $this->member->item("mem_id") == 484 || $this->member->item("mem_id") == 572) {?>
					phn = '080-053-7590';
					<? } ?>
    ...
    //템플릿내용 선택
	function selected_template2(pcr_id) {
        var form = document.createElement("form");
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/personal_v1");
		...
		var selectedField = document.createElement("input");
		selectedField.setAttribute("type", "hidden");
		selectedField.setAttribute("name", "pcr_id");
		selectedField.setAttribute("value", pcr_id);
		form.appendChild(selectedField);
	...
	function smart_choice(code, name=''){
        if(code != ""){
            ...
            $("#btn_url_"+c_btn).val(btn_url); //스마트전단 링크주조 표기 영역
            $("#btn_url_"+c_btn).attr('readonly', true);
            if(name!=""){
                var short_name = "";
                if(name.length > 15){
    ...
    // 소재선택 모달창 닫기
    function close_md_personal_v1() {
      $("#template_select").modal("hide");
    }

    //친구톡 버튼 알림톡 자동적용 함수
    function chg_al_btn(){
        var link_type_cnt = btn_content.length;
        var main_btn_cnt = $('[name="link_type2"]').length;
        ...
        if(se_type=="at"){
            if(link_type_cnt>0){
                ...
                for(var i=0;i <= main_btn_cnt;i++){
                        if($('[name="link_type2"]').eq(i).val() == 'smart'){
