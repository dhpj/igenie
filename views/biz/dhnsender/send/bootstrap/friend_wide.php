...
// 	});
	...
	$(document).ready(function() {
		...
		$('#send_friend_content').html('').load(
			(document.location.href.indexOf("/sender/send/friend_wide") > -1) ? "/dhnbiz/sender/send/select_profile_wide" : "",
	...
	function select_profile() {
		...
		if(activeTr.length == 0 ) {
			...
			$('#send_friend_content').html('').load(
				"/dhnbiz/sender/send/select_profile_wide",
				{
					<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
	...
	*/
	...
	function all_move() {
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) {		// upload 발송
			...
			$.ajaxSettings.traditional = true;
			$.ajax({
				url: "/dhnbiz/sender/friend/wide_all",
				type: "POST",
				data:file_data,
				processData: false,
				contentType: false,
...
//----------------------------------------------------------------------------------------------------------------
	...
	function imageSelect() {
		...
		imgSelectBox = window.open("/dhnbiz/sender/images_wide", 'tst', 'width=' + sw + ',height=' + sh + ',top=' + mt + ',left=' + ml + ',location=no, resizable=no');
	}
