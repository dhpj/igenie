...
<script type="text/javascript">
	...
	function templi_preview(obj) {
		...
		$("#text").html(tpl_imageurl + returnTempli_cont);
		<?}?>
	}
    	...
    	if(url != "") {
    		if(pattern.test(url)) {
				...
				if((link_type == "editor" && bno == "1") || (link_type == "smartEditor" && bno == "2")){ //(에디터 && 1번째 버튼) or (스마트전단+에디터 && 2번째 버튼)
					...
					//alert("html : "+ html); return;
					$.ajax({
						url: "/dhnbiz/sender/talk_img/ums_save",
						type: "POST",
						data: {
							"<?=$this->security->get_csrf_token_name()?>" : "<?=$this->security->get_csrf_hash()?>"
	...
	*----------------------------------*/?>
	...
	function all_send() {
		...
		if(1 == 1){
			...
			$.ajax({
				url: "/dhnbiz/sender/talk_img/equal_send_mem_id",
				type: "POST",
	...
	function all_move() {
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) { //파일업로드 발송
			...
			//alert(couponid);
			$.ajax({
				url: "/dhnbiz/sender/talk_img_v3/adv_send",
				type: "POST",
				data: file_data,
				processData: false,
				contentType: false,
...
/*
	...
	<? if(!empty($spf)) { ?>
		...
		$('#send_template_content').html('').load(
			"/dhnbiz/sender/send/select_template_img_adv_v2",
			{ <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				add: "<?=$add?>",
	...
	function selected_template(bottonId, tmp_flag) {
		...
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/talk_img_adv_v2_2");
	...
	function chg_link_type(id){
		...
		$("div[name='field-data']").each(function (){
			...
			if(btn_type != undefined){
				...
				// 	if(link_type == "smart" || link_type == "smartEditor" || link_type == "coupon" || link_type == "editor"){ //스마트전단 or 스마트전단+에디터 or 스마트쿠폰 or 에디터
				...
				// 		if(link_type == "smart" || link_type == "smartEditor"){ //스마트전단
	...
	function msg_save(use_yn){
		...
		$.ajax({
    		url: "/dhnbiz/sender/send/talk_img_adv_v2_2/save_adv_msg",
    		type: "POST",
    		data: {
    			 <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
	...
	function open_page_user_lms_msg(page, delids) {
		...
		$('#myModalUserMSGList .content').html('').load(
			"/dhnbiz/sender/send/talk_img_adv_v2_2/msg_save_list",
			{
				<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
