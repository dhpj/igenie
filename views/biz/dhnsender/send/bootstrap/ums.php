			...
			<div class="input_content_wrap">
				...
				<div class="input_content">
					<div class="msg_box">
						...
						<span class="txt_byte"><span id="info_desc"><span class="info_desc">90bytes까지만 작성 가능합니다.</span></span><span class="msg_type_txt sms" id="lms_limit_str"></span><span id="lms_num">0</span>/<span id="lms_character_limit">90</span> bytes</span>
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
	...
	function all_send() {
		...
		if (getByteLength($("#lms")) > 90) {
			alert("문자 내용은 최대 90bytes (한글 45자) 이내로\n입력하여야 합니다.");
			$("#lms").focus();
			return;
		}
...
/*-------------------------------------------------------------------------------*/
	...
	function test_send() {
		...
		if ($('#lms').val().replace(/ /gi,"")==""){
			alert("문자 내용을 입력해주세요.");
			$("#lms").focus();
		} else if (getByteLength($("#lms")) > 90) {
			alert("문자 내용은 최대 90bytes (한글 45자) 이내로 입력하여야 합니다.");
			$("#lms").focus();
...
// 			}
		...
		if (totalByte > 90) {
			$("#info_desc").html('<span class="noted_red">※ 90 byte가 넘어 발송할 수 없습니다.</span>');
		}else{
			$("#info_desc").html('<span class="info_desc">90bytes가 넘어가면 발송할 수 없습니다.</span>');
		}
		$num.html(totalByte);
		...
		if (!mms_flag) {
			if (totalByte <= 90) {
				$("#lms_limit_str").html("UMS");
				// $("#lms_limit_str").attr("class", "msg_type_txt sms");
				$(".msg_type").attr("class", "msg_type ums");
                ...
                if(lmsflag=="sms"){
                    showSnackbar("90Byte가 초과되어 UMS로 발송할 수 없습니다.", 1500);
                    lmsflag="lms";
                }
		...
		$.ajax({
			...
			success: function(json){
				...
				if(btn_url != ""){
					...
					document.getElementById("psd_url").value = btn_url; //스마트전단 URL
