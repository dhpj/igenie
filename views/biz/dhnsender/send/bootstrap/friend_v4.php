...
<div class="dh_modal" id="myModal">
	<div class="modal-dialog width_500" id="modal">
		<div class="modal-content">
			<span id="close_myModal" class="dh_close2">&times;</span>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#send_friend_content').html('').load(
			(document.location.href.indexOf("/sender/send/friend_v4") > -1) ? "/dhnbiz/sender/send/select_profile_v2" : "",
			{ <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>", "add" : "<?=$add?>" },
			function() {
				if(document.location.href.indexOf("/sender/send/friend_v4") > -1) {
                    ...
                    if($("#note2nd").html()!=undefined){
                        $("#note2nd").html("*친구톡 실패건을 폰문자로 재발송합니다.");
                    }
...
<?} ?>
	...
	{
		$(".content").html("금일 발송 가능 건수를 초과하였습니다.<br/>"+$str);
		modal_open('myModal'); return false;
	}
...
<?if($mem->mem_2nd_send=='phn') { echo "var resend_price = '".$this->Biz_dhn_model->price_phn."';"; }
	...
	else if($mem->mem_2nd_send=='SMART') { echo "var resend_price = '".$this->Biz_dhn_model->price_imc."';"; }
	else { echo "var resend_price = '0';"; }
?>

	var resend_type = ms;			<?/* 2차발신 종류 */?>
	var resend_type_name = "<?=($mem->mem_2nd_send=='SMART') ? '폰문자' : strtoupper($mem->mem_2nd_send) ?>";	<?/* 2차발신 이름 */?>
	...
	function check_resend_method()
	{
		...
		if ($("input:checkbox[id='lms_select']").is(":checked") == false) {
			...
			var sms_price = '<?=$this->Biz_dhn_model->price_imc ?>';
			var lms_price = '<?=$this->Biz_dhn_model->price_imc ?>';
			// var second_type = value_check("second_type"); //2차발송 구분(ms.문자, at.알림톡) 선택값
			...
			// 		sms_price = '<?=$this->Biz_dhn_model->price_imc ?>';
			// 		lms_price = '<?=$this->Biz_dhn_model->price_imc ?>';
			// 	}
			// 	if(lms_len <= 90){
				...
				success: function (json) {
					...
					if (check == true) {
						...
						if ($("input:checkbox[id='lms_select']").is(":checked") == true)
                            ...
                            if($("#reserve_check").is(":checked") == true) {
                                if(todate!=$("#reserve").val()) {
                                    res_date = $("#reserve").val();
                                }
                            }

						$.ajax({
							url: "/dhnbiz/sender/coin_wp",
							type: "POST",
							data: {<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
                                    "res_date" : res_date,
					 		       "2ndflang":_2ndflag
							...
							success: function (json) {
								...
								if(limit > 0) {
                                    if(res_date == ""){
                                        ...
                                        content_msg += "<div class='modal_info_row'><label>발송가능("+res_date+")</label>" + (limit - dsent).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',').replace(/NaN/,'0') + "건</div>";
                                    }
								}
                                if(limit == 0){ $(".content").html("폰문자 발신등록되지 않아 발송 불가합니다.<br/> 고객센터로 문의하세요.");
                                 modal_open('myModal');return false;}
								...
								if (!$("#switch_upload").is(":checked")) {
                                    ...
                                    if(res_date!=""){
                                        if(limit > 0 && (Number(dsent)+Number(row_index)) > limit) { showLimit("잔여한도 : "+(Number(limit)-Number(dsent))+"건 / 일일한도 : "+limit+"건",res_date); return; }
                                    }else{
                                        if(limit > 0 && (Number(sent)+Number(row_index)) > limit) { showLimit("잔여한도 : "+(Number(limit)-Number(sent))+"건 / 일일한도 : "+limit+"건","금일"); return; }
                                    }
									...
									if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
										...
										if($("input:checkbox[id='lms_select']").is(":checked") == true){ //2차발송의 경우
											if(second_type == "at"){ //2차 알림톡 발송의 경우
												...
												content_msg += "<div class='modal_info_row'><label>2차 폰문자 예상 금액</label>"+ ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + charge_type + ")</div>";
											}
										}
                                        <? } ?>
							...
							success: function (json) {
								...
								if(limit > 0) {
                                    if(res_date == ""){
                                        ...
                                        content_msg += "<div class='modal_info_row'><label>"+res_date+"발송가능</label>" + (limit - dsent).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',').replace(/NaN/,'0') + "건</div>";
                                    }
								}
                                if(limit == 0){ $(".content").html("폰문자 발신등록되지 않아 발송 불가합니다.<br/> 고객센터로 문의하세요.");
                                 modal_open('myModal');return false;}
...
// 			}
		...
		if (totalByte <= 90) {
			$("#lms_limit_str").html("폰문자");
			$("#lms_character_limit").html("2000");
			$(".msg_type").attr("class", "msg_type pms");
