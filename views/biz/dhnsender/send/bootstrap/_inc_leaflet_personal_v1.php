...
<script>
	...
	function linkview(id){
		...
		if(link_type == "editor"){ //에디터
			var url = $('#btn_url_' + id).val(); //에디터 링크 URL
			...
			var btn_url = "";
                var btn_type = link_type;
				if (btn_type != undefined) {
					var focus;
					btn_url =  $('#btn_url_' + id).val();
			...
			if(url == ""){
				alert("스마트쿠폰을 선택하세요.");
				coupon_page('1', id); //스마트쿠폰 불러오기 모달창 열기 => /views/biz/inc/send_menu1.php 있음
				return;
			}else{
				coupon_preview(url, type); //스마트쿠폰 미리보기
			}
		}else{ //스마트전단
			var url = $('#btn_url_' + id).val();
			//alert("url : "+ url);
			if(url == ""){
				alert("스마트전단을 선택하세요.");
				smart_page('1', id); //스마트전단 불러오기 모달창 열기 => /views/biz/inc/send_menu1.php 있음
				return;
			}else{
