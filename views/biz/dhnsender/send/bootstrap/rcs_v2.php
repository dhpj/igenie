...
<script>
    ...
    var rcs_price = '<?=$this->Biz_dhn_model->price_rcs?>';
    var rcs_sms_price = '<?=$this->Biz_dhn_model->price_rcs_sms?>';
    var rcs_mms_price = '<?=$this->Biz_dhn_model->price_rcs_mms?>';
    var rcs_tem_price = '<?=$this->Biz_dhn_model->price_rcs_tem?>';
    ...
    <? if($mem->voucher_deposit>0&&$mem->mem_voucher_yn=='Y'){ ?>
        ...
        rcs_price = '<?=$this->Biz_dhn_model->price_v_rcs?>';
        rcs_sms_price = '<?=$this->Biz_dhn_model->price_v_rcs_sms?>';
        rcs_mms_price = '<?=$this->Biz_dhn_model->price_v_rcs_mms?>';
        rcs_tem_price = '<?=$this->Biz_dhn_model->price_v_rcs_tem?>';
    <? } ?>

    <?
        $fail_flag = !empty($fail_list) ? true : false;
        if ($fail_flag){
            $addr = "?" . "mid=" . $fail_mid . "&uid=" . $fail_uid . "&cnt=" . $fail_cnt . "&date=" . $fail_date . "&type=" . $fail_type . "&mtype=" . $mtype;
        } else {
            $addr = "";
        }
    ?>
...
<!-- 컨텐츠 전체 영역 -->
<!-- form action="/dhnbiz/sender/rcs/send" method="post" id="sendForm" name="sendForm" onsubmit="return false;" enctype="multipart/form-data" -->
...
<div id="mArticle">
					...
					<div class="switch_content" id="hidden_fields_reserve" style="display: none;">
						<div class="widget-content reserve_content">
							...
							function server_time()
							{
                                ...
                                //  // console.log("date : " + ""+year + "년 " + month + "월 " + date + "일("+ week +")");
                                // // $(".rcs_top_date").text(year + "년 " + month + "월 " + date + "일("+ week +")");
                                // $(".rcs_top_date").text(""+year + "년 " + month + "월 " + date + "일("+ week +")");
							}
							</script>
                            ...
                            var WEEKDAY = ['일', '월', '화', '수', '목', '금', '토'];
                                $(document).on('change', '#datetimepicker', function(){
                                     ...
                                     if(idays!=$("#reserve").val()&&$("#reserve").val()!=""){
                                         console.log("Selected date: " + $("#reserve").val() );
                                         var nownownow = new Date($("#reserve").val());
                                         var week = WEEKDAY[nownownow.getDay()];
                                         $(".rcs_top_date").text(""+nownownow.getFullYear() + "년 " + (Number(nownownow.getMonth()) + 1) + "월 " + nownownow.getDate() + "일("+ week +")");
                                         idays = $("#reserve").val();
                                     }
	...
	<div class="btn_send_cen">
		...
		<button class="btn_send"  onclick="all_send();">RCS발송</button>
	</div>
</div><!-- mArticle END -->
<!-- input 숫자만 입력가능/자동 하이픈 -->
<script type="text/javascript">
	var get_buttons = '<?=$tpl->tpl_button?>';
    if(get_buttons&&get_buttons!="null"){
    ...
    console.log("get_btn_cnt : "+ get_btn_cnt);
...
<script type="text/javascript">
	...
	$(".scrolltbody tbody tr").click(function() {
		...
		selected_profile = $(this).find(".tmp_brand").text();
	});
	   ...
	   *--------------------------------------*/?>
       function getArimtalkCont() {
   		...
   		if(dumyTempli_cont != ""){
			...
			dumyTempli_cont = dumyTempli_cont.replace(/<p> <\/p>/gi, '\n\n');
			// if(get_btn_cnt > 0){ //버튼이 있는 경우
			// 	var link_type = document.getElementsByName("link_type0").value; //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력)
			// 	var btn_url = "";
        ...
        returnTempli_cont = returnTempli_cont.replace("{{free}}", "내용작성");
        returnTempli_cont = returnTempli_cont.replace("{{날짜}}", "행사 날짜");
        returnTempli_cont = returnTempli_cont.replace("{{변수}}", "행사 날짜");
        returnTempli_cont = returnTempli_cont.replace("{{등급}}", "등급");
       ...
       function getRcsCont() {
		...
		if(dumyTempli_cont != ""){
            ...
            showSnackbar("내용을 먼저 작성해주십시오.", 1500);
            $("#templi_cont").focus();
        }
        // console.log("dumyTempli_cont : "+dumyTempli_cont);
		//alert("dumyTempli_cont : "+ dumyTempli_cont);
			...
			if (i == 0) {
                ...
                if (var_name == "") {
					...
					if (varName == "{{변수내용}}" || varName == "{{회원정보 등록일}}") {
						var_name = "행사 기간";
					} else if (varName == "{{free}}") {
						var_name = "내용작성";
					} else if (varName == "{{업체명}}") {
						var_name = "업체명";
					} else if (varName == "{{업체전화번호}}" || varName == "{{전화번호}}") {
						var_name = "전화번호";
					} else if (varName == "{{날짜}}" || varName == "{{변수}}") {
						var_name = "행사 날짜";
                    } else if (varName == "{{00}}") {
						var_name = "00";
					} else if (varName == "{{등급}}") {
    	...
    	if(url != "") {
    		if(pattern.test(url)) {
				...
				if((link_type == "editor" && bno == "1") || (link_type == "smartEditor" && bno == "2")){ //(에디터 && 1번째 버튼) or (스마트전단+에디터 && 2번째 버튼)
					...
					//alert("html : "+ html); return;
					$.ajax({
						url: "/dhnbiz/sender/rcs/ums_save",
						type: "POST",
						data: {
							"<?=$this->security->get_csrf_token_name()?>" : "<?=$this->security->get_csrf_hash()?>"
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
    ...
    <? if($mem->voucher_deposit>0&&$mem->mem_voucher_yn=='Y'){ ?>
        ...
        var rcs_price = '<?=$this->Biz_dhn_model->price_v_rcs?>';
        var rcs_sms_price = '<?=$this->Biz_dhn_model->price_v_rcs_sms?>';
        var rcs_mms_price = '<?=$this->Biz_dhn_model->price_v_rcs_mms?>';
        var rcs_tem_price = '<?=$this->Biz_dhn_model->price_v_rcs_tem?>';
    <? }else{ ?>
        var at_price = '<?=$this->Biz_dhn_model->price_at?>';
    	var sms_price = '<?=$this->Biz_dhn_model->price_sms?>';
    	var lms_price = '<?=$this->Biz_dhn_model->price_lms?>';
...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
    ...
    function getRCSTempli() {
		var dumyTempli_cont = $("#dummy_templi_cont").text();
		var temp = dumyTempli_cont.split("{{");
		var temp2 = new Array();
		var returnTempli_cont = dumyTempli_cont;
		for (var i = 0; i < temp.length; i++) {
			if (i == 0) {
                ...
                if(temp.length==1){
    				...
    				//var varName = "#{" + varsplit[0] + "}";
    				//var var_name = $("input[name=var_name]")[(i-1)].value;
    				var var_name = $("textarea[name^='var_name']")[(0)].value;
    				returnTempli_cont = returnTempli_cont + var_name;
	...
	function all_send() {
        ...
        var link_type_cnt = $("[name='link_type0']").length;
        ...
        if(link_type_cnt>1){
            ...
            if(link_type_cnt>0){
                link_type = document.getElementsByName("link_type0").value;
            }
        }
        ...
        if(rcs_select=="T"){
            rcslimit = 90;
            rcs_charge = rcs_tem_price;
        }else if(rcs_select=="S"){
            rcslimit = 100;
            rcstype = "RCS SMS";
            rcs_charge = rcs_sms_price;
        }else if(rcs_select=="L"){
            rcslimit = 1300;
            rcstype = "RCS LMS";
            rcs_charge = rcs_price;
        }else if(rcs_select=="M"){
            rcslimit = 1300;
            rcstype = "RCS MMS";
            rcs_charge = rcs_mms_price;
        }
		...
		// 	return;
		// }
        if (templi_cont.length > rcslimit) {
			alert(rcstype+"는 내용을 "+rcslimit+"자 이내로 입력하여야 합니다.");
			$("#templi_cont").focus();
			return false;
		}
		...
		if(get_btn_cnt > 0){ //링크버튼형
			...
			if((link_type == "smart" || link_type == "smartEditor") && psd_code == ""){ //스마트전단의 경우
				alert("스마트전단을 선택하세요.");
				smart_page('1'); //스마트전단 불러오기 모달창 열기
				return false;
			}else if(link_type == "coupon" && pcd_code == ""){ //스마트쿠폰의 경우
				alert("스마트쿠폰을 선택하세요.");
				coupon_page('1'); //스마트쿠폰 불러오기 모달창 열기
				return false;
			}
		}
		if ((link_type == "editor" || link_type == "smartEditor") && (summ == '' || summ == '<p><br></p>' || summ == '<p><span style="font-size: 18px;">﻿</span><br></p>')){ //에디터의 경우
			alert("에디터 내용을 입력하세요.");
			//$("#link_type").focus();
			window.scroll(0, getOffsetTop(document.getElementById("id_STEP3"))); //STEP 3 영역으로 이동
			return;
		...
		if(1 == 1){
			...
			$.ajax({
				url: "/dhnbiz/sender/rcs/equal_send_mem_id",
				type: "POST",
				...
				success: function (json) {
					...
					if (senderCheck == "F") {
	    				check = false;
	    				$(".content").html("RCS 작성한 계정과 발송계정이 틀립니다.<br>발송계정 다시 로그인 후 다시 작성하세요.");
	    				//$('#myModal').modal({backdrop: 'static'});
	    				//$(document).unbind("keyup").keyup(function (e) {
	    				//	var code = e.which;
	    				//	if (code == 13) {
							...
							success: function (json) {
									...
									if($("#reserve_check").is(":checked") == true) {
					    				if($("#reserve").val().trim() == ""){
					    					...
					    					if($("#reserve_hours").val() < 8 || $("#reserve_hours").val() > 19){
												if(mem_talk_time_clear_yn == "Y" && $('#lms_select').prop('checked') && $("#reserve_hours").val() > 20){
													check = false;
								...
								if (check == true) {
									...
									$("div[name='field-data']").each(function () {
                                        ...
                                        var action_type = $(this).find(document.getElementsByName("link_type0")).val();

										if (btn_type != undefined) {
											var focus;
											var btn_url1 = $(this).find(document.getElementsByName("btn_url21")).val() || "";
											var btn_url2 = $(this).find(document.getElementsByName("btn_url22")).val() || "";
											//alert("["+ btn_no +"] btn_url1 : "+ btn_url1);
											if(btn_url1 != ""&&action_type!="dialer"){
                                                if (btn_type == "WL"&&action_type!="dialer") {
												...
												//if(pattern.test(btn_url1)) {
													if (btn_url1.toLowerCase().indexOf("http://") == 0 || btn_url1.toLowerCase().indexOf("https://") == 0) {
														...
														$(this).find(document.getElementsByName("btn_url21")).val(btn_url1);
													}
												//}else{
												//	check = false;
											...
											if(btn_url2 != ""){
												if (btn_url2.toLowerCase().indexOf("http://") == 0 || btn_url2.toLowerCase().indexOf("https://") == 0) {
													...
													$(this).find(document.getElementsByName("btn_url22")).val(btn_url2);
												}
											}
											if (btn_type == "WL"&&action_type!="dialer") {
												...
												if (btn_url2 == "" || !checkURL(btn_url2)) {
					    							if (btn_url1 == "" || !checkURL(btn_url1)) {
														check = false;
														focus = $(this).find(document.getElementsByName("btn_url22"));
								...
								if (check == true) {
									...
									$.ajax({
										...
										success: function (json) {
											...
											var kakao = Math.floor(Number(coin) / Number(rcs_charge));
											...
											if (!$("#switch_upload").is(":checked")) {
												...
												if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
													...
													if(1 == 1){
														...
														content_msg = "<div class='modal_info_row'><label>발송 종류</label>"+ rcstype +" "+ s2nd_msg + "</div>";
														...
														if (ms!="" && ($('#lms').val().replace(/ /gi, "") != "") && $("#lms_select").is(":checked") == true) {	// 2차문자 발송
															content_msg += "<div class='modal_info_row'><label>RCS 예상 금액</label>"+ ((Number(rcs_charge) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + rcs_charge + ")</div>";
															...
															content_msg += "<div class='modal_info_row'><label>예상 발송 금액</label>"+ ((Number(rcs_charge) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + rcs_charge + ")</div>";
														}
												...
												if (Number(coin) - (Number(charge_type) * Number(upload_send)) < 0) {
													...
													//if (mem_pay_type === "A") {
														...
														if (ms!="" && ($('#lms').val().replace(/ /gi, "") != "") && $("#lms_select").is(":checked") == true) {	// 2차문자 발송
															...
															content_msg += "<div class='modal_info_row'><label>예상 발송 금액</label>"+ ((Number(rcs_charge) * Number(upload_send)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + rcs_charge + ")</div>";
														}
	...
	function all_move() {
		...
		// var templi_cont = getRCSTempli();
        var templi_cont = $("#templi_cont").val();

        // console.log("templi_cont : " + templi_cont);
        // return;
        var rcs_select = $('input[name="rcs_select"]:checked').val();
		...
		$("div[name='field-data']").each(function () {
			var btn_type = $(this).find(document.getElementsByName("btn_type")).val();
            var btn_action = $(this).find(document.getElementsByName("link_type0")).val();
			if (btn_type != undefined) {
				obj[btn_num] = new Object();
				// obj[btn_num].type = btn_type;
				// obj[btn_num].name = $(this).find(document.getElementsByName("btn_name")).val();

                // obj[btn_num].action[0].urlAction[0].openUrl[0].url = $(this).find(document.getElementsByName("btn_url21")).val().trim();
                obj[btn_num].action=new Object();
                if(btn_action!="dialer"){
                    obj[btn_num].action.urlAction=new Object();
                    obj[btn_num].action.urlAction.openUrl=new Object();
                    obj[btn_num].action.urlAction.openUrl.url = $(this).find(document.getElementsByName("btn_url21")).val().trim();
                }else{
                    obj[btn_num].action.dialerAction=new Object();
                    obj[btn_num].action.dialerAction.dialPhoneNumber=new Object();
                    obj[btn_num].action.dialerAction.dialPhoneNumber.phoneNumber = $(this).find(document.getElementsByName("btn_url21")).val().trim();
                }

                obj[btn_num].action.displayText = $(this).find(document.getElementsByName("btn_name2")).val().trim();

                obj[btn_num].action.postback = new Object();
                if(btn_action!="dialer"){
                    obj[btn_num].action.postback.data = "set_by_chatbot_open_url";
                }else{
                    obj[btn_num].action.postback.data = "set_by_chatbot_dial_phone_number";
                }
                // if($(this).find(document.getElementsByName("btn_url22")).val().trim()==""){
                //     obj[btn_num].action.urlAction.openUrl.url = $(this).find(document.getElementsByName("btn_url21")).val().trim();
                // }else{
                //     obj[btn_num].action.urlAction.openUrl.url = $(this).find(document.getElementsByName("btn_url22")).val().trim();
                // }
				// if (btn_type == "WL") {
        ...
        var btnsss = "";
        var btnjson = [];
        btnjson[0] = new Object();
        var newArr = obj.filter((element) => {
          return element !== undefined && element !== null;
        });
        // obj = obj.splice(0, 1);
        btnjson[0].suggestions = newArr;

        var btnall = JSON.stringify(btnjson);
        // console.log("btnall : "+ btnall);
        // return;
        if(btnall=="[{\"suggestions\":[]}]"){
            btnall ="";
        }

        var cont_header_temp = $("#span_adv2").text() + $("#companyName2").val() + "\n";
		var cont_footer_temp = "\n" + $("#span_unsubscribe2").text() + $("#unsubscribe_num2").val();
        templi_cont = cont_header_temp+templi_cont+cont_footer_temp;

        // console.log("templi_cont : "+ templi_cont);
        // return;

        var obj_con = new Object();

        if(rcs_select=="L"||rcs_select=="M"){
            obj_con.title = new Object();
            obj_con.title = $("#span_adv2").text() + $("#companyName2").val();
        }
        obj_con.description = new Object();
        obj_con.description = templi_cont;
        if(rcs_select=="M"){
            obj_con.media = new Object();
            obj_con.media = $("#hdn_rcs_mms_url").val();
        }


        var conall = JSON.stringify(obj_con);

        // console.log("btn1 : "+ btn1 + " /btn2 : " + btn2+ " /btn3 : "+ btn3 + " /btn4 : "+ btn4 + " /btn5 : " +btn5);
        // console.log("btnall : "+ btnall);
        // return;
		...
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) { //파일업로드 발송
            ...
            file_data.append("btnall", btnall);
            file_data.append("mbody", conall);
            ...
            file_data.append("mst_type3", mst_type3);
			...
			//alert(couponid);
			$.ajax({
				url: "/dhnbiz/sender/rcs/adv_send",
				type: "POST",
				data: file_data,
				processData: false,
				contentType: false,
		...
		//} else if ($("#switch_select").is(":checked")) {	// 고객 DB에서 그룹 선택
			...
			$.ajax({
				...
				data: {
					...
					"btn1":btn1,"btn2":btn2,"btn3":btn3,"btn4":btn4,"btn5":btn5,"btnall":btnall,"mbody":conall,"customer_all_count": customer_all_count,"customer_filter": customer_filter,"reserveDt":reserveDt, "couponid":couponid,
					"mst_type1" : mst_type1, "mst_type2" : mst_type2, "mst_type3" : mst_type3,	// 2019.01.31 이수환 추가 ; mst_type1(1차 문자 = 카카오 관련 문자), mst_type2(2차 문자 = 카카오톡 외의 문자)
            ...
            if ($("#fail_flag").length > 0){
                ...
                $.ajaxSettings.traditional = true;
                $.ajax({
                    url: "/dhnbiz/sender/rcs/adv_send_fail",
                    type: "POST",
                    data: {"<?=$this->security->get_csrf_token_name()?>":"<?=$this->security->get_csrf_hash()?>",
                    "tel_number[]":tel_number,"templi_cont":templi_cont,"msg":msg,"kind":kind,
                    "templi_code":code,"pf_key":profile,"senderBox":senderBox,
                    "btn1":btn1,"btn2":btn2,"btn3":btn3,"btn4":btn4,"btn5":btn5,"btnall":btnall,"mbody":conall,"reserveDt":reserveDt, "couponid":couponid,
                    "mst_type1" : mst_type1, "mst_type2" : mst_type2, "mst_type3" : mst_type3,	// 2019.01.31 이수환 추가 ; mst_type1(1차 문자 = 카카오 관련 문자), mst_type2(2차 문자 = 카카오톡 외의 문자)
                    "dhnlurl": $("#dhnl_url").val(),
                    "bizurl": $("#biz_url").val(),
                    "link_type": link_type, //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력) 2020-09-16 추가
	...
	function test_move() {
		...
		$("div[name='field-data']").each(function () {
			...
			if (btn_type != undefined) {
				obj[btn_num] = new Object();
				// obj[btn_num].type = btn_type;
				// obj[btn_num].displayText = $(this).find(document.getElementsByName("btn_name2")).val();
				// if (btn_type == "WL") {
				// 	obj[btn_num].action.urlAction.openUrl.url = $(this).find(document.getElementsByName("btn_url21")).val().trim();
				// 	if ($(this).find(document.getElementsByName("btn_url21")).val() != undefined) {
				// 		obj[btn_num].action.urlAction.openUrl.url = $(this).find(document.getElementsByName("btn_url22")).val().trim();
				// 	}
				// }
                obj[btn_num].action=new Object();
 				...
 				success: function (json) {
 					...
 					if (status == 'error') {
                        ...
                        if(rcs_select=="T"){
                            ...
                            send_price = rcs_tem_price;
                        }else if(rcs_select=="S"){

                            send_price = rcs_sms_price;
                        }else if(rcs_select=="L"){

                            send_price = rcs_price;
                        }else if(rcs_select=="M"){

                            send_price = rcs_mms_price;
                        }
 ...
 if($mem->mem_2nd_send=='mms') { /* 2차발신이 mms인 경우 이미지가 없는 발신은 lms 단가로 계산 : resend_price 적용 */ ?>
 						var total_price = (Number(ft_count) * Number(send_price)) +  (Number(mms_count) * Number(resend_price)) + ((Number(lms_count) - Number(mms_count)) * Number(lms_price));
 <?} else { /* 2차발신이 mms가 아닌 경우 이미지 여부와 관계없이 금액 계산 : resend_price 적용 */ ?>
 						var total_price = (Number(ft_count) * Number(send_price)) + ((Number(lms_count) - Number(sms_count)) * Number(resend_price)) + (Number(sms_count) * Number(sms_price));
 <?}?>
	...
	function onPreviewText() {
		...
		var msg_text = "";
        msg_text = cont_header_temp;

        if ($("#templi_cont").val().replace(/\n/g, "").length > 0) {
			msg_text = msg_text + $("#templi_cont").val() + cont_footer_temp;
		} else {
			msg_text = msg_text + "<span style='color:#999;'>메세지를 입력해주세요.</span>"+ cont_footer_temp;
		}
		//alert(msg_text);
		msg_text = msg_text.replace(/\n/g, "<br>");
...
//				onPreviewLms();
    ...
    function chkword_templi() {
        ...
        var cont_header_temp = $("#span_adv2").text() + $("#companyName2").val() + "\n";
        var cont_footer_temp = "\n"+$("#span_unsubscribe2").text() + $("#unsubscribe_num2").val();
		var msg_text = $("#templi_cont").val();
		var msg_length = cont_header_temp.length + msg_text.length + cont_footer_temp.length || 0;
        if(rcs_select=="T"){
            msg_length = msg_text.length || 0;
        }
        var rcs_select = $('input[name="rcs_select"]:checked').val();
		...
		// } else if ($("#img_url").val() == undefined || $("#img_url").val() == "") {
            ...
            }else if(rcs_select=="L"||rcs_select=="M"){
                var limit_length = 1300;
            }

            console.log("limit_length : "+limit_length);
            //var msg_length = $("#templi_cont").val().length;
            if (msg_length <= limit_length) {
                $("#type_num").html(msg_length);
            } else {
...
/*
		...
		for (var i = 0; i < temp.length; i++) {
			if (i == 0) {
                ...
                if (varName == "{{변수내용}}" || varName == "{{회원정보 등록일}}") {
					...
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var multi\" style=\"height:400px\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" maxlength='90' placeholder=\"내용을 작성하세요\"></textarea>";
				} else if (varName == "{{업체명}}") {
					...
					// 멀티라인이 필요없는 변수 한줄짜리.
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" maxlength='25' placeholder=\"행사 날짜\">"+ strMsg +"</textarea>";
                } else if (varName == "{{등급}}") {
					// 멀티라인이 필요없는 변수 한줄짜리.
					returnTempli_cont = returnTempli_cont + "<textarea name=\"var_name\" class=\"var single\" onkeyup=\"return view_preview();\" onKeypress=\"if(event.keyCode==13) { event.preventDefault(); }\" maxlength='10' placeholder=\"등급\">"+ strMsg +"</textarea>";
                } else if (varName == "{{00}}") {
					// 멀티라인이 필요없는 변수 한줄짜리.
	...
	<? if(!empty($spf)) { ?>
		...
		$('#send_template_content').html('').load(
			"/dhnbiz/sender/send/select_template_rcs_v2",
			{ <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
				add: "<?=$add?>",
				tmp_flag: "<?=$param['tmp_flag']?>",
				tmp_code: "<?=$param['tmp_code']?>",
				tmp_brand: "<?=$param['tmp_brand']?>",
                rcs_type: "<?=$param['rcs_type']?>",
                ...
                <? }else{ ?>
					...
					phn0 = '080-053-7590';
					<? } ?>
                    $("#companyName2").val($("#pf_ynm").val());
                    $("#unsubscribe_num2").val(phn0);
                    onPreviewText();
                    chkword_templi();
                <? } ?>
				//bindKeyUpEvent();
                var rcs_select = $('input[name="rcs_select"]:checked').val();
                if(rcs_select=="T"){
                    $(".rcs_msg_type").attr("class", "rcs_msg_type tem");
                }else if(rcs_select=="S"){
                    $(".rcs_msg_type").attr("class", "rcs_msg_type sms");
                }else if(rcs_select=="L"){
                    $(".rcs_msg_type").attr("class", "rcs_msg_type lms");
                }else if(rcs_select=="M"){
                    $(".rcs_msg_type").attr("class", "rcs_msg_type mms");
                }
    ...
    function selected_template(bottonId, tmp_flag) {
		...
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/rcs_v2");
    ...
    //템플릿내용 선택
    function selected_rcs_type(rcs_type) {
        iscoupon = $("#couponwrite").val();
		nft_value = $("#NFTVALUE").val();
		var psd_code = $("#psd_code").val();
		...
		document.body.appendChild(form);
		form.setAttribute("method", "post");
		form.setAttribute("action", "/dhnbiz/sender/send/rcs_v2<?=$addr?>");
        <?if (!empty($fail_type)){?>rcs_type = "<?=$fail_type?>";<?}?>
		...
		selectedField.setAttribute("type", "hidden");
		selectedField.setAttribute("name", "tmp_code");
		selectedField.setAttribute("value", $("#templi_id").val());
		form.appendChild(selectedField);

        // var selectedField = document.createElement("input");
		// selectedField.setAttribute("type", "hidden");
		// selectedField.setAttribute("name", "templi_cont");
		// selectedField.setAttribute("value", $("#templi_cont").val());
		// form.appendChild(selectedField);
        ...
        var selectedField = document.createElement("input");
		selectedField.setAttribute("type", "hidden");
		selectedField.setAttribute("name", "rcs_type");
		selectedField.setAttribute("value", rcs_type);
		form.appendChild(selectedField);
		...
		selectedField.setAttribute("type", "hidden");
		selectedField.setAttribute("name", "tmp_flag");
		selectedField.setAttribute("value", 'temp');
		form.appendChild(selectedField);
    ...
    function chg_link_type(id){
        ...
        if(link_type_cnt>1){
            ...
            if(buttonArr[idx-1]!=link_type){
                ...
                if(buttonArr.indexOf(link_type)>-1){
                    ...
                    if(link_type != "self"&&link_type != "dialer"){
                        ...
                        if(link_change_ori){
                            ...
                            if(link_change_ori=="smart" || link_change_ori=="editor" || link_change_ori=="coupon"){
                                ...
                                if(link_change_ori=="dialer"){
                                    $("#btn_url"+preindex).attr("type", "number");
                                    $("#btn_url"+preindex).attr("max", "11");
                                }else{
                                    $("#btn_url"+preindex).attr("type", "text");
                                    $("#btn_url"+preindex).removeAttr("max");
                                }

                            }
                            if(link_change_ori=="dialer"){
                                $("#btn_url"+preindex).attr("placeholder", "전화번호를 입력해주세요");
                                console.log("dialer0");
                            }else{
                                $("#btn_url"+preindex).attr("placeholder", "링크주소를 입력해주세요");
                            }
                        }
                    }
                    ...
                    if(link_type){
                        ...
                        if(link_type != "self"&&link_type != "dialer"){
                            ...
                            if(link_change_ori=="self"||link_change_ori=="dialer"){
                                $("#btn_url"+id).val("");
                                $("#btn_url"+preindex).val(nownow);
                                console.log("여기 1-2");
                            }
                        ...
                        if(link_type=="dialer"){
                            $("#btn_url"+id).attr("placeholder", "전화번호를 입력해주세요");
                            console.log("dialer1");
                        }else{
                            $("#btn_url"+id).attr("placeholder", "링크주소를 입력해주세요");
                        }
                    ...
                    if(link_type != "self"&&link_type != "dialer"){
                        if(link_change_ori == "self"||link_change_ori == "dialer"){ //직접입력
		...
		if(link_type == "self"||link_type == "dialer"){ //직접입력
        ...
        if(link_type=="dialer"){
            $("#btn_url"+id).attr("type", "number");
            $("#btn_url"+id).attr("max", "11");
        }else{
            $("#btn_url"+id).attr("type", "text");
            $("#btn_url"+id).removeAttr("max");
        }
		...
		$.ajax({
			...
			success: function(json){
				...
				if(btn_url != ""){
                    ...
                    //     $("#field-data-"+btn_cnt).find("[name=btn_url1]").val(btn_url);
                    //     $("#field-data-"+btn_cnt).find("[name=btn_url2]").val(btn_url);
		...
		$.ajax({
			...
			success: function(json){
				...
				if(btn_url != ""){
                    ...
                    $("#btn_url"+btn_cnt).val(btn_url);
                    // var btn_type = $("#field-data-"+btn_cnt).find("[name=btn_type0]").val();
                    //alert("btn_type : "+ btn_type);
                    // if (btn_type != undefined) { //첫번째 버튼만 링크 추가
	...
	function msg_save(use_yn){
		...
		$.ajax({
    		url: "/dhnbiz/sender/send/rcs_v2/save_adv_msg",
    		type: "POST",
    		data: {
    			 <?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>"
	...
	// 		"/dhnbiz/sender/send/rcs_v2/msg_save_list",
	// 		{
	// 			<?=$this->security->get_csrf_token_name() ?>: "<?=$this->security->get_csrf_hash() ?>",
    ...
    $(document).ready(function (e){
        if ("<?=$fail_flag?>" == "1"){
            ...
            $("#reception_list").append("<input type='hidden' id='fail_flag' value='<?=$fail_mid?>'>");
            $("#reception_list").append("<input type='hidden' id='fail_mtype' value='<?=$mtype?>'>");
            $("#reception_list").append("<input type='hidden' id='fail_uid' value='<?=$fail_uid?>'>");
        }
    });
</script>
