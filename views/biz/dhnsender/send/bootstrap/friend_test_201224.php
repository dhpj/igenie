	...
	<div class="modal-dialog" id="modalCheck">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="content"></div>
				<div class="modal_bottom">
					<button type="button" class="btn md btn-default cancel" onClick="modal_close('myModalAll');">취소</button>
...
<script type="text/javascript">
	...
	function chg_link_type(){
		...
		if(link_type == "self"){ //직접입력
			...
			if(summ == '<p><br></p>' || summ == '<p><span style="font-size: 18px;">﻿</span><br></p>' || summ == '<p><br><span style="font-size: 18px;">﻿</span></p>'){ //에디터의 경우
	...
	function select_profile() {
		...
		if(activeTr.length == 0 ) {
			alert("프로필을 먼저 선택해주세요."); $("#profile_select").focus(); return false;
		} else {
...
<?} ?>
	...
	function SetNum(obj) {
		if($("#pf_key").val()==null || $("#pf_key").val()==''){
			alert("프로필을 먼저 선택해주세요."); obj.value = ""; return false;
		} else {
			var val = obj.value;
			var re = /[^0-9]/gi;
...
?>
	...
	function all_send() {
		...
		if ($("input:checkbox[id='lms_select']").is(":checked")) { //2차발송
			var second_type = value_check("second_type"); //2차발송 구분(ms.문자, at.알림톡) 선택값
			if(second_type == "at"){ //알림톡
				//alert("templi_charCount : "+ templi_charCount());
				var emptyVar = isEmptyVariable(); //템플릿 변수값 체크
				if (emptyVar !== true) {
					if(emptyVar == 1){
						alert("2차발송 알림톡 날짜를 입력해주세요.");
					}else if(emptyVar == 2){
						alert("2차발송 알림톡 행사내용을 입력해주세요.");
					}else{
						alert("2차발송 알림톡 템플릿 "+ (emptyVar+1) +"번째 변수를 입력해주세요.");
					}
				...
				var templi_id = document.getElementById("templi_id").value; //알림톡 템플릿번호
				var tpl_button_cnt = document.getElementById("tpl_button_cnt").value; //알림톡 버튼수
				var summ = $('#summernote').summernote('code'); //에디터내용
				//alert("link_type : "+ link_type +", psd_url : "+ psd_url +", psd_code : "+ psd_code +", templi_id : "+ templi_id); return;
				//alert("summ : "+ summ);
				if(link_type == "smart" && psd_code == "" && tpl_button_cnt > 0){ //스마트전단의 경우
					alert("스마트전단을 선택하세요.");
					smart_page('1'); //스마트전단 불러오기 모달창 열기
					return;
				}else if(link_type == "editor" && (summ == '' || summ == '<p><br></p>' || summ == '<p><span style="font-size: 18px;">﻿</span><br></p>')){ //에디터의 경우
					alert("에디터 내용을 입력하세요.");
					window.scroll(0, getOffsetTop(document.getElementById("div_editor_box"))); //에디터 영역으로 이동
					return;
						...
						if (btn_type != undefined) {
							...
							if (btn_url1 == "" || !checkURL(btn_url1)) {
								...
								if(btn_no == 1){
									...
									alert(btn_no +"번째 버튼 링크주소가 입력되지 않았거나 링크주소가 올바르지 않습니다.");
								}
								focus.focus();
								return false;
							}
							if (btn_url2 == "" || !checkURL(btn_url2)) {
								if (btn_url1 == "" || !checkURL(btn_url1)) {
									...
									if(btn_no == 1){
										alert("버튼 링크주소를 입력해주세요.");
									}else{
										alert(btn_no +"번째 버튼 링크주소를 입력해주세요.");
									}
									focus.focus();
									return false;
								}else{
				...
				if($("#lms").val().replace(/ /gi,"")==""){
					alert("2차발송 문자 내용을 입력해주세요.");
					$("#lms").focus();
					return;
				}
				if(getByteLength($("#lms")) > 2000){
					alert("2차발송 문자 내용은 <?=($mem->mem_2nd_send=='sms') ? '90자(한글 45자)' : '2,000자(한글 1,000자)' ?> 이내로\n입력하여야 합니다.");
					$("#lms").focus();
					return;
				}
			}
		}
		//alert("2차발송 내용 OK"); return;
		    				...
		    				if (btn_type != undefined) {
		    					...
		    					if (btn_type == "WL") {
		    						if($(this).find(document.getElementsByName("btn_name2")).val().trim() == "") {
										...
										alert("웹링크 버튼의 버튼명을 입력해주세요."); focus.focus(); return false;
		    						...
		    						if($(this).find(document.getElementsByName("btn_name3")).val().replace(/ /gi, "") == "") {
		    							...
		    							alert("앱링크 버튼의 Android 버튼링크를 입력해주세요."); focus.focus(); return false;
		    							...
		    							alert("앱링크 버튼의 iOS 버튼링크를 입력해주세요."); focus.focus(); return false;
		    						}
		    						...
		    						alert("봇키워드 링크 버튼의 버튼명을 입력해주세요."); focus.focus(); return false;
		    						...
		    						alert("메시지전달 링크 버튼의 버튼명을 입력해주세요."); focus.focus(); return false;
		    					}
		    				}
		    			});
					}
									...
									if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
										...
										if($("input:checkbox[id='lms_select']").is(":checked") == true){ //2차발송의 경우
											if(ms!="" && ($('#lms').val().replace(/ /gi, "") != "" || ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != ""))) {
												content_msg += "<div class='modal_info_row'><label>2차발송 예상 금액</label>"+ ((Number(charge_type) * Number(row_index)).toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "원(" + row_index + " X " + charge_type + ")</div>";
											}
										}
			...
			$.ajax({
				...
				success: function (json) {
					...
					if (code == "success") {
						...
						alert("발송 요청되었습니다."); document.location.href = $('#navigateURL').val(); return;
					} else {
    						...
    						if (btn_type != undefined) {
    							...
    							if (btn_type == "WL") {
    								if($(this).find(document.getElementsByName("btn_name2")).val().trim() == "") {
    									...
    									modal_open('myModal'); focus.focus(); return false;
    								} else if ($(this).find(document.getElementsByName("btn_url21")).val().trim() == "" || $(this).find(document.getElementsByName("btn_url21")).val().trim() == "http://") {
    									check = false;
    									focus = $(this).find(document.getElementsByName("btn_url21"));
	...
	function chkword_templi() {
		...
		if ($("#img_url").val() != "" && $("#img_url").val() != undefined) {
			...
			if (msg_length <= limit_length) {
				...
				$("#type_num").html('<span class="noted_red">※ 메시지 내용이 '+ comma(limit_length) +'자가 넘어 발송할 수 없습니다.</span>'+ msg_length);
			}
	...
	function urlConfirm(obj) {
		...
		if(url != "") {
			if(pattern.test(url)) {
				...
				modal_open('myModal'); $(parent).find('input').focus(); return false;
			}
		} else {
...
//----------------------------------------------------------------------------------------------------------------
	...
	function btnSelectProfile() {
		...
		if('' == "none") {
			modal_open('profile_danger');
		} else {
			modal_open('profile_select');
...
<?} ?>
	...
	function include_customer(group ) {
		if(($("#sel_all").prop('checked') && $('#searchGroup').val()!='') || group == 'GS') {
			...
			if(confirm("선택하신 " + searchGroupText + " 고객 전체를 추가 하시겠습니까?")){ load_customer_all($('#searchGroup').val()); }
		}
	...
	function msg_save(msg_type) {
		...
		var second_flag = ($("input:checkbox[id='lms_select']").is(":checked") == true) ? "Y" : "N"; //2차발송 설정여부
		var second_type = value_check("second_type"); //2차발송 구분(ms.문자, at.알림톡) 선택값
		if (second_flag == "N" || second_type == "at") msg = "";
		//alert("msg : "+ msg +"\n"+ "msg_type : "+ msg_type +"\n"+ "msg_kind : "+ msg_kind +"\n"+ "kakao_msg : "+ kakao_msg +"\n"+ "kakao_img_filename : "+ kakao_img_filename +"\n"+ "kakao_img_url : "+ kakao_img_url +"\n"+ "second_flag : "+ second_flag +"\n"+ "second_type : "+ second_type); return;
		$.ajax({
	...
	//alert("date : "+ date +"\n"+"hour : "+ hour +"\n"+"minute : "+ minute +"\n"+"time : "+ time);
	if(time < "0800" || time > "2000"){
		alert("발송시간을 8:00 부터 20:00사이로 발송할 수 있습니다.");
	}
	*/
</script>