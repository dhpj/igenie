...
// 	}
	...
	function getArimtalkCont() {
		...
		if(dumyTempli_cont != ""){
			...
			<? if($tpl->tpl_id == "68" or $tpl->tpl_id == "74"){ //링크버튼형 ?>
...
//	 	}
	...
	function templi_preview(obj) {
		...
		var temp2 = new Array();
		var returnTempli_cont = dumyTempli_cont;
		//alert("returnTempli_cont : "+ returnTempli_cont);
...
// 	}
	...
	function all_send() {
		...
		//alert("link_type : "+ link_type +", psd_url : "+ psd_url); return;
		if($("#templi_code").val() == null) {
			//$(".content").html("<h4>템플릿을 먼저 선택해주세요.</h4>");
			//$('#myModal').modal({backdrop: 'static'});
		...
		<? if($rs->tpl_id == "68" or $tpl->tpl_id == "74"){ //링크버튼형 ?>
		if (link_type == "smart" && psd_url == "") { //스마트전단의 경우
			//$(".content").html("스마트전단을 선택하세요.");
			//$('#myModal').modal({backdrop: 'static'});
			//$('#myModal').on('hidden.bs.modal', function () {
			//	//$("input[name=var_name]")[0].focus();
			//	//$("textarea[name=var_name]")[0].focus();
										...
										if (btn_type != undefined) {
											...
											if (btn_type == "WL") {
												...
												if (btn_url2 == "" || !checkURL(btn_url2)) {
					    							...
					    							//$(".content").html("웹링크33 버튼의 PC 버튼링크를 입력해주세요.");
					    							//$("#myModal").modal('show');
					    							//$('#myModal').on('hidden.bs.modal', function () {
					    							//	focus.focus();
					    							//});
													alert(btn_no+"번째 PC 버튼 링크를 입력해주세요.");
													focus.focus();
													return;
					    						}
					    					} else if (btn_type == "AL") {
	...
	function all_move() {
		...
		//alert("link_type : "+ link_type); return;
		if ($("#switch_upload").is(":checked") && parseInt($("#upload_tel_count").text()) > 0) {		// upload 발송
			...
			file_data.append("dhnlurl", $("#dhnl_url").val());
			file_data.append("bizurl", $("#biz_url").val());
			file_data.append("html", $('#summernote').summernote('code'));
...
/*
	...
	function selected_template(bottonId) {
		...
		if (bottonId == "selected_temp2") { //링크버튼<br>[전단보기]
			selected_temp = 68;
		} else if (bottonId == "selected_temp3") { //링크버튼<br>[전단보기]<br>[주문하기]
			selected_temp = 74;
		...
		$.ajax({
			...
			success: function(json){
				...
				if(btn_rul != ""){
					...
					//2차발송 세팅
					document.getElementById("lms").value = "전단보기\n"+ btn_rul; //2차발송 내용
					//문자내용반영
					onPreviewLms();
					chkword_lms();
				}
