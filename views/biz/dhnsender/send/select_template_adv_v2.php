...
<?if($rs->tpl_profile_key) {?>
	...
	<script>
		...
		function adv_template_data() {
			...
			//알림톡버튼 데이타 조회
			$.ajax({
				url: "/dhnbiz/sender/send/talk_adv_v2/ajax_adv_template_data",
				type: "POST",
		...
		function view_templet(tpl_id, tpl_premium_yn, btn_name){
            ...
            // tpl_contents = tpl_contents.replace('#{00}', '00');
		...
		$("#modal_templet_list").sortable({
			stop: function(event, ui) {
				...
				$.ajax({
					url: "/dhnbiz/sender/send/talk_adv_v2/ajax_adv_template_sort",
					type: "POST",
