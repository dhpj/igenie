...
<?if($rs->tpl_profile_key) {?>
	...
	<!-- 알림톡버튼 변경하기 Modal -->
	<div id="modal_templet" class="dh_modal sel_tem_wrap">
	  <!-- Modal content -->
	...
	<script>
		...
		function adv_template_data(temp_type) {
			...
			//알림톡버튼 데이타 조회
			$.ajax({
				url: "/dhnbiz/sender/send/talk_adv_v4/ajax_adv_template_data",
				type: "POST",
				data: {
		...
		$("#modal_templet_list").sortable({
			stop: function(event, ui) {
				...
				$.ajax({
					url: "/dhnbiz/sender/send/talk_adv_v4/ajax_adv_template_sort",
					type: "POST",
