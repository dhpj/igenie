...
<?if($rs->tpl_profile_key) {?>
	...
	<div class="input_content_wrap">
		<div class="mobile_preview">
			...
			<div class="preview_msg_wrap">
				...
				<div class="preview_option flex" <?=($temp_pro2=="Y")? "style='display:none;'" : "" ?>>
	...
	<div class="input_content_wrap" style="display:<?=($tpl_button_cnt == 0) ? "none" : ""?>;">
		...
		<div class="input_content">
			...
			<? }else{ ?>
				<?php include_once($_SERVER['DOCUMENT_ROOT'].'/views/biz/dhnsender/send/bootstrap/_inc_leaflet_new.php'); ?>
			<? } ?>
			...
			for(var i=0;i<btn_content.length;i++) {
				...
				if (btn_type == "DS" || btn_type == "BK" || btn_type == "MD") {
					...
					if (btn_name != "") {
                        ...
                        html2 +=  "		<button class='btn_con_copy btn_myModal' onClick='smart_goods_call(\"ai\");'>행사정보 가져오기</button>";
                        html2 +=  "</div>";
	...
	<script>
		...
		function adv_template_data() {
			...
			//알림톡버튼 데이타 조회
			$.ajax({
				url: "/dhnbiz/sender/send/talk_img_adv_v2/ajax_adv_template_data",
				type: "POST",
		...
		$("#modal_templet_list").sortable({
			stop: function(event, ui) {
				...
				$.ajax({
					url: "/dhnbiz/sender/send/talk_img_adv_v2/ajax_adv_template_sort",
					type: "POST",
