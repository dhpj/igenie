...
<div class="input_content_wrap">
	...
	<div class="input_content">
		...
		<div id="field"></div>
        <?php include_once($_SERVER['DOCUMENT_ROOT'].'/views/biz/dhnsender/send/bootstrap/_inc_leaflet_friend_v2.php'); ?>
	</div>
...
function image_list_open(page){
	//카카오톡 이미지 리스트 조회
	$.ajax({
		url: "/dhnbiz/sender/send/friend_v5/ajax_image_list",
		type: "POST",
...
function add_info(x) {
	...
	if ( fieldid < 5) {	// 5개만 생성
        ...
        if(fieldid == 0){
            ...
            $("#link_type1").val("self").prop("selected", true);
            // chg_link_type('1');
            // $("#btn_url"+(fieldid + 1)).val("");
            buttonArr.push("self");
            $("#link_type_option"+(fieldid + 1)).val("self");
...
function delete_info(obj) {
	...
	for (var i = (fieldIdNo + 1); i < fieldid; i++) {
		...
		rowTargetHtml = rowTargetHtml.replace("\"btn_type0"+ "\"", ("\"btn_type" + i) + "\"");
        rowTargetHtml = rowTargetHtml.replace("\"btn_url" + "\"", ("\"btn_url" + i) + "\"");
        rowTargetHtml = rowTargetHtml.replace("\"link_type00" + "\"", ("\"link_type" + i) + "\"");
        rowTargetHtml = rowTargetHtml.replace("\"link_type_option0" + "\"", ("\"link_type_option" + i) + "\"");
        rowTargetHtml = rowTargetHtml.replace("onchange=\"modify_btn_type(0);link_name(this,0);\"", "onchange=\"modify_btn_type(" + i + ");link_name(this," + i + ");\"");
        rowTargetHtml = rowTargetHtml.replace("onchange=\"chg_link_type(0);\"", "onchange=\"chg_link_type('" + i + "');\"");
        rowTargetHtml = rowTargetHtml.replace("onclick=\"linkview(0)\"", "onclick=\"linkview('" + i + "')\"");
        ...
        rowTargetHtml = rowTargetHtml.replace(/onkeyup=\"link_name\(this,0\);scroll_prevent\(\);\"/g, "onkeyup=\"link_name(this," + i + ");scroll_prevent();\"");
