...
<?if($rs->tpl_profile_key) {?>
					...
					if (btn_name != "") {
						...
						if(btn_type == "WL") { //웹링크
							...
							if(link_type == "smartEditor" && btn_name.replace(/ /g,"").indexOf("전단보기") > -1 && i == 1){ //스마트전단+에디터 & 전단보기 & 2번째 버튼의 경우
								html += "  <button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this, '"+ btn_count +"');\">"+ btn_name +"</button>"; //첫번째 버튼
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;background:#f2f2f2;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"링크주소를 입력해주세요.\" value=\"<?=$dhnl_url?>\" readonly>";
								html += "  </div>";
							}else if(alim_btn_url1 != "" && btn_name.replace(/ /g,"").indexOf("행사보기") > -1){ //알림톡 '행사보기' 버튼 URL 고정 사용의 경우
								$("#link_type").val("self"); //링크타입 직접입력으로 변경
								html += "  <button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this, '"+ btn_count +"');\">"+ btn_name +"</button>";
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"링크주소를 입력해주세요.\" value=\""+ alim_btn_url1 +"\" onkeyup=\"btnUrlCopy(this, '"+ btn_count +"');\">";
								html += "  </div>";
							}else if(alim_btn_url2 != "" && btn_name.replace(/ /g,"").indexOf("전단보기") > -1){ //알림톡 '전단보기' 버튼 URL 고정 사용의 경우
								html += "  <button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this, '"+ btn_count +"');\">"+ btn_name +"</button>";
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"링크주소를 입력해주세요.\" value=\""+ alim_btn_url2 +"\" onkeyup=\"btnUrlCopy(this, '"+ btn_count +"');\">";
								html += "  </div>";
							}else if(alim_btn_url3 != "" && btn_name.replace(/ /g,"").indexOf("주문하기") > -1){ //알림톡 '주문하기' 버튼 URL 고정 사용의 경우
								html += "  <button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this, '"+ btn_count +"');\">"+ btn_name +"</button>";
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"링크주소를 입력해주세요.\" value=\""+ alim_btn_url3 +"\" onkeyup=\"btnUrlCopy(this, '"+ btn_count +"');\">";
								html += "  </div>";
								...
								html += "  <button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"linkview();\">"+ btn_name +"</button>"; //첫번째 버튼
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;background:#f2f2f2;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"스마트전단 불러오기 버튼을 클릭하세요.\" value=\"<?=$psd_url?>\" readonly>";
								html += "  </div>";
							}else if(btn_count == 1 && btn1_type == "C"){ //첫번째 버튼 & 버튼1타입(S.직접입력, E.에디터사용, L.스마트전단, C.스마트쿠폰)
								html += "  <button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"linkview();\">"+ btn_name +"</button>"; //첫번째 버튼
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;background:#f2f2f2;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"스마트쿠폰 불러오기 버튼을 클릭하세요.\" value=\"<?=$pcd_url?>\" readonly>";
								html += "  </div>";
								...
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"링크주소를 입력해주세요.\" value=\"http://<?=$this->member->item('mem_mall_id')?>.pfmall.co.kr\" onkeyup=\"btnUrlCopy(this, '"+ btn_count +"');\">";
								html += "  </div>";
							}else{
								html += "  <button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this, '"+ btn_count +"');\">"+ btn_name +"</button>"; //두번째~ 버튼
								html += "  <div style=\"overflow: hidden;\">";
								html += "    <input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url1\" class=\"btn_loop_"+i+"\" placeholder=\"링크주소를 입력해주세요.\" value=\"<?//=$psd_url?>\" onkeyup=\"btnUrlCopy(this, '"+ btn_count +"');\">";
								html += "  </div>";
							}
							html += "</div>";
	...
	<script>
		...
		function open_templet(){
			...
			//알림톡버튼 데이타 조회
			$.ajax({
				url: "/dhnbiz/sender/send/talk_img_adv/ajax_adv_template_data",
				type: "POST",
		...
		$("#modal_templet_list").sortable({
			stop: function(event, ui) {
				...
				$.ajax({
					url: "/dhnbiz/sender/send/talk_img_adv/ajax_adv_template_sort",
					type: "POST",
