					...
					<?foreach($list as $r) {?>
						<li>
						  ...
						  <div class="img_detail" >
							<button id="dh_myBtn<?=$r->img_id?>" onClick="img_view('<?=$base_url?>/pop/<?=$r->img_filename?>');" class="btn_detail">이미지 상세보기</button>
						  </div>
						</li>
					<?}?>
				</ul>
...
<div id="dh_myModal" class="dh_modal">
	...
	<div class="modal-content2"> <span class="dh_close">&times;</span>
	  ...
	  <div class="modal-img" id="pre_img"><?//이미지 상세보기 영역?>
	  </div>
	</div>
</div>
<script type="text/javascript">
    function img_view(img) {
      ...
      var pre_img = $("#pre_img").html("<img src='"+img +"'>"); //이미지 상세보기 영역
			...
			*/
                $.ajax({
                    ...
                    error: function (data, status, er) {
                        alert("처리중 오류가 발생했습니다. 관리자에게 문의하십시오.");
						return;
                    }
                });
                function showResult(json) {
                    if (json['code'] == 'success') {
                        ...
                        } else if (message == 'InvalidImageSizeException' || message == 'InvalidImageResolutionException') {
                            messagekr = '가로 500px 미만 또는 가로*세로 비율이 1:1.5 초과시 업로드 불가합니다';
	...
	function delete_image() {
		...
		for (var i = 0; i < check_image("chk_image[]").length; i++) {
			//alert("image_name["+ i +"] : "+ image_name[i]);
