<?if($rs->tpl_profile_key) {?>
										...
										<div class="msg_box">
											...
											<textarea style="display:none" name="dumy_templi_cont" id="dumy_templi_cont" cols="30" rows="5" class="form-control autosize" placeholder="템플릿 내용을 입력해주세요." style="resize:none;cursor:default;"
													onkeyup="javascript: var file = document.getElementById('filecount').value;
											                 if (file==''){templi_preview(this); scroll_prevent(); templi_chk();}"><?=$rs->tpl_contents?></textarea>
								...
								<div class="input_content_wrap">
									...
									<script type="text/javascript">
    									function btn_url_variable_split(strUrl) {
    										var temp = strUrl.split("#{");
    										if (temp.length > 1) {
    											var varsplit = temp[1].split("}");
                                                ...
                                                if (btn_name != "") {
    												...
    												if(btn_type == "WL") {
    													<?if($rs->cc_idx) { // 2019.08.08. 쿠폰 발송 관련 시작?>
        													...
        													html += "<input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url1\" placeholder=\"링크주소를 입력해주세요.\" value=\"" + url_link1 + "\" readonly>";
        													html += "</div>";
        													html += "</div>"; 
        													if(url_link2 != null && url_link2 != "") {
            													...
            													//html += "<button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this);\">링크확인</button>";
            													html += "<div style=\"overflow: hidden;\">";
            													html += "<input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url2\" placeholder=\"링크주소를 입력해주세요.\" value=\"" + url_link2 + "\" readonly>";
            													html += "</div>";
            													html += "</div>";
        													}
            											<? } else {?>
            											url_link1 = btn_content[i]["linkMo"];
    													url_link2 = btn_content[i]["linkPc"];
    													var temp_url_link1 = btn_url_variable_split(url_link1);
    													var temp_url_link2 = btn_url_variable_split(url_link2);
    													
    													html += "<div class=\"link_info_wrap\">";
    													html += "<label>Mobile</label>";
    													html += "<div class=\"input_link_wrap\">";
    													if (temp_url_link1.length <= 1) {
        													html += "<label id=\"btn_url_http\" name=\"btn_url_http1\" style=\"float:left;\"></label>";
    													} else {
        													html += "<label class=\"fl\" style=\"margin-right:5px;line-height:38px;\" id=\"btn_url_http\" name=\"btn_url_http1\">" + temp_url_link1[0] + "</label>";
    													}
    													html += "<button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this);\">링크확인</button>";
    													html += "<div style=\"overflow: hidden;\">";
    													if (temp_url_link1.length <= 1) {
    														...
    														html += "<input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url1\" placeholder=\"" + temp_url_link1[1] + "\">";
    													}
    													html += "</div>";
    													html += "</div>"; 
    													html += "</div>"; 
    													if(url_link2 != null && url_link2 != "") {
        													...
        													html += "<label>PC</label>";
        													html += "<div class=\"input_link_wrap\">";
        													if (temp_url_link2.length <= 1) {
        														html += "<label id=\"btn_url_http\" name=\"btn_url_http2\" style=\"float:left;\"></label>";
            												} else {
            													html += "<label class=\"fl\" style=\"margin-right:5px;line-height:38px;\" id=\"btn_url_http\" name=\"btn_url_http2\">" + temp_url_link2[0] + "</label>";
        													}
        													html += "<button class=\"btn md fr\" style=\"margin-left: -1px;\" id=\"find_url\" onclick=\"urlConfirm(this);\">링크확인</button>";
        													html += "<div style=\"overflow: hidden;\">";
        													if (temp_url_link2.length <= 1) {
            													...
            													html += "<input type=\"url\" style=\"width:100%;\" id=\"btn_url\" name=\"btn_url2\" placeholder=\"" + temp_url_link2[1] + "\">";
        													}
        													html += "</div>";
        													html += "</div>";
        													html += "</div>";
    													...
    													html += "</div>"; 
    													html += "<div class=\"link_info_wrap\">";
    													html += '<label <?if($rs->cc_idx) { echo "style=\"display:none\"";} ?>>iOS</label>';
								...
								<div class="input_content_wrap">
									<label class="input_tit">시작일</label>
									<div class="input_content">
										<?=$rs->cc_start_date?>
										<!-- input class="form-control input-width-middle required" id="id_cc_start_date" name="cc_start_date" value="<?=$rs->cc_start_date?>" readonly-->
									</div>
								</div>
								<div class="input_content_wrap">
									<label class="input_tit">종료일</label>
									<div class="input_content">
										<?=$rs->cc_end_date?>
										<input class="form-control input-width-middle required" style="cursor:default;display:none" id="id_cc_end_date" name="cc_end_date" value="<?=$rs->cc_end_date?>" readonly>
									</div>
								</div>
								<div class="input_content_wrap">
									<label class="input_tit">당첨 페이지<BR> 이미지</label>
									<div class="input_content">
										<?=$rs->cc_img_url1?>
								...
								<div class="input_content_wrap">
									<label class="input_tit">당첨 페이지<BR> 텍스트</label>
									<div class="input_content">
										<div class="msg_box">
