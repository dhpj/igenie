...
<!-- 컨텐츠 전체 영역 -->
	<!-- form action="/biz/sender/talk/send" method="post" id="sendForm" name="sendForm" onsubmit="return false;" enctype="multipart/form-data" -->
	 ...
	 <input type='hidden' name='ntf_uplo' id='NFTVALUE' value='NFT' />
				<div id="mArticle">
							...
							<div class="input_content_wrap">
								...
								<div class="input_content">
									<div class="msg_box form_check">
										<div class="send_list" id="not_friend_titel">
											친구 미가입 고객
										</div>
										<div class="bottom tr">
											총 발송 <span class="num" id="not_friend_list">0</span>건
										</div>
...
$('#test_num_list').on(
		...
		<div class="modal-dialog" id="modal">
			<div class="modal-content">
				...
				<div class="modal_bottom">
					<button type="button" class="btn md modal_cancel" data-dismiss="modal">닫기</button>
				</div>
			</div>
		</div>
	</div>
	
	<div class="modal fade" id="template_select" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-lg" id="modal" data-keyboard="false" data-backdrop="static">
			<div class="modal-content alim_templet">
				...
				<!--div>
					...
					<button type="button" class="btn md modal_confirm" id="code" name="code" onclick="select_template();">확인</button>
				</div-->
			</div>
		</div>
	</div>
		...
		<div class="modal-dialog" id="modalCheck">
			<div class="modal-content">
				...
				<div class="modal-body icon_alarm">
					...
					<div class="modal_bottom">
						<button type="button" class="btn md dismiss" data-dismiss="modal">취소</button>
		...
		<div class="modal-dialog" id="modalCheck">
			<div class="modal-content select_send">
				...
				<div class="modal-body icon_alarm">
					<div class="content"></div>
					<div class="modal_bottom">
						<button type="button" class="btn md" data-dismiss="modal">선택추가</button>
	...
	<div class="modal select fade" id="myModalLoadCustomers" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog select-dialog">
         ...
         고객구분
							...
							<input type="text" class="form-control input-width-medium inline" id="searchName" name="searchName" placeholder="고객명 입력" value=""/>
							<input type="button" class="btn md btn-default" id="check" value="조회" onclick="search_question(1)"/>
						</div>
					...
					<div align="center">
						<br/>
						<button type="button" class="btn md dismiss" data-dismiss="modal">취소</button>
						<button type="button" class="btn md include_phns" name="code" onclick="include_customer()">선택 추가</button>
						<button type="button" class="btn md include_phns" name="code" onclick="include_customer('GS')">구분 전체추가</button>
						<br/><br/>
					</div>
				</div>
			</div>
...
// 	}
	...
	function isEmptyBtnUrl1() {
		var returnCheck = true;
		
		var btn_url1 = $("input[name=btn_url1]");
		for (i = 0; i < btn_url1.length; i++) {
			if(btn_url1[i].value.trim() == "") {
				returnCheck = i;
				break;
			}
		}
	...
	function isEmptyBtnUrl2() {
		var returnCheck = true;
		
		var btn_url2 = $("input[name=btn_url2]");
		for (i = 0; i < btn_url2.length; i++) {
			if(btn_url2[i].value.trim() == "") {
				returnCheck = i;
				break;
			}
		}
	...
	function all_send() {
		...
		var emptyBtnUrl1 = isEmptyBtnUrl1();
		var emptyBtnUrl2 = isEmptyBtnUrl2();
		...
		} else if (emptyBtnUrl1 !== true) {
			$(".content").html("링크버튼 URL 입력해주세요.");
			$('#myModal').modal({backdrop: 'static'});
			$('#myModal').on('hidden.bs.modal', function () {
				$("input[name=btn_url1]")[emptyBtnUrl1].focus();
			});
		} else if (emptyBtnUrl2 !== true) {
			$(".content").html("링크버튼 URL 입력해주세요.");
			$('#myModal').modal({backdrop: 'static'});
			$('#myModal').on('hidden.bs.modal', function () {
				$("input[name=btn_url2]")[emptyBtnUrl2].focus();
			});
		...
		} else if (parseInt($("#not_friend_list").text()) == 0) {		// 플러스 친구가 안님 고객
			$(".content").html("플러스 친구 미가입자가 없습니다.");
			$('#myModal').modal({backdrop: 'static'});
    	} else {
			var check = true;
					...
					if (check == true) {	
						...
						$.ajax({
							...
							success: function (json) {
								...
								var row_index = parseInt($("#not_friend_list").text());
	...
	function all_move() {
		...
		$("div[name='field-data']").each(function () {
			...
			if (btn_type != undefined) {
				...
				obj[btn_num].name = $(this).find(document.getElementsByName("btn_name")).val();
				if (btn_type == "WL") {
					var temp_url = $(this).find(document.getElementsByName("btn_url1")).val().trim().split("://");
					//obj[btn_num].url_mobile = $(this).find(document.getElementsByName("btn_url_http1")).text() + $(this).find(document.getElementsByName("btn_url1")).val();
					if (temp_url.length < 2) {
						obj[btn_num].url_mobile = $(this).find(document.getElementsByName("btn_url_http1")).text() + $(this).find(document.getElementsByName("btn_url1")).val();
					} else {
						obj[btn_num].url_mobile = $(this).find(document.getElementsByName("btn_url_http1")).text() + temp_url[1];
					}
					alert(obj[btn_num].url_mobile);
					if ($(this).find(document.getElementsByName("btn_url2")).val() != undefined) {
						var temp_url = $(this).find(document.getElementsByName("btn_url2")).val().trim().split("://");
						if (temp_url.length < 2) {
							obj[btn_num].url_pc = $(this).find(document.getElementsByName("btn_url_http2")).text() + $(this).find(document.getElementsByName("btn_url2")).val();
						} else {
							obj[btn_num].url_pc = $(this).find(document.getElementsByName("btn_url_http2")).text() + temp_url[1];
						}
						alert(obj[btn_num].url_pc);
					}
				} else if (btn_type == "AL") {
					var temp_url = $(this).find(document.getElementsByName("btn_url1")).val().trim().split("://");
					var temp_url2 = $(this).find(document.getElementsByName("btn_url2")).val().trim().split("://");
					if (temp_url.length < 2) {
						obj[btn_num].scheme_android = $(this).find(document.getElementsByName("btn_url_http1")).text() + $(this).find(document.getElementsByName("btn_url1")).val().trim();
					} else {
						obj[btn_num].scheme_android = $(this).find(document.getElementsByName("btn_url_http1")).text() + temp_url[1];
					}
					if (temp_url.length < 2) {
						obj[btn_num].scheme_ios = $(this).find(document.getElementsByName("btn_url_http2")).text() + $(this).find(document.getElementsByName("btn_url2")).val().trim();
					} else {
						obj[btn_num].scheme_ios = $(this).find(document.getElementsByName("btn_url_http2")).text() + temp_url2[1];
					}
		...
		if ($("#customer_all_send").val() != undefined) { // 플러스친구 미가입 고객
...
/*
							...
							*/
							...
							$("#not_friend_list").text(customer_count); 
							$("#customer_all_send").val(customer_count);
							$("#friend_list").val(customer_count);
					  } else {
                            $("#not_friend_titel").text("친구 미가입 고객이 없습니다.");
                            $(".content").html("고객정보가 없습니다.");
                            $('#myModal').modal({backdrop: 'static'});
					  }
				 },
	...
	$(document).ready(function() {
		$('#send_template_content').html('').load(
			"/biz/sender/send/select_template",
			{ <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
