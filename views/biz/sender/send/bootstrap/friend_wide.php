...
<?/*----------------------------------*
	...
	*----------------------------------*/?>
	...
	var ft_img_price = '<?=$this->Biz_model->price_ft_w_img ?>';
...
// 	}
	...
	function all_send() {
		...
		if (ft_img_price <= 0 && ft_img_price < base_wide_price) {
			$(".content").html("친구톡 와이드 이미지를 발송할 수 없습니다.<br>관리자에게 문의하세요.");
			$('#myModal').modal({backdrop: 'static'});	
							...
							success: function (json) {
								...
								if (!$("#switch_upload").is(":checked")) {
									...
									if (Number(coin) - (Number(charge_type) * Number(row_index)) < 0) {
										...
										content_msg += ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != "") ? "<br/><br/>친구톡 와이드 이미지 발송 예상 금액 : " + (Number(ft_img_price) * Number(row_index)).toFixed(2) + " 원(" + row_index + " X " + ft_img_price + ")" : "<br/><br/>친구톡 발송 예상 금액 : " + (Number(ft_price) * Number(row_index)).toFixed(2) + " 원(" + row_index + " X " + ft_price + ")";
										if(ms!="" && ($('#lms').val().replace(/ /gi, "") != "" || ($("#img_url").val().replace(/ /gi, "") != undefined && $("#img_url").val() != ""))) {
