...
function public_temp() {
    ...
    //alert(temp_id);
    $.ajax({
        url: "/dhnbiz/template/public_temp/",
        type: "POST",
        data: {
...
<form action="/dhnbiz/template/view/<?=$rs->tpl_id?>" method="get" id="mainForm">
				<div id="mArticle">
                ...
                <button class="btn md" id="_public_temp"  onclick="public_temp()" style="display:none">공용으로 설정</button>
							...
							<h3>템플릿 상세정보</h3>
              <a class="btn_tr" href="/dhnbiz/template/public_lists">뒤로가기</a>
						</div>
						<div class="inner_content white_box">
							...
							<div class="input_content_wrap">
								...
								<div class="input_content">
									<?if($rs->tpl_mem_id==$member_id && ($rs->tpl_inspect_status=="APL" || $rs->tpl_inspect_status=="REJ")) {?>
							...
							<div class="input_content_wrap">
								...
								<div class="input_content">
									<input type="hidden" id="inspect_status" value="APL">
							...
							<div class="input_content_wrap">
								...
								<div class="input_content">
                  <? if($rs->tpl_emphasizetype=='IMAGE'&&!empty($rs->tpl_imageurl)){   ?>
                    <img src="<?=$rs->tpl_imageurl?>" style="width:300px;"/><br/><br/>
                  <? } ?>
									<?if($rs->tpl_mem_id==$member_id && ($rs->tpl_inspect_status=="APL" || $rs->tpl_inspect_status=="REJ")) {?>
                                    ...
                                    <table class="tpl_ver_form" width="100%" border="1">
                                        ...
                                        <tr id="btn_content_0">
                                                ...
                                                <th style="vertical-align: middle !important;" rowspan="12">버튼 타입
                                                    <button class='btn sm' type='button' onclick='btn_add();'>
							...
							<div class="input_content_wrap">
								<div class="input_content">
        							...
        							<? if(($rs->tpl_mem_id==$member_id) && ($rs->tpl_inspect_status=="APL" || $rs->tpl_inspect_status=="REJ")) {?>
                                            <button class="btn_st1" type="button"
		...
		<? if($tpl_status->data->comments) { foreach($tpl_status->data->comments as $stat) {?>
									<tr>
										...
										<td class="text-left"><p><?=nl2br($stat->content)?></p></td>
									</tr>
		<?} }?>
		                            </tbody>
		                        </table>
						...
						<form action="#">
						<div class="inner_content white_box mg_t20">
							<div class="widget-content">
	                            <table class="tpl_ver_form" width="100%">
	                                ...
	                                <td>
	                                    <div class="flLeft" style="width:91%; float:left;">
		                                    <textarea id=comment_content rows="2" cols="3" name="textarea" maxlength="1999" class="form-control" style="cursor: default;" <? if(($rs->tpl_inspect_status == 'APL' || $rs->tpl_inspect_status == 'REJ' || $rs->tpl_inspect_status == 'COM') && $rs->tpl_status=="R" ) { } else { echo "disabled";} ?>></textarea>
	                                    </div>
	                                    <div class="flRight" style="width:9%; float:left;"> &nbsp;<input class="btn_st1" type="button" value="등록" onclick="javascript:comment_add()">
	                                    </div>
	                                </td>
	                                </tr>
	                                </tbody>
    ...
    <script type="text/javascript">
        ...
        function show_btn_name_url() {
			...
			?>
                    ...
                    for (var i = 0; i < btn_content.length; i++) {
                        ...
                        var url_mobile = btn_content[i]["url_mobile"];
                        var url_pc = btn_content[i]["url_pc"];
                        var scheme_android = btn_content[i]["scheme_android"];
                        var scheme_ios = btn_content[i]["scheme_ios"];
                        ...
                        if (btn_content[i]["type"] == "DS") {
                            ...
                            $("#wl_2_btn_url_" + counter).val(url_mobile);
                            $("#wl_3_btn_url_" + counter).val(url_pc);
                        } else if (btn_content[i]["type"] == "AL") {
                            ...
                            $("#al_2_btn_url_" + counter).val(scheme_android);
                            $("#al_3_btn_url_" + counter).val(scheme_ios);
                            $("#wl_2_btn_url_" + counter).val("http://");
                        } else if (btn_content[i]["type"] == "BK") {
                            $("#bk_1_" + counter).attr("hidden", false);
                            $("#bk_2_" + counter).attr("hidden", false);
                            $("#bk_1_btn_name_" + counter).val(btn_name);
                            $("#wl_2_btn_url_" + counter).val("http://");
                        } else if (btn_content[i]["type"] == "MD") {
			...
			?>
                    ...
                    for (var i = 0; i < btn_content.length; i++) {
                        ...
                        var scheme_android = btn_content[i]["scheme_android"];
                        var scheme_ios = btn_content[i]["scheme_ios"];
						if(btn_content[i]["type"]=="DS") btn_content[i]["linkTypeName"] = "배송조회";
						else if(btn_content[i]["type"]=="WL") btn_content[i]["linkTypeName"] = "웹링크";
						else if(btn_content[i]["type"]=="AL") btn_content[i]["linkTypeName"] = "앱링크";
						else if(btn_content[i]["type"]=="BK") btn_content[i]["linkTypeName"] = "봇키워드";
						else if(btn_content[i]["type"]=="MD") btn_content[i]["linkTypeName"] = "메시지전달";

                        var counter = i + 1;
                        if (btn_content[i]["type"] == "DS" || btn_content[i]["type"] == "BK" || btn_content[i]["type"] == "MD") {
                            ...
                            if (btn_content[i]["type"] == "WL") {
                                if (btn_content[i]["url_pc"] == null) {
                                    url_pc = '&nbsp;';
                                }
                                html += "<tr>";
                                ...
                                html += "<td><label style='font-weight: 100; vertical-align: middle !important; margin-right: 70px;'>Mobile</label><div style='margin-top : -23px; margin-left: 70px; word-break:break-all;'>" + url_mobile + "</div></td>";
                                html += "</tr>";
                                html += "<tr id='btn_content_" + counter + "'>";
                                html += "<td><label style='font-weight: 100; vertical-align: middle !important; margin-right: 70px;'>PC(선택)</label><div style='margin-top : -23px; margin-left: 70px; word-break:break-all;'>" + url_pc + "</div></td>";
                                html += "</tr>";
                            } else if (btn_content[i]["type"] == "AL") {
                                html += "<tr>";
                                html += "<td id='no' align='center' rowspan='2' style='vertical-align: middle !important;'>" + counter + "</td>";
                                html += "<td align='center' rowspan='2' style='vertical-align: middle !important;'>" + btn_content[i]["linkTypeName"] + "</td>";
                                html += "<td align='center' rowspan='2' style='vertical-align: middle !important;'>" + btn_name + "</td>";
                                html += "<td><label style='font-weight: 100; vertical-align: middle !important; margin-right: 70px;'>Android</label><div style='margin-top : -23px; margin-left: 70px; word-break:break-all;'>" + scheme_android + "</div></td>";
                                html += "</tr>";
                                html += "<tr id='btn_content_" + counter + "' class='" + counter + "'>";
                                html += "<td><label style='font-weight: 100; vertical-align: middle !important; margin-right: 70px;'>iOS</label><div style='margin-top : -23px; margin-left: 70px; word-break:break-all;'>" + scheme_ios + "</div></td>";
                                html += "</tr>";
                            }
                        }
                        $("#btn_content_" + i).after(html);
        ...
        function comment_add() {
            ...
            if (comment_content) {
                if ($('#comment_content').val().trim().length == 0) { //공백문자 일 경우
                    ...
                    $.ajax({
                        url: "/dhnbiz/template/comment_add/",
                        type: "POST",
                        data: {
                            <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
                            pf_key: pf_key,
        ...
        function delete_template() {
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    $("#myModal").modal('show');
                    $('#myModal').on('hidden.bs.modal', function () {
                        window.location.href = "/dhnbiz/template/lists/";
                    })
                },
                error: function (data, status, er) {
                    $(".content").html("처리중 오류가 발생하였습니다.<br/>관리자에게 문의해주시기 바랍니다.");
        ...
        function modify_template(tmp_code) {
            ...
            if ($('#input_tmp_code').val().trim().length == 0) { //공백문자 일 경우
                ...
                if (inspect_status == "APL" || inspect_status == "REJ") {
                    var isValid = true;
                    ...
                    $("#addedFormDiv tr").each(function () {
                        ...
                        if (btn_type != undefined) {
                            if (btn_type == "DS") {
                                ...
                                btnInfo.url_mobile = $(this).find(document.getElementsByName("btn_url21")).val();
                                btnInfo.url_pc = $(this).next('tr').find(document.getElementsByName("btn_url22")).val();
                                btn_content.push(btnInfo);
                                ordering++;
                            } else if (btn_type == "AL") {
                                btnInfo = new Object();
                                ...
                                btnInfo.scheme_android = $(this).find(document.getElementsByName("btn_url31")).val();
                                btnInfo.scheme_ios = $(this).next('tr').find(document.getElementsByName("btn_url32")).val();
                                btn_content.push(btnInfo);
                                ordering++;
                            } else if (btn_type == "BK") {
                                btnInfo = new Object();
                ...
                //등록 또는 반려일 경우 수정가능
                $.ajax({
                    url: "/dhnbiz/template/modify_success/",
                    type: "POST",
                    data: {
                        <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
                        pf_key: pf_key,
            ...
            if (inspect_status == "COM" || inspect_status == "REQ") {
