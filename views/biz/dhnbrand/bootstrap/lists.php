	...
	<div class="form_section">
		<div class="inner_tit">
			<!--<? if($public == "Y") { ?>
				...
				<button class="btn_tr" onclick="location.href='/dhnbiz/template/write/N'">공용템플릿 등록하기</button>
			<? } else {?>
...
<?
  ...
  <input type='hidden' name='<?=$this->security->get_csrf_token_name()?>' value='<?=$this->security->get_csrf_hash()?>' />
	<div class="white_box">
		<select class="input-width-large" id="pf_ynm" name="pf_ynm" onChange="search_template(0);">
		...
		<select id="inspect_status" name="inspect_status" onChange="search_template(0);">
			...
			<option value="REG" <?=($param['inspect_status']=="APL") ? 'selected' : ''?>>등록</option>
			<option value="REQ" <?=($param['inspect_status']=="REQ") ? 'selected' : ''?>>검수요청</option>
			<option value="COM" <?=($param['inspect_status']=="COM") ? 'selected' : ''?>>승인</option>
