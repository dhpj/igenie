							...
							<table>
								...
								<tbody>
										...
										<tr>
											...
											<td style="cursor:pointer;vertical-align:middle !important;" width="190" onclick="javascript:clickTrEvent(<?=$r->tpl_id?>)">
													<span rel="<?=$r->tpl_id?>"
											...
											<td>
												<div class="" id="tmplate_btn_<?=$r->tpl_id?>"></div>
												<script>
													var buttons = '<?=$r->tpl_button?>';
													//tmplate_buttons(buttons, 'tmplate_btn_<?=$r->tpl_id?>', '<?=$num?>');
													var btn = buttons.replace(/&quot;/gi, "\"");
													var btn_content = JSON.parse(btn);
													//if(num == 1) alert("btn_content.length : "+ btn_content.length);
													var html = '';
													for(var i=0;i<btn_content.length;i++) {
														var btn_name = btn_content[i]["name"];
														html += '<div class="tem_btn1">'+ btn_name +'</div>';
													}
													$("#tmplate_btn_<?=$r->tpl_id?>").html(html);
												</script><?//버튼정보?>
											</td>
											<?} else {?>
