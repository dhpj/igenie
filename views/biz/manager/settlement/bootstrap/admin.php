                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="align-center">문자<BR>SMS</th>
                            <th class="align-center">문자<br/>LMS</th>

                            <th class="align-center">RCS<BR>SMS</th>
                            <th class="align-center">RCS<br/>LMS</th>
                    ...
                    <?php
						...
						foreach ($list as $r) {
							...
							$row_margin_sum = $r->total_amt + $r->full_care + $r->month_deposit;
							$sum_charge = $sum_charge  + $r->month_deposit;
							...
							$sum_monthly_fee = $sum_monthly_fee  + $r->full_care;
                            ?>
                            <tr>
                                <? if(($r->mem_id!=$pre_mem_id&&$r->cnt>1)||$r->cnt<2){ ?>
                                	<? if ($r->mem_level >= 10) { ?>
                                    <th class="align-center" <?=($r->cnt>1)? "rowspan='".$r->cnt."'" : "" ?>><a href="#" onclick="show_page('<?=$r->mem_userid?>')"><?php echo $r->vendor ?></a></th><?//업체명?>
                                    <? } else { ?>
                                    <th class="align-center" <?=($r->cnt>1)? "rowspan='".$r->cnt."'" : "" ?>><?php echo $r->vendor ?></th><?//업체명?>
                                    <? } ?>

                                <td class="align-right" <?=($r->cnt>1)? "rowspan='".$r->cnt."'" : "" ?>><?php echo number_format($r->full_care); ?></td><?//월사용료?>
                                <? } ?>
                                <td class="align-right"><?php echo number_format($r->month_deposit); ?></td><?//충전금액?>
                                ...
                                <td class="align-right"><?php echo number_format($r->rcs_sms); ?></td><?//WEB(C) SMS?>
                                <td class="align-right"><?php echo number_format($r->rcs); ?></td><?//WEB(C)?>
                    ...
                    ?>
                            <tr>
                                <th class="align-center sum_bg">합  계</th>
                                <td class="align-right sum_bg"><?php echo number_format($sum_monthly_fee); ?></td><?//월사용료?>
                                <td class="align-right sum_bg"><?php echo number_format($sum_charge); ?></td><?//충전금액?>
                                <td class="align-right sum_bg"><?php echo number_format($sum_at); ?></td><?//알림톡?>
                                <td class="align-right sum_bg"><?php echo number_format($sum_ft); ?></td><?//친구톡 텍스트?>
                                <td class="align-right sum_bg"><?php echo number_format($sum_ft_img); ?></td><?//친구톡 이미지?>
                                <!--<td class="align-right sum_bg"><?php echo number_format($sum_ft_w_img); ?></td><?//친구톡 와이드 이미지?>-->
                                <!-- <td class="align-right sum_bg"><?php echo number_format($sum_grs); ?></td><?//WEB(A) KT,LG?> -->
                                <!-- <td class="align-right sum_bg"><?php echo number_format($sum_grs_biz); ?></td><?//WEB(A) SKT?> -->
                                <td class="align-right sum_bg"><?php echo number_format($sum_smt_sms); ?></td><?//WEB(C) SMS?>
                                <td class="align-right sum_bg"><?php echo number_format($sum_smt); ?></td><?//WEB(C)?>

                                <td class="align-right sum_bg"><?php echo number_format($sum_rcs_sms); ?></td><?//WEB(C) SMS?>
                                <td class="align-right sum_bg"><?php echo number_format($sum_rcs); ?></td><?//WEB(C)?>

                                <td class="align-right sum_bg"><?php echo number_format($sum_send); ?></td><?//발송 합계?>
                                <!-- <td class="align-right sum_bg"><?php echo number_format($sum_send_amount); ?></td><?//전송매출?> -->
                                <td class="align-right sum_bg"><?php echo number_format($sum_margin_amt); ?></td><?//수익금액?>
                            </tr>
                    <? } ?>
                    </tbody>
                </table>
	...
	function download_summary() {
		...
		if (rowCount < 1) {
           ...
           <? $sum_charge=0;
            ...
            foreach ($list as $r) {
                $row_sum = $r->total_sum;
                // $row_sum = $r->at + $r->ft + $r->ft_img + $r->ft_w_img + $r->grs + $r->grs_biz + $r->grs_sms + $r->nas + $r->nas_sms + $r->nas_mms + $r->grs_mms + $r->grs_biz + $r->smt + $r->smt_sms + $r->smt_mms + $r->rcs + $r->rcs_sms + $r->rcs_mms + $r->rcs_tem;
                $row_amount_sum = $r->at_amount + $r->ft_amount + $r->ft_img_amount + $r->grs_amount + $r->grs_biz_amount + $r->grs_sms_amount + $r->nas_amount + $r->nas_sms_amount + $r->nas_mms_amount + $r->grs_mms_amount + $r->smt_amount + $r->smt_sms_amount + $r->smt_mms_amount + $r->rcs_amount + $r->rcs_sms_amount + $r->rcs_mms_amount + $r->rcs_tem_amount;
                $row_margin_sum = $r->at_amt + $r->ft_amt + $r->ft_img_amt + $r->grs_amt + $r->grs_biz_amt + $r->grs_sms_amt + $r->nas_amt + $r->nas_sms_amt + $r->nas_mms_amt + $r->grs_mms_amt + $r->smt_amt + $r->smt_sms_amt + $r->smt_mms_amt + $r->rcs_amt + $r->rcs_sms_amt + $r->rcs_mms_amt + $r->rcs_tem_amt + $r->full_care;
