...
<div id="mArticle">
					<div class="form_section">
						...
						<div class="inner_content">
                        		...
                        		<div class="fl">
			                            <a href="javascript:void(0);" onclick="addForm();" class="btn md mr10"><i class="icon-plus"></i>입력란 추가</a>
										<a href="javascript:void(0);" onclick="standard_del();" class="btn md"><i class="icon-minus"></i>입력란 삭제</a>
                        		</div>
		                        <div class="mt30 tc">
			                            <button class="btn lg dark mr-20" type="button" id="cancel" name="cancel" onclick="location.href='/template/add/'">취소</button>
			                            <button class="btn lg yellow" type="button" onclick="javascript:register_template()">등록</button>
		                        </div>
    ...
    <script type="text/javascript">
        ...
        function addForm() {
            ...
            col_html += "			<div class='table_list mb-30'>";
            ...
            col_html += "					<td colspan='5' style='text-align:left'>";
            col_html += "						<input type='checkbox' id='securityFlag|col_no|' name='securityFlag'><label for='securityFlag'>보안 템플릿 체크 시, 메인 디바이스 모바일 외 모든 서브 디바이스에서는 메시지 내용이 노출되지 않습니다.</label>";
            col_html += "					</td>";
            col_html += "				</tr>";
            col_html += "				<tr>";
            col_html += "					<th>카테고리<span class='required'>*</span></th>";
            col_html += "					<td colspan='5' style='text-align:left'>";
            col_html += "                       <select style='width:300px' class='select2 input-width-medium none-search' id='categoryCode|col_no|' name='categoryCode'>";
            ...
            col_html += "                       <select style='width:300px' class='select2 input-width-medium none-search' id='templateMessageType|col_no|' name='templateMessageType'>";
            col_html += "                           <option value='BA'>기본형</option>";
            col_html += "                           <option value='EX'>부가정보형</option>";
            col_html += "                       </select>";
            col_html += "					</td>";
            col_html += "					<th>강조 유형</th>";
            col_html += "					<td colspan='2' style='text-align:left'>";
            col_html += "                       <select style='width:300px' class='select2 input-width-medium none-search' id='templateEmphasizeType|col_no|' name='templateEmphasizeType' onchange='changeEmpType(\"|col_no|\")'>";
            col_html += "                           <option value='NONE'>선택안함</option>";
            col_html += "                           <option value='IMAGE'>이미지형</option>";
            col_html += "                           <option value='TEXT'>강조표기형</option>";
            col_html += "                           <option value='ITEM_LIST'>아이템리스트형</option>";
            col_html += "                       </select>";
            col_html += "					</td>";
            col_html += "				</tr>";
            col_html += "				<tr id='image_row|col_no|' style='display:none'>";
            col_html += "					<th>이미지</th>";
            col_html += "					<td colspan='4' style='text-align:left'>";
            col_html += "						<div class='send_list'>";
		    col_html += "                         <input type='file' name='image|col_no|' id='image|col_no|' accept='.jpg, .png' />";
            col_html += "						  <span id='image_url_span|col_no|'></span>";
            col_html += "						  <input id='image_url|col_no|' type='hidden'  >";
		    col_html += "					    </div>";
            col_html += "					</td>";
            col_html += "					<td  style='text-align:left'>";
		    col_html += "                         <button class='btn md' type='button' onclick='image_upload(\"|col_no|\")'>업로드</button>";
            col_html += "					</td>";
            col_html += "				</tr>";
            col_html += "				<tr>";
            ...
            col_html += "					<td colspan='5' style='text-align:left'>";
            col_html += "                       <textarea name='input_tmp_cont' onkeyup='check_word_count(1000);' id='input_tmp_cont|col_no|' cols='30' rows='12' class='form-control required' placeholder='내용을 입력해주세요.'></textarea>";
            col_html += "						<DIV class=remaining style='color: #8c8c8c'><SPAN class='count'>0</SPAN>/1000</DIV>";
            col_html += "					</td>";
            col_html += "				</tr>";

            col_html += "               <tr id='btn_content_|col_no|'>";
            col_html += "					<th>버튼 타입</th>";
            ...
            col_html += "                       </select>";
            col_html += "                   </td>";
            col_html += "                   <td id='btn_add_msg|col_no|' align='center' rowspan='2' colspan='5'>버튼을 추가할 수 있습니다.<br><button class='btn md' type='button' onclick='javascript:btn_add(|col_no|);'><i class='icon-plus'></i> 추가</button></td>";
            //N
            ...
            //공통
            col_html += "                   <td id='btn_del|col_no|' align='center' rowspan='2' hidden><button class='btn sm delete' type='button' style='margin-top: 5px; padding: 3px 5px;' onclick='javascript:btn_del(|col_no|,1);'><i class='icon-trash'></i> 삭제</button></td>";
            col_html += "               </tr>";
...
//                 });
        ...
        function btn_add(obj) {
            ...
            //var tr_length = $("#addedFormDiv_"+obj+" tr").length - 2;
            var current_btn_num = lastItemNo;
            var add_btn_num = Number(current_btn_num)+1;
            //alert(tr_length);
            if(tr_length == 7 && $("#no" + obj).is(":hidden") == true) {
            ...
            // } else if (tr_length < 15) { // 업체명,템플릿코드,템플릿명,템플릿내용,버튼5개 = 9
            } else if (tr_length < 19) { // 업체명,템플릿코드,템플릿명,템플릿내용,버튼5개 = 9
                var btn_tr = tr_length-3-4;
                var no = btn_tr/2;
                var col_html = "";
                //공통
                ...
                //공통
                col_html += "                   <td id='btn_del" + obj + "' align='center' rowspan='2'><button class='btn sm delete' type='button' onclick='javascript:btn_del(" + obj + "," + add_btn_num + ");'><i class='icon-trash'></i> 삭제</button></td>";
                col_html += "               </tr>";
        ...
        function image_upload(col) {
    		var formData = new FormData();
			formData.append("<?=$this->security->get_csrf_token_name()?>", "<?=$this->security->get_csrf_hash()?>");
			formData.append("image", $("input[name=image" + col + "]")[0].files[0]);
    		...
    		$.ajax({
    			...
    			success: function (json) {
 					console.log(json);
 					if(json['code']=='0000') {
 						$("#image_url_span" + col).text(json['image']);
 						$("#image_url" + col).val(json['image']);
 					} else {
 	 					$("#image_url_span" + col).text(json['message']);
 	 					$("#image_url" + col).val("");
 					}
    			}
    		});            
        ...
        function changeEmpType(col) {
			var a = $("#templateEmphasizeType"+col).val();
			if(a == "IMAGE") {
				$("#image_row" + col).css("display","");
			} else {
				$("#image_row" + col).css("display","none");
			}
        }
    </script>
    ...
    <script type="text/javascript">
        ...
        function register_template() {
            ...
            for (var i = 1; i <= count; i++) {
                ...
                } else if (!$('#categoryCode'+i).val()) {
                    isValid = false;
                    $(".content").html("카테고리를 선택해주세요.");
                    $("#myModal").modal('show');
                    $('#myModal').on('hidden.bs.modal', function () {
                        $('#categoryCode'+i).focus();
                    });
				...
				if($("input:checkbox[id=securityFlag"+i+"]").is(":checked") == true) {
					securityFlag = true;	
				}		
                var item = {
                    "senderKey": pf_key,
                    ...
                    "templateEmphasizeType":emp_type,
                    "templateImageName":img_name,
                    "templateImageUrl":img_url,
            ...
            function show_response(str, success, fail) {
                if (jsonObj.length > 1 || str == null) {
                    ...
                    text = '템플릿 등록을 요청하였습니다.' + '\n' + '[요청결과] 성공 : ' + success + '건, ' + '실패 :' + fail + '건' +
                            '\n\n' + '<p class="comment highlight danger">템플릿 목록에서 <em class="marker">검수요청 버튼</em>을 클릭하셔야 정상적으로 요청이 완료됩니다.</p>';
                    $(".content").html(text.replace(/\n/g, "<br/>"));
                    $("#myModal").modal('show');
                    $('#myModal').on('hidden.bs.modal', function () {
                        window.location.href = "/dhnbiz/template/write<?=($public_flag == "Y") ? "/Y" : ""?>";
                    });
                }
