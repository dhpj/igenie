							...
							<div class="table_list">
								<table cellpadding="0" cellspacing="0" border="0">
									<colgroup>
								        ...
								        <col width="220px"><?//업체명?>
								        <col width="220px"><?//플러스친구 ?>
								        <col width="*"><?//발신 프로필 키?>
								        <col width="70px"><?//상태	?>
								        <col width="140px"><?//SMS발신번호?>
								        <col width="70px"><?//수정?>
								        <col width="70px"><?//확인?>
                        ...
                        <th>영업자</th>
								    ...
								    <tbody>
									    ...
									    <tr>
										    ...
										    <td><?=$row->spf_datetime?></td><?//등록일시	?>
										    <td><?=$row->spf_company?></td><?//업체명?>
										    <td>
												<? if($this->member->item('mem_level') >= 100){ ?>
													<input type="text" class="form-control" style="width: 100%;" id="pf_friend" name="pf_friend" maxlength="50" value="<?=$row->spf_friend?>" onchange="noSpaceForm(this);">
												<?} else { ?>
													<input type="hidden" name="pf_friend" id="pf_friend" value="<?=$row->spf_friend?>">
													<?=$row->spf_friend?>
												<? } ?>
											</td><?//플러스친구?>
										    <td>
												<input type="hidden" name="pf_mem_id" id="pf_mem_id" value="<?=$row->spf_mem_id?>">
												<? if($this->member->item('mem_level') >= 100){ ?>
													<input type="text" class="form-control" style="width: 100%;" id="pro_key" name="pro_key" maxlength="50" value="<?=$row->spf_key?>" onchange="noSpaceForm(this);">
												<?} else { ?>
													<input type="hidden" name="pro_key" id="pro_key" value="<?=$row->spf_key?>">
													<?=$row->spf_key?>
												<? } ?>
											</td><?//발신 프로필 키?>
										    <td><? if($row->spf_status=='S') { echo "차단"; } else if($row->spf_status=='D') {echo "삭제";} else { echo "정상";} ?></td><!-- SMS발신하는업체만 표시 -->
										    <td>
												<? if($row->spf_mem_id==$this->member->item('mem_id') or $this->member->item('mem_level') >= 100){ ?>
										    ...
										    <td>
												<? if($row->spf_mem_id==$this->member->item('mem_id') or $this->member->item('mem_level') >= 100){ ?>
													<button class="btn sm" type="button" onclick="sender_modify(this);">수정</button>
												<? } ?>
											</td><?//수정?>
										    <td><button class="btn sm" onclick="sender_refresh('<?=$row->spf_key?>', '<?=$row->spf_appr_id?>');">확인</button></td>
                                            <? if($this->member->item('mem_level') >= 100){ ?>
                                            <td><button class="btn sm" onclick="sender_smart('<?=$row->spf_friend?>', '<?=$row->spf_mem_id?>', '<?=$row->spf_public_id?>');">업데이트</button></td>
                                            <? } ?>
										    <td><?=$row->mem_sales_name?></td>
                      </tr>
									    <?}?>
								    </tbody>
								</table>
    ...
    <script type="text/javascript">
        ...
        function sender_modify(obj) {
			var pf_mem_id = $(obj).parent().parent().find("#pf_mem_id").val().trim();
			var pf_friend = $(obj).parent().parent().find("#pf_friend").val().trim();
        ...
        function sender_smart(frd, bid, pid) {

            if(frd == ""||bid==""){
                $(".content").html("업데이트 오류가 있습니다.");
                $('#myModal').modal({backdrop: 'static'});
            } else {
				$.ajax({
					 url: "/dhnbiz/sendprofile/sender_smart",
					 type: "POST",
					 data: {
						  <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
						  pf_mem_id: bid,
					 ...
					 success: function (json) {
                         if(json.code=="0"){
                             ...
                             alert("전단서버에 멤버정보가 없습니다.");
                         }else if(json.code=="2"){
                             alert("동일한 업데이트 정보가 이미 있습니다.");
                         }else{
                             alert("업데이트에 문제가 있습니다.");
                         }
					 },
					 error: function (data, status, er) {
						  //$(".content").html("처리중 오류가 발생하였습니다.<br/>관리자에게 문의해주시기 바랍니다.");
