...
<div id="mArticle">
	<div class="form_section">
		<div class="inner_tit">
			<h3>충전내역</h3>
			<button class="btn_st1 fr mg_t20" type="button" onclick="download_charge()"><i class="icon-arrow-down"></i>엑셀 다운로드</button>
		</div>
		<div class="white_box">
			<form id="mainForm" name="mainForm" method="post" action="/biz/myinfo/charge_history">
				...
				<div class="table_list">
					<table id="table_id">
						...
						<tbody>
							<?foreach($list as $row) {?>
							<tr>
								<td class="td01" width="25%" style="word-break:break-all;"><?=$row->amt_datetime?></td>
								<td width="20%" style="word-break:break-all;">
									<?if($row->amt_kind=="1") {?>
										₩<?=number_format($row->amt_amount)?>
									<?} else if ($row->amt_kind=="2") { ?>
										- ₩<?=number_format($row->amt_amount)?>
									<?} ?>
								</td>
								<td width="20%" style="word-break:break-all;">
									<?if($row->amt_kind!="1" && $row->amt_kind!="2") {?>₩<?=number_format($row->amt_amount)?><?}?>
								</td>
								<td width="35%" style="word-break:break-all;"><?=$row->amt_memo?></td>
							</tr>
							<?}?>
						</tbody>
					</table>
...
<script type="text/javascript">
	...
	function download_charge() {
		...
		if(list==''){
			...
			form.setAttribute("action", "/biz/myinfo/download_charge_history");
