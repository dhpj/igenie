    <!-- form action="/biz/sendprofile/remove" method="post" id="mainForm" -->
...
<!-- 타이틀 영역 END -->
						...
						<div class="white_box">
							...
							<div class="table_list">
								<table cellpadding="0" cellspacing="0" border="0">
								    ...
								    <thead>
									    <tr>
										    <th><input type="checkbox" class="uniform"></th>
										    <th>등록일시</th>
										    <th>업체명</th>
										    <th>플러스친구</th>
										    <th>발신 프로필 키</th>
								    ...
								    <tbody>
									    <?foreach($list as $row) {?>
									    <tr>
										    <td><input type="checkbox" class="uniform" id="pf_key" value="<?=$row->spf_key?>" name="pf_key[]"></td>
										    ...
										    <td><?if($row->spf_mem_id==$this->member->item('mem_id')) {?><input type="tel" class="form-control" style="width: 100%;" id="sms_sender" name="sms_sender" maxlength="25" onkeyup="SetNum(this);" value="<?=$row->spf_sms_callback?>" onchange="noSpaceForm(this);"><?} else { echo $row->spf_sms_callback; }?></td>
										    <td><input type="hidden" name="pro_key" id="pro_key" value="<?=$row->spf_key?>"><?if($row->spf_mem_id==$this->member->item('mem_id')) {?><button class="btn" type="button" onclick="sender_modify(this);"><i class="icon-pencil"></i>수정</button><?}?></td>
    ...
    <script type="text/javascript">
        ...
        function open_page(page) {
            ...
            form.setAttribute("action", "/biz/sendprofile/lists");
            document.body.appendChild(form);
