 ...
 <div id="mArticle">
					<div class="form_section">
						...
						<form class="form-horizontal row-border" id="writeform" name="writeform" method="post"
                          action="/biz/sendprofile/write" enctype="multipart/form-data">
								...
								<div class="input_content">
									<select name="category" id="category">
                                        	<?foreach($category["data"] as $row ) {
                                        	    ...
                                        	    ?>
                                        	    <option value="<?echo $row["code"];?>"><?echo $row["name"];?></option>
						...
						<div class="btn_group_lg mg_t30">
							<input type="submit" onclick="check();" value="등록" class="btn md dark btn-custom" name="register" id="register">
							<input class="btn md" type="button" id="cancel" name="cancel" value="취소" onclick="location.href='/biz/sendprofile/lists'">
						</div>
					</div>
 </div>
</form>
            ...
            var token = $("#token").val().replace(/ /gi,"");
				...
				try {
					...
					var settings = {
					  "async": false,
					  "crossDomain": true,
					  "url": "http://dhnmsg.com/api/sender/create",
//					  "url": "https://dev-alimtalk-api.bizmsg.kr:1443/v1/sender/create",
					  "method": "POST",
					  "headers": {
					...
					$.ajax(settings).done(function (text) {
						...
						$('#api_code').val(json['code']);
						$('#api_data').val(text);
						//alert(json['message']+","+json['code']+","+$('#pf_key').val()+","+text );

						if(json['code']=="fail") {
							if(json['message']=="이미 사용중인 플러스친구 입니다.") {
								if($('#pf_key').val()=="" || $('#pf_key').val().length < 40) {
									//alert($('#pf_key').val().length);
									$('#profile_key').show();
									alert(json['message']+"\n\n프로필 키를 입력하고 등록하실 수 있습니다.");
									$('#pf_key').focus();
									return false;
								} else {
									json['message'] = "";
				...
				if (success && phoneNumber != "" && biz_license != "" && yellowId != "" && token !="") {
					$('#overlay').fadeOut();
					return true;
				}
				return false;
