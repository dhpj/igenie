    ...
    public function form()
    {
        ...
        if ($form_validation === false OR $file_error !== '' OR $file_error2 !== '') {
            ...
            $mem_id = $this->Member_model->insert($insertdata);
				...
				/* 2017.11.27 : 신규등록인 경우 필요 테이블 생성 */
				if (!$this->cbconfig->item('use_register_email_auth')) {
					$this->load->model("Biz_free_model");
					$this->Biz_free_model->make_new_member_data($mem_id);
					$this->Biz_free_model->make_msg_log_table($this->input->post('mem_userid'));
					$this->Biz_free_model->make_customer_book($this->input->post('mem_userid'));
					$this->Biz_free_model->make_user_image_table($this->input->post('mem_userid'));
					$this->Biz_free_model->make_user_deposit_table($this->input->post('mem_userid'));
					$this->Biz_free_model->make_send_cache_table($this->input->post('mem_userid'));
				}
				/*-----------------------------------------------*/
