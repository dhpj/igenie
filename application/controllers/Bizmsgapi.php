...
class Bizmsgapi extends CB_Controller
{
    ...
    public function alimtalk()
    {
        ...
        if ($this->db->error()['code'] == 0) {
            if(!empty($kakao['profile_key'])) {
                ...
                //ib플래그
                $sql = "select count(1) as cnt,max(b.mem_userid) as userid, max(b.mem_id) as mem_id , max(a.spf_sms_callback) as sender_callback from ".config_item('ib_profile')." a inner join cb_member b on b.mem_id = a.spf_mem_id where spf_key = '".$kakao['profile_key']."' and spf_use = 'Y'";
                        ...
                        if(!empty($tmp->tpl_id)) {
                            ...
                            if ($this->db->error()['code'] > 0) {
                                ...
                                if(config_item('ib_flag')=="Y"){
                                    ...
                                    $this->dhnkakao_ib->sendAgentSendAPI('at', $gid,$mem_id, $userid,$kakao['profile_key'],'00000000000000', $sc_id);
                                }else{
    ...
    public function alimtalk_v()
    {
        ...
        if ($this->db->error()['code'] == 0) {
            if(!empty($kakao['profile_key'])) {
                ...
                //ib플래그
                if(!empty($kakao_d->mid)){
                    $sql = "select count(1) as cnt,max(b.mem_userid) as userid, max(b.mem_id) as mem_id, b.mem_2nd_send  from ".config_item('ib_profile')." a inner join cb_member b on b.mem_id = a.spf_mem_id where b.mem_id = ".$kakao_d->mid." and spf_key = '".$kakao['profile_key']."' and spf_use = 'Y'";
                }else{
                    $sql = "select count(1) as cnt,max(b.mem_userid) as userid, max(b.mem_id) as mem_id, b.mem_2nd_send  from ".config_item('ib_profile')." a inner join cb_member b on b.mem_id = a.spf_mem_id where spf_key = '".$kakao['profile_key']."' and spf_use = 'Y'";
                }


                // log_message('ERROR', '/bizmsgapi/alimtalk_v > sql : '. $sql);
                ...
                // 2019-02-25 이수환 cb_wt_msg_sent에 단가(9개 필드) 추가
                $sql2 = "select mad_mem_id, mad_price_at, mad_price_ft, mad_price_ft_img, mad_price_grs, mad_price_grs_sms, mad_price_grs_mms, mad_price_nas, mad_price_nas_sms, mad_price_nas_mms, mad_price_ft_w_img, mad_price_smt, mad_price_smt_sms, mad_price_smt_mms from cb_wt_member_addon where mad_mem_id=".$mem_id;
                // log_message('ERROR', '/bizmsgapi/alimtalk_v > sql2 : '. $sql2);
                $price = $this->db->query($sql2)->row();

                if($kakao_d->voucher == 'V'){
                    $sql2 = "select vad_mem_id, vad_price_at, vad_price_ft, vad_price_ft_img, vad_price_smt, vad_price_smt_sms, vad_price_smt_mms from cb_wt_voucher_addon where vad_mem_id=".$mem_id;
                    // log_message('ERROR', '/bizmsgapi/alimtalk_v > sqlv : '. $sql2);
                    $price_v = $this->db->query($sql2)->row();
                }

                if($kakao_d->voucher == 'V'){
                    $mst_price_at = $price_v->vad_price_at;
                    $mst_price_ft = $price_v->vad_price_ft;
                    $mst_price_ft_img = $price_v->vad_price_ft_img;
                    $mst_price_smt = $price_v->vad_price_smt;
                    $mst_price_smt_sms = $price_v->vad_price_smt_sms;
                    $mst_price_smt_mms = $price_v->vad_price_smt_mms;
                }else{
                    $mst_price_at = $price->mad_price_at;
                    $mst_price_ft = $price->mad_price_ft;
                    $mst_price_ft_img = $price->mad_price_ft_img;
                ...
                if($cnt>0) {
                    ...
                    $sql = "select * from ".config_item('ib_template')." where tpl_code = '".$kakao['tmp_number']."' and tpl_profile_key = '".$kakao['profile_key']."'";
                    ...
                    if(empty($tmp)) {
                        ...
                        $sql = "select * from ".config_item('ib_template')." where tpl_code = '".$kakao['tmp_number']."' and tpl_mem_id = '0'";
                        $tmp = $this->db->query($sql)->row();
                        //log_message('ERROR', "/bizmsgapi/alimtalk_test > sql : ". $sql);
                    }
                    ...
                    if ($this->db->error()['code'] == 0) {
                        if(!empty($tmp->tpl_id)) {
                            ...
                            while($btn_cnt >=1) {
                                ...
                                //ib플래그
                                if(config_item('ib_flag')=="Y"){
                                    if (strpos($btns[$idx]->url_mobile, "#{") !==false) {
                                        if (empty($kakao_url_m[$idx])) {
                                            $btnstr['url_mobile'] = $btns[$idx]->url_mobile;
                                        } else {
                                            $loc = 0;
                                            if(strpos($btns[$idx]->url_mobile, "://") !== false){
                                                $loc = strpos($btns[$idx]->url_mobile, "://") + 3;
                                            }
                                            $pre_url = substr($btns[$idx]->url_mobile, 0, $loc);
                                            $loc = 0;
                                            if(strpos($kakao_url_m[$idx], "://") !== false){
                                                $loc = strpos($kakao_url_m[$idx], "://") + 3;
                                            }
                                ...
                                //ib플래그
                                if(config_item('ib_flag')=="Y"){
                                    if (strpos($btns[$idx]->url_pc, "#{") !==false) {
                                        if (empty($kakao_url_pc[$idx])) {
                                            $btnstr['url_pc'] = $btns[$idx]->url_pc;
                                        } else {
                                            $loc = 0;
                                            if(strpos($btns[$idx]->url_pc, "://") !== false){
                                                $loc = strpos($btns[$idx]->url_pc, "://") + 3;
                                            }
                                            $pre_url = substr($btns[$idx]->url_pc, 0, $loc);
                                            $loc = 0;
                                            if(strpos($kakao_url_pc[$idx], "://") !== false){
                                                $loc = strpos($kakao_url_pc[$idx], "://") + 3;
                                            }
                            ...
                            // 2021-10-14 발송 플랫폼 추가
                            $data['mst_sent_platform'] = 'A';
                            // 2022-07-20 바우처 발송여부 추가 - 윤재박
                            // $data['mst_sent_voucher'] = $kakao_d->voucher;
                            if ($kakao_d->voucher == 'N'){
                                $sql = "
                                SELECT SUM(amt_amount * amt_deduct) as bamt FROM cb_amt_" . $userid . " WHERE FIND_IN_SET('바우처', amt_memo)=0 AND FIND_IN_SET('보너스', amt_memo)>0 AND amt_kind != '9'
                                ";
                                $bamt = $this->db->query($sql)->row()->bamt;
                            ...
                            // log_message("ERROR", "kakao_d->voucher > ".$kakao_d->voucher);
                            $this->db->insert("cb_wt_msg_sent", $data);
                            $gid = $this->db->insert_id();
                            $sc = array();
                            $sc['sc_tel'] = str_replace('-','',$kakao['kakao_phone']);
                            ...
                            if(config_item('ib_flag')=="Y"){
                                $this->load->library('Dhnkakao_ib');
                                $this->dhnkakao_ib->sendAgent2SendAPIs('at', $gid,$mem_id, $userid,$kakao['profile_key'],'00000000000000', $sc_id, $mem_send, $sms_kind);
                            }else{
                                $this->load->library('Dhnkakao');
    ...
    public function alimtalk_test()
    {
        ...
        $kakao['kakao_url5_2']= $kakao_d->kakao_url5_2;
        $kakao['kakao_2nd']= $kakao_d->kakao_2nd;
        $mst_type1 = !empty($kakao_d->mst_type1) ? $kakao_d->mst_type1 : 'at';
                    ...
                    if ($this->db->error()['code'] == 0) {
                        if(!empty($tmp->tpl_id)) {
                            ...
                            if(config_item('ib_flag')=="Y"){
                                ...
                                $this->load->library('Dhnkakao');
                                $this->dhnkakao->sendAgent2SendAPIs($mst_type1, $gid,$mem_id, $userid,$kakao['profile_key'],'00000000000000', $sc_id, $mem_send, $sms_kind);
                            }
    ...
    public function alimtalk2nds()
    {
        ...
        if ($this->db->error()['code'] == 0) {
            if(!empty($kakao['profile_key'])) {
                ...
                //ib플래그
                $sql = "select count(1) as cnt,max(b.mem_userid) as userid, max(b.mem_id) as mem_id  from ".config_item('ib_profile')." a inner join cb_member b on b.mem_id = a.spf_mem_id where spf_key = '".$kakao['profile_key']."' and spf_use = 'Y'";
                $cnt = $this->db->query($sql)->row()->cnt;
                $userid = $this->db->query($sql)->row()->userid;
                $mem_id = $this->db->query($sql)->row()->mem_id;
...
//';
    ...
    public function sample_img_alimtk()
    {
        $kakao_h = getallheaders();
        $kakao_d = json_decode(file_get_contents('php://input'));
        $debug = file_get_contents('php://input');
                            ...
                            if ($this->db->error()['code'] == 0) {
                                if(!empty($tmp->tpl_id)) {
                                    ...
                                    if ($this->db->error()['code'] > 0  ) {
                                        ...
                                        if(config_item('ib_flag')=="Y"){
                                            $this->load->library('dhnkakao_ib');
                                            $this->dhnkakao_ib->sendAgentSendAPI('ai', $gid,$mem_id, $userid,$kakao['profile_key'],'00000000000000', $sc_id);
                                        }else{
                                            $this->load->library('dhnkakao');
                                            $this->dhnkakao->sendAgentSendAPI('ai', $gid,$mem_id, $userid,$kakao['profile_key'],'00000000000000', $sc_id);
                                        }
                                    }
    ...
    public function msg()
    {
        log_message("ERROR",'Bizmsgapi/msg 시작');
        $msg_h = getallheaders();
        $msg_d = json_decode(file_get_contents('php://input'));
        ...
        $msg['userid'] = $msg_h["Authorization"];
        
        //log_message("ERROR","msg_d->msg_phn : ".$msg_d->msg_phn);
        $msg['msg_text'] = $msg_d->msg_text;
        $msg['msg_sender']= str_replace('-', '', $msg_d->msg_sender);
        $msg['msg_phn']= $msg_d->msg_phn;
        $msg['msg_mms_id']= $msg_d->msg_mms_id;
        $msg['msg_method']= 'WC';
        
        //log_message("ERROR","msg['msg_phn'] : ".$msg['msg_phn'] );
        
        $respo_no = 0;
        $respo_msg = "success";
        $mst_msg_id ="";
        
        if(empty($msg['msg_text'])) {
            $respo_no = '102';
            $respo_msg = '메시지 내용이 없습니다.';
            $error = 'Y';
        }
        
        if(empty($msg['msg_sender'])) {
            $respo_no = '103';
            $respo_msg = '발신자 전화번호가 없습니다.';
            $error = 'Y';
        }
        
        if(empty($msg['msg_phn'])) {
            $respo_no = '104';
            $respo_msg = '수신자 전화번호가 없습니다.';
            $error = 'Y';
        }
        
        if(empty($msg['msg_method'])) {
            $respo_no = '105';
            $respo_msg = '발송 방법을 선택 하세요.';
            $error = 'Y';
        }
        
        //log_message("ERROR",'userid : '.$msg['userid'] );
        
        $userinfo = $this->db->query("SELECT cm.mem_userid, cm.mem_id FROM cb_wt_send_profile_dhn p INNER JOIN cb_member cm ON p.spf_mem_id = cm.mem_id WHERE p.spf_key = '".$msg['userid']."'")->row();
        $userinfo_mem_userid = $userinfo->mem_userid;
        if (empty($userinfo->mem_userid)) {
            $userinfo_mem_userid = $msg['userid'];
        }
        
        $check1 = "select count(1) as cnt, max(cm.mem_id) as mem_id, cm.mem_2nd_send from cb_member cm where cm.mem_userid = '" .$userinfo_mem_userid. "' and cm.mem_useyn = 'Y'";
        //log_message("ERROR", "SQL : ".$check1);
        $rs = $this->db->query($check1)->row();
        $check_cnt = $rs->cnt;
        $mem_id = $rs->mem_id;
        $msg['msg_method'] =  $rs->mem_2nd_send;
        $second_msg_type = $rs->mem_2nd_send;
        ...
        // 2019-02-25 이수환 cb_wt_msg_sent에 단가(9개 필드) 추가
        $sql2 = "select mad_mem_id, mad_price_at, mad_price_ft, mad_price_ft_img, mad_price_grs, mad_price_grs_sms, mad_price_grs_mms, mad_price_nas, mad_price_nas_sms, mad_price_nas_mms, mad_price_ft_w_img, mad_price_smt, mad_price_smt_sms, mad_price_smt_mms, mad_price_cs, mad_price_il from cb_wt_member_addon where mad_mem_id=".$mem_id;
        //log_message("ERROR", "SQL2 : ".$sql2);
        $price = $this->db->query($sql2)->row();
        $mst_price_at = $price->mad_price_at;
        $mst_price_ft = $price->mad_price_ft;
        $mst_price_ft_img = $price->mad_price_ft_img;
        ...
        $mst_price_smt_sms = $price->mad_price_smt_sms;
        $mst_price_smt_mms = $price->mad_price_smt_mms;
        $mst_price_cs = $price->mad_price_cs; //캐러셀 단가
        $mst_price_il = $price->mad_price_il; //아이콘 리스트 단가
        
        if($error == 'N') {
            if($check_cnt > 0 ) {
                $exist = $this->db->query("show tables like 'cb_api_sc_".$msg['userid']."'")->result_array();
                if ($this->db->error()['code'] > 0) { echo '<pre> :: ';print_r($this->db->error()); exit; }
                //- API 처럼 사용자이면 테이별 생성
                if(count($exist) < 1) {
                    $sql = "CREATE TABLE `cb_api_sc_".$msg['userid']."` (
                                        `sc_id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '식별번호',
                                        `sc_name` VARCHAR(100) NULL DEFAULT '' COMMENT '고객명',
                                        `sc_tel` VARCHAR(15) NULL DEFAULT '' COMMENT '전화번호',
                                        `sc_content` VARCHAR(5000) NULL DEFAULT '' COMMENT '내용(친구톡/알림톡)',
                ...
                $del = "delete from cb_api_sc_".$msg['userid'];
                $this->db->query($del);
                
                $phns = explode(',', $msg['msg_phn']);
                $msg_cnt = 0;
                
                $msgLen = mb_strwidth($msg['msg_text'], 'UTF-8');
                
                $msgType = 'L';
                if(empty($msg_d->msg_mms_id)) {
                    if($msgLen <=90 ) {
                        $msgType = 'S';
                    }
                }
                
                //log_message("ERROR", "길이 : ".$msgLen."  / 메시지 Type : ".$msgType." :".$msg['msg_text']);
                $data = array();
                $data['mst_mem_id'] = $mem_id;
                $data['mst_template'] = '';
                $data['mst_profile'] = '';
                $data['mst_datetime'] =  cdate('Y-m-d H:i:s');
                $data['mst_kind'] =  "MS";
                $data['mst_content'] = $msg['msg_text'];
                $data['mst_sms_send'] =  ( $msgType == 'S' ? 'S': '');
                $data['mst_lms_send'] =  ( $msgType == 'L' ? 'L': '');
                $data['mst_mms_send'] =  "";
                $data['mst_sms_content'] =  ( $msgType == 'S' ? $msg['msg_text']: '');
                $data['mst_lms_content'] =  $msg['msg_text'];
                $data['mst_mms_content'] =  $msg_d->msg_mms_id;
                $data['mst_img_content'] =  "";
                $data['mst_button'] = '["[]","[]","[]","[]","[]"]';
                $data['mst_reserved_dt'] =  "00000000000000";
                $data['mst_sms_callback'] =  $msg['msg_sender'];
                $data['mst_type2'] = $msg['msg_method'].( $msgType == 'S' ? 'S': '');
                $data['mst_status'] = '1';
                $data['mst_qty'] =  1;
                $data['mst_amount'] = 0;	// ($this->Biz_model->price_ft * (($customer_all_count) ? $customer_all_count : count($tel_number)));
                ...
                if ($second_msg_type == "GREEN_SHOT") {
                    if ($msgType == 'L') {
                        $data['mst_type2'] = "wa";
                    } else if ($msgType == 'S') {
                        $data['mst_type2'] = "was";
                    }
                } else if ($second_msg_type == "NASELF") {
                    if ($msgType == 'L') {
                        $data['mst_type2'] = "wb";
                    } else if ($msgType == 'S') {
                        $data['mst_type2'] = "wbs";
                    }
                } else if ($second_msg_type == "SMART") {
                    if ($msgType == 'L') {
                        $data['mst_type2'] = "wc";
                    } else if ($msgType == 'S') {
                        $data['mst_type2'] = "wcs";
                ...
                $data['mst_price_smt_sms'] = $mst_price_smt_sms;
                $data['mst_price_smt_mms'] = $mst_price_smt_mms;
                $data['mst_price_cs'] = $mst_price_cs; //캐러셀 단가
                $data['mst_price_il'] = $mst_price_il; //아이콘 리스트 단가
                
                // 2021-02-18 발송 플랫폼 추가
                $data['mst_sent_platform'] = 'A';
                
                $sql = "
                SELECT SUM(amt_amount * amt_deduct) as bamt FROM cb_amt_" . $msg['userid'] . " WHERE FIND_IN_SET('바우처', amt_memo)=0 AND FIND_IN_SET('보너스', amt_memo)>0 AND amt_kind != '9'
                ";
                $bamt = $this->db->query($sql)->row()->bamt;
                $data['mst_sent_voucher'] = $bamt > 0 ? 'B' : 'N';
                ...
                foreach($phns as $phn) {
                    ...
                    $sc = array();
                    $sc['sc_tel'] = $phn;
                    $sc['sc_content'] =  $msg['msg_text'];
                    $sc['sc_lms_content'] =  $msg['msg_text'];
                    $sc['sc_sms_yn'] = 'N';
                    $sc['sc_sms_callback'] = $msg['msg_sender'];
                    $sc['sc_group_id'] = $gid;
                    $this->db->insert('cb_api_sc_'.$msg['userid'], $sc);
                    
                }
                
                $this->load->library('Message');
                $this->message->sendAgentSendAPIs($msgType, $gid,$mem_id, $msg['userid'],$second_msg_type,'00000000000000');
                $mst_msg_id = $gid;
                
            } else {
                $respo_no = '101';
                $respo_msg = 'User ID 가 존재 하지 않습니다.';
            }
        } else {
            log_message("ERROR",'userid : '.$msg['userid'] );
            log_message("ERROR",'msg_text : '.$msg['msg_text']);
            log_message("ERROR",'msg_sender : '.$msg['msg_sender']);
            log_message("ERROR",'msg_phn : '.$msg['msg_phn']);
            log_message("ERROR",'msg_method : '.$msg['msg_method']);
        }
        
        //log_message("ERROR", '{"response_code": "'.$respo_no.'", "response_message":"'.$respo_msg.'"}');
        
        header('Content-Type: application/json');
        echo '{"response_code": "'.$respo_no.'", "response_message":"'.$respo_msg.'", "msg_id":"'.$mst_msg_id.'"}';
    }
}
