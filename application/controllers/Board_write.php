...
class Board_write extends CB_Controller
{
    ...
    public function _write_common($board, $origin = '', $reply = '')
    {
        ...
        if ( check_use_captcha($this->member, $board) && element('brd_key', $board)!="request_tel") {
        ...
        if ($form_validation === false OR $file_error) {
			...
			if($this->member->item("mem_level") >= 100){ //2020-12-16
             ...
             */
				if($this->input->post('mode')=='iframe' || element('brd_key', $board)=="request_tel") {
					if(element('brd_key', $board)=="request") {
						echo '<script type="text/javascript">alert("사용문의가 접수되었습니다.");document.location.href="/write/'.element('brd_key', $board).'?mode=iframe";</script>"';
					} else if(element('brd_key', $board)=="request_tel") {
						$this->load->model("Biz_free_model");
						$this->Biz_free_model->adm_send_message($post_content);
						echo '<script type="text/javascript">alert("상담예약이 접수되었습니다.");parent.hide_cs();</script>"';
					} else {
						echo '<script type="text/javascript">alert("저장되었습니다.");document.location.href="/write/'.element('brd_key', $board).'?mode=iframe";</script>"';
					}
					exit;
				}
