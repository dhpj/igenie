 ...
 */
class Vbank extends CB_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = '/biz/manager/vbank';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Deposit', 'Biz');
    ...
    public function index()
    {
        ...
        //$view['view']['page'] = $page;
		$view['view']['page'] = preg_replace("/\<li\>\<a\ href\=\"\?page\=([0-9]*)\"/","<li style='cursor: pointer;' onclick='open_page($1)'><a herf='#' ",$view['view']['paging']);
		*/
		...
		$search_option = array('mem_realname' => '입금자', 'member.mem_username' => '회원명', 'member.mem_userid' => '회원아이디', 'dep_deposit_request' => $this->depositconfig->item('deposit_name'), 'dep_cash_request' => '총결제해야할 금액', 'dep_ip' => 'IP');
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_deposit_vbank_write';
        $this->load->event($eventname);
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_deposit_vbank_listdelete';
        $this->load->event($eventname);
