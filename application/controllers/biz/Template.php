...
class Template extends CB_Controller {
	...
	*/
	...
	public function lists()
	{
		...
		$view['total_rows'] = $this->Biz_model->get_template_count($this->member->item('mem_id'), $where, $param);	//get_table_count("cb_wt_template", $where, $param);
		...
		//	from cb_wt_template a inner join cb_wt_send_profile b on a.tpl_profile_key=b.spf_key and b.spf_use='Y'
	...
	function sync_template_status()
	{
		...
		//$req = $this->db->query("select tpl_profile_key, tpl_code from cb_wt_template where tpl_mem_id=? and tpl_inspect_status='REQ'", array($this->member->item('mem_id')))->result();
		$sql ="select ws.spf_appr_id, cwt.* from cb_wt_template cwt inner join cb_wt_send_profile ws on cwt.tpl_profile_key = ws.spf_key where ( tpl_mem_id in 
(WITH RECURSIVE cmem(mem_id, parent_id, mem_username) AS (
 ...
 or tpl_mem_id = 1 )";
				...
				if($result->code=="success") {
				    if($result->data->inspectionStatus != $r->tpl_inspect_status || $result->data->status!=$r->tpl_status || empty($r->tpl_inspect_status) || $r->tpl_status) {
				        if($result->data->inspectionStatus!=$r->tpl_inspect_status || empty($r->tpl_inspect_status)) {
							...
							$this->db->query("update cb_wt_template set tpl_status='".$result->data->status."' where tpl_mem_id=? and tpl_profile_key=? and tpl_code=?", array($this->member->item('mem_id'), $r->tpl_profile_key, $r->tpl_code));
						}
					}
				} else {
	...
	public function btn()
	{
		...
		$view = array();
		$view['rs'] = $this->Biz_model->get_row("cb_wt_template", "tpl_id=?", array($tmpl_id));
	...
	public function check_inspect()
	{
		...
		for($n=0;$n<$count;$n++) {
			...
			$this->db->query("update cb_wt_template set tpl_inspect_status='REQ', tpl_check_datetime=? where tpl_profile_key=? and tpl_code=? and tpl_use='Y' and tpl_inspect_status='REG'", array(cdate('Y-m-d H:i:s'), $pf_key[$n], $tmp_code[$n]));
	...
	public function check_delete()
	{
		...
		for($n=0;$n<$count;$n++) {
			...
			if($result[0]->code!="success") {
				...
				$this->db->query("update cb_wt_template set tpl_use='N' where tpl_profile_key=? and tpl_code=? and tpl_inspect_status='REJ'", array($pf_key[$n], $tmp_code[$n]));

				$err++;
				continue;
			}

			$this->db->query("update cb_wt_template set tpl_use='N' where tpl_profile_key=? and tpl_code=? and (tpl_inspect_status='REG' OR tpl_inspect_status='REJ')", array($pf_key[$n], $tmp_code[$n]));
			if($this->db->affected_rows() > 0) { $ok++; } else { $err++; }
		}
		header('Content-Type: application/json');
		echo '{"message": '.(($ok>0) ? 'null' : '"'.$err_msg.'"').', "success": '.$ok.', "fail": '.$err.' }';
	...
	public function hide_proc()
	{
	    ...
	    for($n=0;$n<$count;$n++) {
	        $this->db->query("update cb_wt_template set tpl_use='N' where tpl_profile_key=? and tpl_code=? ", array($pf_key[$n], $tmp_code[$n]));
	        if($this->db->affected_rows() > 0) { $ok++; } else { $err++; }
	    }
	    header('Content-Type: application/json');
	    echo '{"message": '.(($ok>0) ? 'null' : '"'.$err_msg.'"').', "success": '.$ok.', "fail": '.$err.' }';
	...
	public function check_delete_public()
	{
	    ...
	    for($n=0;$n<$count;$n++) {
	        ...
	        $this->db->query("update cb_wt_template set tpl_share_flag='U' where tpl_profile_key=? and tpl_code=? ", array($pf_key[$n], $tmp_code[$n]));
	...
	public function add_process()
	{
	    ...
	    foreach($json as $tr) {
	        ...
	        $data['tpl_id'] = ($partner) ? $this->Biz_dhn_model->get_table_next_id('cb_wt_template_dhn', 'tpl_id', "", array()) : 1;	//'tpl_mem_id=?', array($partner->spf_mem_id)) : 0;
	        if($dup || !$partner || !$data['tpl_id']) {
	            $err++;
	            log_message('ERROR', '처리중 오류 1:'.$dup.'/'.$data['tpl_id']);
	            continue;
	        }
	        
	        $img = $this->imgUpload();
	        // $filedata['file_path'], $filedata['file_type'], $filedata['file_name']
	        log_message("ERROR", "img path ".$img['file_path'] );
	        ...
	        $data['tpl_quickreplies'] = json_encode($tr->quickReplies);
	        $data['tpl_datetime'] = cdate('Y-m-d H:i:s');
	        $data['tpl_share_flag'] = ($partner->spf_key_type == 'G')?"P":"U";
	        $data['tpl_categorycode'] = $tr->categoryCode;
	        $data['tpl_securityflag'] = $tr->securityFlag;
	        $data['tpl_messagetype'] = $tr->templateMessageType;
	        $data['tpl_emphasizetype'] = $tr->templateEmphasizeType;
	        $data['tpl_extra'] = $tr->templateExtra;
	        $data['tpl_ad'] = $tr->templateAd;
	        $data['tpl_title'] = $tr->templateTitle;
	        $data['tpl_subtitle'] = $tr->templateSubtitle;
	        $data['tpl_type_group'] = "none";
	        //추가 2021-11-25 윤재박
	        $data['tpl_imageurl'] = $tr->templateImageUrl;
	        $data['tpl_imagename'] = $tr->templateImageName;
	        
	        /* 스윗트렉커로 전송 */
	        $swData = array(
	            "senderKey"=>$tr->senderKey
	            ...
	            ,"templateContent"=>$tr->templateContent
	            ,"templateExtra"=>$tr->templateExtra
	            ,"templateAd"=>$tr->templateAd
	            ,"templateImageName"=>$tr->templateImageName
	            ,"templateImageUrl"=>$tr->templateImageUrl
	            ,"templateTitle"=>$tr->templateTitle
	            ,"templateSubtitle"=>$tr->templateSubtitle
	            ,"senderKeyType"=>$tr->senderKeyType
	            ,"categoryCode"=>$tr->categoryCode
	            ,"securityFlag"=>$tr->securityFlag
	            ,"buttons"=>$tr->buttons
	            ,"quickReplies"=>$tr->quickReplies
	        );
	        ...
	        //if(!empty($img)) {
	        //    $result = $this->dhnkakao->template_create_with_image($swData, $img['file_path'], $img['file_type'],$img['file_name']);
	        //} else {
	    ...
	    header('Content-Type: application/json');
	    echo '{"message": '.(($ok>0) ? '"템플릿 등록 성공"' : '"'.$err_msg.'"').', "success": '.$ok.', "fail": '.$err.' }';
	}
	
	public function atimgupload() {
	    
	    $this->load->library('upload');
	    if (isset($_FILES) && isset($_FILES['image']) && isset($_FILES['image']['name']) && isset($_FILES['image']['name'])) {
	        ...
	        log_message("ERROR", "File : ".$upload_path);
	        $uploadconfig = '';
	        ...
	        if ($this->upload->do_upload()) {
	            ...
	            $this->load->library('dhnkakao');
	            $swResult = $this->dhnkakao->upload_at_image($filedata['file_path'], $filedata['file_type'], $filedata['file_name']);
	            
	            header('Content-Type: application/json');
	            echo json_encode($swResult);
	            // return $filedata;
	        }
	...
	public function view()
	{
		...
		$view = array();
		$view['view'] = array();
		$view['rs'] = $this->db->query("select a.*, b.spf_friend, b.spf_key_type, b.spf_sms_callback, b.spf_appr_id from cb_wt_template a inner join cb_wt_send_profile b on a.tpl_profile_key=b.spf_key and b.spf_use='Y' where tpl_id=? limit 1", array($key))->row();
		...
		if($view['tpl_status']->code=="success") {
			if(($view['rs']->tpl_inspect_status=="REQ" && $view['rs']->tpl_inspect_status!=$view['tpl_status']->data->inspectionStatus) || $view['tpl_status']->data->status!=$view['rs']->tpl_status) {
				if($view['rs']->tpl_inspect_status=="REQ" && $view['rs']->tpl_inspect_status!=$view['tpl_status']->data->status) {
					$sql = "update cb_wt_template set tpl_inspect_status='".$view['tpl_status']->data->inspectionStatus."', tpl_appr_id='admin', tpl_appr_datetime='".cdate('Y-m-d H:i:s')."' where tpl_id=?";
				} else {
					$sql = "update cb_wt_template set tpl_status='".$view['tpl_status']->data->status."' where tpl_id=?";
				}
	...
	public function modify_success()
	{
		...
		if($json->code!="success") {
			...
			$this->db->update('wt_template', $data, array("tpl_profile_key"=>$pf_key, "tpl_code"=>$tmp_code));
	...
	public function public_temp() {
	    ...
	    $sql = "update cb_wt_template set tpl_share_flag = 'P' where tpl_id = '".$temp_id."'";
	    $this->db->query($sql);
...
// 		);
	...
	public function download()
	{
		...
		foreach($ids as $id) {
			...
			$rs = $this->db->query("select * from cb_wt_template where tpl_id=? and tpl_use='Y'", array($id))->row();
...
*/
	...
	public function public_lists()
	{
	    ...
	    $view['total_rows'] = $this->Biz_model->get_public_template_count($where, $param);	//get_table_count("cb_wt_template", $where, $param);
