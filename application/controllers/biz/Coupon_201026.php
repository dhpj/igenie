...
class Coupon extends CB_Controller {
    ...
    public function index()
    {
        ...
        $view['total_rows'] = $this->Biz_model->get_table_count("cb_coupon", $where, $param);
        ...
        $sql = "select cc.*, (select count(1) as cnt from cb_coupon_list ccl where ccl.cl_cc_id = cc.cc_idx and ccl.cl_phn is not null) as used_cnt from cb_coupon cc where ".$where." order by cc_creation_date desc limit ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage'];
    ...
    public function view($cc_idx)
    {
        ...
        $sql = "select cc.cc_idx
                    ...
                    ,cc.cc_rate_txt
                    ,cc.cc_status
               from cb_coupon cc inner join cb_wt_template cwt on cc.cc_tpl_id = cwt.tpl_id
              where cc.cc_idx = '".$cc_idx."'";       
//log_message("ERROR", $sql);
    ...
    public function template()
    {
        ...
        $page_cfg['link_mode'] = 'open_page';
        $page_cfg['base_url'] = '';
        $page_cfg['total_rows'] = $this->Biz_model->get_table_count("cb_wt_template", $where, $param);
        $page_cfg['per_page'] = $view['perpage'];
        $this->pagination->initialize($page_cfg);
        $this->pagination->cur_page = intval($view['param']['page']);
        $view['page_html'] = $this->pagination->create_links();
