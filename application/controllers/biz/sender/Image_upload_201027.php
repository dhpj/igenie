...
class Image_upload extends CB_Controller {
    ...
    public function index()
    {
        ...
        $pf_key = $mem->spf_key;
		log_message("ERROR", $_SERVER['REQUEST_URI'] ."> pf_key : ".$pf_key);
        if(!$pf_key) { header('Content-Type: application/json'); echo '{"code":"error", "message", "발신 프로필이 없습니다. "}'; exit; }
        $msg = "";
        $file_error = '';
        $uploadfiledata = '';
        ...
        if (isset($_FILES) && isset($_FILES['image_file']) && isset($_FILES['image_file']['name']) && isset($_FILES['image_file']['name'])) {
            ...
            //log_message("ERROR", "Image_upload upload_path : ".$uploadconfig['upload_path']);
            //log_message("ERROR", "Image_upload allowed_types : ".$uploadconfig['allowed_types']);
            //log_message("ERROR", "Image_upload max_size : ".$uploadconfig['max_size']);
            //log_message("ERROR", "Image_upload encrypt_name : ".$uploadconfig['encrypt_name']);
            ...
            log_message("ERROR", $_SERVER['REQUEST_URI'] ."> Image_w_upload name : ".$_FILES['userfile']['name']);
            ...
            if ($this->upload->do_upload()) {
                log_message("ERROR", $_SERVER['REQUEST_URI'] ."> this->upload->do_upload()");
                ...
                $swResult = $this->sweettracker->upload_image($pf_key, $filedata['file_path'], $filedata['file_type'], $filedata['file_name']);
				log_message("ERROR", $_SERVER['REQUEST_URI'] ."> swResult->code : ". $swResult->code .", swResult->message : ". $swResult->message);
				...
				log_message("ERROR", $_SERVER['REQUEST_URI'] ."> img_url : ".$img_url);
