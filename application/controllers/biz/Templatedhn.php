<?php
class Templatedhn extends CB_Controller {
    /**
     * 모델을 로딩합니다
     */
    ...
    function sync_template_status()
    {
        ...
        $sql ="select ws.spf_appr_id, cwt.* from cb_wt_template_dhn cwt inner join cb_wt_send_profile_dhn ws on cwt.tpl_profile_key = ws.spf_key where ( tpl_mem_id in
(SELECT distinct  a.mem_id
        ...
        if($req) {
            ...
            foreach($req as $r) {
                ...
                $result = $this->dhnkakao->template($r->tpl_profile_key, $r->tpl_code);
        ...
        if($this->input->post('public_flag') == 'Y') {
            redirect('/biz/dhntemplate/public_lists');
        } else {
            redirect('/biz/dhntemplate/lists');
        }
    }
    ...
    public function btn()
    {
        ...
        $view = array();
        $view['rs'] = $this->Biz_dhn_model->get_row("cb_wt_template_dhn", "tpl_id=?", array($tmpl_id));
    ...
    public function take($public_flag='N')
    {
        ...
        $view['profile'] = $this->db->query("select a.* from cb_wt_send_profile_dhn a")->result();
        $view['profile_group'] = $this->Biz_dhn_model->get_profile_group();
        $view['view']['canonical'] = site_url();
    ...
    public function taket() 
    {
        ...
        $sql="select * from cb_wt_send_profile_dhn where spf_key='".$senderkey."'";
        $spf = $this->db->query($sql)->row();
        $skt = ((empty($spf->spf_key_type))?"S":"G");
        $json = $this->template($senderkey, $tcode, $skt);
        
        //log_message("ERROR", "Template Result : ".$json->code);
        $rtn = $json->code;
        if($json->code == "200" &&!empty($json->data)) {
            $temp = array();
            
            $sql="select * from cb_wt_template_dhn where tpl_profile_key='".$json->data->senderKey."' and tpl_code = '".$json->data->templateCode."'" ;
            $tpl = $this->db->query($sql)->row();
            
            if(empty($tpl->tpl_id )) {
                $temp['tpl_id'] = ($json->data) ? $this->Biz_dhn_model->get_table_next_id('cb_wt_template_dhn', 'tpl_id', "", array()) : 1;
    ...
    public function template($senderKey, $templateCode, $senderKeyType = "S")
    {
        ...
        curl_close($ch);
        //log_message('error', 'Templeate buffer : '.$buffer);
    ...
    public function add_process()
    {
        ...
        foreach($json as $tr) {
            ...
            //log_message("ERROR", json_encode($swData));
    ...
    public function view()
    {
        ...
        $view['tpl_status'] = $this->dhnkakao->template($view['rs']->tpl_profile_key, $view['rs']->tpl_code);
    ...
    public function public_lists()
    {
        ...
        $view['param']['searchStr'] = $this->input->post('searchStr');					//한국
        $param = array();	//$this->member->item('mem_id'));
        $where = " tpl_use='Y' and tpl_share_flag = 'P' and exists (select 1 from cb_wt_send_profile_group_dhn where spg_pf_key = tpl_profile_key)";	//and tpl_mem_id=? ";
        if($view['param']['pf_ynm']!="ALL" && $view['param']['pf_ynm']) { $where.=" and tpl_profile_key=? "; array_push($param, $view['param']['pf_ynm']); }
        if($view['param']['inspect_status']!="ALL" && $view['param']['inspect_status']) { $where.=" and tpl_inspect_status=? "; array_push($param, $view['param']['inspect_status']); }
        if($view['param']['tmpl_status']!="ALL" && $view['param']['tmpl_status']) { $where.=" and tpl_status=? "; array_push($param, $view['param']['tmpl_status']); }
        if($view['param']['comment_status']!="ALL" && $view['param']['comment_status']) { $where.=" and tpl_comment_status=? "; array_push($param, $view['param']['comment_status']); }
