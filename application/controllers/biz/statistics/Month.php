<?php
class Month extends CB_Controller {
	/**
	* ���� �ε��մϴ�
	*/
	...
	*/
	...
	public function index()
	{
		...
		$view['param']['startDate'] = ($this->input->post('startDate')) ? $this->input->post('startDate') : date('Y-m', strtotime(date('Y-m-d')."-3month"));
		$view['param']['endDate'] = ($this->input->post('endDate')) ? $this->input->post('endDate') : date('Y-m');
		...
		if($this->member->item('mem_level')==17) {
		    ...
		    if ($sales_cnt > 0) {
		        ...
		        $view['list'] = $this->Biz_model->get_sales_send_report('month', $view['param']['startDate'], $view['param']['endDate'], $view['param']['pf_key'], $sales_id, $view['param']['managerType']);
		    } else {
		        $view['list'] = $this->Biz_model->get_send_report('month', $view['param']['startDate'], $view['param']['endDate'], $view['param']['pf_key']);
		    }
