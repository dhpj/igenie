	...
	*/
	...
	public function index(){
        ...
        $in_ip = $_SERVER['REMOTE_ADDR'];
					// if($in_ip=="61.75.230.209"||$in_ip=="180.228.46.27"){
					// 	$skin = "main_type";
					// }
        // if($this->member->item('mem_id')==3||$this->member->item('mem_id')==2464){// if($in_ip=="61.75.230.209"){
        //     $skin = "main_type";
        // }
	...
	public function write(){
		...
		$skin = "write";
        $in_ip = $_SERVER['REMOTE_ADDR'];
				if($in_ip=="61.75.230.209"||$in_ip=="49.166.228.22"){
					$skin = "write_type_test";
				}
        // if($in_ip=="61.75.230.209"){
        //     $skin = "write_type";
        // }
        // if($this->member->item('mem_id')==3||$this->member->item('mem_id')==2464){
        //     $skin = "write_type";
        // }
        ...
        // if($this->member->item('mem_id')==3||$this->member->item('mem_id')==2464){// if($in_ip=="61.75.230.209"){
    		...
    		FROM cb_pop_screen_goods a
            left join cb_pop_screen_text c on a.psg_psd_id =c.pst_psd_id  AND a.psg_step_no= c.pst_step_no AND c.pst_mem_id = '". $this->member->item("mem_id") ."'  AND c.pst_seq = 0
    		WHERE psg_mem_id = '". $this->member->item("mem_id") ."' /* 회원번호 */

    		AND psg_psd_id = '". $psd_id ."' /* 전단번호 */
    		AND psg_step_no > 0 /* 스텝 */ and psg_box_no > 0
    		ORDER BY ". $sort ." ";
        // }else{
        //     $sql = "
    	// 	SELECT a.*
		...
		SELECT
            ...
            , tit_type
		FROM cb_design_title dt
		WHERE dt.tit_type in ('T','C')
		AND dt.tit_useyn = 'Y'
		ORDER BY dt.tit_type DESC, dt.tit_seq ASC ";
		// echo $sql ."<br>";
		$view['design_title_custom_list_t'] = $this->db->query($sql)->result();
		...
		SELECT
            ...
            , tit_type
		FROM cb_design_title dt
		WHERE dt.tit_type in ('I','C')
		AND dt.tit_useyn = 'Y'
		ORDER BY dt.tit_type DESC, dt.tit_seq ASC ";
		// echo $sql ."<br>";
		$view['design_title_custom_list_i'] = $this->db->query($sql)->result();
	...
	public function smart_temp_save(){
		...
		$sql = "INSERT INTO cb_pop_screen_goods(psg_mem_id, psg_psd_id, psg_tit_id, psg_step, psg_step_no, psg_imgpath, psg_name, psg_option, psg_price, psg_dcprice, psg_badge, psg_seq, psg_box_no, psg_option2, psg_mng_yn, psg_code, psg_stock, psg_no_order) SELECT psg_mem_id, psg_psd_id, psg_tit_id, psg_step, psg_step_no, psg_imgpath, psg_name, psg_option, psg_price, psg_dcprice, psg_badge, psg_seq, psg_box_no, psg_option2, psg_mng_yn, psg_code, psg_stock, psg_no_order FROM cb_pop_screen_goods_temp WHERE psg_psd_id = '".$psd_id."'";
		$this->db->query($sql);
    ...
    public function check_shop(){
        ...
        if($shopcnt == 0){
            ...
            // log_message("ERROR", $_SERVER['REQUEST_URI'] ." > check_shop 0 smd_mem_shop_notice : ".$this->member->item("mem_shop_notice"));
    ...
    public function check_shop2(){
        ...
        if($shopcnt == 0){
            ...
            $mem_data["smd_mem_shop_addr"] = $this->member->item("mem_shop_addr");
            $mem_data["smd_mem_shop_notice"] = $this->member->item("mem_shop_notice");
            // log_message("ERROR", $_SERVER['REQUEST_URI'] ." > check_shop2 0 smd_mem_shop_notice : ".$this->member->item("mem_shop_notice"));
            $mem_data["smd_mem_shop_template"] = $this->member->item("mem_shop_template");
            $mem_data["smd_spf_friend"] = $spf_friend;
            $mem_data["smd_mem_stmall_yn"] = $this->member->item("mem_stmall_yn");
            $mem_data["smd_mem_stmall_alim_phn"] = $this->member->item("mem_stmall_alim_phn");
	...
	public function check_psd(){
        ...
        if($smartcnt==0){
            ...
            $data["psd_font_LN"] = $oridata->psd_font_LN;
            $data["psd_from"] = '133g';
	...
	public function screen_copy(){
		...
		$code = $this->base62->encode($surl_id);
        log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 스마트전단 데이타 복사 code = ". $code." surl_id = ".$surl_id);
    ...
    public function screen_goods_arr_save_batch(){
		$mem_id = $this->member->item("mem_id"); //회원번호
		$updata = $this->input->post('updata'); //배열 데이타
	    $datas = json_decode( $updata );
		...
		foreach($datas as $r) {
            ...
            $data[] = array(
                    ...
                    , "psg_no_order" => (!empty($r->goods_noorder))? $r->goods_noorder : "N" //주문불가
                ...
                // if($this->member->item("mem_id")==3||$this->member->item("mem_id")==2464){// if($in_ip=="61.75.230.209"){
                    ...
                    if($r->goods_seq=="0"){
                        $data_text[] = array(
                            ...
                            , "pst_tit_bgcolor" => $r->bgcolor
                            , "pst_tit_txcolor" => $r->txcolor
                            , "pst_tit_align" => $r->tit_align
                            , "pst_sub_title" => $r->sub_title
                            , "pst_sub_title_size" => $r->sub_title_size
                            , "pst_sub_title_color" => $r->sub_title_color
                        );
            ...
            if(($cnt!=1&&$divide == 1)||$datas_cnt==$cnt){
                if(!empty($data)){
                    ...
                    $db->insert_batch("cb_pop_screen_goods", $data);
                }
                if(!empty($data_text)){
        			$this->db->insert_batch("cb_pop_screen_text", $data_text);
                    $db->insert_batch("cb_pop_screen_text", $data_text2);
                }
                    // $db->insert_batch("cb_pop_screen_goods", $data);
                    // $db->insert_batch("cb_pop_screen_text", $data_text2);
        			// $this->db->insert_batch("cb_pop_screen_text", $data_text);
		...
		if($result['library_type'] == "goods"){ //최근상품 조회
				...
				foreach ($list as $r){
					...
					<li class=\"img_select\" title=\"".$r->img_name."\">
						<a onClick=\"set_goods_library('". $r->img_path ."', '". addslashes($r->img_name) ."', '". addslashes($r->psg_price) ."', '". addslashes($r->psg_dcprice) ."')\">
							...
							<div class=\"thumb_img\" style=\"background-image: url('". config_item('igenie_path') . $r->img_path ."');background-size: contain;\"><input type=\"hidden\" id=\"img_". $r->id ."\" value=\"". $r->img_path ."\"></div>
						</a>
				...
				foreach ($list as $r){
					...
					<li class=\"img_select\" title=\"".$r->img_name."\" >
						<a onClick=\"set_img_library('". $r->img_path ."')\">
							<input type=\"radio\" name=\"chk_image\" id=\"chk". $r->img_id ."\" value=\"". $r->img_id ."\" title=\"". $r->img_name ."\"><label for=\"chk". $r->img_id ."\"></label>
							<div class=\"thumb_img\" style=\"background-image: url('". config_item('igenie_path').$r->img_path_thumb ."');background-size: contain;\"><input type=\"hidden\" id=\"img_". $r->id ."\" value=\"". $r->img_path ."\"></div>
						</a>
           ...
           select
			...
			if(!empty($list)){
				foreach ($list as $r){
                    // if($this->member->item("mem_level")>=100){
                        //스크롤페이징 오류 버그픽스 - 3줄의 제목을 줄임 - 2022-12-08 윤재박
                        if(mb_strlen($r->img_name)>25){
                            $img_name = mb_substr($r->img_name,0,25).'...';
                        }else{
                            $img_name = $r->img_name;
                        }
                    ...
                    if(!empty($r->img_path)&&$r->img_path != ""){
                        $file_ext = substr(strrchr($r->img_path, "."), 1);
                        $fileNameWithoutExt = substr($r->img_path, 0, strrpos($r->img_path, "."));
					...
					<li class=\"img_select\" title=\"".$r->img_name."\" >
						<a onClick=\"set_img_library('". $r->img_path ."')\">
							<input type=\"radio\" name=\"chk_image\" id=\"chk". $r->img_id ."\" value=\"". $r->img_id ."\" title=\"". $r->img_name ."\"><label for=\"chk". $r->img_id ."\"></label>
							<div class=\"thumb_img\" style=\"background-image: url('". config_item('igenie_path').$imgnamepext ."');background-size: contain;\"><input type=\"hidden\" id=\"img_". $r->id ."\" value=\"". $r->img_path ."\"></div>
						</a>
						<div class=\"file_name\">". $img_name ."</div>
					</li>";
				}
		...
		if(!empty($list)){
			foreach ($list as $r){
				...
				<li class=\"img_select\" title=\"".$img_name."\">
					...
					<div class=\"thumb_img\" style=\"background-image: url('". config_item('igenie_path').$img_path ."');background-size: contain;\"><input type=\"hidden\" id=\"img_". $r->id ."\" value=\"". $img_path ."\"></div>
					<div class=\"goods_name\">". $img_name ."</div>
    ...
    //상품 엑셀 업로드
	public function excel_upload_ok_new(){
	    $result = array();
	    ...
	    if(file_exists($_FILES['file']['tmp_name'])) {
			...
			for($i = 0; $i < $sheetsCount; $i++){
				...
				for ($row = 2; $row <= $highestRow; $row++){ //2번줄부터 읽기
                    ...
                    $img_auto = (empty($rowData[0][7])) ? "Y" : $rowData[0][7];
                    $goods_stock = (empty($rowData[0][8])) ? "Y" : $rowData[0][8];
                    $goods_noorder = (empty($rowData[0][9])) ? "N" : $rowData[0][9];
                    ...
                    $result[$row-2]['stock'] = $goods_stock;
                    $result[$row-2]['noorder'] = $goods_noorder;
                    if($img_auto=="Y"){
                        $result[$row-2]['img_path'] = $this->img_path($code, $sch_name, $name, $option, $ipath); //이미지 경로
                    }else{
                        $result[$row-2]['img_path'] = $ipath; //이미지 경로
                    }
    ...
    //상품 엑셀 업로드
	public function excel_upload_ok_neo(){
	    $result = array();
		$rtn_data = $this->input->post('rtn_data'); //리턴값
        $conner = $this->input->post('inconner');
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > file : ". $_FILES['file']['tmp_name']);
		//업로드된 엑셀 파일을 처리함
	    if(file_exists($_FILES['file']['tmp_name'])) {
            $iiicate = "N";
            ...
            if(!empty($conner)){
                if($iiicate=="N"){
                    if(strpos($conner,'채소')==true||strpos($conner,'야채')==true){
                        $iiicate = "Y";
                        $cate1 = "1";
                        $cate2 = "7";
                    }
                ...
                if($iiicate=="N"){
                    if(strpos($conner,'생활')==true||strpos($conner,'공산')||strpos($conner,'비식품')==true){
                        $iiicate = "Y";
                        $cate1 = "3";
                        $cate2 = "0";
                    }
                ...
                if($iiicate=="N"){
                    if(strpos($conner,'과일')==true||strpos($conner,'청과')==true){
                        $iiicate = "Y";
                        $cate1 = "1";
                        $cate2 = "6";
                    }
                ...
                if($iiicate=="N"){
                    if(strpos($conner,'축산')==true||(strpos($conner,'정육')==true&&strpos($conner,'정육계란')==false)||strpos($conner,'육즙')==true||strpos($conner,'소고기')==true||strpos($conner,'돈육')==true||strpos($conner,'수입산')==true||strpos($conner,'말복')==true){
                        $iiicate = "Y";
                        $cate1 = "1";
                        $cate2 = "8";
                    }
                ...
                if($iiicate=="N"){
                    if(strpos($conner,'건어물')==true||(strpos($conner,'계란')==true&&strpos($conner,'정육계란')==false)){
                        $iiicate = "Y";
                        $cate1 = "1";
                        $cate2 = "5";
                    }
                ...
                if($iiicate=="N"){
                    if(strpos($conner,'조미')==true||strpos($conner,'양념')==true){
                        $iiicate = "Y";
                        $cate1 = "2";
                        $cate2 = "13";
                    }
                ...
                if($iiicate=="N"){
                    if(strpos($conner,'냉장')==true||strpos($conner,'냉동')==true||(strpos($conner,'식품')==true&&strpos($conner,'비식품')==false)||strpos($conner,'식자재')==true){
                        $iiicate = "Y";
                        $cate4 = "3";
                    }
                }
			...
			for($i = 0; $i < $sheetsCount; $i++){
				...
				for ($row = 2; $row <= $highestRow; $row++){ //2번줄부터 읽기
                    ...
                    $result[$row-2]['noorder'] = $goods_noorder;
                    if($img_auto=="Y"){
                        $result[$row-2]['img_path'] = $this->img_path_new($code, $sch_name, $name, $option, $ipath, $cate1, $cate2, $cate3, $cate4); //이미지 경로
                    }else{
                        $result[$row-2]['img_path'] = $ipath; //이미지 경로
                    }
					$result[$row-2]['seq'] = ($row-2); //정렬순서
	...
	//이미지 경로 조회
	public function img_path($code, $sch_name, $name, $option, $ipath){
        $img_path = "";
        if($ipath != ""&&$ipath != "/dhn/images/leaflets/sample_img.jpg"){
        ...
        if($img_path == "" and $name != "" and $this->member->item("mem_id")!='3'){ //최근상품 > 이미지명으로 이미지경로 조회
			//이미지 경로
			$sql = "
			SELECT psg_imgpath AS img_path
			FROM cb_pop_screen_goods
    ...
    //이미지 경로 조회
	public function img_path_new($code, $sch_name, $name, $option, $ipath, $cate1='0',$cate2='0',$cate3='0',$cate4='0'){
		$img_path = "";
        $where = "";
        $where2 = "";
        $where_ori1 = "";
        ...
        if($img_path == "" and $name != ""){ //이미지 라이브러리 > 상품코드로 이미지경로 조회
			...
			FROM cb_images
			WHERE img_useyn = 'Y'
			AND ( img_name = '". addslashes($name) ."' OR img_name = '". addslashes($sch_name) ."') order by img_id desc
			LIMIT 1 ";
            if($name == "국산 특 새우젓"){

                log_message("ERROR", $_SERVER['REQUEST_URI'] ." > sql : ". $sql);
                log_message("ERROR", $_SERVER['REQUEST_URI'] ." > option : ". $option);
            }
        ...
        if($img_path == "" and !empty($name)) {
            ...
            $in_ip = $_SERVER['REMOTE_ADDR'];
            // if($in_ip=="61.75.230.209"){
                $get_names = $this->funn->get_img_name($name, $option);

                $name_t = $get_names["name_t"];
                $wherewhere = $get_names["wherewhere"];
            ...
            // if(!empty($titid)) {
                ...
                if($cate1!="0"||$cate2!="0"||$cate3!="0"||$cate4!="0"){

                    if($cate4!="0"){
                        $where .= " and img_category1 <> '".$cate4."' ";
                        $where2 .= " and img_category1 <> '".$cate4."' ";
                    }else if($cate3!="0"){
                        $where .= " and img_category1 = '".$cate1."' and ( img_category2 = '".$cate2."' or img_category2 = '".$cate3."') ";
                        $where2 .= " and img_category1 = '".$cate1."' and ( img_category2 = '".$cate2."' or img_category2 = '".$cate3."') ";
                    }else{
                        // $where .= " and img_category1 = '".$cate1."' and  img_category2 = '".$cate2."' ";
                        $where .= " and img_category1 = '".$cate1."' ";
                        $where2 .= " and img_category1 = '".$cate1."' ";
                        if($cate2!="0"){
                            $where .= " and  img_category2 = '".$cate2."' ";
                            $where2 .= " and  img_category2 = '".$cate2."' ";
                        }

                        // $where2 .= " and img_category1 = '".$cate1."' and  img_category2 = '".$cate2."' ";
                    }
                }
            // }
        }
		...
		if($img_path == "" and $name != ""){ //이미지 라이브러리 > 이미지명으로 이미지경로 조회
			...
			FROM cb_images
			WHERE img_useyn = 'Y'
			".$where.$wherewhere." and img_category1 <> '0' and img_category2 <> '0' order by img_category1 asc, img_id desc
			LIMIT 1 ";
            if($name == "국산 특 새우젓"){
                log_message("ERROR", $_SERVER['REQUEST_URI'] ." > sql : ". $sql);
            }
        ...
        if($img_path == "" and $name != ""){ //이미지 라이브러리 > 이미지명으로 이미지경로 조회
			...
			FROM cb_images
			WHERE img_useyn = 'Y'
			".$where2.$wherewhere." and img_category1 <> '0' and img_category2 <> '0' order by img_category1 asc, img_id desc
			LIMIT 1 ";
            if($name == "국산 특 새우젓"){
                log_message("ERROR", $_SERVER['REQUEST_URI'] ." > sql : ". $sql);
            }
        ...
        if($img_path == "" and $name != ""){ //이미지 라이브러리 > 이미지명으로 이미지경로 조회
			...
			FROM cb_images
			WHERE img_useyn = 'Y'
			".$where_ori1.$wherewhere." and img_category1 <> '0' and img_category2 <> '0' order by img_category1 asc, img_id desc
			LIMIT 1 ";
            if($name == "국산 특 새우젓"){
                log_message("ERROR", $_SERVER['REQUEST_URI'] ." > sql : ". $sql);
            }
        ...
        if($img_path == "" and $name != ""){ //이미지 라이브러리 > 이미지명으로 이미지경로 조회
			...
			FROM cb_images
			WHERE img_useyn = 'Y'
			".$where_ori2.$wherewhere." and img_category1 <> '0' and img_category2 <> '0' order by img_category1 asc, img_id desc
			LIMIT 1 ";
            if($name == "국산 특 새우젓"){
                log_message("ERROR", $_SERVER['REQUEST_URI'] ." > sql : ". $sql);
            }
	...
	public function smart_dhnlurl(){
		...
		$result = array();
		$result['dhnlurl'] = 'http://'. $this->dhnlurl->get_short("http://smart.dhn.kr/smart/view/". $code);
		$json = json_encode($result,JSON_UNESCAPED_UNICODE);
		header('Content-Type: application/json');
		echo $json;
	}
	...
	public function title_self_save(){
        ...
        $db->replace("cb_design_title", $data);
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > goods_id : ". $goods_id .", rtn = ". $rtn);
    ...
    public function conner_download_new() {
        ...
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(7, 1, "이미지 자동매칭(Y)");
	    // $this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, 1, "처리일시");
	    $this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, 1, "품절");
	    $this->excel->getActiveSheet()->setCellValueByColumnAndRow(9, 1, "주문X");
	    ...
	    foreach($list as $r) {
            ...
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, "");
	        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $r->goods_stock);
	        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(9, $row, $r->goods_noorder);
    ...
    //전단전체 엑셀 다운로드 탭별 테스트
    public function psd_download_tab_new() {
        if($this->member->is_member() == false) { //로그인이 안된 경우
			redirect('login');
		}
        ...
        foreach($boxlist as $r) {
            ...
            if($r->psg_box_no=="0"&&$psg_step == "1단"){
                $this->excel->getActiveSheet()->setTitle("대표&할인");
            }else{
                $this->excel->getActiveSheet()->setTitle($psg_step);
            }
    	    ...
    	    // $this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, 1, "처리일시");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(7, 1, "이미지 자동매칭(Y)");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, 1, "재고");

            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(9, 1, "주문X");

    	    $this->excel->getActiveSheet()->setCellValueByColumnAndRow(10, 1, "타이틀ID");
    	    $this->excel->getActiveSheet()->setCellValueByColumnAndRow(11, 1, "타이틀");
    	    $this->excel->getActiveSheet()->setCellValueByColumnAndRow(12, 1, "뱃지");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(13, 1, "형태");
            // $this->excel->getActiveSheet()->setCellValueByColumnAndRow(11, 1, "단락번호");
            // $this->excel->getActiveSheet()->setCellValueByColumnAndRow(12, 3, "순서");

            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(14, 1, "타이틀 색상");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(15, 1, "타이틀 정렬");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(16, 1, "부제목");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(17, 1, "부제목 배경");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(18, 1, "부제목 크기");
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(19, 1, "부제목 색상");
    	    ...
    	    foreach($list as $r) {
                ...
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $r->psg_imgpath);
    	        // $this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $r->dep_deposit_datetime);
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $r->psg_stock);
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(9, $row, $r->psg_no_order);
    	        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(10, $row, $r->psg_tit_id);
    	        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $r->pst_tit_text_info);
    	        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(13, $row, $r->psg_badge);

                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(13, $row, $psg_step);
                // $this->excel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $r->psg_box_no);
                // $this->excel->getActiveSheet()->setCellValueByColumnAndRow(12, $row, $r->psg_seq);

                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(14, $row, $r->pst_tit_txcolor);
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(15, $row, $r->pst_tit_align);
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(16, $row, $r->pst_sub_title);
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(17, $row, $r->pst_tit_bgcolor);
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(18, $row, $r->pst_sub_title_size);
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(19, $row, $r->pst_sub_title_color);
    	        $row++;
    	        $cnt++;
    	    }
    ...
    //전단 엑셀 업로드
	public function excel_psd_upload_ok_tab_new(){
        if($this->member->is_member() == false) { //로그인이 안된 경우
			redirect('login');
		}
			...
			for($i = 0; $i < $sheetsCount; $i++){
                ...
                if($i==0){
    				...
    				for ($row = 2; $row <= $highestRow; $row++){ //2번줄부터 읽기
    					...
    					$option2 = (empty($rowData[0][5])) ? "" : $rowData[0][5]; //추가란 2021-01-22
                        $ipath = (empty($rowData[0][6])) ? "" : $rowData[0][6]; //이미지 경로 2022-10-21
                        $badge = (empty($rowData[0][12])) ? "" : $rowData[0][12];
                        $noorder = (empty($rowData[0][9])) ? "N" : $rowData[0][9];
                        if($row == 2){
                            $titid = (empty($rowData[0][10])) ? "" : $rowData[0][10];
                            $tittext = (empty($rowData[0][11])) ? "" : $rowData[0][11];
                            $step = (empty($rowData[0][13])) ? "" : $rowData[0][13];

                            $tittxcolor = (empty($rowData[0][14])) ? "" : $rowData[0][14];
                            $titalign = (empty($rowData[0][15])) ? "" : $rowData[0][15];
                            $subtitle = (empty($rowData[0][16])) ? "" : $rowData[0][16];
                            $titbgcolor = (empty($rowData[0][17])) ? "" : $rowData[0][17];
                            $subtitle_size = (empty($rowData[0][18])) ? "" : $rowData[0][18];
                            $subtitle_color = (empty($rowData[0][19])) ? "" : $rowData[0][19];
                        ...
                        $stock = (empty($rowData[0][8])) ? "" : $rowData[0][8];
                        $autoImg = (empty($rowData[0][7])) ? "" : $rowData[0][7];
                        if($psg_step!="9"&&$psg_step!="3"&&empty($ipath)&&$autoImg=="Y"){
                            $sch_name = $name;
        					if($option != "") $sch_name .= " ". $option;
                            $ipath = $this->img_path($code, $sch_name, $name, $option, "");
                        ...
                        if($row == 2){
                            ...
                            $pstData["pst_seq"] = $psg_seq;
                            $pstData["pst_tit_text_info"] = $tittext;
                            $pstData["pst_tit_bgcolor"] = $titbgcolor;
                            $pstData["pst_tit_txcolor"] = $tittxcolor;
                            $pstData["pst_tit_align"] = $titalign;
                            $pstData["pst_sub_title"] = $subtitle;
                            $pstData["pst_sub_title_size"] = $subtitle_size;
                            $pstData["pst_sub_title_color"] = $subtitle_color;
    ...
    //전단 엑셀 업로드
	public function excel_psd_upload_ok_tab_new_fix(){
        if($this->member->is_member() == false) { //로그인이 안된 경우
			redirect('login');
		}
                ...
                if($i==0){
    				...
    				for ($row = 2; $row <= $highestRow; $row++){ //2번줄부터 읽기
                        ...
                        if($row == 2){
                            ...
                            $titidid = 0;
                            $conner = $tittext;
                            if(!empty($conner)&&empty($titid)&&$step!='대표'&&$step!='이미지'){
                        ...
                        if($psg_step!="9"&&$psg_step!="3"&&(empty($ipath)||$autoImg=="Y")){
                            $sch_name = $name;
        					if($option != "") $sch_name .= " ". $option;
                            $ipath = $this->img_path_new($code, $sch_name, $name, $option, "", $cate1, $cate2, $cate3, $cate4);
                        }
