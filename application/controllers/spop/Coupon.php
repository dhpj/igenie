...
class Coupon extends CB_Controller {
	...
	*/
	...
	public function index(){
        ...
        if($in_ip=="61.75.230.209"||$in_ip=="112.223.165.38"){
            // $skin = "main_type";
        }
	...
	public function coupon_save(){
        ...
        $sql = "DELETE FROM cb_pop_coupon_data_detail WHERE pcdd_parent_id = '".$data_id."'";
        $this->db->query($sql);
        foreach($pcdd_arr as $a){
            $a = json_decode($a);
            $data = array();
            $data["pcdd_parent_id"] = $data_id;
            $data["pcdd_type"] = $a[0];
            $data["pcdd_date"] = $a[1];
            $data["pcdd_title"] = $a[2];
            $data["pcdd_option1"] = $a[3];
            $data["pcdd_option2"] = $a[4];
            $data["pcdd_price"] = $a[5];
            $data["pcdd_date_size"] = $a[6];
            $data["pcdd_title_size"] = $a[7];
            $data["pcdd_option1_size"] = $a[8];
            $data["pcdd_option2_size"] = $a[9];
            $data["pcdd_price_size"] = $a[10];
            $data["pcdd_imgpath"] = $a[11];
            $data["barcode_txt_length_flag"] = $a[12];
            $data["barcode_txt"] = $a[13];
            $data["barcode_img_path"] = $a[14];
            $data['pcdd_message'] = $a[15];
            $data['pcdd_message_size'] = $a[16];
            $data['pcdd_stan'] = $a[17];
            $data['pcdd_stan_size'] = $a[18];
            $data['pcdd_dcprice'] = $a[19];
            $data['pcdd_dcprice_size'] = $a[20];
            $data['pcdd_ucomment'] = $a[21];
            $data['pcdd_ucomment_size'] = $a[22];
            $data['pcdd_backcolor'] = $a[23];

            $rtn = $this->db->insert("cb_pop_coupon_data_detail", $data); //데이타 추가
        }
	...
	public function coupon_copy(){
        ...
        $sql = "
		INSERT INTO cb_pop_coupon_data_detail (
			  pcdd_parent_id /* 부모 쿠폰번호 */
			, pcdd_type /* 쿠폰타입(1.무료증정, 2.가격할인) */
			, pcdd_date /* 행사기간 */
			, pcdd_title /* 쿠폰제목 */
			, pcdd_option1 /* 상품정보/쿠폰옵션1 */
			, pcdd_option2 /* 쿠폰옵션/쿠폰옵션2 */
			, pcdd_price /* 할인금액 */
			, pcdd_imgpath /* 이미지경로 */
			, pcdd_date_size /* 행사기간 폰트크기 */
			, pcdd_title_size /* 쿠폰제목 폰트크기 */
			, pcdd_option1_size /* 상품정보/쿠폰옵션1 폰트크기 */
			, pcdd_option2_size /* 쿠폰옵션/쿠폰옵션2 폰트크기 */
			, pcdd_price_size /* 할인금액 폰트크기 */
            , barcode_txt_length_flag /* 바코드 글자수 */
            , barcode_txt /* 바코드 텍스트 */
            , barcode_img_path /* 바코드 이미지 경로 */
		)
		SELECT
			  ". $new_id ." AS pcdd_parent_id /* 부모 쿠폰번호 */
			, pcdd_type /* 쿠폰타입(1.무료증정, 2.가격할인) */
			, pcdd_date /* 행사기간 */
			, CONCAT(pcdd_title, ' (복사)') AS pcdd_title /* 쿠폰제목 */
			, pcdd_option1 /* 상품정보/쿠폰옵션1 */
			, pcdd_option2 /* 쿠폰옵션/쿠폰옵션2 */
			, pcdd_price /* 할인금액 */
			, pcdd_imgpath /* 이미지경로 */
            ...
            , barcode_txt /* 바코드 텍스트 */
            , barcode_img_path /* 바코드 이미지 경로 */
		FROM cb_pop_coupon_data_detail
		WHERE pcdd_id IN (SELECT pcdd_id FROM cb_pop_coupon_data_detail WHERE pcdd_parent_id = '".$data_id."')";
        $rtn = $this->db->query($sql);
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 스마트쿠폰 데이타 복사 ". $sql);
		if($rtn){ //성공
			$code = "0";
