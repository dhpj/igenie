...
class Download extends CB_Controller {
	...
	*/
	...
	public function index()
	{
		...
		$rs = $this->db->query("select a.*, b.mem_userid, b.mem_username, b.mem_tel_mark_flag, b.mem_parent_tel_mark_flag, c.* from cb_wt_msg_sent a inner join cb_member b on a.mst_mem_id=b.mem_id left join cb_wt_send_profile_dhn c on a.mst_profile=c.spf_key where a.mst_id=?", $param)->row();

		$mem_tel_mark_flag = $rs->mem_tel_mark_flag;
		$mem_parent_tel_mark_flag = $rs->mem_parent_tel_mark_flag;

		log_message("ERROR", "SENDER Massege TEL Download => mem_tel_mark_flag : ".$mem_tel_mark_flag);
		log_message("ERROR", "SENDER Massege TEL Download => mem_parent_tel_mark_flag : ".$mem_parent_tel_mark_flag);
		...
		foreach($list as $r) {
            ...
            //2021-01-18 mem_id : 738 (주)티지테크놀러지 예외 추가
		    //2022 03-17 수신자 전화번호 국번 **** 처리구문 변경. 기존: dhnadmin, dhn, TG 계정, (주)지니 계정 보이게 처리, 변경: TG계정, (주)지니 계정 보이게 처리
//            if($this->member->item('mem_id') != '3' && $this->member->item('mem_id') != '6' && $this->member->item('mem_id') != '2' && $this->member->item('mem_id') != '738' && $this->member->item('mem_id') != '962') {
