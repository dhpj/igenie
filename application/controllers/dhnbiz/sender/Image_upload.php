...
class Image_upload extends CB_Controller {
    ...
    public function index()
    {
        ...
        if (isset($_FILES) && isset($_FILES['image_file']) && isset($_FILES['image_file']['name']) && isset($_FILES['image_file']['name'])) {
            ...
            if ($this->upload->do_upload()) {
                ...
                if($swResult->code != "200" && !is_null($swResult->code)) {header('Content-Type: application/json'); echo '{"code":"error", "message":"'.$swResult->code.'"}'; exit; }
	...
	public function talkadvimg()
    {
        ...
        $uploadfiledata = '';
        $this->load->library('upload');
        //log_message("ERROR", $_SERVER['REQUEST_URI'] ." > Img File Name :".$_FILES['image_file']);
        if (isset($_FILES) && isset($_FILES['image_file']) && isset($_FILES['image_file']['name']) && isset($_FILES['image_file']['name'])) {
            $filecount = count($_FILES['image_file']['name']);
            ...
            if ($this->upload->do_upload()) {
                ...
                $filedata = $this->upload->data();
                $this->image_lib->initialize($imgconfig);
                $this->image_api->api_image("/".$upload_path.$filedata['file_name']);
                // $msg = "http://". $_SERVER['HTTP_HOST'] ."/".$upload_path.$filedata['file_name'];
                $msg = config_item('igenie_path').$upload_path.$filedata['file_name'];
				//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > upload_url : ". $msg);
            } else {
