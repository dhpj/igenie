<?php
class Talk_img_adv extends CB_Controller {
    /**
     * 모델을 로딩합니다
     */
    ...
    public function index()
    {
        if ($this->member->item("mem_level") < 100){
            ...
            echo "  location.replace('http://igenie.co.kr/dhnbiz/sender/send/talk_img_adv_v3');";
            echo "</script>";
        }
		...
		$sql = "
		SELECT
			IFNULL(MAX(a.tpl_id),186) AS tmp_code /* 템플릿번호 */
	    ...
	    $sql = "
            ...
            FROM cb_wt_template_dhn cwtd
            	JOIN (SELECT DISTINCT tci_tem_id as tpl_id
            			FROM cb_wt_template_categoryid a
            			WHERE a.tci_type = 'I' ".$where;
	    $sql .= "
            		) as sd
            		ON cwtd.tpl_id = sd.tpl_id AND cwtd.tpl_adv_yn = 'Y' AND cwtd.tpl_profile_key = '".$this->tmp_profile."'
