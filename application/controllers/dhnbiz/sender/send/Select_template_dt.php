...
class Select_template_dt extends CB_Controller {
    ...
    public function index()
    {
        ...
        if(!$profileKey) {
            ...
            $view['rs1'] = $this->db->query("select A.*, B.mem_linkbtn_name FRom (select * from cb_wt_send_profile_dhn where spf_key=? and spf_appr_id<>'' and spf_use='Y' and spf_status='A') as A Left join cb_member B ON A.spf_mem_id = B.mem_id", array($profileKey))->row();
        }
        ...
        if(empty($sms_callback))
            ...
            $this->load->view('biz/dhnsender/send/select_template_dt',$view);
    }
}
?>