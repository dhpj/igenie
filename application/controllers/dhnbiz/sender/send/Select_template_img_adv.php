...
class Select_template_img_adv extends CB_Controller {
    ...
    public function index()
    {
		...
		$sql = "SELECT COUNT(cwtc.tc_id) as category_cnt FROM cb_wt_template_category cwtc WHERE cwtc.tc_useyn='Y' and cwtc.tc_type='I'";
		$view['category_cnt'] =  $this->db->query($sql)->row()->category_cnt;

		$sql = "SELECT cwtc.tc_id, cwtc.tc_description, cwtc.tc_seq FROM cb_wt_template_category cwtc WHERE cwtc.tc_useyn='Y' and cwtc.tc_type='I' ORDER BY cwtc.tc_seq ASC";
		$view['category_list'] =  $this->db->query($sql)->result();

		// 템플릿 버튼갯수 최대값 조회 2020-09-19
		$sql = "SELECT COUNT(DISTINCT cwtc.tci_btn_cnt) AS max_btn_cnt FROM cb_wt_template_categoryid cwtc WHERE cwtc.tci_type ='I'";
		$view['max_btn_cnt'] = $this->db->query($sql)->row()->max_btn_cnt;

		//$sql = "SELECT MAX(cwtc.tci_btn_cnt) AS max_btn_cnt FROM cb_wt_template_categoryid cwtc WHERE cwtc.tci_type ='I'";
		$sql = "SELECT DISTINCT cwtc.tci_btn_cnt FROM cb_wt_template_categoryid cwtc WHERE cwtc.tci_type ='I' ORDER BY cwtc.tci_btn_cnt ASC";
		//$view['max_btn_cnt'] = $this->db->query($sql)->row()->max_btn_cnt;
