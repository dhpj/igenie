<?php
class Talk_adv_v4 extends CB_Controller {
    /**
     * 모델을 로딩합니다
     */
    ...
    public function index()
    {
        ...
        if(config_item('ib_flag')=="Y"){
            ...
            $sql = "
    		SELECT
    			IFNULL(MAX(a.tpl_id),349) AS tmp_code /* 템플릿번호 */
    		FROM ".config_item('ib_template')." a
    		INNER JOIN ".config_item('ib_profile')." b ON a.tpl_profile_key=b.spf_key AND b.spf_use='Y'
    		WHERE ( a.tpl_profile_key = '". $this->tmp_profile ."' OR a.tpl_profile_key = '". $this->tmp_profile2 ."') /* 프로필키 */
    		AND a.tpl_adv_yn = 'Y' /* 광고성 알림톡 여부 */
    ...
    public function check_psd($code){
    ...
    //에디터 불러오기
	public function editor_save_list() {
        if($this->member->is_member() == false) { //로그인이 안된 경우
			redirect('login');
		}
        ...
        $where = " mem_id = ".$this->member->item('mem_id')." and html <> '' and creation_date BETWEEN DATE_ADD(NOW(),INTERVAL -2 WEEK ) AND NOW()";
        $param = array();
        ...
        $this->pagination->initialize($page_cfg);
        $this->pagination->cur_page = intval($view['param']['page']);
        $view['page_html'] = str_replace('open_page','open_page_user_editer_msg',$this->pagination->create_links());
        //ib플래그 + 그룹추가
        $sql = "
			select
				 *
			from cb_alimtalk_ums
			where ".$where." order by idx desc limit ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage'];
        //echo $sql ."<br>";
		//log_message("ERROR", $sql);
        $view['list'] = $this->db->query($sql, $param)->result();
        $view['view']['canonical'] = site_url();

        $this->load->view('biz/dhnsender/adv_save_editor_list',$view);
    }
	...
	public function ajax_adv_template_data(){
	    ...
	    $sql = "
			...
			WHERE a.tci_type = 'T' and tci_btn_cnt < 3 ";

        if (empty($temp_type) || $temp_type == "2"){
        //ib플래그 + 그룹추가
	    $sql .= $where . "
            		) as sd ON cwtd.tpl_id = sd.tpl_id AND cwtd.tpl_adv_yn = 'Y'
            ...
            $sql .= "
                ...
                ".$whereplus."  and cwtfd.mem_id = " . $this->member->item("mem_id") . "
                ORDER BY cwtd.tpl_adv_sort DESC, cwtd.tpl_id DESC";
        }
	    //log_message("ERROR", $_SERVER['REQUEST_URI'] ." > Query : ".$sql);
	    $result = $this->db->query($sql)->result();
    ...
    public function select_load_editor(){
		$save_msg_id  = $this->input->post('save_msg_id'); //에디터
