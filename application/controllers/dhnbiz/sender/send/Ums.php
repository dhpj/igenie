<?php
class Ums extends CB_Controller {
    /**
     * 모델을 로딩합니다
     */
    ...
    public function index()
    {
        ...
        $view['rs'] = $this->db->query("select * from ".config_item('ib_profile')." where spf_mem_id=? and spf_appr_id<>'' and spf_use='Y' order by spf_appr_datetime desc limit 1", $this->member->item('mem_id'))->row();
    ...
    public function resend($fail_mst_id = -1)
    {
        ...
        $view['rs'] = $this->db->query("select * from ".config_item('ib_profile')." where spf_mem_id=? and spf_appr_id<>'' and spf_use='Y' order by spf_appr_datetime desc limit 1", $mem_id)->row();
