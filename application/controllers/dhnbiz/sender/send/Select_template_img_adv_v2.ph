...
class Select_template_img_adv extends CB_Controller {
    ...
    public function index()
    {
        ...
        //ib플래그
		$sql = "SELECT COUNT(cwtc.tc_id) as category_cnt FROM ".config_item('ib_categoryid')." cwtc WHERE cwtc.tc_useyn='Y' and cwtc.tc_type='I'";
		$view['category_cnt'] =  $this->db->query($sql)->row()->category_cnt;
        //ib플래그
		$sql = "SELECT cwtc.tc_id, cwtc.tc_description, cwtc.tc_seq FROM ".config_item('ib_categoryid')." cwtc WHERE cwtc.tc_useyn='Y' and cwtc.tc_type='I' ORDER BY cwtc.tc_seq ASC";
		$view['category_list'] =  $this->db->query($sql)->result();

		// 템플릿 버튼갯수 최대값 조회 2020-09-19
        //ib플래그
		$sql = "SELECT COUNT(DISTINCT cwtc.tci_btn_cnt) AS max_btn_cnt FROM ".config_item('ib_category')."id_ib cwtc WHERE cwtc.tci_type ='I'";
        ...
        //ib플래그
		$sql = "SELECT DISTINCT cwtc.tci_btn_cnt FROM ".config_item('ib_category')."id_ib cwtc WHERE cwtc.tci_type ='I' ORDER BY cwtc.tci_btn_cnt ASC";
		//$view['max_btn_cnt'] = $this->db->query($sql)->row()->max_btn_cnt;
