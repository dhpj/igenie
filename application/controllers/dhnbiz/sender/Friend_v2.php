...
//             $data['mst_price_smt_mms'] = $this->Biz_dhn_model->price_smt_mms;   // 그린샷 웹(C) mms 단가
			...
			$data['mst_2nd_alim'] = ($mst_type2 == "at") ? "1" : "0"; //2차 알림톡 구분(0.알림톡 발송안함, 1.대형 카카오 발송, 2.스윗트래커 카카오 발송)
			...
			if($mst_type2 == "at"){
                ...
                if(config_item('ib_flag')=="Y"){
                    ...
                    $this->dhnkakao->sendAgentSendCache2nd($gid, $alim_code, $alim_cont, $btn1_alim, $btn2_alim, $btn3_alim, $btn4_alim, $btn5_alim);
                }
			...
			// if($mst_type2 == "at" and ($link_type == "smart" or $link_type == "coupon" or $link_type == "editor")){ //전단종류(smart.스마트전단, coupon.스마트쿠폰, editor.에디터 사용, self.직접입력) 2021-01-05 추가
...
//         $data['mst_price_smt_mms'] = $this->Biz_dhn_model->price_smt_mms;   // 그린샷 웹(C) mms 단가
		...
		//2021-01-05 2차 알림톡 추가
		$data['mst_2nd_alim_code'] = ($mst_type2 == "at") ? $alim_code : ""; //2차 알림톡 템플릿코드
		$data['mst_2nd_alim_cont'] = $alim_cont; //2차 알림톡 내용
		$data['mst_2nd_alim_btn'] = json_encode(array($btn1_alim, $btn2_alim, $btn3_alim, $btn4_alim, $btn5_alim), JSON_UNESCAPED_UNICODE); //2차 알림톡 버튼
		$data['mst_2nd_alim'] = ($mst_type2 == "at") ? "1" : "0"; //2차 알림톡 구분(0.알림톡 발송안함, 1.대형 카카오 발송, 2.스윗트래커 카카오 발송)
        ...
        log_message('error', 'song btn12345 : ' . json_encode(array($btn1, $btn2, $btn3, $btn4, $btn5), JSON_UNESCAPED_UNICODE));
