...
class Settlement extends CB_Controller {
    ...
    public function index()
    {
        ...
        $view['param']['userid'] = (!$this->input->post('userid') || $this->input->post('userid')=="ALL") ? ($this->member->item('mem_level')>100) ? "dhnadmin" : $this->member->item('mem_userid') : $this->input->post('userid');
        //$view['param']['startDate'] = ($this->input->post('startDate')) ? $this->input->post('startDate') : date('Y-m');
        $view['param']['startDate'] = ($this->input->post('startDate')) ? $this->input->post('startDate') : date('Y-m', strtotime("-1 months"));
        ...
        if(($view['param']['userid'] == 'dhn' || $view['param']['userid'] == 'dhnadmin') && $view['param']['companyName'] == '')
        {
            ...
            log_message("ERROR", "query 1 ========================". $view['param']['userid']);
            $sql = "
                ...
                FROM (
                   SELECT
                        period_name,
                        -- manager_mem_id,
                        CASE WHEN id_level1 = 2 THEN CASE WHEN id_level3 = 0 THEN manager_mem_id ELSE id_level3 END ELSE CASE WHEN id_level2 = 0 THEN manager_mem_id ELSE id_level2 END END manager_mem_id,
                       ...
                       -- end_mem_id,
                    	  ...
                    	  where exists ( select 1 from cb_member_register cmr
                                            where cmr.mrg_recommend_mem_id = CASE WHEN id_level1 = 2 THEN CASE WHEN id_level3 = 0 THEN manager_mem_id ELSE id_level3 END ELSE CASE WHEN id_level2 = 0 THEN manager_mem_id ELSE id_level2 END END -- manager_mem_id
                        ...
                        id_level1, 
                        id_level2";
            if ($view['param']['startDate'] > '2019-11') {
                $sql .= ",
                            id_level3 ";
            }
            $sql .= "
            ...
            if ($this->member->item('mem_userid') == 'dhn' || $this->member->item('mem_userid') == 'dhnadmin') {
                ...
                $sql = "
                    ...
                    FROM
                        ...
                        cb_wt_member_addon cwma on CASE WHEN LEFT('".$view['param']['userid']."',4) = 'dhn_' THEN CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level4 ELSE cwma.mad_mem_id = cmr.id_level3 END ELSE CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level3 ELSE cwma.mad_mem_id = cmr.id_level2 END END left join
...
/*             $sql = "
    ...
    private function excelTopTable($r, $cur_worksheet, $start_date, $end_date, $period_name) {
        ...
        $cur_worksheet->setTitle(trim($r->mem_username).'_'.$period_name)
        ->mergeCells("B2:E2")
...
//                 $row_count += 1;
    ...
    private function excelTopTable2($r, $cur_worksheet, $start_date, $end_date, $period_name) {
        $cur_worksheet->getColumnDimension('A')->setWidth(1.38 + 0.63);
        $cur_worksheet->getColumnDimension('B')->setWidth(14.63 + 0.63);
        $cur_worksheet->getColumnDimension('C')->setWidth(19.75 + 0.63);
        $cur_worksheet->getColumnDimension('D')->setWidth(17.50 + 0.63);
...
//             	sum(ft) ft, 
//             	ft_price, 
//             	sum(s_ft_amount) s_ft_amount,
//             	sum(ft_img) ft_img,
//             	ft_img_price,
//             	sum(s_ft_img_amount) s_ft_img_amount,
//             	sum(ft_w_img) ft_w_img,
//             	ft_w_img_price,
//             	sum(s_ft_w_img_amount) s_ft_w_img_amount,
//             	sum(at) at,
//             	at_price,
//             	sum(s_at_amount) s_at_amount,
//             	sum(grs) grs,
//             	grs_price,
//             	sum(s_grs_amount) s_grs_amount,
//             	sum(grs_sms) grs_sms,
//             	grs_sms_price,
//             	sum(s_grs_sms_amount) s_grs_sms_amount,
//             	sum(grs_mms) grs_mms,
//             	grs_mms_price,
//             	sum(s_grs_mms_amount) s_grs_mms_amount,
//             	sum(nas) nas,
//             	nas_price,
//             	sum(s_nas_amount) s_nas_amount,
//             	sum(nas_sms) nas_sms,
//             	nas_sms_price,
//             	sum(s_nas_sms_amount) s_nas_sms_amount,
//             	sum(nas_mms) nas_mms,
//             	nas_mms_price,
//             	sum(s_nas_mms_amount) s_nas_mms_amount,
//             	sum(smt) smt,
//             	smt_price,
//             	sum(s_smt_amount) s_smt_amount,
//             	sum(smt_sms) smt_sms,
//             	smt_sms_price,
//             	sum(s_smt_sms_amount) s_smt_sms_amount,
//             	sum(smt_mms) smt_mms,
//             	smt_mms_price,
//             	sum(s_smt_mms_amount) s_smt_mms_amount,
//             	sum(tot_send) tot_send,
//                 sum(send_amount) send_amount,
//             	sum(tot_amount) + mad_price_full_care tot_amount
// 			from (
//             select 
//             	a.period_name, 
//                 a.end_mem_id as mem_id,
...
//             	a.smt_mms_price,
//             	(a.smt_mms * a.smt_mms_price) as s_smt_mms_amount,
//                 c.mad_price_full_care,
//             	(a.ft + a.ft_img + a.at + a.grs + a.grs_biz + a.grs_sms + a.grs_mms + a.nas + a.nas_sms + a.nas_mms + a.smt + a.smt_sms + a.smt_mms) as tot_send,
//                 ((a.ft * a.ft_price) + (a.ft_img * a.ft_img_price) + (a.at * a.at_price) + ((a.grs + a.grs_biz) * a.grs_price) + (a.grs_sms * a.grs_sms_price) + (a.grs_mms * a.grs_mms_price) + (a.nas * a.nas_price) + (a.nas_sms * a.nas_sms_price) + (a.nas_mms * a.nas_mms_price) + (a.smt * a.smt_price) + (a.smt_sms * a.smt_sms_price) + (a.smt_mms * a.smt_mms_price)) as send_amount,
//             	((a.ft * a.ft_price) + (a.ft_img * a.ft_img_price) + (a.at * a.at_price) + ((a.grs + a.grs_biz) * a.grs_price) + (a.grs_sms * a.grs_sms_price) + (a.grs_mms * a.grs_mms_price) + (a.nas * a.nas_price) + (a.nas_sms * a.nas_sms_price) + (a.nas_mms * a.nas_mms_price) + (a.smt * a.smt_price) + (a.smt_sms * a.smt_sms_price) + (a.smt_mms * a.smt_mms_price)) as tot_amount
//             from cb_monthly_report a left join 
//                  cb_member b on a.end_mem_id = b.mem_id left join
//                  cb_wt_member_addon c on a.end_mem_id = c.mad_mem_id
//             where a.period_name in ('2019-12', '2020-01') and  a.end_mem_id in (".$value->mem_id.")
//             order by b.mem_id
//             ) as temp
//             group by temp.mem_id, temp.mem_username, temp.mem_email, temp.full_care, temp.ft_price, temp.ft_img_price, temp.ft_w_img_price, temp.at_price,
//                      temp.grs_price, temp.grs_sms_price, temp.grs_mms_price, temp.nas_price, temp.nas_sms_price, temp.nas_mms_price, temp.smt_price, temp.smt_sms_price,
//                      temp.smt_mms_price        ";
        ...
        $result_query = $this->db->query($sql)->result();
        $sheet_count = 0;
        $start_date = $value->period_name.'-01'; // 2019-02-03
        $end_date = $value->period_name.'-'.date('t', $start_date);
        $SD = $start_date;
        $ED = $end_date;
        $start_date .= ' 00:00:00';
...
//                 ";
    ...
    public function download_monthly_settlement_parent() {
        ...
        FROM
            ...
            cb_member cm on cm.mem_id = cmr.manager_mem_id left join
            cb_wt_member_addon cwma on CASE WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 50 THEN CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level3 ELSE cwma.mad_mem_id = cmr.id_level2 END 
            									WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 30 THEN CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level4 ELSE cwma.mad_mem_id = cmr.id_level3 END
            									WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 10 THEN CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level5 ELSE cwma.mad_mem_id = cmr.id_level4 END END left join
            cb_member tmcm on CASE WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 50 THEN CASE WHEN cmr.id_level1 = 2 THEN tmcm.mem_id = cmr.id_level3 ELSE tmcm.mem_id = cmr.id_level2 END 
        							WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 30 THEN CASE WHEN cmr.id_level1 = 2 THEN tmcm.mem_id = cmr.id_level4 ELSE tmcm.mem_id = cmr.id_level3 END
        							WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 10 THEN CASE WHEN cmr.id_level1 = 2 THEN tmcm.mem_id = cmr.id_level5 ELSE tmcm.mem_id = cmr.id_level4 END END
        WHERE
        ...
        FROM (
            ...
            FROM
                ...
                cb_wt_member_addon cwma on CASE WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 50 THEN CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level3 ELSE cwma.mad_mem_id = cmr.id_level2 END  
                									WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 30 THEN CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level4 ELSE cwma.mad_mem_id = cmr.id_level3 END
                									WHEN (select mem_level from cb_member where mem_userid = '".$value->mem_userid."') = 10 THEN CASE WHEN cmr.id_level1 = 2 THEN cwma.mad_mem_id = cmr.id_level5 ELSE cwma.mad_mem_id = cmr.id_level4 END END
            WHERE
                cmr.period_name = '".$value->period_name."'
            ORDER BY period_name , mem_level DESC, manager_mem_id , end_mem_id
        ) ABC 
    ...
    public function download($dl_type)
    {
        ...
        if ($dl_type == "admin")
        {
            ...
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, 3, 'Fullcare');
            ...
            foreach($value as $val) {
                ...
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $val->full_care);
            ...
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $total[0]->full_care);
    ...
    public function download_email() {
        ...
        $cur_worksheet->setCellValue('E1', "첨부파일명1");
        ...
        $cur_worksheet->setCellValue('F1', "첨부파일명2");
        $cur_worksheet->getStyle('F1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        $cur_worksheet->getStyle('F1')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
        $cur_worksheet->getStyle('F1')->getFont()->setSize(13);
        $cur_worksheet->getStyle('F1')->getFont()->setBold(true);
        $cur_worksheet->setCellValue('G1', "첨부파일명3");
        $cur_worksheet->getStyle('G1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        $cur_worksheet->getStyle('G1')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
        $cur_worksheet->getStyle('G1')->getFont()->setSize(13);
        $cur_worksheet->getStyle('G1')->getFont()->setBold(true);
...
감사합니다.";
                ...
                //$cur_worksheet->getStyle('E'.$next_row_count)->getBorders()->getAllBorders()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
                $cur_worksheet->setCellValue('F'.$next_row_count, '');
                ...
                $cur_worksheet->setCellValue('G'.$next_row_count, '');
