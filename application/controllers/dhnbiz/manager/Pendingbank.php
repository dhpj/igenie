...
class Pendingbank extends CB_Controller
{
    ...
    public function index()
    {
        ...
        $view['perpage'] = ($this->input->post("perpage")) ? $this->input->post("perpage") : 20;;
        $view['param']['page'] = ($this->input->post("page")) ? $this->input->post("page") : 1;
        $view['param']['dep_status'] = $this->input->post("dep_status");
...
(	SELECT distinct a.mem_id
			...
			where ".$where." order by a.dep_datetime desc limit ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage'];
    ...
    public function write($pid = 0)
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->input->post($primary_key)) {
                $selectCount = $this->db->query("select count(*) cnt from cb_deposit where dep_id = '".$this->input->post($primary_key)."' and status <> 'OK'" )->row()->cnt;
                ...
                if ($selectCount > 0) {
                    ...
                    $msg = '이미 완납처리가 되었습니다.';
                    log_message("ERROR", "TEST : ".$this->input->post($primary_key)."     이미 완납처리가 되었습니다.");
                }
