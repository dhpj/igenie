...
class Sendprofile extends CB_Controller {
    ...
    public function download() {
        ...
        $list = $this->Biz_model->get_partner_profile_excel($uid, $where, $param);
        ...
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(0, 1, "등록일시");
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(1, 1, "업체명");
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, 1, "플러스친구");
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(3, 1, "발신프로필키");
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(4, 1, "상태");
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(5, 1, "SMS발신번호");
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, 1, "비고");
        ...
        foreach($list as $r) {
            $strStatus = "";
            switch($r->spf_status) { case 'S' : $strStatus = '차단'; break; case 'D': $strStatus = '삭제'; break; default:$strStatus = '정상';}
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $r->spf_datetime);
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $r->spf_company);
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $r->spf_friend);
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $r->spf_key);
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $strStatus);
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $r->spf_sms_callback);
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $r->mem_sales_name);
            $row++;
            $cnt++;
        }
        ...
        header('Content-Disposition: attachment;filename="profile_list.xls"');
    ...
    function sendprofile_save()
    {
        ...
        // id 중복검사
        $cnt = $this->Biz_model->get_table_count("cb_wt_send_profile_dhn", "spf_friend=? and spf_use='Y'", array($id));
	...
	public function refresh() 
	{
		...
		$cnt = $this->Biz_model->get_table_count("cb_wt_send_profile_dhn", "spf_key=? ", $result->{'data'}->{'senderKey'});
		...
		if($cnt = 0) {
			...
			$this->db->insert("wt_send_profile_dhn", $ins);
			$ins_cnt = $this->db->affected_rows();
		} else {
		    log_message("ERROR", "user id : ".$userId. " / key : ".$senderKey."/".$result->{'data'}->{'status'});
            $this->db->query("update cb_wt_send_profile_dhn set spf_status = '".$result->{'data'}->{'status'}."' where spf_key=?", $senderKey);
		}
		header('Content-Type: application/json');
		echo '{"success": true}';
	}
