...
class Partner extends CB_Controller {
    ...
    public function download() {
        ...
        $this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, 1, "2nd");
        ...
        foreach($list as $r) {
            ...
            $this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $str2n);
            $row++;
            $cnt++;
        }
        ...
        header('Content-Disposition: attachment;filename="partner_list.xls"');
    ...
    function partner_save($new=false)
    {
        ...
        $this->mem_full_care_type = $mem_full_care_type;
    ...
    public function partner_charge()
    {
        ...
        //$view['param']['endDate'] = $this->input->post('endDate');
        //$view['param']['startDate'] = $this->input->post('startDate');

        //log_message("ERROR", "this->input->post('startDate') : ".$this->input->post("startDate"));
        //log_message("ERROR", "this->input->post('endDate') : ".$this->input->post("endDate"));
        ...
        if ($view['param']['search_type'] == 'all') {
            ...
            $view['total_rows'] = $this->Biz_model->get_table_count("cb_amt_".$view['rs']->mem_userid, "amt_kind = '".$view['param']['search_type']."' and amt_datetime between ? and ?", $param);
        }
        $this->load->library('pagination');
        $page_cfg['link_mode'] = 'open_page';
        $page_cfg['base_url'] = '';
        ...
        if ($view['param']['search_type'] == 'all') {
            ...
            $sql = "
    			select *
    			from cb_amt_".$view['rs']->mem_userid."
    			where amt_kind = '".$view['param']['search_type']."' and amt_datetime between ? and ? order by amt_datetime desc limit ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage'];
        }
...
// 			where a.mst_mem_id=? and a.mst_datetime between ? and ? order by a.mst_datetime desc limit ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage'];
    ...
    public function view()
    {
        ...
        $view['rs'] = $this->Biz_model->get_member($view['param']['key'], true);
        //ib플래그
        $sql = "select spf_biz_sheet from ".config_item('ib_profile')." where spf_mem_id=? and spf_biz_sheet is not null and spf_biz_sheet<>''";
