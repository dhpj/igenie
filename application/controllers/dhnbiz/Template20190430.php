...
class Template extends CB_Controller {
	...
	*/
	...
	public function lists()
	{
		...
		$view['total_rows'] = $this->Biz_model->get_template_count($this->member->item('mem_id'), $where, $param);	//get_table_count("cb_wt_template_dhn_dhn", $where, $param);
		...
		//	from cb_wt_template_dhn_dhn a inner join cb_wt_send_profile_dhn b on a.tpl_profile_key=b.spf_key and b.spf_use='Y'
	...
	function sync_template_status()
	{
		...
		//$req = $this->db->query("select tpl_profile_key, tpl_code from cb_wt_template_dhn_dhn where tpl_mem_id=? and tpl_inspect_status='REQ'", array($this->member->item('mem_id')))->result();
		$sql ="select ws.spf_appr_id, cwt.* from cb_wt_template_dhn_dhn cwt inner join cb_wt_send_profile_dhn ws on cwt.tpl_profile_key = ws.spf_key where ( tpl_mem_id in 
(SELECT distinct  a.mem_id
				...
				if($result->code=="success") {
				    if($result->data->inspectionStatus != $r->tpl_inspect_status || $result->data->status!=$r->tpl_status || empty($r->tpl_inspect_status) || $r->tpl_status) {
				        if($result->data->inspectionStatus!=$r->tpl_inspect_status || empty($r->tpl_inspect_status)) {
							...
							$this->db->query("update cb_wt_template_dhn_dhn set tpl_status='".$result->data->status."' where tpl_mem_id=? and tpl_profile_key=? and tpl_code=?", array($this->member->item('mem_id'), $r->tpl_profile_key, $r->tpl_code));
						}
					}
				} else {
	...
	public function btn()
	{
		...
		$view = array();
		$view['rs'] = $this->Biz_model->get_row("cb_wt_template_dhn_dhn", "tpl_id=?", array($tmpl_id));
	...
	public function check_inspect()
	{
		...
		for($n=0;$n<$count;$n++) {
			...
			$this->db->query("update cb_wt_template_dhn_dhn set tpl_inspect_status='REQ', tpl_check_datetime=? where tpl_profile_key=? and tpl_code=? and tpl_use='Y' and tpl_inspect_status='REG'", array(cdate('Y-m-d H:i:s'), $pf_key[$n], $tmp_code[$n]));
	...
	public function check_delete()
	{
		...
		for($n=0;$n<$count;$n++) {
			...
			if($result[0]->code!="success") {
				...
				$this->db->query("update cb_wt_template_dhn_dhn set tpl_use='N' where tpl_profile_key=? and tpl_code=? and tpl_inspect_status='REJ'", array($pf_key[$n], $tmp_code[$n]));

				$err++;
				continue;
			}

			$this->db->query("update cb_wt_template_dhn_dhn set tpl_use='N' where tpl_profile_key=? and tpl_code=? and (tpl_inspect_status='REG' OR tpl_inspect_status='REJ')", array($pf_key[$n], $tmp_code[$n]));
			if($this->db->affected_rows() > 0) { $ok++; } else { $err++; }
		}
		header('Content-Type: application/json');
		echo '{"message": '.(($ok>0) ? 'null' : '"'.$err_msg.'"').', "success": '.$ok.', "fail": '.$err.' }';
	...
	public function hide_proc()
	{
	    ...
	    for($n=0;$n<$count;$n++) {
	        $this->db->query("update cb_wt_template_dhn_dhn set tpl_use='N' where tpl_profile_key=? and tpl_code=? ", array($pf_key[$n], $tmp_code[$n]));
	        if($this->db->affected_rows() > 0) { $ok++; } else { $err++; }
	    }
	    header('Content-Type: application/json');
	    echo '{"message": '.(($ok>0) ? 'null' : '"'.$err_msg.'"').', "success": '.$ok.', "fail": '.$err.' }';
	...
	public function check_delete_public()
	{
	    ...
	    for($n=0;$n<$count;$n++) {
	        ...
	        $this->db->query("update cb_wt_template_dhn_dhn set tpl_share_flag='U' where tpl_profile_key=? and tpl_code=? ", array($pf_key[$n], $tmp_code[$n]));
	...
	public function add_process()
	{
		...
		foreach($json as $tr) {
			...
			$data['tpl_id'] = ($partner) ? $this->Biz_model->get_table_next_id('cb_wt_template_dhn_dhn', 'tpl_id', "", array()) : 1;	//'tpl_mem_id=?', array($partner->spf_mem_id)) : 0;
			...
			$this->db->insert('wt_template_dhn_dhn', $data);
	...
	public function view()
	{
		...
		$view = array();
		$view['view'] = array();
		$view['rs'] = $this->db->query("select a.*, b.spf_friend, b.spf_key_type, b.spf_sms_callback, b.spf_appr_id from cb_wt_template_dhn_dhn a inner join cb_wt_send_profile_dhn b on a.tpl_profile_key=b.spf_key and b.spf_use='Y' where tpl_id=? limit 1", array($key))->row();
		...
		if($view['tpl_status']->code=="success") {
			if(($view['rs']->tpl_inspect_status=="REQ" && $view['rs']->tpl_inspect_status!=$view['tpl_status']->data->inspectionStatus) || $view['tpl_status']->data->status!=$view['rs']->tpl_status) {
				if($view['rs']->tpl_inspect_status=="REQ" && $view['rs']->tpl_inspect_status!=$view['tpl_status']->data->status) {
					$sql = "update cb_wt_template_dhn_dhn set tpl_inspect_status='".$view['tpl_status']->data->inspectionStatus."', tpl_appr_id='admin', tpl_appr_datetime='".cdate('Y-m-d H:i:s')."' where tpl_id=?";
				} else {
					$sql = "update cb_wt_template_dhn_dhn set tpl_status='".$view['tpl_status']->data->status."' where tpl_id=?";
				}
	...
	public function modify_success()
	{
		...
		if($json->code!="success") {
			...
			$this->db->update('wt_template_dhn_dhn', $data, array("tpl_profile_key"=>$pf_key, "tpl_code"=>$tmp_code));
			if($this->db->error()['code'] > 0) { echo '<pre> :: ';print_r($this->db->error());$err++; } else { $ok++; }
		}
	...
	public function public_temp() {
	    ...
	    $sql = "update cb_wt_template_dhn_dhn set tpl_share_flag = 'P' where tpl_id = '".$temp_id."'";
	    $this->db->query($sql);
	...
	public function download()
	{
		...
		foreach($ids as $id) {
			...
			$rs = $this->db->query("select * from cb_wt_template_dhn_dhn where tpl_id=? and tpl_use='Y'", array($id))->row();
...
*/
	...
	public function public_lists()
	{
	    ...
	    $view['total_rows'] = $this->Biz_model->get_public_template_count($where, $param);	//get_table_count("cb_wt_template_dhn_dhn", $where, $param);
	    ...
	    //$sql = "
	    //	select a.*, b.spf_friend, b.spf_key_type
	    //	from cb_wt_template_dhn_dhn a inner join cb_wt_send_profile b on a.tpl_profile_key=b.spf_key and b.spf_use='Y'
	    //	where ".$where." order by tpl_datetime desc limit ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage'];
