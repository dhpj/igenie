...
class Design extends CB_Controller {
	...
	*/
	...
	public function templet(){
        ...
        $view['tag'] = ($this->input->get("tag")) ? $this->input->get("tag") : "all"; //태그번호
		$where = "WHERE dt.tem_useyn = 'Y' ";
		// if($view['tagid'] != "all"){
		// 	$where .= "AND dt.tem_tag_id = '". $view['tagid'] ."' ";
		// }
        if($view['tag'] != "all"){
			$where .= "AND find_in_set ('". $view['tag'] ."',dt.tem_tag_txt) ";
		}
		//echo $where ."<br>";
	...
	public function templet_save(){
		...
		if (isset($_FILES) && isset($_FILES['imgfile']) && isset($_FILES['imgfile']['name'])) { //이미지 업로드
			...
			if ($this->upload->do_upload()) {
				...
				if($this->image_lib->resize()) {
                    ...
                    $str_path_thumb = str_replace("/var/www/igenie", "", $imgconfig['new_image']);
                    $this->image_api->api_image($str_path_thumb);

                    $simgconfig = array();
                   $simgconfig['image_library'] = 'gd2';
                   $simgconfig['source_image']	= $filedata['full_path'];
                   $simgconfig['maintain_ratio'] = false;
                   $simgconfig['width']	= 640; //썸네일 가로 사이즈
                   $simgconfig['height']= 640; //썸네일 가로 사이즈
                   $simgconfig['new_image'] = $filedata['file_path'].$filedata['raw_name'].'_og'.$filedata['file_ext'];
		...
		$data["tem_tag_txt"] = $this->input->post("tem_tag_txt"); //태그
		...
		if(!empty($tem_id)) { //수정의 경우
            ...
            $sql = "select count(1) as cnt from cb_design_templet where tem_mem_id = '".$this->member->item("mem_id")."' and tem_id = '".$tem_id."'";
            $tem_cnt = $db->query($sql)->row()->cnt;
            if($tem_cnt == 0){
                $data["tem_id"] = $tem_id; //템플릿번호
    			$data["tem_seq"] = $tem_id;
                $sql = "select * from cb_design_templet where tem_mem_id = '".$this->member->item("mem_id")."' and tem_id = '".$tem_id."' limit 1";
                $tem = $this->db->query($sql)->row();
                $data["tem_imgpath"] = $tem->tem_imgpath;
                $data["tem_cre_date"] = $tem->tem_cre_date;
                $data["tem_upd_date"] = $tem->tem_upd_date;
                $db->replace("cb_design_templet", $data);
            }else{
                $db->update("cb_design_templet", $data, $where);
            }
		}else{ //등록
            ...
            $rtn2 = $db->replace("cb_design_templet", $data);
		}
	...
	public function title(){
		...
		WHERE tit_type = 'I' /* 구분(I.이미지형, T.텍스트형, B.뱃지이미지) */
		...
		$sql = "SELECT *
		FROM cb_design_title
		WHERE tit_type = 'T' /* 구분(I.이미지형, T.텍스트형, B.뱃지이미지) */
		AND tit_useyn = 'Y' /* 사용여부 (Y/N) */
		ORDER BY tit_seq ASC ";
		//echo $sql ."<br>";
        ...
        $sql = "SELECT *
		FROM cb_design_title
		WHERE tit_type = 'C' /* 구분(I.이미지형, T.텍스트형, B.뱃지이미지) */
		AND tit_useyn = 'Y' /* 사용여부 (Y/N) */
		ORDER BY tit_seq ASC ";
		//echo $sql ."<br>";
		$view['data_cus_img_list'] = $this->db->query($sql)->result();
		...
		ORDER BY tit_seq ASC ";
		//echo $sql ."<br>";
		$view['data_badge_list'] = $this->db->query($sql)->result();
	...
	public function title_save(){
			...
			if ($this->upload->do_upload()) {
				...
				if($this->image_lib->resize()) {
                    ...
                    $simgconfig = array();
                     ...
                     $simgconfig['width']	= 120; //썸네일 가로 사이즈
                     $simgconfig['height']= 33; //썸네일 가로 사이즈
		...
		$data["tit_text_info"] = $this->input->post("tit_text_info"); //타이틀정보
		if(!empty($this->input->post("tit_useyn"))) {
			$data["tit_useyn"] = $this->input->post("tit_useyn") ; //사용여부
		}
        if($data["tit_type"]=="C"){
            $data["tit_align"] = $this->input->post("tit_align");
            $data["tit_bgcolor"] = $this->input->post("tit_bgcolor");
            $data["tit_txcolor"] = $this->input->post("tit_txcolor");
        }
		if(!empty($tit_id)) { //수정의 경우
			$where = array();
			$where["tit_id"] = $tit_id; //타이틀번호
			$rtn = $this->db->update("cb_design_title", $data, $where);
            $sql = "select count(1) as cnt from cb_design_title where tit_mem_id = '".$this->member->item("mem_id")."' and tit_id = '".$tit_id."'";
           $tit_cnt = $db->query($sql)->row()->cnt;
           if($tit_cnt == 0){
               $data["tit_id"] = $tit_id; //템플릿번호
               $data["tit_seq"] = $tit_id;
               $sql = "select * from cb_design_title where tit_mem_id = '".$this->member->item("mem_id")."' and tit_id = '".$tem_id."' limit 1";
               $tit = $this->db->query($sql)->row();
               $data["tit_imgpath"] = $tit->tit_imgpath;
               $data["tit_cre_date"] = $tit->tit_cre_date;
               $data["tit_upd_date"] = $tit->tit_upd_date;
               $data["tit_type_self"] = $tit->tit_type_self;
               $db->replace("cb_design_title", $data);
           }else{
               $db->update("cb_design_title", $data, $where);
           }
		}else{ //등록
            ...
            $rtn2 = $db->replace("cb_design_title", $data);
		}
	...
	public function images(){
		...
		if(!empty($view['imgcode'])) { //상품코드
			if($view['imgcode'] == "Y") {
				$where .= " AND length(img_code) >= 10 AND length(img_code) <= 20 "; //상품코드(Y)
			} else if($view['imgcode'] == "N") {
				$where .= " AND (length(ifnull(img_code,'')) < 10 OR length(img_code) > 20) "; //상품코드(N) 또는 오류코드
			}
		}
		$sort = "";
		if($view['sort'] == "2"){ //제품명순
		...
		SELECT
			...
			, (SELECT description FROM cb_images_category c WHERE c.id = i.img_category1 LIMIT 1) AS img_category1_nm /* 대분류명 */
			, (SELECT description FROM cb_images_category c WHERE c.id = i.img_category2 LIMIT 1) AS img_category2_nm /* 소분류명 */
		FROM cb_images i
		". $where ."
		ORDER BY ". $sort ."
		LIMIT ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage']." ";
	...
	public function images_download() {
		...
		// 필드명을 기록한다.
		// 글꼴 및 정렬
		$this->excel->getActiveSheet()->duplicateStyleArray( array('font' => array('bold' => true, 'size' => 11),
		...
		$this->excel->getActiveSheet()->getColumnDimension('A')->setWidth(10); //일련번호
		$this->excel->getActiveSheet()->getColumnDimension('B')->setWidth(10); //대분류번호
		$this->excel->getActiveSheet()->getColumnDimension('C')->setWidth(10); //소분류번호
		$this->excel->getActiveSheet()->getColumnDimension('D')->setWidth(40); //제품명
		$this->excel->getActiveSheet()->getColumnDimension('E')->setWidth(20); //상품코드
		$this->excel->getActiveSheet()->getColumnDimension('F')->setWidth(60); //이미지경로
		$this->excel->getActiveSheet()->getColumnDimension('G')->setWidth(60); //썸네일경로
		$this->excel->getActiveSheet()->getColumnDimension('H')->setWidth(20); //등록일
		$this->excel->getActiveSheet()->getColumnDimension('I')->setWidth(20); //최종수정일

		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(0, 1, "ID");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(1, 1, "대분류");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, 1, "소분류");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(3, 1, "제품명");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(4, 1, "상품코드");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(5, 1, "이미지");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, 1, "썸네일");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(7, 1, "등록일");
		$this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, 1, "최수정일");

		$cnt = 1;
		$row = 2;
		foreach($list as $r) {
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(0, $row, $r->img_id);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $r->img_category1);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $r->img_category2);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $r->img_name);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $r->img_code);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, $r->img_path);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $r->img_path_thumb);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $r->img_cre_date);
			$this->excel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $r->img_upd_date);
			$row++;
			$cnt++;
		}
		...
		header('Content-Disposition: attachment;filename="images_list.xls"');
	...
	public function images_save(){
		...
		$data = array();
		$data["img_category1"] = ($this->input->post("img_category1") != "") ? $this->input->post("img_category1") : "0"; //대분류
		$data["img_category2"] = ($this->input->post("img_category2") != "") ? $this->input->post("img_category2") : "0"; //소분류
	...
	public function images_del(){
		...
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 상품이미지 관리 삭제 : ". $sql);
		$this->db->query($sql);
		$result = array();
		$result['code'] = '0';
		$result['msg'] = $img_id;
	...
	//디자인관리 > 상품이미지 관리 > 선택삭제 2021-02-03
	public function images_chk_del(){
		$chkIds = $this->input->post("chkIds"); //이미지번호
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > chkIds : ". $chkIds);
		$sql = "update cb_images set img_useyn='N' where img_id IN (". $chkIds .") ";
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 상품이미지 관리 선택삭제 : ". $sql);
		$this->db->query($sql);
		$result = array();
		$result['code'] = '0';
		$result['msg'] = "OK";
	...
	//디자인관리 > 상품이미지 관리 > 데이타 조회
	public function images_data(){
		$img_id = $this->input->post("img_id"); //이미지번호
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 상품이미지 관리 > 데이타 조회");
		$sql = "select * from cb_images where img_id = '". $img_id ."' ";
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 상품이미지 관리 > 데이타 조회 : ". $sql);
	...
	public function images_import(){
        ...
        $db_e = $this->load->database("59em", TRUE);
        $db_m = $this->load->database("91m", TRUE);
		...
		foreach ($files as $fn) {
			...
			if(file_exists($old_filepath) && 1 == 1){
				if(!copy($old_filepath, $new_filepath)) {
					...
					$simgconfig = array();
					$simgconfig['image_library'] = 'gd2';
					$simgconfig['source_image']	= $old_filepath;
					$simgconfig['maintain_ratio'] = TRUE;
					$simgconfig['width']	= 200; //썸네일 가로 사이즈
					$simgconfig['height']= 200; //썸네일 가로 사이즈
					$simgconfig['new_image'] = $new_dir."/".explode(".", $new_name)[0] ."_thumb.". $file_ext;
					//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > new_image : ". $simgconfig['new_image']);
					$this->image_lib->initialize($simgconfig);
					if(!$this->image_lib->resize()) {
						//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 썸네일 이미지 저장 실패 : ". $this->upload->display_errors());
					...
					if($img_cnt == 0){ //신규등록
                        ...
                        // $db_e->replace($tbl_nm, $data);
                        // $db_m->replace($tbl_nm, $data);
						$file_mode = "등록";
                        ...
                        // $db_e->update($tbl_nm, $data, $where);
                        // $db_m->update($tbl_nm, $data, $where);
						$file_mode = "수정";
	...
	//디자인관리 > 상품이미지 관리 > 소분류 조회
	public function search_category2(){
		$category1 = $this->input->post("category1"); //대분류
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 상품이미지 관리 > 소분류 조회");
		$sql = "SELECT id, code, description FROM cb_images_category WHERE parent_id = '". $category1 ."' and useyn = 'Y' ORDER BY id ASC";
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 상품이미지 관리 > 소분류 조회 : ". $sql);
		$result = $this->db->query($sql)->result();
		$json = json_encode($result,JSON_UNESCAPED_UNICODE);
		//$json = str_replace('null', '""', $json);
		header('Content-Type: application/json');
	...
	//디자인관리 > 상품이미지 관리 > 카테고리 엑셀 업데이트 2021-02-09
	public function images_category_upload(){
		$cnt = 0;
		$upd = 0;
		$result = array();
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > file : ". $_FILES['file']['tmp_name']);
	    ...
	    if(file_exists($_FILES['file']['tmp_name'])) {
			...
			for($i = 0; $i < $sheetsCount; $i++){
				...
				//log_message("ERROR", "Rows : ".$highestRow);
				// 한줄읽기
				for ($row = 2; $row <= $highestRow; $row++){ //1번줄부터 읽기
					$cnt++;
					$rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row, NULL, TRUE, FALSE);
					$img_id = (empty($rowData[0][0])) ? "0" : trim($rowData[0][0]); //일련번호
					$img_cate1 = (empty($rowData[0][1])) ? "0" : trim($rowData[0][1]); //대분류번호
					$img_cate2 = (empty($rowData[0][2])) ? "0" : trim($rowData[0][2]); //소분류번호
					if($img_id > 0){
						//이미지 라이브러리 확인
						$sql = "SELECT COUNT(*) AS cnt FROM cb_images WHERE img_id = '". $img_id ."' ";
						$img_cnt = $this->db->query($sql)->row()->cnt;

						//상품명이 일치하고 상품코드가 없는 경우
						if($img_cnt > 0){
							$sql = "UPDATE cb_images SET img_category1 = '". $img_cate1 ."', img_category2 = '". $img_cate2 ."' WHERE img_id = '". $img_id ."' ";
							$rtn = $this->db->query($sql);
							if($rtn == 1){
								$upd++;
							}
							//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 카테고리 업데이트 [". $upd ."] : ". $sql);
						}
						//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > file[". $cnt ."] file_code : ". $file_code .", file_name : ". $file_name .", full_name : ". $full_name);
					}
				}
	...
	public function customer_images(){
		...
		WHERE psg_id IN (
			...
			WHERE psg_mng_yn = 'Y' /* 관리여부 */
			AND psg_mem_id != '3' /* 관리자 제외 */
			AND LEFT(psg_imgpath,16) = '/uploads/goods/Y' /* 회원이 직접 올린 경우 */
			AND psg_name != '' /* 상품명 있는 것만 조회 */
			AND psg_dcprice != '' /* 할인가 있는 것만 조회 */ ";
		if(!empty($view['searchStr'])) {
			if($view['searchType'] == "1"){ //제품명
				$orstr = explode(' ', $view['searchStr']);
				$orcnt = count($orstr);
				if($orcnt > 1) {
					...
					$prefix = '';
					for($i=0; $i<$orcnt; $i++){
						$where .= $prefix."CONCAT(a.psg_name, ' ', a.psg_option) LIKE '%". addslashes($orstr[$i]) ."%' ";
						$prefix = "AND ";
					}
					$where .= ") ";
				} else {
					$where .= "	AND CONCAT(a.psg_name, ' ', a.psg_option) LIKE '%". addslashes($orstr[0]) ."%' ";
				}
			} else if($view['searchType'] == "2"){ //업체명
				$where .= "	AND b.mem_username LIKE '%". addslashes($view['searchStr']) ."%'  ";
			}
		}
		$where .= "	GROUP BY psg_imgpath ";
		$where .= ") ";
		$sort = "";
		if($view['sort'] == "2"){ //제품명순
			$sort = "a.psg_name ASC";
		...
		$sql = "
		SELECT
			  a.psg_id AS img_id /* 상품번호 */
			, CONCAT(a.psg_name, ' ', a.psg_option) AS img_name /* 상품명 규격 */
			, a.psg_imgpath AS img_path /* 이미지경로 */
			, a.psg_imgpath AS img_path_thumb /* 썸네일 이미지경로 */
			, b.mem_username /* 업체명 */
		FROM cb_pop_screen_goods a
		LEFT JOIN cb_member b ON b.mem_id = a.psg_mem_id
		". $where ."
		ORDER BY ". $sort ."
		LIMIT ".(($view['param']['page'] - 1) * $view['perpage']).", ".$view['perpage']." ";
		//echo $sql ."<br>";
	...
	//디자인관리 > 고객이미지 관리 > 저장 (상품이미지 관리에 추가 처리)
	public function customer_images_save(){
		//상품이미지 관리에 추가
		$data = array();
		$data["img_category1"] = ($this->input->post("img_category1") != "") ? $this->input->post("img_category1") : "0"; //대분류
		$data["img_category2"] = ($this->input->post("img_category2") != "") ? $this->input->post("img_category2") : "0"; //소분류
		$data["img_name"] = $this->input->post("img_name"); //제품명
		$data["img_code"] = $this->input->post("img_code"); //상품코드
		$data["img_path"] = $this->input->post("img_path"); //이미지경로
		$data["img_path_thumb"] = $data["img_path"]; //썸네일 이미지경로
		$data["img_useyn"] = "Y"; //사용여부
		$data["img_cre_id"] = $this->member->item("mem_id"); //등록자 회원번호
		$data["img_upd_id"] = $this->member->item("mem_id"); //수정자 회원번호
		$rtn = $this->db->replace("cb_images", $data); //이미지 라이브러리 추가
		...
		//고객이미지 관리여부 N 처리
		$psg_id = $this->input->post("img_id"); //이미지번호
		$sql = "update cb_pop_screen_goods set psg_mng_yn='N' where psg_id = '". $psg_id ."' ";
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 고객이미지 관리 삭제 : ". $sql);
		$this->db->query($sql);
	...
	//디자인관리 > 고객이미지 관리 > 삭제
	public function customer_images_del(){
		$img_id = $this->input->post("img_id"); //이미지번호
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > img_id : ". $img_id);
		//$sql = "delete from cb_images where img_id = '". $img_id ."' ";
		$sql = "update cb_pop_screen_goods set psg_mng_yn='N' where psg_id = '". $img_id ."' ";
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 고객이미지 관리 삭제 : ". $sql);
		$this->db->query($sql);
		$result = array();
		$result['code'] = '0';
	...
	//디자인관리 > 고객이미지 관리 > 선택삭제
	public function customer_images_chk_del(){
		$chkIds = $this->input->post("chkIds"); //이미지번호
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > chkIds : ". $chkIds);
		$sql = "update cb_pop_screen_goods set psg_mng_yn='N' where psg_id IN (". $chkIds .") ";
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 고객이미지 관리 선택삭제 : ". $sql);
		$this->db->query($sql);
		$result = array();
		$result['code'] = '0';
		$result['msg'] = "OK";
	...
	//디자인관리 > 고객이미지 관리 > 데이타 조회
	public function customer_images_data(){
		$img_id = $this->input->post("img_id"); //이미지번호
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 고객이미지 관리 > 데이타 조회");
		$sql = "
		SELECT
			  a.psg_id AS img_id /* 상품번호 */
			, CONCAT(a.psg_name, ' ', a.psg_option) AS img_name /* 상품명 규격 */
			, a.psg_imgpath AS img_path /* 이미지경로 */
			, a.psg_imgpath AS img_path_thumb /* 썸네일 이미지경로 */
			, 0 AS img_category1 /* 대분류 */
			, 0 AS img_category2 /* 소분류 */
			, '' AS img_code /* 상품코드 */
		FROM cb_pop_screen_goods a
		where psg_id = '". $img_id ."' ";
		//log_message("ERROR", $_SERVER['REQUEST_URI'] ." > 디자인관리 > 고객이미지 관리 > 데이타 조회 : ". $sql);
		$result = $this->db->query($sql)->row();
		$json = json_encode($result,JSON_UNESCAPED_UNICODE);
		$json = str_replace('null', '""', $json);
		header('Content-Type: application/json');
