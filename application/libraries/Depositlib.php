...
class Depositlib extends CI_Controller
{
    ...
    {
        $use = $this->CI->depositconfig->item('use_deposit');
        return $use;
    }
    ...
    {
        $deposit = floor($point / $this->CI->depositconfig->item('deposit_point'));
        return $deposit;
    }
    ...
    {
        $point = $deposit * $this->CI->depositconfig->item('deposit_refund_point');
        return $point;
    }
    ...
    public function do_point_to_deposit($mem_id = 0, $point = '', $pay_type = '', $content = '', $admin_memo = '')
    {
        if ( ! $this->use_deposit()) {
            $return = array(
                'result' => 'fail',
                'reason' => '이 사이트는 ' . $this->CI->depositconfig->item('deposit_name') . ' 기능을 사용하지 않습니다',
            );
        ...
        if ($deposit === 0) {
            $return = array(
                'result' => 'fail',
                'reason' => '전환하려는 ' . $this->CI->depositconfig->item('deposit_name') . '가(이) 0 이므로 진행하지 않습니다',
            );
            return json_encode($return);
        }
        if ($this->CI->depositconfig->item('deposit_point_min') && $point < $this->CI->depositconfig->item('deposit_point_min')) {
            $return = array(
                'result' => 'fail',
                'reason' => '최소 ' . $this->CI->depositconfig->item('deposit_point_min') . ' 포인트 이상만 전환이 가능합니다',
            );
            return json_encode($return);
        }
        ...
        $this->CI->point->insert_point(
            $mem_id,
            $_point,
            $this->CI->depositconfig->item('deposit_name') . ' ' . $deposit . ' ' . $this->CI->depositconfig->item('deposit_unit') . ' (으)로 전환 ',
            'deposit_point',
            $dep_id,
            '전환'
        );
        ...
        $return = array(
            'result' => 'success',
            'reason' => '포인트(' . $point . ')를 ' . $this->CI->depositconfig->item('deposit_name') . '(' . $deposit . ') 으로 전환합니다',
            'deposit' => $deposit,
            'dep_id' => $dep_id,
        );
        return json_encode($return);
    ...
    public function do_deposit_to_point($mem_id = 0, $deposit = '', $pay_type = '', $content = '', $admin_memo = '')
    {
        ...
        if (empty($deposit)) {
            $return = array(
                'result' => 'fail',
                'reason' => $this->CI->depositconfig->item('deposit_name') . ' 값이 입력되지 않았습니다',
            );
            return json_encode($return);
        }
        ...
        if ($deposit > element('total_deposit', $meta)) {
            $return = array(
                'result' => 'fail',
                'reason' => '전환하려는 ' . $this->CI->depositconfig->item('deposit_name') . '(' . $deposit . ') 보다 가지고 계신 ' . $this->CI->depositconfig->item('deposit_name') . '(' . element('total_deposit', $meta) . ') 가 작습니다',
            );
            return json_encode($return);
        }
        ...
        $this->CI->point->insert_point(
            $mem_id,
            $point,
            $this->CI->depositconfig->item('deposit_name') . ' ' . $deposit . ' ' . $this->CI->depositconfig->item('deposit_unit') . ' 을(를) 포인트로 전환 ',
            'deposit_point',
            $dep_id,
            '전환'
        );
        ...
        $return = array(
            'result' => 'success',
            'reason' => $this->CI->depositconfig->item('deposit_name') . '(' . $deposit . ') 을(를) 포인트(' . $point . ')로 전환합니다',
            'point' => $point,
            'dep_id' => $dep_id,
        );
    ...
    public function do_deposit_to_contents($mem_id = 0, $deposit = '', $pay_type = '', $content = '', $admin_memo = '')
    {
        ...
        if ($deposit > element('total_deposit', $meta)) {
            $return = array(
                'result' => 'fail',
                'reason' => '전환하려는 ' . $this->CI->depositconfig->item('deposit_name') . '(' . $deposit . ') 보다 가지고 계신 '
                    . $this->CI->depositconfig->item('deposit_name') . '(' . element('total_deposit', $meta) . ') 가 작습니다',
            );
            return json_encode($return);
        }
    ...
    public function alarm($type = '', $dep_id = 0)
    {
        ...
        $this->CI->load->library(array('email', 'notelib', 'smslib'));
        ...
        $superadminlist = '';
        if ($this->CI->depositconfig->item('deposit_email_admin_' . $type)
            OR $this->CI->depositconfig->item('deposit_note_admin_' . $type)
            OR $this->CI->depositconfig->item('deposit_sms_admin_' . $type)) {

            $mselect = 'mem_id, mem_email, mem_nickname, mem_phone';
            $superadminlist = $this->CI->Member_model->get_superadmin_list($mselect);

        }
        if ($this->CI->depositconfig->item('deposit_email_admin_' . $type) && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $emailsendlistadmin[$value['mem_id']] = $value;
            }
        }
        if (($this->CI->depositconfig->item('deposit_email_user_' . $type) && element('mem_receive_email', $member))
            OR $this->CI->depositconfig->item('deposit_email_alluser_' . $type)) {
            $emailsendlistuser['mem_email'] = element('mem_email', $member);
        }
        if ($this->CI->depositconfig->item('deposit_note_admin_' . $type) && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $notesendlistadmin[$value['mem_id']] = $value;
            }
        }
        if ($this->CI->depositconfig->item('deposit_note_user_' . $type) && element('mem_use_note', $member)) {
            $notesendlistuser['mem_id'] = element('mem_id', $member);
        }
        if ($this->CI->depositconfig->item('deposit_sms_admin_' . $type) && $superadminlist) {
            foreach ($superadminlist as $key => $value) {
                $smssendlistadmin[$value['mem_id']] = $value;
            }
        }
        if (($this->CI->depositconfig->item('deposit_sms_user_' . $type) && element('mem_receive_sms', $member))
            OR $this->CI->depositconfig->item('deposit_sms_alluser_' . $type)) {
        ...
        $replaceconfig = array(
            ...
            $money1,
            $money2,
            $this->CI->depositconfig->item('deposit_name'),
            $this->CI->depositconfig->item('deposit_unit'),
            $changepoint,
        ...
        $replaceconfig_escape = array(
            ...
            $money1,
            $money2,
            html_escape($this->CI->depositconfig->item('deposit_name')),
            html_escape($this->CI->depositconfig->item('deposit_unit')),
            $changepoint,
        ...
        if ($emailsendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->depositconfig->item('deposit_email_admin_' . $type . '_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->depositconfig->item('deposit_email_admin_' . $type . '_content')
            );
        ...
        if ($emailsendlistuser) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->depositconfig->item('deposit_email_user_' . $type . '_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->depositconfig->item('deposit_email_user_' . $type . '_content')
            );
            $this->CI->email->clear(true);
            $this->CI->email->from($this->CI->cbconfig->item('webmaster_email'), $this->CI->cbconfig->item('webmaster_name'));
        ...
        if ($notesendlistadmin) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->depositconfig->item('deposit_note_admin_' . $type . '_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->depositconfig->item('deposit_note_admin_' . $type . '_content')
            );
        ...
        if ($notesendlistuser && element('mem_id', $notesendlistuser)) {
            $title = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->depositconfig->item('deposit_note_user_' . $type . '_title')
            );
            $content = str_replace(
                $searchconfig,
                $replaceconfig_escape,
                $this->CI->depositconfig->item('deposit_note_user_' . $type . '_content')
            );
            $note_result = $this->CI->notelib->send_note(
                $sender = 0,
        ...
        if ($smssendlistadmin) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->depositconfig->item('deposit_sms_admin_' . $type . '_content')
            );
            $sender = array(
        ...
        if ($smssendlistuser) {
            $content = str_replace(
                $searchconfig,
                $replaceconfig,
                $this->CI->depositconfig->item('deposit_sms_user_' . $type . '_content')
            );
            $sender = array(
