...
class Message {
    ...
    public function phoneBookToSendCache($msgType, $customer_filter="")
    {
        ...
        if($customer_filter == "NONE") {
            //log_message("ERROR", "customer_filter : ".$customer_filter);
            //array_push($param, "");
            $sql = "insert into cb_sc_".$CI->member->item('mem_userid')."
				    (sc_name , sc_tel, sc_content, sc_button1, sc_button2, sc_button3, sc_button4, sc_button5, sc_sms_yn, sc_lms_content, sc_sms_callback, sc_img_url, sc_img_link, sc_template)
			     select ab_name, ab_tel, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? from cb_ab_".$CI->member->item('mem_userid')." where ab_status='1' and length(ab_tel) >=8  and  ifnull(ab_kind, '') = ''";

        } else if ($customer_filter == "FT") {      // 플러스 친구만 발송 2019-07-09 추가
            $sql = "insert into cb_sc_".$CI->member->item('mem_userid')."
            	    (sc_name , sc_tel, sc_content, sc_button1, sc_button2, sc_button3, sc_button4, sc_button5, sc_sms_yn, sc_lms_content, sc_sms_callback, sc_img_url, sc_img_link, sc_template)
                select ab_name, ab_tel, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? from cb_ab_".$CI->member->item('mem_userid')." where ab_status='1' and length(ab_tel) >=8 and exists ( select 1 from cb_friend_list b where b.mem_id = '".$CI->member->item("mem_id")."' and b.phn = ab_tel)";

        } else if ($customer_filter == "NFT") {     // 플러스 친구아닌 고객만 발송 2019-07-09 추가
            $sql = "insert into cb_sc_".$CI->member->item('mem_userid')."
            	    (sc_name , sc_tel, sc_content, sc_button1, sc_button2, sc_button3, sc_button4, sc_button5, sc_sms_yn, sc_lms_content, sc_sms_callback, sc_img_url, sc_img_link, sc_template)
                select ab_name, ab_tel, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? from cb_ab_".$CI->member->item('mem_userid')." where ab_status='1' and length(ab_tel) >=8 and not exists ( select 1 from cb_friend_list b where b.mem_id = '".$CI->member->item("mem_id")."' and b.phn = ab_tel)";

		} else if(substr($customer_filter, 0, 7) == "groupId") { // 추가 2020-09-10
            $groupId = str_replace("groupId", "", $customer_filter); //그룹번호
			$sch = "";
			if($groupId == "all"){ //그룹 전체 선택의 경우
				$sch .= "WHERE u.ab_status = '1' AND LENGTH(u.ab_tel) >=8 ";
			}else{ // //그룹 전체 선택이 아닌경우
				$sql = "SELECT cg_level, cg_parent_id FROM cb_customer_group c WHERE cg_mem_id = '". $CI->member->item('mem_id') ."' and c.cg_id = '". $groupId ."' ";
				$group = $CI->db->query($sql)->row();
				$sch .= "LEFT JOIN cb_ab_".$CI->member->item('mem_userid')."_group b ON u.ab_id = b.aug_ab_id ";
				$sch .= "WHERE u.ab_status = '1' AND LENGTH(u.ab_tel) >=8 ";
				if($group->cg_level == 1){ //1차 그룹
					$sch .= "
					AND b.aug_group_id IN (
						SELECT cg_id FROM cb_customer_group c WHERE c.cg_parent_id = '". $group->cg_parent_id ."' AND c.cg_use_yn = 'Y'
					) ";
				}else{ //2차 그룹
					$sch .= "AND b.aug_group_id = '". $groupId ."' ";
				}
			}
			$sql = "
			INSERT INTO cb_sc_".$CI->member->item('mem_userid')."
			(sc_name , sc_tel, sc_content, sc_button1, sc_button2, sc_button3, sc_button4, sc_button5, sc_sms_yn, sc_lms_content, sc_sms_callback, sc_img_url, sc_img_link, sc_template)
			SELECT ab_name, ab_tel, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
			FROM cb_ab_".$CI->member->item('mem_userid')." u
			". $sch;
            ...
            //if($customer_filter) { array_push($param, $customer_filter_temp); }
            //log_message("ERROR", "customer_filter : ".$customer_filter);
            //log_message("ERROR", "customer_filter_temp : ".$customer_filter_temp);
            ...
            $sql = "insert into cb_sc_".$CI->member->item('mem_userid')."
            	    (sc_name , sc_tel, sc_content, sc_button1, sc_button2, sc_button3, sc_button4, sc_button5, sc_sms_yn, sc_lms_content, sc_sms_callback, sc_img_url, sc_img_link, sc_template)
                select ab_name, ab_tel, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? from cb_ab_".$CI->member->item('mem_userid')." where ab_status='1' and length(ab_tel) >=8 ".(($customer_filter) ? " and ifnull(ab_kind, '') in (".$customer_filter_temp.") " : "");
        }

        //log_message("ERROR", "lms SQL : ".$sql);
        $CI->db->query($sql, $param);
        //log_message("ERROR", "CI->db->error() : ".$CI->db->error()['code']);
    ...
    public function sendAgentSendCache($msgType, $group_id, $mem_id, $mem_userid, $pf_key, $reserveDt='00000000000000', $limit=9999999)
    {
        ...
        //ib플래그
        $request_table = "DHN_REQUEST_RESULT";
        if(config_item('ib_flag')=="Y"){
            $request_table = "DHN_REQUEST_RESULT_IB";
        }
        //ib플래그
    ...
    public function sendAgentSendAPIs($msgType, $group_id, $mem_id, $mem_userid, $pf_key, $reserveDt='00000000000000', $limit=9999999)
    {
        $CI =&get_instance();
        $param = array(
            "N",	//(($msgType=="at") ? 'N'	: 'Y'),		// 광고구분(친구톡 필수 광고성정보 구분:내용에 (광고)글자가 있는 경우 N으로 보내면 됨)
        ...
        $sql = "insert into DHN_REQUEST_RESULT
			(
				MSGID, AD_FLAG,
				BUTTON1, BUTTON2, BUTTON3, BUTTON4, BUTTON5,
				IMAGE_LINK, IMAGE_URL,
			...
			select
				concat('".$mem_id."_".$group_id."_', sc_id), ?,
				sc_button1, sc_button2, sc_button3, sc_button4, sc_button5,
				sc_img_link IMAGE_LINK, sc_img_url IMAGE_URL,
				?, sc_content, sc_lms_content, ? , ?, ?,
				(CASE WHEN SUBSTR(sc_tel, 1, 2)='01' THEN CONCAT('82', SUBSTR(sc_tel,2)) ELSE sc_tel END), ?, now(), ?, sc_sms_yn,
        ...
        $sql = "update cb_wt_msg_sent set mst_qty=".($mst_qty).", mst_send_server='133' where mst_id = '".$group_id."'";
        $CI->db->query($sql);
