...
class Sweettracker {
    ...
    var $adm_server = "https://api-dev.wideshot.co.kr"; // 개발 서버
//    var $adm_server = "https://api.wideshot.co.kr";     // 운영 서버
    ...
    {
        $url = $this->adm_server."/api/v1/sender/token";
        $postfields = json_encode(array("yellowId"=>$plusFriend, "phoneNumber"=>$phoneNumber), JSON_UNESCAPED_UNICODE);
        $param = "?yellowId=".$plusFriend."&phoneNumber=".$phoneNumber;
        $ch = curl_init();
        ...
        $options = array(
            ...
            CURLOPT_HTTPHEADER => array('Accept: application/json', 'Content-Type: application/json', 'sejongApiKey:'.$apikey),
    ...
    {
        $url = $this->adm_server."/api/v1/sender/category/all";
        $ch = curl_init();
        $options = array(
            CURLOPT_URL => $url,
            CURLOPT_HEADER => false,
    ...
    public function get_one_category($categoryCode)
    {
        $url = $this->adm_server."/api/v1/sender/category";
    ...
    public function sender_create($userId, $yellowId, $phoneNumber,$category, $licenseNumber, $token)
    {
        ...
        $url = $this->adm_server."/api/v1/sender/create";
        ...
        $options = array(
            ...
            CURLOPT_HEADER => false,
            CURLOPT_POST => 1,
            CURLOPT_HTTPHEADER => array("Content-Type:multipart/form-data", 'sejongApiKey:'.$apikey, 'token:'.$token, 'phoneNumber:'.$phoneNumber),
...
//            CURLOPT_INFILESIZE => $filesize,
    ...
    {
        $url = $this->adm_server."/api/v1/sender/delete";
    ...
    {
        $url = $this->adm_server."/api/v1/sender/recover";
    ...
    {
        $url = $this->adm_server."/api/v1/sender?senderKey=".$senderKey;
    ...
    {
        $url = $this->adm_server."/api/v1/template/create";
        ...
        log_message("ERROR", "CREATE ===>".$this->userId);
    ...
    public function template_request($data)
    {
        ...
        $user_Id = $this->getUserId($data[0]['senderKey']);
    ...
    {
        $url = $this->adm_server."/api/v1/template";
        ...
        $options = array(
            ...
            CURLOPT_HEADER => false,
            CURLOPT_POST => 1,
            CURLOPT_HTTPHEADER => array('sejongApiKey:'.$apikey),
            CURLOPT_POSTFIELDS => $postfields,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_USERAGENT => $this->agent,
            CURLOPT_REFERER => "",
    ...
    {
        $url = $this->adm_server."/api/v1/template/update";
        $postfields = json_encode($data, JSON_UNESCAPED_UNICODE);
        
        $user_Id = $this->getUserId($data['senderKey']);
    ...
    {
        $url = $this->adm_server."/api/v1/template/delete";
        $postfields = json_encode($data, JSON_UNESCAPED_UNICODE);
        $user_Id = $this->getUserId($data[0]['senderKey']);
        $ch = curl_init();
        $options = array(
        ...
        log_message("ERROR", "DELETE ===>".$buffer);
        curl_close($ch);
        if ($cinfo['http_code'] != 200) { return ($buffer) ? json_decode($buffer) : ''; } else { return json_decode($buffer); }
    }
    ...
    {
        $url = $this->adm_server."/api/v1/template/comment";
        $postfields = json_encode($data, JSON_UNESCAPED_UNICODE);
    ...
    public function template_last_modified($senderKey='')
    {
        ...
        $url = "";
        if(empty($senderKey)) {
            $url = $this->adm_server."/api/v1/template/last_modified";
        } else {
            $url = $this->adm_server."/api/v1/template/last_modified?senderKey=".$senderKey;
        }
