...
class Sweettracker {
    ...
    public function sender_token($plusFriend, $phoneNumber)
    {
        ...
        log_message("ERROR", "인증번호요청 결과".$cinfo['http_code']);
        curl_close($ch);
    ...
    public function template_crate($data, $userId = '')
    {
        ...
        log_message("ERROR", "CREATE2 ===>".$userId);
    ...
    public function template_update($data)
    {
        ...
        log_message("ERROR", "template_update ====> User_Id : ".$user_Id);
        $ch = curl_init();
        $options = array(
            CURLOPT_URL => $url,
            CURLOPT_HEADER => false,
    ...
    public function upload_wide_image($profileKey, $img_file, $img_type, $img_name)
    {
        ...
        $cinfo = curl_getinfo($curl);
        curl_close($curl);
        log_message("ERROR", "File upload_wide_image : ".$buffer);
        if ($cinfo['http_code'] != 200) { return ($buffer) ? json_decode($buffer) : ''; } else { return json_decode($buffer); }
    }
    ...
    /* 메시지 전송 결과 */
    /* [{"msgid":""},{"msgid":""}] */
    public function response($profile_key, $data)
    {
        ...
        $cinfo = curl_getinfo($ch);
        curl_close($ch);
        log_message("ERROR", "SW Response : ".$buffer);
        if ($cinfo['http_code'] != 200) { return ($buffer) ? json_decode($buffer) : ''; } else { return json_decode($buffer); }
    }
    ...
    public function sendAgentSendCache($msgType, $group_id, $mem_id, $mem_userid, $pf_key, $reserveDt='00000000000000', $limit=9999999, $wide='N')
    {
        ...
        $sql = "insert ignore into TBL_REQUEST
			(
				...
				RESERVE_DT, S_CODE, SMS_KIND, SMS_LMS_TIT, SMS_SENDER, TMPL_ID, REMARK2, WIDE
			)
			select
				concat('".$mem_id."_', sc_id, '_O2O'), ?,
    ...
    public function sendAgentSendAPI($msgType, $group_id, $mem_id, $mem_userid, $pf_key, $reserveDt='00000000000000', $sc_id)
    {
        ...
        $sql = "insert ignore into TBL_REQUEST
			...
			select
				concat('".$mem_id."_', sc_id, '_API_O2O'), ?,
				sc_button1, sc_button2, sc_button3, sc_button4, sc_button5,
				sc_img_link IMAGE_LINK, sc_img_url IMAGE_URL,
				?, sc_content, sc_lms_content, ? , ?, ?,
				(CASE WHEN SUBSTR(sc_tel, 1, 2)='01' THEN CONCAT('82', SUBSTR(sc_tel,2)) ELSE sc_tel END), ?, now(), ?, sc_sms_yn,
