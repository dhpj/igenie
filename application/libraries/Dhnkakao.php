...
class Dhnkakao {
    ...
    //var $adm_server = "http://222.122.203.68:8282";
    var $adm_server = "http://210.114.225.57:8282"; //원래꺼
...
//            CURLOPT_INFILESIZE => $filesize,
    ...
    public function upload_image($profileKey, $img_file, $img_type, $img_name)
    {
        ...
        curl_close($curl);
        log_message("ERROR", "F : ".$buffer." / ".$cinfo['http_code']);
        if ($cinfo['http_code'] != 200) { return ($buffer) ? json_decode($buffer) : ''; } else { return json_decode($buffer); }
    }
    ...
    /* 이미지 전송 */
    public function upload_at_image($img_file, $img_type, $img_name)
    {
        $url = $this->adm_server."/at/image";
        $cfile = $this->getCurlValue($img_file, $img_type, $img_name);
...
//     }
	...
	public function sendAgentSendCache2nd($msgid, $alim_code, $alim_cont, $btn1_alim, $btn2_alim, $btn3_alim, $btn4_alim, $btn5_alim, $message_type="at")
	{
		...
		$sql .= "	IMAGE_LINK, IMAGE_URL, ";
		$sql .= "	'".$message_type."' AS MESSAGE_TYPE,  ";
    ...
    public function NewUploadToSendCache2($msgType, $data)
    {
        $CI =& get_instance();
        /* 고객전체 발송 -> sendCache 삭제후 AddrBook으로 insert */
        $CI->db->query("delete from cb_sc_stest");
        if($CI->db->error()['code'] > 0) { return 0; }
        ...
        /* 고객 정보를 sendCache에 insert */
        $param = array(
            $data['mst_content'],	// 템플릿내용
            $data['btn1']=="[]" ? null : str_replace(array("[", "]"), "", $data['btn1']),
            $data['btn2']=="[]" ? null : str_replace(array("[", "]"), "", $data['btn2']),
            $data['btn3']=="[]" ? null : str_replace(array("[", "]"), "", $data['btn3']),
            $data['btn4']=="[]" ? null : str_replace(array("[", "]"), "", $data['btn4']),
            $data['btn5']=="[]" ? null : str_replace(array("[", "]"), "", $data['btn5']),
            "N",			// SMS재발신여부
            '',				// LMS/SMS내용
            '',
            '',
            '', //($CI->input->post('img_link')) ? $CI->input->post('img_link') : null,
            '' // 템플릿코드
        );

        $sql = "insert into cb_sc_stest
                (sc_name , sc_tel, sc_content, sc_button1, sc_button2, sc_button3, sc_button4, sc_button5, sc_sms_yn, sc_lms_content, sc_sms_callback, sc_img_url, sc_img_link, sc_template)
            values('', '".$data['tel']."', ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";
        $CI->db->query($sql, $param);
        if($CI->db->error()['code'] > 0) { echo '<pre> :: ';print_r($CI->db->error());return 0; }
    ...
    // 템플릿 휴면 해제
    public function template_dormant_uplock($data)
    {
        $url = "https://bzm-center.kakao.com/api/v2/51d3ab9bc25dceeee44c6f472fd16691c8b49d65/alimtalk/template/dormant/release";
        $postfields = json_encode($data, JSON_UNESCAPED_UNICODE);
        ...
        $options = array(
            ...
            CURLOPT_POSTFIELDS => $postfields,
            CURLOPT_RETURNTRANSFER => true,
            //CURLOPT_USERAGENT => $this->agent,    // 나중에 DHNCenter 수정할때 넣어주기
            CURLOPT_REFERER => "",
            CURLOPT_TIMEOUT => 3
            //CURLOPT_SSLVERSION => 3,			// SSL 버젼 (https 접속시에 필요)
            //CURLOPT_SSL_VERIFYPEER => false	// 인증서 체크같은데 true 시 안되는 경우가 많다. default 값이 true 이기때문에 이부분을 조심 (https 접속시에 필요)
        ...
        curl_close($ch);
        if ($cinfo['http_code'] != 200) {
            return ($buffer) ? json_decode($buffer) : '';
        } else {
            return json_decode($buffer);
        }
    }
    
    public function template_check($data){
