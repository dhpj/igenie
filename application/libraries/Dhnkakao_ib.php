...
class Dhnkakao_ib {
    ...
    var $userid = "dhncorp2_talk";
    var $apikey = "YjNjZDRkMzc0N2RlM2EyN2U4ZjVhODRlNmQ5NDBiNTE4MzhlODY1YjYxNDFmYTE5YzczZDAxNTEzOGE3NjljNQ==";
    var $agent = 'Mozilla/4.0 (compatible; MSIE 9.0; Windows NT 5.0)';

    var $adm_server = "https://bzm-center.ibizplus.co.kr";
    //ib플래그
    var $request_table = "DHN_REQUEST_IB";
    ...
    public function getUserId($profileKey = "")
    {
        ...
        if(is_null($profileKey) || $profileKey == "") {
            ...
            $str = "select * from cb_wt_send_profile_ib where spf_key = '".$profileKey."'";
    ...
    public function sender_token($yellowId, $phoneNumber, $category)
    {
        $url = $this->adm_server."/v3/sender/create";
    ...
    {
        $url = $this->adm_server."/v1/category/all";

        $ch = curl_init();
        //log_message("ERROR", $this->userid);
        $options = array(
            CURLOPT_URL => $url,
            CURLOPT_HEADER => false,
            CURLOPT_HTTPGET => TRUE,
            CURLOPT_HTTPHEADER => array('Accept: application/json', 'Content-Type: application/json', 'auth_key: '.$this->apikey, 'siteid: '.$this->userid),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_USERAGENT => $this->agent,
            CURLOPT_REFERER => "",
            CURLOPT_TIMEOUT => 30
    ...
    {
        $url = $this->adm_server."/v1/category";
    ...
    /* 발신프로필 추가 */
    public function sender_create($yellowId, $token, $etc)
    {

        $url = $this->adm_server."/v3/sender/token";
    ...
    public function sender_delete($senderKey)
    {
        ...
        $options = array(
            ...
            CURLOPT_HEADER => false,
            CURLOPT_POST => 1,
            CURLOPT_HTTPHEADER => array("cache-control: no-cache","content-type: multipart/form-data",'auth_key: '.$this->apikey, 'siteid: '.$this->userid),
            CURLOPT_POSTFIELDS => $postfields,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_USERAGENT => $this->agent,
            CURLOPT_REFERER => "",
    ...
    /* 발신프로필 삭제 */
    public function sender_delete_search($senderKey)
    {
        $url = $this->adm_server."/v1/sender/delete_search?senderKey=".$senderKey;
    ...
    {
        $url = $this->adm_server."/v3/sender?senderKey=".$senderKey;
        //log_message("ERROR", $url);
        $ch = curl_init();
        $options = array(
            CURLOPT_URL => $url,
            CURLOPT_HEADER => false,
            CURLOPT_HTTPHEADER => array('auth_key: '.$this->apikey, 'siteid: '.$this->userid),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_USERAGENT => $this->agent,
            CURLOPT_REFERER => "",
            CURLOPT_TIMEOUT => 3
    ...
    {
        $url = $this->adm_server."/v1/template/create";
        //$postfields = json_encode($data, JSON_UNESCAPED_UNICODE);
        //log_message("ERROR", $this->str_urlencode($data));
        $ch = curl_init();
        $options = array(
            CURLOPT_URL => $url,
            CURLOPT_HEADER => false,
    ...
    /* 이미지 템플릿 추가 */
    public function template_create_with_image($data )
    {
        $url = $this->adm_server."/v2/alimtalk/template/create_with_image";
    ...
    {
        $url = $this->adm_server."/v1/template/request";
    ...
    /* 템플릿 검수요청 */
    /* [{"senderKey":"", "senderKeyType":"", "templateCode":""},{"senderKey":"", "senderKeyType":"", "templateCode":""}] */
    public function template_req_cancel($data)
    {
        $url = $this->adm_server."/v1/template/cancel_request";
    ...
    {
        $url = $this->adm_server."/v1/template";
    ...
    {
        $url = $this->adm_server."/v1/template/update";
        //$postfields = json_encode($data, JSON_UNESCAPED_UNICODE);
    ...
    /* 이미지 템플릿 수정 */
    public function template_update_with_image($data )
    {
        $url = $this->adm_server."/v2/alimtalk/template/update_with_image";
//         $postfields = array();
...
//         $postfields['image'] = $cfile;
    ...
    {
        $url = $this->adm_server."/v1/template/delete";
        //$postfields = json_encode($data, JSON_UNESCAPED_UNICODE);
    ...
    {
        $url = $this->adm_server."/v1/template/comment_file";
    ...
    {
        $url = $this->adm_server."/v2/alimtalk/category/all";
        $ch = curl_init();
    ...
    /* 템플릿 카테고리 조회 */
    public function template_category($categoryCode)
    {
        $url = $this->adm_server."/v2/alimtalk/category?categoryCode=".$categoryCode;
        $ch = curl_init();
    ...
    /* 템플릿의 카테고리를 변경합니다. */
    public function template_category_update($data)
    {
        $url = $this->adm_server."/v2/alimtalk/category/update";
    ...
    {
        $url = $this->adm_server."/v1/group/list";
...
//         ); // cURL options
    ...
    {
        $url = $this->adm_server."/v1/group?groupKey=".$groupKey;
        $ch = curl_init();
        $options = array(
            CURLOPT_URL => $url,
            CURLOPT_HEADER => false,
    ...
    {
        $url = $this->adm_server."/v1/group/add";
        ...
        $options = array(
            ...
            CURLOPT_POST => 1,
            CURLOPT_HTTPHEADER => array('Accept: application/json', 'Content-Type: application/json', 'auth_key: '.$this->apikey, 'siteid: '.$this->userid),
            CURLOPT_POSTFIELDS => json_encode($pf, JSON_UNESCAPED_UNICODE),
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_USERAGENT => $this->agent,
            CURLOPT_REFERER => "",
            CURLOPT_TIMEOUT => 3
    ...
    {
        $url = $this->adm_server."/v1/group/remove";
    ...
    /* 이미지 전송 */
    public function upload_image($img_file, $img_type, $img_name)
    {
        $url = $this->adm_server."/v2/friendtalk/upload_image";
        $cfile = $this->getCurlValue($img_file, $img_type, $img_name);

        $post   = array('image' => $cfile, 'image_name' => $img_name);
        $curl = curl_init();
    ...
    /* 이미지 전송 */
    public function upload_wide_image($img_data)
    {
        $url = $this->adm_server."/v2/friendtalk/upload_wide_image";
    ...
    /* 이미지 전송 */
    public function upload_at_image($img_data)
    {
        $url = $this->adm_server."/v1/image/alimtalk/template";

        $post   = array('image' => $img_data);
    ...
    public function moveUploadToSendCache($msgType, $mem_id, $callback="")
    {
        ...
        foreach($lists as $r)
        {
            if($agreeid != "") { //2021-01-20
                ...
                // $btn1_array = array();
                $btn1_array->url_mobile = base_url()."short_url/agree/".$md5;
                $btn1_array->url_pc = base_url()."short_url/agree/".$md5;
    ...
    public function NewUploadToSendCache($msgType )
    {
        ...
        if($msgType == 'at' || $msgType=="ai") {
            $tempsql = "select tpl_contents from cb_wt_template_ib where tpl_code = '".$CI->input->post('templi_code')."'";
            $tempmsg = $CI->db->query($tempsql)->row()->tpl_contents;
            if(empty($tempmsg))
            {
                log_message("ERROR", "템플릿을 확인 하세요.(".$CI->input->post('templi_code').")");
                return 0;
            }
        }
        ...
        foreach($lists as $r)
        {
            if($agreeid != "") { //2021-01-20
                ...
                if(($msgType == 'at' || $msgType=="ai") && !empty($r->var1)) {
                    $msg = $tempmsg;

                    $pattern = '/\#{[^}]*}/';
                    $m_cnt = preg_match_all($pattern, $msg, $match );
                    $idx = 0;
                    $m_idx = 0;
                    ...
                    while($m_cnt >= 1) {
                        ...
                        if (($idx - $m_idx) == 0) {
                            $msg= preg_replace($variable, $r->var1,$msg, 1);
                        } else if (($idx - $m_idx) == 1) {
                            $msg= preg_replace($variable, $r->var2,$msg, 1);
                        } else if (($idx - $m_idx) == 2) {
                            $msg= preg_replace($variable, $r->var3,$msg, 1);
                        } else if (($idx - $m_idx) == 3) {
                            $msg= preg_replace($variable, $r->var4,$msg, 1);
                        } else if (($idx - $m_idx) == 4) {
                            $msg= preg_replace($variable, $r->var5,$msg, 1);
                        }
                        $m_cnt = $m_cnt - 1;
                        $idx = $idx + 1;
                    }
    ...
    {
        $CI =& get_instance();
        $mt = ($msgType == 'at') ? "AL":strtoupper($msgType);
        $param = array(
    ...
    public function sendAgentSendAPI($msgType, $group_id, $mem_id, $mem_userid, $pf_key, $reserveDt='00000000000000', $sc_id)
    {
        ...
        $sql = "insert ignore into  ".$this->request_table."
			(
				...
				message_type, msg, msg_sms, only_sms, p_com, p_invoice,
				phn, profile, reg_dt, remark4,  remark5,
				reserve_dt, s_code, sms_kind, sms_lms_tit, sms_sender, tmpl_id, remark2, supplement, price, currency_type
			)
			select
				...
				?, sc_content, sc_lms_content, ? , ?, ?,
				(CASE WHEN SUBSTR(sc_tel, 1, 2)='01' THEN CONCAT('82', SUBSTR(sc_tel,2)) ELSE sc_tel END), ?, now(), ?, sc_sms_yn,
				?, ?, ?, substr(sc_lms_content, 1, 20), sc_sms_callback, sc_template, '".$mem_id."', supplement, price, currency_type
    ...
    public function sendAgent2SendAPIs($msgType, $group_id, $mem_id, $mem_userid, $pf_key, $reserveDt='00000000000000', $mem_send, $sms_kind)
    {
        ...
        $sql = "insert ignore into  ".$this->request_table."
			...
			select
				...
				?, sc_content, sc_lms_content, ? , ?, ?,
				(CASE WHEN SUBSTR(sc_tel, 1, 2)='01' THEN CONCAT('82', SUBSTR(sc_tel,2)) ELSE sc_tel END), ?, now(), ?, sc_sms_yn,
				?, ?, ?, substr(sc_lms_content, 1, 20), sc_sms_callback, sc_template, '".$mem_id."', '' as supplement, '0' as price, '' as currency_type 
			from cb_api_sc_".$mem_userid." where sc_group_id='".$group_id."'";
        //			echo '<pre> :: ';print_r($sql);
        // log_message("ERROR", "/application/libraries/Dhnkakao > sql : ". $sql);
        $CI->db->query($sql, $param);
        $mst_qty = $CI->db->affected_rows();
